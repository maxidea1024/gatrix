  ...
  try {
    ...
    async function sendImpactMetrics() {
      try {
        ...
        if (response.ok) {
          logger.info('Impact metrics sent successfully', {
    ...
    const autoEvalInterval = setInterval(() => {
      ...
      logger.debug('Auto-evaluated feature flags', {
        count: evalCount,
    ...
    function printFeatureFlags() {
      ...
      logger.info('FEATURE FLAGS:', {
        ...
        flags: flags.map((f) => ({
          ...
          strategies: f.strategies?.length || 0,
    ...
    logger.info(
      `Evaluating every ${EVAL_INTERVAL_MS / 1000}s, flushing every ${FLUSH_INTERVAL_MS / 1000}s`
    );
