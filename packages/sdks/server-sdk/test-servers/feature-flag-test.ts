...
async function main() {
    ...
    try {
        ...
        allFlags.forEach(flag => {
            console.log(`      - ${flag.name}: enabled=${flag.isEnabled}, strategies=${flag.strategies.length}, variants=${flag.variants?.length || 0}`);
        });
        ...
        for (const testFlag of allFlags.slice(0, 3)) {  // Test up to 3 flags
            console.log(`\n    Flag: ${testFlag.name}`);
            for (const context of testContexts) {
                ...
                console.log(`      userId=${context.userId} → enabled=${result.enabled}, reason=${result.reason}${variantInfo}`);
            }
        }
        ...
        const boolDetailResult = sdk.featureFlag.boolVariationDetail(testFlagName, testContext, environment, false);
        console.log(`    boolVariationDetail: value=${boolDetailResult.value}, reason=${boolDetailResult.reason}`);

        // String variation
        const stringResult = sdk.featureFlag.stringVariation(testFlagName, testContext, environment, 'default');
