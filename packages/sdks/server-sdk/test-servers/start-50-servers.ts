...
function startLobbyServer(instanceId: number, port: number): ChildProcess {
  ...
  const proc = spawn('npx', ['ts-node', scriptPath, instanceId.toString(), port.toString(), 'production', 'true'], {
...
function startWorldServer(instanceId: number, port: number, worldId: string): ChildProcess {
  ...
  const proc = spawn('npx', ['ts-node', scriptPath, instanceId.toString(), port.toString(), worldId, 'true'], {
    cwd: path.join(__dirname, '..'),
    stdio: ['ignore', 'pipe', 'pipe'],
    shell: true,
  });
  ...
  proc.stdout?.on('data', (data) => {
    const output = data.toString().trim();
    if (output) {
      console.log(`${colors.magenta}[worldd-${instanceId}:${port}:${worldId}]${colors.reset} ${output}`);
    }
  });
  ...
  proc.stderr?.on('data', (data) => {
    const output = data.toString().trim();
    if (output) {
      console.error(`${colors.red}[worldd-${instanceId}:${port}:${worldId} ERROR]${colors.reset} ${output}`);
    }
  });

  proc.on('exit', (code) => {
    log(`worldd instance ${instanceId} (port ${port}, ${worldId}) exited with code ${code}`, colors.yellow);
  });
