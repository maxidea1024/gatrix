...
function startLobbyServer(instanceId: number, port: number): ChildProcess {
  ...
  proc.stdout?.on("data", (data) => {
    ...
    if (output) {
      console.log(
        `${colors.cyan}[lobbyd-${instanceId}:${port}]${colors.reset} ${output}`,
      );
    }
  });
  ...
  proc.stderr?.on("data", (data) => {
    ...
    if (output) {
      console.error(
        `${colors.red}[lobbyd-${instanceId}:${port} ERROR]${colors.reset} ${output}`,
      );
    }
  });
  ...
  proc.on("exit", (code) => {
    log(
      `lobbyd instance ${instanceId} (port ${port}) exited with code ${code}`,
      colors.yellow,
    );
  });
...
function startWorldServer(
  ...
  log(
    `Starting worldd instance ${instanceId} (${worldId}) on port ${port}...`,
    colors.magenta,
  );
...
function printSummary() {
  ...
  console.log(
    `${colors.cyan}Lobbyd Servers: ${lobbydServers.length}${colors.reset}`,
  );
  console.log(
    `  Ports: ${LOBBYD_START_PORT}-${LOBBYD_START_PORT + LOBBYD_COUNT - 1}`,
  );
  console.log(
    `${colors.magenta}Worldd Servers: ${worlddServers.length}${colors.reset}`,
  );
  console.log(
    `  Ports: ${WORLDD_START_PORT}-${WORLDD_START_PORT + WORLDD_COUNT - 1}`,
  );
  console.log(`  World IDs: kr-1, kr-2, us-1, us-2, jp-1 (distributed)`);
  console.log(
    `${colors.green}Total: ${servers.length} servers${colors.reset}\n`,
  );
}
...
function stopAllServers() {
  ...
  servers.forEach((server) => {
    try {
      ...
      log(
        `Stopped ${server.type} instance ${server.instanceId} (port ${server.port})`,
        colors.dim,
      );
    } catch (error) {
      log(
        `Failed to stop ${server.type} instance ${server.instanceId}: ${error}`,
        colors.red,
      );
    }
  });
