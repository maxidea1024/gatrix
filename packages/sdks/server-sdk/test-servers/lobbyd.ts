...
class LobbyServer extends BaseTestServer {
  ...
  private setupMaintenanceEventHandlers(): void {
    ...
    this.sdk.on('local.maintenance.started', (event) => {
      ...
      this.log(`   forceDisconnect=${details?.forceDisconnect}, gracePeriodMinutes=${details?.gracePeriodMinutes}`);
  ...
  private notifyPlayersAboutMaintenance(
    ...
    details: { forceDisconnect?: boolean; gracePeriodMinutes?: number; message?: string } | undefined
  ): void {
    ...
    if (source === 'service') {
      ...
      for (const [lobbyId, lobby] of this.lobbies) {
        if (lobby.worldId === worldId) {
          this.log(`📢 Notifying ${lobby.players.length} players in lobby ${lobbyId} about world maintenance`);
   ...
   */
  private kickPlayersForMaintenance(source: string, worldId: string | undefined, message?: string): void {
    ...
    if (source === 'service') {
      ...
      for (const [lobbyId, lobby] of this.lobbies) {
        if (lobby.worldId === worldId) {
          this.log(`🚫 Kicking ${lobby.players.length} players from lobby ${lobbyId} (world: ${worldId})`);
          for (const player of lobby.players) {
  ...
  protected async onStop(): Promise<void> {
    this.log(`Shutting down with ${this.lobbies.size} active lobbies and ${this.connectedPlayers.size} connected players`);
    this.lobbies.clear();
  ...
  private async manageLobby(): Promise<void> {
    ...
    this.log(`Created lobby ${lobbyId} for world ${world.name} with ${playersInLobby.length} players (Total lobbies: ${this.lobbies.size})`);
