...
export class CacheManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (this.environments === "*" && this.environmentService) {
        ...
        this.logger.info(
          "Multi-environment mode (*): fetched environment list from backend",
          {
        ...
        Array.isArray(this.environments) &&
        this.environments.length > 0
      ) {
        this.logger.info(
          "Multi-environment mode (explicit): using configured environments",
          {
      ...
      if (isMultiEnvMode && envList.length === 0) {
        this.logger.warn(
          "Multi-environment mode enabled but no environments available. Skipping initial data load.",
          {
      ...
      if (
        this.features.serviceMaintenance !== false &&
  ...
  private async waitForBackendReady(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const response = await this.apiClient.get<{ status: string }>(
          "/api/v1/ready",
        );
  ...
  async refreshEnvironmentList(): Promise<{
    ...
    const added = this.cachedEnvironmentList.filter(
      (e) => !oldEnvList.includes(e),
    );
    const removed = oldEnvList.filter(
      (e) => !this.cachedEnvironmentList.includes(e),
    );
  ...
  private async loadDataForEnvironments(environments: string[]): Promise<void> {
    ...
    if (this.features.gameWorld !== false && this.gameWorldService) {
      promises.push(
        this.gameWorldService
          .listByEnvironments(environments)
          .catch((error) => {
            this.logger.warn(
              "Failed to load game worlds for new environments",
              { error: error.message },
            );
          }),
      );
    ...
    if (this.features.popupNotice !== false && this.popupNoticeService) {
      promises.push(
        this.popupNoticeService
          ...
          .catch((error) => {
            this.logger.warn(
              "Failed to load popup notices for new environments",
              { error: error.message },
            );
          }),
      );
    ...
    if (
      ...
      promises.push(
        this.serviceMaintenanceService
          ...
          .catch((error) => {
            this.logger.warn(
              "Failed to load service maintenance for new environments",
              { error: error.message },
            );
          }),
      );
    ...
    if (this.features.clientVersion === true && this.clientVersionService) {
      promises.push(
        this.clientVersionService
          ...
          .catch((error) => {
            this.logger.warn(
              "Failed to load client versions for new environments",
              { error: error.message },
            );
          }),
      );
    ...
    if (this.features.serviceNotice === true && this.serviceNoticeService) {
      promises.push(
        this.serviceNoticeService
          ...
          .catch((error) => {
            this.logger.warn(
              "Failed to load service notices for new environments",
              { error: error.message },
            );
          }),
      );
    ...
    if (this.features.storeProduct === true && this.storeProductService) {
      promises.push(
        this.storeProductService
          ...
          .catch((error) => {
            this.logger.warn(
              "Failed to load store products for new environments",
              { error: error.message },
            );
          }),
      );
  ...
  private checkMaintenanceStateChanges(environment?: string): void {
    try {
      if (environment) {
        ...
        const serviceStatus =
          this.serviceMaintenanceService?.getCached(environment) || null;
