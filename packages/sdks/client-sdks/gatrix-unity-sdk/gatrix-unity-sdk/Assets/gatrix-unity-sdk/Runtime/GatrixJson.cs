...
namespace Gatrix.Unity.SDK
{
    ...
    internal static class GatrixJson
    {
        ...
        /// <summary>
        /// Deserialize a JSON string to a Dictionary (for JSON variant payloads)
        /// </summary>
        ...
        private static EvaluatedFlag MapEvaluatedFlag(Dictionary<string, object> dict)
        {
            ...
            {
                flag.Variant = MapVariant(variantDict, flag.ValueType);
            }

            return flag;
        }

        private static Variant MapVariant(Dictionary<string, object> dict, ValueType variantType)
        {
            ...
            if (dict.TryGetValue("value", out var payload))
            {
                ...
                switch (variantType)
                {
                    case ValueType.Number:
                        if (payload is int pi) variant.Value = (double)pi;
                        else if (payload is long pl) variant.Value = (double)pl;
                        else if (payload is double pd) variant.Value = pd;
                        else if (payload is string ps && double.TryParse(ps, NumberStyles.Float, CultureInfo.InvariantCulture, out var parsed))
                            variant.Value = parsed;
                        break;
                    ...
                    case ValueType.String:
                        variant.Value = payload?.ToString();
                        break;
