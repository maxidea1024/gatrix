/**
 * Config Validation - Validates SDK configuration options (CocosCreator)
 *
 * Ensures required fields are present and optional fields have valid values/ranges.
 * Uses UrlBuilder.isValidUrl() instead of native URL constructor.
 */
import { type GatrixClientConfig } from './types';
import { GatrixError } from './errors';
import { isValidUrl } from './url-builder';
...
export function validateConfig(config: GatrixClientConfig): void {
  ...
  // Streaming config (WebSocket only)
  const streaming = config.features?.streaming;
  if (streaming) {
    // Validate transport type
    if (streaming.transport !== undefined) {
      if (streaming.transport !== 'websocket') {
        throw new GatrixError(
          `Invalid config: streaming.transport must be 'websocket' in CocosCreator, got '${streaming.transport}'`
        );
      }
    }
