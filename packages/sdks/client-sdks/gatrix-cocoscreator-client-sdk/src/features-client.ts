...
export class FeaturesClient implements VariationProvider {
  ...
  private watchCallbacks: Map<
    string,
    Set<(flag: FlagProxy) => void | Promise<void>>
  > = new Map();
  private syncedWatchCallbacks: Map<
    string,
    Set<(flag: FlagProxy) => void | Promise<void>>
  > = new Map();
  private flagEnabledCounts: Map<string, { yes: number; no: number }> =
    new Map();
  ...
  constructor(emitter: EventEmitter, config: GatrixClientConfig) {
    ...
    const cachePrefix = fc?.cacheKeyPrefix
      ? `${fc.cacheKeyPrefix}:`
      : undefined;
    this.storage = fc?.storageProvider ?? new CocosStorageProvider(cachePrefix);
  ...
  boolVariationInternal(
    ...
    if (flag.valueType !== 'boolean') {
      this.trackFlagAccess(
        flagName,
        flag,
        'getVariant',
        VALUE_SOURCE.TYPE_MISMATCH
      );
      return fallbackValue;
    }
    this.trackFlagAccess(flagName, flag, 'getVariant', flag.variant.name);
  ...
  boolVariationDetailsInternal(
    ...
    if (!flag) {
      ...
      return {
        ...
        flagExists: false,
        enabled: false,
        variantName: VALUE_SOURCE.MISSING,
      };
    }
    if (flag.valueType !== 'boolean') {
      this.trackFlagAccess(
      ...
      return {
        ...
        variantName: VALUE_SOURCE.TYPE_MISMATCH,
      };
    }
    this.trackFlagAccess(flagName, flag, 'getVariant', flag.variant.name);
    return {
  ...
  numberVariationDetailsInternal(
    ...
    return {
      ...
      reason: isNaN(value)
        ? 'type_mismatch:value_not_number'
        : (flag.reason ?? 'evaluated'),
      flagExists: true,
      enabled: flag.enabled,
      variantName: isNaN(value)
        ? VALUE_SOURCE.TYPE_MISMATCH
        : flag.variant.name,
    };
  }
  ...
  boolVariationOrThrowInternal(
    ...
    this.trackFlagAccess(flagName, flag, 'isEnabled', flag.variant.name);
    if (flag.valueType !== 'boolean') {
      throw GatrixFeatureError.typeMismatch(
        flagName,
        'boolean',
        flag.valueType
      );
  ...
  stringVariationOrThrowInternal(
    flagName: string,
    forceRealtime: boolean = true
  ): string {
    const flag = this.lookupFlag(flagName, forceRealtime);
  ...
  numberVariationOrThrowInternal(
    flagName: string,
    forceRealtime: boolean = true
  ): number {
    const flag = this.lookupFlag(flagName, forceRealtime);
    ...
    if (isNaN(value)) {
      throw GatrixFeatureError.typeMismatch(
        flagName,
        'number',
        typeof flag.variant.value
      );
    }
    return value;
  }

  jsonVariationOrThrowInternal<T>(
    flagName: string,
    forceRealtime: boolean = true
  ): T {
    const flag = this.lookupFlag(flagName, forceRealtime);
  ...
  boolVariationDetails(
    ...
    return this.boolVariationDetailsInternal(
      flagName,
      fallbackValue,
      forceRealtime
    );
  ...
  stringVariationDetails(
    ...
    return this.stringVariationDetailsInternal(
      flagName,
      fallbackValue,
      forceRealtime
    );
  ...
  numberVariationDetails(
    ...
    return this.numberVariationDetailsInternal(
      flagName,
      fallbackValue,
      forceRealtime
    );
  ...
  jsonVariationDetails<T>(
    ...
    return this.jsonVariationDetailsInternal(
      flagName,
      fallbackValue,
      forceRealtime
    );
  ...
  public hasPendingSyncFlags(): boolean {
    return this.isExplicitSyncEnabled() ? this.pendingSync : false;
  }
  ...
  watchRealtimeFlagWithInitialState(
    ...
    if (this.readyEventEmitted) {
      void callback(this.createProxyForWatch(flagName, true));
    } else {
      this.emitter.once(EVENTS.FLAGS_READY, () => {
        void callback(this.createProxyForWatch(flagName, true));
      });
    }
    return () => {
  ...
  watchSyncedFlagWithInitialState(
    ...
    this.syncedWatchCallbacks.get(flagName)!.add(callback);
    if (this.readyEventEmitted) {
      void callback(this.createProxyForWatch(flagName, false));
    } else {
      this.emitter.once(EVENTS.FLAGS_READY, () => {
        void callback(this.createProxyForWatch(flagName, false));
      });
  ...
  private async fetchFlagsInternal(
    ...
    if (this.featuresConfig.offlineMode) {
      this.logger.warn(
        'fetchFlags called but client is in offline mode, ignoring'
      );
      return;
    }
    ...
    this.logger.info(
      `fetchFlags [${caller}]: starting fetch. etag=${this.etag}`
    );
    ...
    try {
      ...
      const nonRetryableStatusCodes = retryOptions.nonRetryableStatusCodes ?? [
        401, 403,
      ];
      ...
      if (this.featuresConfig.usePOSTrequests) {
        ...
        response = await httpGet(url.toString(), {
          headers,
      ...
      if (this.sdkState === 'error' && response.status < 400) {
        ...
        this.logger.info(
          `SDK recovered from error state (recovery #${this.recoveryCount})`
        );
      ...
      if (response.ok) {
        ...
        this.pollingStopped = true;
        this.logger.error(
          `Polling stopped due to non-retryable status code ${response.status}. Call fetchFlags() manually to retry.`
        );
      ...
      if (this.lastContextHash !== fetchStartContextHash) {
        this.devLog(
          'Context changed during fetch, triggering re-fetch with new context'
        );
        this.etag = '';
  ...
  private scheduleNextRefresh(): void {
    ...
    if (isStreamingEnabled && this.streamingState !== 'degraded') {
      this.devLog(
        `scheduleNextRefresh: Skipped because streaming is enabled and state is '${this.streamingState}'`
      );
      return;
    }
    if (this.timerRef) {
    ...
    this.devLog(
      `scheduleNextRefresh: delay=${delay}ms, consecutiveFailures=${this.consecutiveFailures}`
    );
  ...
  private setFlags(flags: EvaluatedFlag[], forceSync: boolean = false): void {
    ...
    this.devLog(
      `setFlags: ${flags.length} flags loaded, forceSync=${forceSync}`
    );
  ...
  private emitRealtimeFlagChanges(
    ...
    for (const [name, newFlag] of newFlags) {
      ...
      if (
        !oldFlag ||
        !isEqualFlag(oldFlag, newFlag, oldContextHash, newContextHash)
      ) {
        ...
        if (callbacks && callbacks.size > 0) {
          ...
          callbacks.forEach((callback) => {
            try {
              ...
              this.logger.error(
                `Error in watch callback for removed flag ${name}:`,
                error
              );
            }
          });
  ...
  private trackImpression(
    ...
    const shouldTrack =
      this.featuresConfig.impressionDataAll || flag?.impressionData;
    if (!shouldTrack) {
      return;
    }
  ...
  private connectWebSocket(): void {
    ...
    this.webSocket.onopen = () => {
      ...
      const pingInterval =
        (this.featuresConfig.streaming?.websocket?.pingInterval ?? 30) * 1000;
      ...
      try {
        ...
        switch (msg.type) {
          case 'connected': {
            ...
            this.devLog(
              `WS 'connected' event: globalRevision=${data.globalRevision}`
            );
            this.emitter.emit(EVENTS.FLAGS_STREAMING_CONNECTED, {
              globalRevision: data.globalRevision,
            });
            if (
              data.globalRevision > this.localGlobalRevision &&
              this.localGlobalRevision > 0
            ) {
              this.devLog(
          ...
          case 'flags_changed': {
            ...
            this.devLog(
              `WS 'flags_changed': globalRevision=${data.globalRevision}, changedKeys=[${data.changedKeys.join(',')}]`
            );
  ...
  private scheduleStreamingReconnect(): void {
    ...
    if (
      this.streamingReconnectAttempt >= 5 &&
  ...
  private handleStreamingInvalidation(changedKeys: string[]): void {
    ...
    if (
      ...
      totalFlags === 0 ||
      changedKeys.length >= totalFlags / 2
    ) {
      this.etag = '';
      if (!this.isFetchingFlags) {
    ...
    if (!this.isFetchingFlags) {
      void this.fetchPartialFlagsInternal(new Set(changedKeys));
    } else {
  ...
  private async fetchPartialFlagsInternal(
    flagKeys: Set<string>
  ): Promise<void> {
    ...
    this.devLog(
      `fetchPartialFlags: starting partial fetch for keys=[${keysStr}]`
    );
    ...
    try {
      ...
      if (this.featuresConfig.usePOSTrequests) {
        ...
        response = await httpPost(
          url.toString(),
          { context: this.context, flagNames: Array.from(flagKeys) },
          { headers, cancelToken: this.cancelToken, timeout }
        );
      } else {
      ...
      if (response.ok) {
        ...
        this.devLog(
          `fetchPartialFlags: error ${response.status}, falling back to full fetch`
        );
        this.etag = '';
        this.isFetchingFlags = false;
        void this.fetchFlagsInternal('manual');
      ...
      if (this.lastContextHash !== fetchStartContextHash) {
        this.devLog(
          'Context changed during partial fetch, triggering full re-fetch'
        );
        this.etag = '';
        this.pendingInvalidationKeys.clear();
        void this.fetchFlagsInternal('contextChange');
