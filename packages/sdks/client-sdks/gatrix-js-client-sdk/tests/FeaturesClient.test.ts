...
import { GatrixFeatureError } from '../src/errors';
...
describe('FeaturesClient', () => {
  ...
  describe('variationDetails', () => {
    ...
    describe('boolVariationDetails', () => {
      it('should return correct details for existing flag', () => {
        const result = client.boolVariationDetails('bool-enabled', false);
      ...
      it('should return flag_not_found for non-existent flag', () => {
        const result = client.boolVariationDetails('non-existent', false);
    ...
    describe('numberVariationDetails', () => {
      it('should return correct details for number flag', () => {
        const result = client.numberVariationDetails('number-variant', 0);
  ...
  describe('getAllFlags', () => {
    ...
    it('should return all flags', () => {
      ...
      expect(flags.map((f) => f.name)).toEqual(
        expect.arrayContaining(bootstrapFlags.map((f) => f.name))
      );
    });
  });
  ...
  describe('offline mode', () => {
    ...
    it('should warn and skip fetchFlags in offline mode', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('offline mode'));
    });
