...
export class FeaturesClient implements VariationProvider {
  ...
  private scheduleNextRefresh(): void {
    ...
    // Do not scale refresh if streaming is healthy
  ...
  private scheduleStreamingReconnect(): void {
    ...
    if (this.streamingReconnectAttempt >= 5 && this.streamingState !== 'degraded') {
      ...
      // Trigger polling since streaming is degraded
      this.scheduleNextRefresh();
    }
  ...
  /**
   * Get lightweight feature flag statistics.
   * Returns only scalar/primitive values ??no Map iteration.
