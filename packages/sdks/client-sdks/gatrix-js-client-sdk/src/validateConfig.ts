...
export function validateConfig(config: GatrixClientConfig): void {
    ...
    if (feat) {
        ...
        if (feat.metricsIntervalInitial !== undefined) {
            validatePositiveNumber(feat.metricsIntervalInitial, 'features.metricsIntervalInitial', 0, 3600);
        }
        ...
        if (feat.fetchRetryOptions) {
            ...
            if (retry.backoffLimit !== undefined) {
                validatePositiveNumber(retry.backoffLimit, 'features.fetchRetryOptions.backoffLimit', 0, 300000);
            }
