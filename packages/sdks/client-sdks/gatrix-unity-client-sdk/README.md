...
```mermaid
flowchart LR
    DASHBOARD["?ñ•Ô∏?Dashboard<br/>Toggle: ON"]:::dash ==> GAME["?éÆ Game Client<br/>if (IsEnabled)"]:::game
...
```mermaid
flowchart LR
    DASHBOARD["?ñ•Ô∏?Dashboard<br/>game-speed: 2.0<br/>welcome-msg: 'Hello!'"]:::dash ==> GAME["?éÆ Game Client<br/>FloatVariation / StringVariation"]:::game
    GAME --> VALUE1("?? Speed = 2.0"):::feature
    GAME --> VALUE2("?í¨ Text = 'Hello!'"):::feature
...
```mermaid
flowchart LR
    RULE["?ñ•Ô∏?Rule:<br/>IF Level >= 10<br/>THEN 'difficulty' = 'Hard'"]:::dash ==> GAME["?éÆ Game Client<br/>Context: { Level: 15 }"]:::game
    GAME --> RESULT("?î• Difficulty: Hard"):::feature
...
// ?Ä?Ä Example 1: VIP tier branching ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
// Dashboard rules:
...
// ?Ä?Ä Example 2: Region-specific event rollout ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
// Dashboard rules:
...
// ?Ä?Ä Example 3: JSON variant for a full config table ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä Example 4: Percentage rollout with new feature ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
### ?éØ Real-World Scenarios

#### ?ì± Mobile App Store Review
...
```mermaid
sequenceDiagram
    participant Dev as ?ßë?çüí?Developer
    participant Store as ?ì± App Store
    participant Dashboard as ?ñ•Ô∏?Gatrix Dashboard
    participant Game as ?éÆ Game Client

    Dev->>Store: Submit build v2.1<br/>(new feature code included, flag OFF)
    Note over Store: Review in progress<br/>(3?? days)
    ...
    Note over Game: ?éâ Feature goes live instantly!

    Note over Dev: Issue detected!
    Dev->>Dashboard: Disable "summer-event" flag
    Dashboard->>Game: Flag updated via streaming
    Note over Game: ?õë Feature disabled instantly<br/>No new build needed
```

> ?í° **Without Gatrix:** You'd either have to time your submission perfectly, or submit *another* update and wait for another review cycle just to enable or disable a feature.

#### ?ñÔ∏è Regulatory & Legal Compliance (GDPR, etc.)
...
#### Ôø?A/B Testing & Data-Driven Decisions
...
#### ?õ°Ô∏?SafeGuard (Automated Circuit Breaker)
...
### ?§î When you might NOT need Gatrix
...
- **Very small or prototype projects** ??if your team is 1?? people and the game's scope is small, hard-coded values may be simpler to manage.
...
|---|---|---|
| **Security** | ??Rules never leave server | ?†Ô∏è Rules exposed to client |
| **Consistency** | ??Same result on all SDKs | ?†Ô∏è SDK must re-implement rules |
| **Payload** | ??Small (final values only) | ?†Ô∏è Large (full rule set) |
| **First-run offline** | ?†Ô∏è Needs at least one prior fetch or bootstrap data | ??Rules can be bundled at build time |

> ?åê **Offline & Availability:** The SDK always serves from local cache if the server is unreachable. Fallback values ensure the game never crashes due to connectivity issues.

> ?ìñ Full details ??value resolution flow, reserved variant names (`$missing`, `$env-default-enabled`, ??, and `fallbackValue` rationale:  
> **[??Evaluation Model ?êÏÑ∏??Î≥¥Í∏∞](docs/EVALUATION_MODEL.md)**
---
...
{
  "dependencies": {
    "com.gatrix.unity.client.sdk": "file:../../path/to/gatrix-unity-client-sdk"
  }
}
```
...
> ?ìñ Full Watch API reference ??`FlagProxy` properties, `FlagProxy` API table, Watch Groups, `forceRealtime`, and real-world sync scenarios:  
> **[??Watch API ?êÏÑ∏??Î≥¥Í∏∞](docs/WATCH_API.md)**
---

## ?ß© Zero-Code Components
...
> ?ìñ Detailed component reference ??flag value types, all modes, use cases & A/B test scenarios:  
> **[??Component Reference ?êÏÑ∏??Î≥¥Í∏∞](docs/COMPONENTS.md)**
---
...
> ?í° **Think of it like a function call.** Context is the **argument** you pass in; the targeting rules on the server are the **conditions inside the function**. The server runs the function and returns the result ??you never see the logic itself.
...
### ?†Ô∏è Side Effects of Context Changes
...
- ?éÆ **Mid-combat stat changes** that feel like bugs or cheating
- ?îó **Dependency issues** where flags change before dependent systems are ready
- ?ò§ **Jarring UX** where the UI shifts while the player is interacting with it
...
> ?ìñ Deep dive ??polling intervals, ETag behaviour, streaming transports, hybrid failure scenarios, and configuration reference:  
> **[??Polling / Streaming / Hybrid ?êÏÑ∏??Î≥¥Í∏∞](docs/OPERATING_MODES.md)**
...
```mermaid
flowchart LR
    INIT["?ì¶ Bootstrap / Cache"] --> SDK["SDK"]

    subgraph M1 ["??Streaming + Polling (Default)"]
        direction LR
        A1["?ñ•Ô∏?Server"] -->|stream| B1["SDK"]
        A1 -->|poll| B1
    end

    subgraph M2 ["?îÑ Polling Only"]
        direction LR
        A2["?ñ•Ô∏?Server"] -->|poll| B2["SDK"]
    end

    subgraph M3 ["?ì¥ Offline"]
        direction LR
...
> ?ì¶ **Bootstrap / Cache is always the first step.** Regardless of the operating mode, the SDK loads bootstrap data and cached flags on startup for instant availability before any network request.
...
| `GetStats()` | `GatrixClientSDKStats` | Get combined SDK statistics |
...
## ?îÑ Lifecycle-Bound Watch Extensions
...
> ?í° **Namespace:** These extension methods live in `Gatrix.Unity.SDK.Extensions`. Add `using Gatrix.Unity.SDK.Extensions;` to use them.
...
> ?í° **Tip:** Built-in components ([`GatrixFlagToggle`](docs/COMPONENTS.md#gatrixflagtoggle), [`GatrixFlagValue`](docs/COMPONENTS.md#gatrixflagvalue), etc.) already handle lifecycle through `GatrixFlagComponentBase`. The lifecycle extensions are for your **custom MonoBehaviours**.
...
- [?úÍµ≠??Î¨∏ÏÑú (README.ko.md)](./README.ko.md)
