...
namespace Gatrix.Unity.SDK
{
    ...
    internal class Metrics
    {
        ...
        public async UniTask SendMetricsAsync()
        {
            if (_disabled) return;
        ...
        private async UniTask RunMetricsLoop(int intervalMs, int initialDelayMs, CancellationToken ct)
        {
            ...
            try
            {
                if (initialDelayMs > 0)
                {
                    await UniTask.Delay(initialDelayMs, DelayType.Realtime, cancellationToken: ct);
                }
