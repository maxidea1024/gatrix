...
DECLARE_DELEGATE_TwoParams(FGatrixAnyEventDelegate, const FString&, const TArray<FString>&);
...
public:
  struct FListenerInfo {
    ...
    FListenerInfo() : Handle(0), bOnce(false), CallCount(0), RegisteredAt(FDateTime::UtcNow()) {}
  };
  ...
  void Emit(const FString& EventName, const TArray<FString>& Args = TArray<FString>());
...
private:
  int32 AddListener(const FString& EventName, TFunction<void(const TArray<FString>&)> Callback,
