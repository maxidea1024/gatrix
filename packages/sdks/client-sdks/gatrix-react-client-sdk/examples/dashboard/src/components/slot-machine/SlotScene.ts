...
function checkWins(
  ...
  multiplier: number,
  bet: number,
  customPayouts: Record<string, number> | null
): WinLine[] {
  const wins: WinLine[] = [];
  for (let li = 0; li < PAYLINES.length; li++) {
    ...
    if (matched && matchSym) {
      ...
      wins.push({
        lineIdx: li,
        symbol: matchSym,
        payout: Math.floor(basePay * multiplier * bet * wildBonus),
        positions: line,
      });
    }
  }
...
export class SlotScene extends Phaser.Scene {
  ...
  private drawFrame() {
    ...
    this.add
      .text(REEL_CX, FRAME_Y - 28, '✦ GATRIX SLOT SAGA ✦', {
        fontSize: '22px',
  ...
  private onAllStopped() {
    ...
    const wins = checkWins(
      this.finalResult,
      this.bridge.config.wildSymbolEnabled,
      this.bridge.config.winMultiplier || 1,
      this.bridge.bet,
    ...
    if (wins.length > 0) {
      ...
      if (
        this.bridge.config.bonusRoundEnabled &&
        this.bridge.comboCount >= 3 &&
        this.bridge.freeSpinsLeft === 0
      ) {
  ...
  private createUI() {
    ...
    this.creditsTxt = this.add
      .text(FRAME_X + 6, infoY, `CREDITS: ${this.bridge.credits}`, {
        fontSize: '11px',
    ...
    const betY = infoY + 22;
    this.betTxt = this.add
      .text(REEL_CX, betY, `BET: ${this.bridge.bet}`, {
        fontSize: '10px',
        fontFamily: PIXEL_FONT,
        color: '#fff',
        stroke: '#000',
                ...
                this.tweens.add({
                  ...
                  onComplete: () => {
                    ...
                    const flash = this.add
                      .circle(creditPos.x, creditPos.y, 8, 0xffd700, 0.8)
                      .setDepth(16);
                    this.tweens.add({
                      targets: flash,
                      scaleX: 2,
    ...
    this.ambientTimer = this.time.addEvent({
      ...
      callback: () => {
        ...
        const emitter = this.add
          .particles(x, GAME_H + 10, 'particle_dot', {
