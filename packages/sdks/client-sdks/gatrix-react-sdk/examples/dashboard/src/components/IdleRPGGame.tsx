...
function drawHero(g: PIXI.Graphics, frame: number) {
    ...
    // Shield (left hand)
    g.roundRect(-20, -28 + bob, 8, 14, 2).fill(0xe76e55).stroke({ width: 1, color: 0xaa4422 });
}
...
function drawEnemy(g: PIXI.Graphics, frame: number, isBoss: boolean) {
    ...
    // Belly highlight
    g.ellipse(0, -10 * scale + bob, 12 * scale * squish, 10 * scale / squish).fill({ color: 0xffffff, alpha: 0.15 });
    // Eyes
    ...
    if (isBoss) {
        // Crown for boss
        g.moveTo(-12, -36 * scale + bob).lineTo(-8, -42 * scale + bob).lineTo(-4, -36 * scale + bob)
            .lineTo(0, -44 * scale + bob).lineTo(4, -36 * scale + bob)
            .lineTo(8, -42 * scale + bob).lineTo(12, -36 * scale + bob)
            .fill(0xf7d51d);
    }
}
...
function drawGround(g: PIXI.Graphics, width: number) {
    ...
    for (let x = 0; x < width; x += 30) {
        ...
        g.moveTo(x, 0).lineTo(x + 3, -h).lineTo(x + 6, 0).fill(0x5aaf3a);
    }
}
...
function drawMountains(g: PIXI.Graphics, width: number, offset: number) {
    ...
    for (let i = -1; i < width / 120 + 2; i++) {
        ...
        g.moveTo(x - 70, 0).lineTo(x, -60 - (i % 3) * 20).lineTo(x + 70, 0).fill(0x3a5f8a).stroke({ color: 0x4a6f9a, width: 1 });
    }
}
...
function drawClouds(g: PIXI.Graphics, width: number, offset: number) {
    g.clear();
    const positions = [
        { x: 100, y: 40, s: 1.0 }, { x: 320, y: 25, s: 0.7 }, { x: 550, y: 50, s: 1.2 },
        { x: 700, y: 30, s: 0.8 }, { x: 900, y: 45, s: 0.9 },
    ];
...
const IdleRPGGame: React.FC<IdleRPGGameProps> = ({ onExit }) => {
    ...
    const stateRef = useRef({
        level: 1, gold: 0, exp: 0, kills: 0, stage: 1, subStage: 1,
        enemyHp: 100, enemyMaxHp: 100,
        attackTimer: 0, heroFrame: 0, enemyFrame: 0, hitFrame: -1,
        isAttacking: false, attackAnimFrame: 0,
        cloudOffset: 0, mountainOffset: 0, treeOffset: 0,
        gameState: 'playing' as string,
        enemyX: 700, heroX: 180,
    });
    ...
    useEffect(() => {
        ...
        const initGame = async () => {
            ...
            const hpText = new PIXI.Text({ text: '', style: { fontSize: 10, fill: 0xffffff, fontFamily: '"Press Start 2P", monospace' } });
            ...
            app.ticker.add((ticker) => {
                ...
                hpBarBg.clear().rect(s.enemyX - 30, 220, 60, 8).fill(0x333333).stroke({ width: 1, color: 0x666666 });
                hpBarFill.clear().rect(s.enemyX - 29, 221, 58 * hpPercent, 6).fill(hpPercent > 0.3 ? 0xff4444 : 0xff0000);
    ...
    const expPercent = stateRef.current.level > 0
        ? Math.min(100, (exp / (stateRef.current.level * 100)) * 100)
        : 0;
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                {gameState === 'stageClear' && (
                    <div style={{
                        ...
                        <h1 className="nes-text is-warning" style={{ fontSize: '36px' }}>STAGE CLEAR!</h1>
                    </div>
                )}
                ...
                <div style={{
                    ...
                    <div style={{
                        height: '100%', background: '#92cc41',
                        width: `${expPercent}%`, transition: 'width 0.3s',
                    }} />
                </div>
                ...
                {client.features.canSyncFlags() && (
                    <div style={{
                        ...
                        <button className="nes-btn is-warning" onClick={startSyncTransition}
            ...
            <div style={{ display: 'flex', gap: '8px', width: '808px' }}>
                {/* Log */}
                <div className="nes-container is-dark with-title" style={{
                    flex: 1, height: '110px', overflow: 'hidden',
                }}>
                    <p className="title">BATTLE LOG</p>
                    <div style={{ fontSize: '9px', textAlign: 'left', lineHeight: '1.6' }}>
                        {log.map((m, i) => (
                            <div key={i} style={{ opacity: 1 - i * 0.15 }}>{m}</div>
                        ))}
                    </div>
                </div>

                {/* Controls */}
                <div className="nes-container is-dark with-title" style={{ width: '250px', height: '110px' }}>
                    <p className="title">ACTIONS</p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        <button className="nes-btn is-primary" onClick={useSkill}
                            style={{ fontSize: '10px' }}>
                            🗡️ POWER BASH
                        </button>
            ...
            {/* Flag status bar */}
            <div style={{
                fontSize: '9px', color: '#666', textAlign: 'center', width: '808px',
                display: 'flex', justifyContent: 'center', gap: '16px',
            }}>
                <span>MODE: {client.features.boolVariation('idle-boss-mode', false) ? '🔴 BOSS' : '🟢 NORMAL'}</span>
