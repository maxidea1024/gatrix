...
import type { Enemy, Projectile, Particle, DamageText, HeroStats, SkillDef, InventoryItem, MailItem } from './GameTypes';
import { ENEMY_CONFIGS } from './GameTypes';
import { getDefaultSkills, getInitialMails, generateWaveClearMail, rollDrop, generateShopItems } from './GameData';
...
export interface GameState {
    ...
    mails: MailItem[];
    shopItems: ShopItem[];
    droppedItems: import('./GameTypes').DroppedItem[];
...
function spawnParticles(gs: GameState, x: number, y: number, color: number, count: number): void {
...
function addDamageText(gs: GameState, x: number, y: number, amount: number, isSkill: boolean): void {
    gs.damageTexts.push({
...
function dealDamageToEnemy(gs: GameState, enemy: Enemy, rawDamage: number, isSkill: boolean): void {
    ...
    spawnParticles(gs, enemy.x, enemy.y - 15 * enemy.size, isSkill ? 0xffdd00 : 0xffffff, isSkill ? 12 : 6);

    if (enemy.hp <= 0) {
        enemy.hp = 0;
        enemy.dead = true;
        enemy.deathFrame = 0;
        // Death particles - RED for blood effect
        spawnParticles(gs, enemy.x, enemy.y - 15 * enemy.size, 0xff0000, 25);
    }
}

function processEnemyDeath(gs: GameState, enemy: Enemy, flags: FeatureFlagReader,
    addLog: (m: string) => void): void {
...
export function useSkill(gs: GameState, skillId: string, _flags: FeatureFlagReader,
    addLog: (m: string) => void): void {
...
export function tick(gs: GameState, delta: number, flags: FeatureFlagReader,
    addLog: (m: string) => void): void {
    ...
    if (gs.isDead) {
        // Update visuals only, no combat logic
    ...
    if (gs.hero.kills >= waveKillTarget * gs.hero.wave &&
        gs.enemies.filter(e => !e.dead).length === 0 &&
        gs.hero.kills > 0 && gs.hero.kills % waveKillTarget === 0) {
    ...
    for (let i = gs.droppedItems.length - 1; i >= 0; i--) {
        ...
        if (item.life < 100) {
            ...
            if (dist < 15) {
                ...
                if (!item.collected) {
                    item.collected = true;
                    const existing = gs.inventory.find(it => it.name === item.item.name && it.rarity === item.item.rarity);
                    if (existing) {
                        existing.quantity += 1;
                    } else {
