      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        {mails.map((mail) => (
          ...
          >
            ...
            <button
              className={`nes-btn is-small ${mail.claimed ? 'is-disabled' : 'is-warning'}`}
...
export const GameHUD: React.FC<GameHUDProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ color: flagStatus.bossMode ? '#f7d51d' : '#888' }}>
          ● BOSS: {flagStatus.bossMode ? 'ON' : 'OFF'}
        </span>
        <span style={{ color: flagStatus.gameSpeed > 1 ? '#92cc41' : '#888' }}>
          ● SPEED: x{flagStatus.gameSpeed}
        </span>
        <span style={{ color: flagStatus.expBooster > 1 ? '#92cc41' : '#888' }}>
          ● EXP: x{flagStatus.expBooster}
        </span>
        <span style={{ color: flagStatus.autoSkill ? '#209cee' : '#888' }}>
          ● AUTO-SKILL: {flagStatus.autoSkill ? 'ON' : 'OFF'}
        </span>
      </div>
    </div>
  );
