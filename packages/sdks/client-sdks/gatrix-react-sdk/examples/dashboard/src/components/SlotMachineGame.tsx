...
import {
    SlotConfig, DEFAULT_SLOT_CONFIG, FLAG_NAMES, ThemeKey,
    SYMBOL_CONFIGS, type FlagChangeLog,
...
function FlagStatusPanel({ config, bridge, flagLogs, hasPending, onSync, onVipChange, playerName, vipLevel }: FlagPanelProps) {
    ...
    const realTimeFlags = [
        ...
        { name: FLAG_NAMES.WILD_SYMBOL, label: 'Wild Symbol', value: String(config.wildSymbolEnabled), type: 'bool' },
        { name: FLAG_NAMES.BONUS_ROUND, label: 'Bonus Round', value: String(config.bonusRoundEnabled), type: 'bool' },
        { name: FLAG_NAMES.AUTO_SPIN, label: 'Auto Spin', value: String(config.autoSpinEnabled), type: 'bool' },
    ];

    // Manual sync flags (applied only via syncFlags)
    const syncFlags = [
        { name: FLAG_NAMES.SPIN_SPEED, label: 'Spin Speed', value: String(config.spinSpeed), type: 'number' },
        { name: FLAG_NAMES.WIN_MULTIPLIER, label: 'Win Multiplier', value: String(config.winMultiplier), type: 'number' },
        { name: FLAG_NAMES.INITIAL_CREDITS, label: 'Initial Credits', value: String(config.initialCredits), type: 'number' },
        { name: FLAG_NAMES.PAYOUT_TABLE, label: 'Payout Table', value: config.payoutTable ? 'Custom' : 'Default', type: 'json' },
    ];
            ...
            {bridge && (
                <div className="slot-panel-section">
                    ...
                    <div className="slot-stats-grid">
                        <span className="slot-stat-label">Credits:</span><span className="slot-stat-value">{bridge.credits}</span>
                        <span className="slot-stat-label">Total Bet:</span><span className="slot-stat-value">{bridge.totalBet}</span>
                        <span className="slot-stat-label">Total Win:</span><span className="slot-stat-value">{bridge.totalWin}</span>
                        <span className="slot-stat-label">Combo:</span><span className="slot-stat-value">{bridge.comboCount}</span>
                        <span className="slot-stat-label">Free Spins:</span><span className="slot-stat-value">{bridge.freeSpinsLeft}</span>
                    </div>
                </div>
            )}
            ...
            <div className="slot-panel-section">
                ...
                <div className="slot-flag-log">
                    {flagLogs.length === 0 ? (
                        ...
                        flagLogs.slice(-8).reverse().map((log) => (
                            <div key={log.id} className="slot-log-entry">
                                <span className="slot-log-flag">{log.flagName}</span>
                                <span className="slot-log-change">{log.oldValue} → {log.newValue}</span>
                            </div>
                        ))
                    )}
                </div>
...
export default function SlotMachineGame({ onExit }: { onExit: () => void }) {
    ...
    const handleSync = useCallback(() => {
        ...
        const payout = payoutRaw && typeof payoutRaw === 'object' ? payoutRaw as Record<string, number> : null;

        setConfig((c) => {
            const newConfig = { ...c, spinSpeed: speed, winMultiplier: mult, initialCredits: credits, autoSpinEnabled: autoSpin, payoutTable: payout };
            // Update bridge
            if (bridgeRef.current) {
    ...
    const handleVipChange = useCallback((newVip: number) => {
        setVipLevel(newVip);
        if (client) {
    ...
    const handleLogin = useCallback((name: string, vip: number) => {
        setPlayerName(name);
        setVipLevel(vip);
        if (client) {
            client.features.updateContext({ playerName: name, vipLevel: vip } as Record<string, unknown>);
        }
        setPhase('playing');
    }, [client]);
    ...
    useEffect(() => {
        ...
        features.syncFlags().then(() => {
            ...
            const themeNow = (features.stringVariation(FLAG_NAMES.THEME, 'classic') || 'classic') as SlotConfig['theme'];
    ...
    if (phase === 'title') {
        return (
            <div className="slot-container">
                <button className="slot-exit-btn" onClick={onExit}>✕</button>
