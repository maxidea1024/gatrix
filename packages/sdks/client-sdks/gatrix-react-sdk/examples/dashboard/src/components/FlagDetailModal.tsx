        ...
        <div className="modal-overlay" onClick={onClose} style={{ zIndex: 5000 }}>
            ...
            >
                <div className="flag-header" style={{ margin: '0', padding: '8px 16px' }}>
                    ...
                    <span className={`badge ${flag.enabled ? 'badge-success' : 'badge-error'}`} style={{ fontSize: '9px' }}>
                        {flag.enabled ? 'ON' : 'OFF'}
                    </span>
                </div>
                        ...
                        <div className="flag-payload" style={{ marginTop: '12px' }}>
                            ...
                            {hasPayload ? (
                                <>
                                    ...
                                    <div className="flag-payload-size" style={{ fontSize: '8px', color: '#888', marginTop: '6px', textAlign: 'right' }}>
                                        {payloadSize} BYTES
                                    </div>
                                </>
                            ) : (
                                <div className="flag-payload-value no-payload" style={{ padding: '20px' }}>✕ NO PAYLOAD</div>
                            )}
                        </div>

                        {flag.impressionData && (
                            <div className="flag-detail" style={{ marginTop: '12px', border: 'none', justifyContent: 'flex-start', gap: '10px' }}>
                                <span className="flag-detail-label">Impressions:</span>
                                <span className="pixel-chip" style={{ backgroundColor: '#e8f5e9', color: '#2e7d32', borderColor: '#2e7d32' }}>ENABLED</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="modal-footer" style={{
                    ...
                    <button type="button" className="nes-btn is-primary" onClick={onClose} style={{ fontSize: '9px', padding: '6px 12px' }}>
                        CLOSE
                    </button>
                </div>
            </div>
