...
public:
  ...
  /**
   * Update context and re-fetch flags (C++ only).
   * onComplete(bSuccess, errorMessage) is called when the resulting fetch
   * completes, or immediately when offline/not started.
   */
  void updateContext(const GatrixContext& context,
  ...
  /**
   * Sync flags with completion callback (C++ only).
   * When fetchNow=true, onComplete is called after the fetch completes.
   * When fetchNow=false, onComplete is called immediately after sync.
   */
  void syncFlags(bool fetchNow, std::function<void(bool, const std::string&)> onComplete);

  // ==================== Watch Pattern ====================
  using WatchCallback = std::function<void(FlagProxy)>;
  std::function<void()> watchRealtimeFlag(const std::string& flagName, WatchCallback callback,
                                          const std::string& name = "");
  std::function<void()> watchRealtimeFlagWithInitialState(const std::string& flagName,
                                                          WatchCallback callback,
                                                          const std::string& name = "");
  std::function<void()> watchSyncedFlag(const std::string& flagName, WatchCallback callback,
                                        const std::string& name = "");
  std::function<void()> watchSyncedFlagWithInitialState(const std::string& flagName,
                                                        WatchCallback callback,
                                                        const std::string& name = "");
   ...
   * onComplete(bSuccess, errorMessage) is called when the client first becomes
   * ready, or immediately if already ready. In offline mode resolves
   * immediately.
   */
  ...
  /**
   * Explicitly fetch flags (C++ only).
   * onComplete(bSuccess, errorMessage) is called when the fetch completes.
   */
  void fetchFlags(std::function<void(bool, const std::string&)> onComplete);
...
private:
  ...
  // Pending completion callbacks (MoveTemp-drained on fetch result)
  using CompletionCallback = std::function<void(bool, const std::string&)>;
  std::vector<CompletionCallback> _pendingStartCallbacks;
  std::vector<CompletionCallback> _pendingFetchCallbacks;
  std::vector<CompletionCallback> _pendingContextCallbacks;
  ...
  void saveToStorage();
  void setFlags(const std::vector<EvaluatedFlag>& flags, bool forceSync = false);
  void onFetchResponse(int statusCode, const std::string& body, const std::string& etag);
  void onFetchError(int statusCode, const std::string& error);
  void trackAccess(const std::string& flagName, bool enabled, const std::string& variantName,
