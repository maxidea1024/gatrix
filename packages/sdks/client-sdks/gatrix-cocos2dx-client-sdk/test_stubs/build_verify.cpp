...
int main() {
  ...
  client->on(
      EVENTS::FLAGS_READY,
      [](const std::vector<std::string>&) { std::cout << "Ready!" << std::endl; },
      "ready_listener");

  client->once(
      EVENTS::FLAGS_CHANGE,
      [](const std::vector<std::string>&) { std::cout << "Changed!" << std::endl; }, "change_once");
  ...
  bool canSync = features->hasPendingSyncFlags();
  features->syncFlags(false);

  // 15. Watch pattern
  auto unwatch = features->watchRealtimeFlag(
      "test-flag",
      [](FlagProxy flag) { std::cout << "Flag changed: " << flag.enabled() << std::endl; },
      "watch_test");
  unwatch(); // unsubscribe

  auto unwatchInit = features->watchRealtimeFlagWithInitialState(
      "test-flag",
      [](FlagProxy flag) { std::cout << "Flag state: " << flag.enabled() << std::endl; },
      "watch_init");
  unwatchInit();
