...
import { useGatrixClient } from '@gatrix/gatrix-vue-client-sdk';
      ...
      <div class="stats-grid-layout">
        ...
        <div class="stats-right-group" style="display: flex; align-items: center; gap: 0">
          ...
          <div class="stats-modes-compact">
            ...
            <div :class="['mode-item', config.features?.streaming?.enabled !== false && !client.features.isOfflineMode() ? 'is-success' : 'is-warning']">
              <span class="mode-label">STREAM</span>
              <span class="mode-value">
                {{ client.features.isOfflineMode() ? 'OFF' : config.features?.streaming?.enabled === false ? 'OFF' : (config.features?.streaming?.transport || 'SSE').toUpperCase() }}
              </span>
            </div>
          </div>
