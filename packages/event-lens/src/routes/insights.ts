...
const insightsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/:projectId/timeseries', {
    preHandler: [authenticateClient, requireReadAccess],
  }, async (request, reply) => {
    try {
      const { projectId } = request.params as { projectId: string };
  ...
  fastify.get('/:projectId/referrers', {
    preHandler: [authenticateClient, requireReadAccess],
  }, async (request, reply) => {
    try {
      const { projectId } = request.params as { projectId: string };
  ...
  fastify.get('/:projectId/retention', {
    preHandler: [authenticateClient, requireReadAccess],
  }, async (request, reply) => {
    try {
      const { projectId } = request.params as { projectId: string };
