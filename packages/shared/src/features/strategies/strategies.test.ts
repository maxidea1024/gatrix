...
describe('FlexibleRolloutStrategy', () => {
    ...
    it('should enable at 100% rollout', () => {
        expect(
            strategy.isEnabled({ rollout: 100, stickiness: 'userId' }, { ...defaultContext, userId: 'user-1' })
        ).toBe(true);
    });

    it('should disable at 0% rollout', () => {
        expect(
            strategy.isEnabled({ rollout: 0, stickiness: 'userId' }, { ...defaultContext, userId: 'user-1' })
        ).toBe(false);
    });
    ...
    it('should use default stickiness (userId -> sessionId -> undefined)', () => {
        expect(
            strategy.isEnabled({ rollout: 100, stickiness: 'default' }, { ...defaultContext, sessionId: 'session-1' })
        ).toBe(true);
    });
...
describe('UserWithIdStrategy', () => {
    ...
    it('should match userId in list', () => {
        expect(
            strategy.isEnabled({ userIds: 'user-1,user-2,user-3' }, { ...defaultContext, userId: 'user-2' })
        ).toBe(true);
    });

    it('should not match userId not in list', () => {
        expect(
            strategy.isEnabled({ userIds: 'user-1,user-2,user-3' }, { ...defaultContext, userId: 'user-4' })
        ).toBe(false);
    });
...
describe('GradualRolloutUserIdStrategy', () => {
    ...
    it('should enable at 100% percentage', () => {
        expect(
            strategy.isEnabled({ percentage: 100 }, { ...defaultContext, userId: 'user-1' })
        ).toBe(true);
    });

    it('should disable at 0% percentage', () => {
        expect(
            strategy.isEnabled({ percentage: 0 }, { ...defaultContext, userId: 'user-1' })
        ).toBe(false);
    });
...
describe('RemoteAddressStrategy', () => {
    ...
    it('should match exact IP', () => {
        expect(
            strategy.isEnabled({ IPs: '192.168.1.1' }, { ...defaultContext, remoteAddress: '192.168.1.1' })
        ).toBe(true);
    });
    ...
    it('should match CIDR range', () => {
        expect(
            strategy.isEnabled({ IPs: '192.168.1.0/24' }, { ...defaultContext, remoteAddress: '192.168.1.100' })
        ).toBe(true);
    });

    it('should not match outside CIDR range', () => {
        expect(
            strategy.isEnabled({ IPs: '192.168.1.0/24' }, { ...defaultContext, remoteAddress: '192.168.2.1' })
        ).toBe(false);
    });
...
describe('Strategy Registry', () => {
    it('should find all built-in strategies', () => {
        const names = [
            'default', 'flexibleRollout', 'userWithId',
            'gradualRolloutUserId', 'gradualRolloutRandom', 'gradualRolloutSessionId',
