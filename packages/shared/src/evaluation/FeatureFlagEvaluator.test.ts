...
describe('String Operators', () => {
    describe('str_eq', () => {
        it('should match equal strings', () => {
            expect(evalConstraint(
                c('str_eq', 'region', { value: 'us-east' }),
                { ...defaultContext, properties: { region: 'us-east' } }
            )).toBe(true);
        });
        ...
        it('should not match different strings', () => {
            expect(evalConstraint(
                c('str_eq', 'region', { value: 'us-east' }),
                { ...defaultContext, properties: { region: 'eu-west' } }
            )).toBe(false);
        });
    ...
    describe('str_contains', () => {
        it('should match substring', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { email: 'user@example.com' } }
            )).toBe(true);
        });
        ...
        it('should not match missing substring', () => {
            expect(evalConstraint(
                c('str_contains', 'email', { value: '@example.com' }),
                { ...defaultContext, properties: { email: 'user@other.com' } }
            )).toBe(false);
        });
        ...
        it('should support case insensitive', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { name: 'SuperAdmin' } }
            )).toBe(true);
        });
    ...
    describe('str_starts_with', () => {
        it('should match prefix', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { path: '/api/v1/users' } }
            )).toBe(true);
        });
        ...
        it('should not match non-prefix', () => {
            expect(evalConstraint(
                c('str_starts_with', 'path', { value: '/api/' }),
                { ...defaultContext, properties: { path: '/web/page' } }
            )).toBe(false);
        });
    ...
    describe('str_ends_with', () => {
        it('should match suffix', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { file: 'config.json' } }
            )).toBe(true);
        });
        ...
        it('should not match non-suffix', () => {
            expect(evalConstraint(
                c('str_ends_with', 'file', { value: '.json' }),
                { ...defaultContext, properties: { file: 'config.yaml' } }
            )).toBe(false);
        });
    });
    ...
    describe('str_in', () => {
        it('should match value in list', () => {
            expect(evalConstraint(
                c('str_in', 'country', { values: ['US', 'CA', 'MX'] }),
                { ...defaultContext, properties: { country: 'CA' } }
            )).toBe(true);
        });
        ...
        it('should not match value not in list', () => {
            expect(evalConstraint(
                c('str_in', 'country', { values: ['US', 'CA', 'MX'] }),
                { ...defaultContext, properties: { country: 'JP' } }
            )).toBe(false);
        });

        it('should support case insensitive', () => {
            expect(evalConstraint(
                c('str_in', 'country', { values: ['us', 'ca'], caseInsensitive: true }),
                { ...defaultContext, properties: { country: 'US' } }
            )).toBe(true);
        });

        it('should invert result (str_not_in equivalent)', () => {
            expect(evalConstraint(
                c('str_in', 'country', { values: ['US', 'CA', 'MX'], inverted: true }),
                { ...defaultContext, properties: { country: 'CA' } }
            )).toBe(false);
        });
    ...
    describe('str_regex', () => {
        it('should match regex pattern', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { version: '1.2.3' } }
            )).toBe(true);
        });
        ...
        it('should not match non-matching pattern', () => {
            expect(evalConstraint(
                c('str_regex', 'version', { value: '^\\d+\\.\\d+\\.\\d+$' }),
                { ...defaultContext, properties: { version: 'v1.2.3' } }
            )).toBe(false);
        });
        ...
        it('should handle invalid regex gracefully', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { value: 'test' } }
            )).toBe(false);
        });
        ...
        it('should support case insensitive flag', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { name: 'ADMIN_USER' } }
            )).toBe(true);
        });
    });
});
...
describe('Number Operators', () => {
    describe('num_eq', () => {
        it('should match equal numbers', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { level: 10 } }
            )).toBe(true);
        });
        ...
        it('should not match different numbers', () => {
            expect(evalConstraint(
                c('num_eq', 'level', { value: '10' }),
                { ...defaultContext, properties: { level: 20 } }
            )).toBe(false);
        });
    ...
    describe('num_gt', () => {
        it('should match greater values', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { age: 25 } }
            )).toBe(true);
        });
        ...
        it('should not match equal values', () => {
            expect(evalConstraint(
                c('num_gt', 'age', { value: '18' }),
                { ...defaultContext, properties: { age: 18 } }
            )).toBe(false);
        });
        ...
        it('should not match lesser values', () => {
            expect(evalConstraint(
                c('num_gt', 'age', { value: '18' }),
                { ...defaultContext, properties: { age: 10 } }
            )).toBe(false);
        });
    });
    ...
    describe('num_in', () => {
        it('should match value in list', () => {
            expect(evalConstraint(
                c('num_in', 'tier', { values: ['1', '2', '3'] }),
                { ...defaultContext, properties: { tier: 2 } }
            )).toBe(true);
        });
        ...
        it('should not match value not in list', () => {
            expect(evalConstraint(
                c('num_in', 'tier', { values: ['1', '2', '3'] }),
                { ...defaultContext, properties: { tier: 5 } }
            )).toBe(false);
        });
...
describe('Boolean Operators', () => {
    describe('bool_is', () => {
        it('should match true', () => {
            expect(evalConstraint(
                c('bool_is', 'premium', { value: 'true' }),
                { ...defaultContext, properties: { premium: true } }
            )).toBe(true);
        });

        it('should match false', () => {
            expect(evalConstraint(
                c('bool_is', 'premium', { value: 'false' }),
                { ...defaultContext, properties: { premium: false } }
            )).toBe(true);
        });
...
describe('Date Operators', () => {
    ...
    describe('date_eq', () => {
        it('should match equal dates', () => {
            expect(evalConstraint(
                c('date_eq', 'signup', { value: testDate }),
                { ...defaultContext, properties: { signup: testDate } }
            )).toBe(true);
        });
        ...
        it('should not match different dates', () => {
            expect(evalConstraint(
                c('date_eq', 'signup', { value: testDate }),
                { ...defaultContext, properties: { signup: beforeDate } }
            )).toBe(false);
        });
    ...
    describe('date_gt', () => {
        it('should match later dates', () => {
            expect(evalConstraint(
                c('date_gt', 'signup', { value: testDate }),
                { ...defaultContext, properties: { signup: afterDate } }
            )).toBe(true);
        });
    ...
    describe('date_gte', () => {
        it('should match equal dates', () => {
            expect(evalConstraint(
                c('date_gte', 'signup', { value: testDate }),
                { ...defaultContext, properties: { signup: testDate } }
            )).toBe(true);
        });
    ...
    describe('date_lt', () => {
        it('should match earlier dates', () => {
            expect(evalConstraint(
                c('date_lt', 'signup', { value: testDate }),
                { ...defaultContext, properties: { signup: beforeDate } }
            )).toBe(true);
        });
    ...
    describe('date_lte', () => {
        it('should match equal dates', () => {
            expect(evalConstraint(
                c('date_lte', 'signup', { value: testDate }),
                { ...defaultContext, properties: { signup: testDate } }
            )).toBe(true);
        });
...
describe('Semver Operators', () => {
    describe('semver_eq', () => {
        it('should match equal versions', () => {
            expect(evalConstraint(
                c('semver_eq', 'appVersion', { value: '1.2.3' }),
        ...
        it('should not match different versions', () => {
            expect(evalConstraint(
                c('semver_eq', 'appVersion', { value: '1.2.3' }),
                { ...defaultContext, appVersion: '1.2.4' }
            )).toBe(false);
        });
    });
    ...
    describe('semver_gt', () => {
        it('should match greater versions', () => {
            expect(evalConstraint(
                c('semver_gt', 'appVersion', { value: '1.2.3' }),
                { ...defaultContext, appVersion: '1.3.0' }
            )).toBe(true);
        });
        ...
        it('should detect major version difference', () => {
            expect(evalConstraint(
                c('semver_gt', 'appVersion', { value: '1.2.3' }),
                { ...defaultContext, appVersion: '2.0.0' }
            )).toBe(true);
        });
    ...
    describe('semver_gte', () => {
        it('should match equal versions', () => {
            expect(evalConstraint(
                c('semver_gte', 'appVersion', { value: '1.2.3' }),
                { ...defaultContext, appVersion: '1.2.3' }
            )).toBe(true);
        });
    });
    ...
    describe('semver_lt', () => {
        it('should match lesser versions', () => {
            expect(evalConstraint(
                c('semver_lt', 'appVersion', { value: '2.0.0' }),
                { ...defaultContext, appVersion: '1.9.9' }
            )).toBe(true);
        });
    });
    ...
    describe('semver_lte', () => {
        it('should match equal versions', () => {
            expect(evalConstraint(
                c('semver_lte', 'appVersion', { value: '1.2.3' }),
                { ...defaultContext, appVersion: '1.2.3' }
            )).toBe(true);
        });
    });
...
describe('Common Operators', () => {
    describe('exists', () => {
        ...
        it('should return true for empty string (value exists)', () => {
            expect(evalConstraint(
                c('exists', 'region'),
                { ...defaultContext, properties: { region: '' } }
            )).toBe(true);
        });
        ...
        it('should return true for zero (value exists)', () => {
            expect(evalConstraint(
                c('exists', 'count'),
                { ...defaultContext, properties: { count: 0 } }
            )).toBe(true);
        });
        ...
        it('should return true for false (value exists)', () => {
            expect(evalConstraint(
                c('exists', 'active'),
                { ...defaultContext, properties: { active: false } }
            )).toBe(true);
        });
    ...
    describe('not_exists', () => {
        ...
        it('should invert result (becomes exists)', () => {
            expect(evalConstraint(
                c('not_exists', 'region', { inverted: true }),
                { ...defaultContext, properties: {} }
            )).toBe(false);
        });
    });
...
describe('Array Operators', () => {
    describe('arr_includes', () => {
        it('should match when array includes any target value', () => {
            expect(evalConstraint(
                c('arr_includes', 'tags', { values: ['premium', 'vip'] }),
                { ...defaultContext, properties: { tags: ['basic', 'premium', 'trial'] } }
            )).toBe(true);
        });
        ...
        it('should not match when array includes none of the target values', () => {
            expect(evalConstraint(
                c('arr_includes', 'tags', { values: ['premium', 'vip'] }),
                { ...defaultContext, properties: { tags: ['basic', 'trial'] } }
            )).toBe(false);
        });

        it('should handle empty arrays', () => {
            expect(evalConstraint(
                c('arr_includes', 'tags', { values: ['premium'] }),
                { ...defaultContext, properties: { tags: [] } }
            )).toBe(false);
        });

        it('should support case insensitive', () => {
            expect(evalConstraint(
                c('arr_includes', 'tags', { values: ['PREMIUM'], caseInsensitive: true }),
                { ...defaultContext, properties: { tags: ['basic', 'premium'] } }
            )).toBe(true);
        });

        it('should invert result', () => {
            expect(evalConstraint(
                c('arr_includes', 'tags', { values: ['premium'], inverted: true }),
                { ...defaultContext, properties: { tags: ['basic', 'premium'] } }
            )).toBe(false);
        });
        ...
        it('should handle non-array values as empty arrays', () => {
            expect(evalConstraint(
                c('arr_includes', 'tags', { values: ['premium'] }),
                { ...defaultContext, properties: { tags: 'premium' } }
            )).toBe(false);
        });
    });
    ...
    describe('arr_all', () => {
        it('should match when array includes all target values', () => {
            expect(evalConstraint(
                c('arr_all', 'tags', { values: ['premium', 'vip'] }),
                { ...defaultContext, properties: { tags: ['basic', 'premium', 'vip'] } }
            )).toBe(true);
        });
        ...
        it('should handle empty target values', () => {
            expect(evalConstraint(
                c('arr_all', 'tags', { values: [] }),
                { ...defaultContext, properties: { tags: ['basic'] } }
            )).toBe(false);
        });
        ...
        it('should invert result', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { tags: ['premium', 'vip'] } }
            )).toBe(false);
        });
    });
    ...
    describe('arr_empty', () => {
        ...
        it('should return false for non-empty array', () => {
            expect(evalConstraint(
                c('arr_empty', 'tags'),
                { ...defaultContext, properties: { tags: ['a'] } }
            )).toBe(false);
        });
        ...
        it('should return true for non-array values (treated as empty)', () => {
            expect(evalConstraint(
                c('arr_empty', 'tags'),
                { ...defaultContext, properties: { tags: 'not-array' } }
            )).toBe(true);
        });
        ...
        it('should invert result (arr_not_empty equivalent)', () => {
            expect(evalConstraint(
                c('arr_empty', 'tags', { inverted: true }),
                { ...defaultContext, properties: { tags: ['a'] } }
            )).toBe(true);
        });
...
describe('Edge Cases', () => {
    ...
    describe('built-in context fields', () => {
        it('should resolve userId', () => {
            expect(evalConstraint(
                c('str_eq', 'userId', { value: 'user-123' }),
                { ...defaultContext, userId: 'user-123' }
            )).toBe(true);
        });

        it('should resolve sessionId', () => {
            expect(evalConstraint(
                c('str_eq', 'sessionId', { value: 'session-abc' }),
                { ...defaultContext, sessionId: 'session-abc' }
            )).toBe(true);
        });

        it('should resolve appName', () => {
            expect(evalConstraint(
                c('str_eq', 'appName', { value: 'test-app' }),
                { ...defaultContext, appName: 'test-app' }
            )).toBe(true);
        });

        it('should resolve appVersion', () => {
            expect(evalConstraint(
                c('str_eq', 'appVersion', { value: '1.0.0' }),
                { ...defaultContext, appVersion: '1.0.0' }
            )).toBe(true);
        });

        it('should resolve remoteAddress', () => {
            expect(evalConstraint(
                c('str_eq', 'remoteAddress', { value: '192.168.1.1' }),
    ...
    describe('unknown operator', () => {
        it('should return false for unknown operator', () => {
            expect(evalConstraint(
                ...
                { ...defaultContext, properties: { field: 'test' } }
            )).toBe(false);
        });
    });
    ...
    describe('multiple constraints', () => {
        it('should require all constraints to pass (AND logic)', () => {
            ...
            // Both match
            expect(FeatureFlagEvaluator.evaluate(
                flag,
