...
const RealtimeEventsPage: React.FC = () => {
  ...
  const [customEventSettings, setCustomEventSettings] = useState<
    Record<string, { iconName: string; color: string }>
  >(() => {
  ...
  const loadEvents = useCallback(async () => {
    try {
      ...
      if (isInitialLoadRef.current || !latestEventTimestampRef.current) {
        ...
        console.log(
          '[RealtimeEvents] Incremental update - fetching since:',
        ...
        setEvents((prevEvents) => {
          ...
          if (!isInitialLoadRef.current) {
            ...
            if (newIdsToFlash.size > 0) {
              ...
              unseenEventIdsToProcess = Array.from(newIdsToFlash).filter(
                (id) => !seenEventIds.has(id)
              );

              console.log('?îî New events detected:', {
  ...
  useEffect(() => {
    ...
    if (!firstEvent) {
      console.log('?†Ô∏è First event ref not found');
      return;
    }
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            console.log('?ëÅÔ∏?First event is visible - marking events as seen');
    ...
    return () => {
      console.log('?ßπ Cleaning up Intersection Observer');
      observer.disconnect();
    };
  }, [events]);
