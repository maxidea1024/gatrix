...
const MaintenancePage: React.FC = () => {
  ...
  const [kickDelayMinutes, setKickDelayMinutes] = useState<number>(5); // ?†Ïòà?úÍ∞Ñ (Î∂? - Default values 5Î∂?
  // Input mode
  ...
  useSSENotifications({
    ...
    onEvent: (event) => {
      if (event.type === 'maintenance_status_change') {
        ...
        setMaintenanceStatus(status);
        setCurrentMaintenanceDetail(detail);
        // ?êÍ? Ï§ëÏùº ?åÎßå Existing Settings??Î∂àÎü¨??(SSE)
        if (detail && !!isUnderMaintenance) {
          setType(detail.type);
          setStartsAt(
            detail.startsAt
          ...
          // ?êÍ? Ï§ëÏù¥ ?ÑÎãàÎ©?Íπ®ÎÅó??StatusÎ°?Initialization
          setType('regular');
          setStartsAt(null);
          setEndsAt(null);
          setBaseMsg('');
  ...
  const validateMaintenanceTime = () => {
    ...
    if (!startsAt && endsAt) {
      ...
      // ?†Ïòà?úÍ∞Ñ Validation (kickExistingPlayersÍ∞Ä Active?îÎêú Í≤ΩÏö∞)
    ...
    // ?úÏûë ?úÍ∞Ñ??Settings??Í≤ΩÏö∞
    if (startsAt) {
      // Ï¢ÖÎ£å ?úÍ∞Ñ???úÏûë ?úÍ∞ÑÎ≥¥Îã§ ?¥Î•∏ÏßÄ Confirm
      ...
      // Ï¢ÖÎ£å ?úÍ∞Ñ??Settings??Í≤ΩÏö∞ Í∏∞Í∞Ñ Validation
      if (endsAt) {
  ...
  // ?úÏûë ?úÍ∞Ñ??Í≥ºÍ±∞?∏Ï? Confirm?òÎäî Helper function
                          ...
                          {currentMaintenanceDetail?.updatedBy && (
                            <Box component="tr">
                              ...
                              >
                                ...
                                {currentMaintenanceDetail.updatedBy.email})
                              </Box>
                            </Box>
                          )}
                    ...
                    {inputMode === 'direct' && (
                      <MultiLanguageMessageInput
                        ...
                        onLocalesChange={(newLocales) => {
                          ...
                          // Translation ResultsÍ∞Ä ?àÏúºÎ©??êÎèô?ºÎ°ú ?§Íµ≠??ÏßÄ??Active??                          const hasNonEmptyLocales = newLocales.some(
              ...
              >
                ...
                >
                  <Box component="tbody">
                    ...
                    {/* Settings???ïÎ≥¥ (Dialog?êÏÑú???ÑÏû¨ Used?? */}
                    {user && (
                      <Box component="tr">
                        <Box
                          component="td"
