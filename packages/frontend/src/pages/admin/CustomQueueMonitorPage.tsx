...
const CustomQueueMonitorPage: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.role === 'admin') {
      fetchQueues();
      const interval = setInterval(fetchQueues, 5000); // 5Ï¥àÎßà??Í∞±Ïã†
  ...
  return (
    <Box sx={{ p: 3 }}>
            ...
            >
              <CardContent>
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                  <Chip 
                    label={`?ÄÍ∏? ${queue.counts?.waiting || 0}`} 
                    color="info" 
                    size="small" 
                  />
                  <Chip 
                    label={`?§ÌñâÏ§? ${queue.counts?.active || 0}`} 
                    color="primary" 
                    size="small" 
                  />
                  <Chip 
                    label={`?ÑÎ£å: ${queue.counts?.completed || 0}`} 
                    color="success" 
                    size="small" 
                  />
                  <Chip 
                    label={`?§Ìå®: ${queue.counts?.failed || 0}`} 
                    color="error" 
                    size="small" 
                  />
                </Box>
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>?¥Î¶Ñ</TableCell>
                    <TableCell>?ÅÌÉú</TableCell>
                    <TableCell>?ùÏÑ±?úÍ∞Ñ</TableCell>
                    <TableCell>ÏßÑÌñâÎ•?/TableCell>
                    <TableCell>?°ÏÖò</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
      ...
      <Dialog 
        ...
        fullWidth
      >
        <DialogTitle>?ëÏóÖ ?ÅÏÑ∏ ?ïÎ≥¥</DialogTitle>
        <DialogContent>
          {selectedJob && (
            <Box>
              ...
              <Typography variant="subtitle1" gutterBottom>
                <strong>?¥Î¶Ñ:</strong> {selectedJob.name}
              </Typography>
              <Typography variant="subtitle1" gutterBottom>
                <strong>?∞Ïù¥??</strong>
              </Typography>
        ...
        <DialogActions>
          <Button onClick={() => setJobDialogOpen(false)}>?´Í∏∞</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
