...
const RemoteConfigPage: React.FC = () => {
  ...
  // SSE connection for real-time updates (?ÑÏãú ÎπÑÌôú?±Ìôî)
  ...
  const getValueTypeIcon = (valueType: string) => {
    const iconProps = {
      ...
      sx: {
        mr: 0.5,
        fontSize: '0.875rem', // Í∏Ä???¨Í∏∞?Ä ÎπÑÏä∑?òÍ≤å
    ...
    switch (valueType) {
      case 'string':
        return <StringIcon {...iconProps} color="primary" title="Î¨∏Ïûê?? />;
      case 'number':
        return <NumberIcon {...iconProps} color="success" title="?´Ïûê" />;
      case 'boolean':
        return <BooleanIcon {...iconProps} color="warning" title="Î∂àÎ¶∞" />;
      case 'json':
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.description.trim()) {
      errors.description = '?§Î™Ö?Ä ?ÑÏàò ?ÖÎ†• ??™©?ÖÎãà??';
    }
  ...
  const loadConfigs = async () => {
    try {
      ...
      // ?ÑÌÑ∞ ?åÎùºÎØ∏ÌÑ∞ Ï∂îÍ? (undefined Í∞??úÏô∏)
  ...
  const handleStageConfigs = async () => {
    try {
      console.log('Starting stage configs...', { selectedConfigs, stageFormData }); // ?îÎ≤ÑÍπÖÏö©
      ...
      console.log('Full response:', response); // ?ÑÏ≤¥ ?ëÎãµ ?ïÏù∏
      console.log('Stage response:', response.data); // ?îÎ≤ÑÍπÖÏö©

      // ?±Í≥µ Ï°∞Í±¥: successÍ∞Ä true?¥Í±∞?? stagedConfigIdsÍ∞Ä ?àÏúºÎ©??±Í≥µ?ºÎ°ú Í∞ÑÏ£º
      if (response.data.success || response.data.stagedConfigIds) {
        ...
        console.log('Stage failed - success is false'); // ?îÎ≤ÑÍπÖÏö©
  ...
  const handlePublishChanges = async () => {
    try {
      console.log('Starting publish...', { publishFormData }); // ?îÎ≤ÑÍπÖÏö©
  ...
  const handleDiscardChanges = async () => {
    try {
      ...
      enqueueSnackbar(`${configsToDiscard.length}Í∞??§Ï†ï??Î≥ÄÍ≤ΩÏÇ¨??ù¥ Ï∑®ÏÜå?òÏóà?µÎãà??`, { variant: 'success' });
      ...
      const errorMessage = error.response?.data?.error?.message || error.message || 'Î≥ÄÍ≤ΩÏÇ¨??Ï∑®ÏÜå Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.';
      enqueueSnackbar(errorMessage, { variant: 'error' });
    }
  };
  ...
  const CampaignsTab = () => {
    ...
    const handleCreateCampaign = async () => {
      try {
        ...
        const createData = {
          ...campaignFormData,
          status: 'draft' // Í∞ïÏ†úÎ°?draft ?ÅÌÉúÎ°??§Ï†ï
        };
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 600 }} align="center">?∏Îûò??ÎπÑÏú®</TableCell>
          ...
          <DialogContent>
            <Stack spacing={3} sx={{ mt: 1 }}>
              ...
              <TextField
                ...
                rows={3}
                required
                helperText="Ï∫†Ìéò?∏Ïùò Î™©Ï†ÅÍ≥??¥Ïö©??Í∞ÑÎã®???§Î™Ö?¥Ï£º?∏Ïöî"
              />
              <TextField
                fullWidth
              ...
              <TextField
                ...
                helperText="Ï∫†Ìéò?∏Ïù¥ ?ÅÏö©???¨Ïö©??ÎπÑÏú® (0-100%)"
              />
  ...
  const TargetingTab = () => {
    ...
    const loadTargetings = async () => {
      try {
        ...
        // ?§Ï†ú ?∞Ïù¥?∞Î≤†?¥Ïä§???∏Í∑∏Î®ºÌä∏ Î°úÎìú
        const segmentsResponse = await api.getSegments();
        const segments = segmentsResponse.data.segments || [];

        // ?∏Í∑∏Î®ºÌä∏ ?∞Ïù¥?∞Î? ?ÄÍ≤üÌåÖ ?ïÏãù?ºÎ°ú Î≥Ä??        const targetingSegments = segments.map((segment: any) => ({
          id: segment.id,
          name: segment.ruleName,
          description: segment.value || '?∏Í∑∏Î®ºÌä∏ ?§Î™Ö ?ÜÏùå',
    ...
    const handleUpdateTargeting = async () => {
      try {
        // ?ÄÍ≤üÌåÖ ?ÖÎç∞?¥Ìä∏ (?•ÌõÑ API ?∞Îèô ?àÏ†ï)
  ...
  const ContextFieldsTab = () => {
    ...
    const handleCreateField = async () => {
      try {
        ...
        // defaultValue Ï≤òÎ¶¨ (Îπ?Î¨∏Ïûê?? false, 0???†Ìö®??Í∏∞Î≥∏Í∞?
        ...
        // optionsÍ∞Ä ?àÏùÑ ?åÎßå Ï∂îÍ? (array ?Ä?ÖÏù¥Í≥??µÏÖò???àÎäî Í≤ΩÏö∞)
        ...
        if (response.success) {
          ...
          loadContextFields();
          enqueueSnackbar('Ïª®ÌÖç?§Ìä∏ ?ÑÎìúÍ∞Ä ?±Í≥µ?ÅÏúºÎ°??ùÏÑ±?òÏóà?µÎãà??', { variant: 'success' });
        } else {
          console.error('Failed to create context field:', response.message);
          enqueueSnackbar(response.message || 'Ïª®ÌÖç?§Ìä∏ ?ÑÎìú ?ùÏÑ±???§Ìå®?àÏäµ?àÎã§.', { variant: 'error' });
        }
      } catch (error) {
    ...
    const handleUpdateField = async () => {
      try {
        ...
        if (response.success) {
          ...
          setFieldFormData({ key: '', name: '', description: '', type: 'string', defaultValue: '', isRequired: false, options: [] });
          loadContextFields();
          enqueueSnackbar('Ïª®ÌÖç?§Ìä∏ ?ÑÎìúÍ∞Ä ?±Í≥µ?ÅÏúºÎ°??òÏ†ï?òÏóà?µÎãà??', { variant: 'success' });
        } else {
          console.error('Failed to update context field:', response.message);
          enqueueSnackbar(response.message || 'Ïª®ÌÖç?§Ìä∏ ?ÑÎìú ?òÏ†ï???§Ìå®?àÏäµ?àÎã§.', { variant: 'error' });
        }
      } catch (error) {
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>Í∏∞Î≥∏Í∞?/TableCell>
                  <TableCell align="center">?ÑÏàò ?¨Î?</TableCell>
                  <TableCell>{t('remoteConfig.description')}</TableCell>
                  <TableCell align="center">{t('remoteConfig.actions')}</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {contextFields.map((field) => (
                  <TableRow key={field.key}>
                    ...
                    <TableCell align="center">
                      <Chip
                        label={field.isRequired ? '?ÑÏàò' : '?†ÌÉù'}
                        size="small"
            ...
            <Stack spacing={3} sx={{ mt: 1 }}>
              ...
              {fieldFormData.type === 'boolean' ? (
                <FormControl fullWidth>
                  ...
                  <FormHelperText>Ïª®ÌÖç?§Ìä∏?êÏÑú Í∞íÏùÑ Î∞õÏ? Î™ªÌñà?????¨Ïö©??Í∏∞Î≥∏Í∞?/FormHelperText>
                </FormControl>
              ) : (
                <TextField
                  fullWidth
                  ...
                  helperText="Ïª®ÌÖç?§Ìä∏?êÏÑú Í∞íÏùÑ Î∞õÏ? Î™ªÌñà?????¨Ïö©??Í∏∞Î≥∏Í∞?
                  placeholder={
              ...
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                />
                <Typography variant="body2" color="text.primary">
                  ?ÑÏàò ?ÑÎìú (Ï°∞Í±¥ ?âÍ? ??Î∞òÎìú???úÍ≥µ?òÏñ¥????
                </Typography>
              </Box>
          ...
          <>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
              <Typography variant="subtitle1" color="text.primary">
                {selectedConfig.keyName}??Î≥Ä??              </Typography>
              <Button
                variant="contained"
                startIcon={<AddIcon />}
          ...
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              {modifiedConfigs.length > 0 && (
                <Chip
                  size="small"
                  label={`${modifiedConfigs.length}Í∞??òÏ†ï??}
                  color="warning"
                  variant="outlined"
                />
              )}
              {stagedConfigs.length > 0 && (
                <Chip
                  size="small"
                  label={`${stagedConfigs.length}Í∞??§ÌÖå?¥Ïßï??}
                  color="info"
                  variant="outlined"
                />
              )}
            ...
            <Stack direction="row" spacing={2}>
              ...
              {stagedConfigs.length > 0 ? (
                ...
                >
                  ?§Ï†ï Î∞∞Ìè¨ ({stagedConfigs.length}Í∞?
                </Button>
              ) : modifiedConfigs.length === 0 && (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Typography variant="body2" color="text.secondary">
                    ?í° Î∞∞Ìè¨???§ÌÖå?¥Ïßï???§Ï†ï???ÜÏäµ?àÎã§.
                  </Typography>
                  <Typography variant="body2" color="primary.main" sx={{ fontWeight: 500 }}>
                    ?§Ï†ï???òÏ†ï????"Î≥ÄÍ≤ΩÏÇ¨???§ÌÖå?¥Ïßï"??Î®ºÏ? ÏßÑÌñâ?òÏÑ∏??
                  </Typography>
                </Box>
              )}
            </Stack>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Î≤ÑÏ†Ñ ?ÅÌÉú</TableCell>
              <TableCell>{t('remoteConfig.description')}</TableCell>
          ...
          <Stack spacing={3} sx={{ mt: 1 }}>
            <Alert severity="info" sx={{ mb: 2 }}>
              <Typography variant="body2">
                ?§Ïùå {selectedConfigs.length}Í∞úÏùò ?§Ï†ï???§ÌÖå?¥Ïßï?©Îãà?? ?§ÌÖå?¥Ïßï???§Ï†ï?Ä Î∞∞Ìè¨ ?ÄÍ∏??ÅÌÉúÍ∞Ä ?©Îãà??
              </Typography>
            </Alert>
                  ...
                  >
                    <Box sx={{ flex: 1 }}>
                      ...
                      <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                        {config.description || '?§Î™Ö ?ÜÏùå'}
                      </Typography>
                      <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>
                        ?Ä?? {config.valueType} | ?ÑÏû¨ Í∞? {config.defaultValue || '(?ÜÏùå)'}
                      </Typography>
                    </Box>
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
                      <Chip
                        size="small"
                        label={config.status === 'draft' ? '?òÏ†ï?? : config.status}
                        color={config.status === 'draft' ? 'warning' : 'default'}
                        variant="outlined"
                      />
                      <Chip
                        size="small"
                        label={config.isActive ? '?úÏÑ±' : 'ÎπÑÌôú??}
                        color={config.isActive ? 'success' : 'default'}
                        variant="filled"
                      />
                    </Box>
            ...
            <TextField
              ...
              required
              helperText="?§ÌÖå?¥Ïßï?òÎäî ?¥Ïú†??Î≥ÄÍ≤??¥Ïö©??Í∞ÑÎã®???§Î™Ö?¥Ï£º?∏Ïöî."
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 1 }}>
            ...
            <TextField
              ...
              required
              helperText="Î∞∞Ìè¨Î•??ùÎ≥Ñ?????àÎäî Í≥†Ïú†???¥Î¶Ñ???ÖÎ†•?òÏÑ∏?? (?? v1.2.0_release)"
            ...
            <TextField
              ...
              required
              helperText="?¥Î≤à Î∞∞Ìè¨???¨Ìï®??Î≥ÄÍ≤ΩÏÇ¨??ù¥??Î™©Ï†Å???§Î™Ö?¥Ï£º?∏Ïöî."
                    ...
                    >
                      <Box sx={{ flex: 1 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            ?ÑÏû¨ Í∞? {config.defaultValue || 'null'}
                          </Typography>
                        </Box>
                      </Box>
                      <Box sx={{ ml: 2 }}>
      ...
      >
        ...
        <DialogContent>
          <Typography color="text.primary" sx={{ mb: 2 }}>
            ?§Ïùå ?§Ï†ï?§Ïùò Î≥ÄÍ≤ΩÏÇ¨??ùÑ Ï∑®ÏÜå?òÏãúÍ≤†Ïäµ?àÍπå? ???ëÏóÖ?Ä ?òÎèåÎ¶????ÜÏäµ?àÎã§.
          </Typography>
