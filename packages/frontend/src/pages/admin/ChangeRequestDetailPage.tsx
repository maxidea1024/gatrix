...
const ChangeRequestDetailPage: React.FC = () => {
    ...
    if (error || !cr) {
        return <Box sx={{ p: 3 }}><Alert severity="error">{t('common.loadFailed')}</Alert></Box>;
    }
    ...
    return (
        <Box sx={{ p: 3, bgcolor: 'background.default', minHeight: '100vh' }}>
            {/* GitHub-style Header */}
            <Box sx={{ borderBottom: 1, borderColor: 'divider', bgcolor: 'background.paper', borderRadius: 1, mb: 3 }}>
                <Box sx={{ px: 3, py: 2 }}>
                    {/* Back + Title */}
                    <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2, mb: 1 }}>
                        <IconButton size="small" onClick={() => navigate('/admin/change-requests')} sx={{ mt: 0.5 }}>
                        ...
                        <Box sx={{ width: 256, flexShrink: 0 }}>
                            <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
                                    <Avatar sx={{ width: 20, height: 20 }}><PersonIcon sx={{ fontSize: 14 }} /></Avatar>
                                    <Typography variant="body2">{cr.requester?.name || cr.requester?.email}</Typography>
                                </Box>
                            </Paper>
                                    ...
                                    <Box sx={{ mt: 1 }}>
                                        {cr.approvals.map((a) => (
                                            <Box key={a.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                                                ...
                                                <Typography variant="body2">{a.approver?.name || a.approver?.email}</Typography>
                                            </Box>
                                        ))}
                                    </Box>
                                ) : (
                        ...
                        {cr.status === 'open' && (
                            ...
                            >
                                <TextField
                                    ...
                                    autoFocus
                                />
                                <Button variant="outlined" color="error" onClick={handleReject} disabled={actionLoading || !comment.trim()}>
                                    {t('changeRequest.actions.reject')}
                                </Button>
                                <Button variant="contained" color="success" startIcon={<CheckIcon />} onClick={handleApprove} disabled={actionLoading}>
                                    {t('changeRequest.actions.approve')}
                                </Button>
                            </Paper>
                        )}
