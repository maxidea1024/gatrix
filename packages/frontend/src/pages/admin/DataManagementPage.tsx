...
const DataManagementPage: React.FC = () => {
  ...
  const handleImportConfirm = async () => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: t("Data imported successfully. Please refresh the page."),
      });
      setImportFile(null);
    } catch (error) {
    ...
    <Box p={3}>
      ...
      >
        ...
        <Card>
          <CardHeader
            title={t("Export Data")}
            subheader={t("Download a full backup of the system data")}
          />
          <Divider />
          <CardContent>
            <Typography variant="body2" color="textSecondary" paragraph>
              {t(
                "This will export all database tables, planning data files, and uploads into a single ZIP file.",
              )}
            </Typography>
            <Button
              variant="contained"
        ...
        <Card>
          <CardHeader
            title={t("Import Data")}
            subheader={t("Restore system data from a backup file")}
          />
          <Divider />
          <CardContent>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            {t(
              "Are you sure you want to import this data? This action will overwrite all existing database records and files. This cannot be undone.",
            )}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
