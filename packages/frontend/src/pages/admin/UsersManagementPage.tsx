...
const UsersManagementPage: React.FC = () => {
  ...
  // ?òÏù¥ÏßÄ State management (URL params ?∞Îèô)
  ...
  // Debouncing??Search??  const debouncedSearchTerm = useDebounce(searchTerm, 500);
  ...
  // ?ôÏ†Å Filter?êÏÑú Í∞?Ï∂îÏ∂ú (useMemoÎ°?Ï∞∏Ï°∞ ?àÏ†ï??
  ...
  // Î∞∞Ïó¥??Î¨∏Ïûê?¥Î°ú Î≥Ä?òÌïò???òÏ°¥??Î∞∞Ïó¥??Used
  ...
  // Ï¥àÎ?ÎßÅÌÅ¨ Event Ï≤òÎ¶¨ (MainLayout?êÏÑú ?ÑÎã¨Î∞õÏùå)
  useEffect(() => {
    const handleInvitationChange = (event: CustomEvent) => {
      ...
      if (
        ...
        // Ï¥àÎ?ÎßÅÌÅ¨ StatusÍ∞Ä Î≥ÄÍ≤ΩÎêòÎ©??ÑÏû¨ Ï¥àÎ? ?ïÎ≥¥Î•??§Ïãú Î°úÎìú
  ...
  useEffect(() => {
    ...
    loadTags();
    loadCurrentInvitation(); // Ï¥àÎ? Í∏∞Îä• Active??  }, []);
  ...
  // ?ôÏ†Å Filter ?∏Îì§??  const handleFilterAdd = (filter: ActiveFilter) => {
  ...
  // ?ºÍ¥Ñ Ï≤òÎ¶¨ ?∏Îì§??  const handleBulkAction = (
    actionType: 'status' | 'role' | 'tags' | 'emailVerified' | 'delete'
  ) => {
    // ?§Ìîà ??Í∞?Initialization (?¥Ï†Ñ???†ÌÉù??Í∞??†Ï? Î∞©Ï?)
  ...
  // ?ºÍ¥Ñ ?ëÏóÖ Save Î≤ÑÌäº Active??Ï°∞Í±¥ Confirm
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      // API ?§Î•ò Response?êÏÑú Íµ¨Ï≤¥?ÅÏù∏ Î©îÏãúÏßÄ Ï∂îÏ∂ú
      const errorMessage =
  ...
  // ?¥Î©î??Í∞ïÏ†ú Authentication Ï≤òÎ¶¨
  ...
  const handleAddUser = async () => {
    ...
    // Î∏åÎùº?∞Ï? ?êÎèô?ÑÏÑ±??Î∞©Ï??òÍ∏∞ ?ÑÌï¥ ?ΩÍ∞Ñ??ÏßÄ?????§Ïãú Initialization
    setTimeout(() => {
      setNewUserData({
        name: '',
        email: '',
  ...
  // Ï¥àÎ? Í¥Ä???∏Îì§??  const handleCreateInvitation = async (data: CreateInvitationRequest) => {
    try {
      ...
      setInvitationDialogOpen(false); // Ï¥àÎ? Form Close
      await loadCurrentInvitation(); // ?ÑÏû¨ Ï¥àÎ? ?ïÎ≥¥ Refresh
      // Success ?†Ïä§?∏Îäî SSE Event?êÏÑú Ï≤òÎ¶¨ (Ï§ëÎ≥µ Î∞©Ï?)
    } catch (error: any) {
      console.error('Failed to create invitation:', error);
      enqueueSnackbar(error.message || 'Ï¥àÎ? ÎßÅÌÅ¨ ?ùÏÑ±???§Ìå®?àÏäµ?àÎã§.', {
        variant: 'error',
      });
    }
  };
  ...
  const handleDeleteInvitation = async () => {
    ...
    try {
      ...
      enqueueSnackbar(error.message || 'Ï¥àÎ? ÎßÅÌÅ¨ ??†ú???§Ìå®?àÏäµ?àÎã§.', {
        variant: 'error',
      });
    }
  };
    ...
    <Box sx={{ px: 2, pb: 2, pt: 1.5 }}>
      ...
      >
        ...
        <Box sx={{ flex: 1, overflow: 'auto', p: 2 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            {/* ?¥Î©î??Authentication Status Î∞??°ÏÖò */}
            {editUserDialog.user && !isCurrentUser(editUserDialog.user) && (
              <Box
                sx={{
                  display: 'flex',
      ...
      >
        ...
        <Box sx={{ flex: 1, overflow: 'auto', p: 2 }}>
          ...
          {/* Í≥µÌÜµ ?Ä??ÎØ∏Î¶¨Î≥¥Í∏∞ (Delete ???°ÏÖò?êÎèÑ ?úÏãú) */}
