...
const MessageTemplatesPage: React.FC = () => {
  ...
  // Pagination
  const [page, setPage] = useState(0); // SimplePagination?Ä 0-based
  const [rowsPerPage, setRowsPerPage] = useGlobalPageSize();

  // Filter
  const [searchQuery, setSearchQuery] = useState('');

  // Debouncing??Search??(500ms ÏßÄ??
  const debouncedSearchQuery = useDebounce(searchQuery, 500);

  // ?ôÏ†Å Filter Status (localStorage?êÏÑú Î≥µÏõê)
  ...
  // FilterÎ•?Î¨∏Ïûê?¥Î°ú Î≥Ä?òÌïò???òÏ°¥??Î∞∞Ïó¥??Used
  ...
  // Form ?ÑÎìú ref??  const nameFieldRef = useRef<HTMLInputElement>(null);
  ...
  // ?òÏù¥ÏßÄ ?¨Í∏∞ Î≥ÄÍ≤??∏Îì§??  const handleRowsPerPageChange = useCallback(
  ...
  // ?†ÌÉù Í¥Ä???∏Îì§??  const handleSelectAll = useCallback(
    (checked: boolean) => {
  ...
  // ?ºÍ¥Ñ Used Í∞Ä??Î∂àÍ? Î≥ÄÍ≤?  const handleBulkToggleAvailability = useCallback(
  ...
  const handleSave = async (skipCr: boolean = false) => {
    ...
    try {
      ...
      // Handle duplicate name error - ??Í∞ÄÏßÄ ?§Î•ò Íµ¨Ï°∞ All Ï≤òÎ¶¨
    ...
    <Box sx={{ px: 2, pb: 2, pt: 1.5 }}>
      ...
      <Box sx={{ mb: 2 }}>
        ...
        >
          ...
          >
            {/* Search Ïª®Ìä∏Î°§ÏùÑ Îß??ûÏúºÎ°??¥Îèô?òÍ≥† Í∞úÏÑ† */}
            <SearchTextField
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <MultiLanguageMessageInput
              ...
              onLocalesChange={(locales) => {
                ...
                // Translation ResultsÍ∞Ä ?àÏúºÎ©??êÎèô?ºÎ°ú ?§Íµ≠??ÏßÄ??Active??                const hasNonEmptyLocales = locales.some(
