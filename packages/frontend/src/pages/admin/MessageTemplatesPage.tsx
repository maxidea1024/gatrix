...
const MessageTemplatesPage: React.FC = () => {
  ...
  // ?òÏù¥ÏßÄ?§Ïù¥??  const [page, setPage] = useState(0); // SimplePagination?Ä 0-based
  ...
  // ?ôÏ†Å ?ÑÌÑ∞ ?ÅÌÉú (localStorage?êÏÑú Î≥µÏõê)
  ...
  // ?ÑÌÑ∞Î•?Î¨∏Ïûê?¥Î°ú Î≥Ä?òÌïò???òÏ°¥??Î∞∞Ïó¥???¨Ïö©
  ...
  // ?†ÌÉù Í¥Ä??  const [selectedIds, setSelectedIds] = useState<number[]>([]);
  ...
  // ???ÑÎìú ref??  const nameFieldRef = useRef<HTMLInputElement>(null);
  ...
  // ?†ÌÉù Í¥Ä???∏Îì§??  const handleSelectAll = useCallback((checked: boolean) => {
  ...
  // ?ºÍ¥Ñ ?¨Ïö© Í∞Ä??Î∂àÍ? Î≥ÄÍ≤?  const handleBulkToggleAvailability = useCallback(async (isEnabled: boolean) => {
    if (selectedIds.length === 0) return;
  ...
  // ?úÍ∑∏ Í¥Ä???∏Îì§??  const handleOpenTagDialog = useCallback(async (template: MessageTemplate) => {
    try {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Handle duplicate name error - ??Í∞ÄÏßÄ ?§Î•ò Íµ¨Ï°∞ Î™®Îëê Ï≤òÎ¶¨
  ...
  return (
    <Box sx={{ p: 3 }}>
        ...
        <CardContent>
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'space-between' }}>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap', flex: 1 }}>
              {/* Í≤Ä??Ïª®Ìä∏Î°§ÏùÑ Îß??ûÏúºÎ°??¥Îèô?òÍ≥† Í∞úÏÑ† */}
              <TextField
      ...
      <ResizableDrawer
        ...
        zIndex={1300}
        onEntered={() => {
          // DrawerÍ∞Ä ?¥Î¶∞ ???¥Î¶Ñ ?ÑÎìú???¨Ïª§??          setTimeout(() => {
            nameFieldRef.current?.focus();
          }, 100);
        }}
      >
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <MultiLanguageMessageInput
              ...
              onLocalesChange={(locales) => {
                ...
                // Î≤àÏó≠ Í≤∞Í≥ºÍ∞Ä ?àÏúºÎ©??êÎèô?ºÎ°ú ?§Íµ≠??ÏßÄ???úÏÑ±??                const hasNonEmptyLocales = locales.some(l => l.message && l.message.trim() !== '');
            ...
            <TextField
              ...
              helperText="Î©îÏãúÏßÄ ?úÌîåÎ¶øÏóê ?ÅÏö©???úÍ∑∏Î•??†ÌÉù?òÏÑ∏??
            >
              {allTags.map((tag) => (
