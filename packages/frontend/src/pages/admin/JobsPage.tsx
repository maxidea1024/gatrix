...
const JobsPage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      enqueueSnackbar('?ëÏóÖ Î™©Î°ù??Î∂àÎü¨?§Îäî???§Ìå®?àÏäµ?àÎã§.', { variant: 'error' });
    } finally {
      setLoading(false);
    }
  ...
  // ?úÍ∑∏ ?ÑÌÑ∞ Î≥ÄÍ≤??∏Îì§??  const handleTagFilterChange = useCallback((tags: Tag[]) => {
    setTagFilter(tags);
    setPage(0);
  }, []);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      enqueueSnackbar('?ëÏóÖ ??†ú???§Ìå®?àÏäµ?àÎã§.', { variant: 'error' });
    }
  };
