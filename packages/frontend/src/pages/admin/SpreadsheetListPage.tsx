...
// ?Ä?Ä?Ä SpreadsheetRowMenu (list view context menu) ?Ä?Ä?Ä
...
const SpreadsheetListPage: React.FC = () => {
  ...
  useListRestoration(
    ...
    [loading, items] // Î°úÎî©???ùÎÇòÍ≥?itemsÍ∞Ä ?åÎçîÎß???ÏßÅÌõÑ ?§ÌÅ¨Î°?Î≥µÏõê
  );
  ...
  const handleTogglePin = useCallback(
    async (item: SpreadsheetListItem) => {
      ...
      // ?ôÍ????ÖÎç∞?¥Ìä∏(Optimistic Update): UI Ï¶âÏãú Í∞±Ïã† Î∞??ïÎ†¨
      setItems((prev) => {
      ...
      try {
        ...
        // ?úÎ≤Ñ?êÏÑú ?±Í≥µ?òÎ©¥ ?ïÎ†¨ ?†Ï?Î•??ÑÌï¥ Íµ≥Ïù¥ ?§Ïãú Î°úÎìú?òÏ? ?äÏùå
      } catch {
        // ?§Ìå® ??Î°§Î∞±
        setItems((prev) => {
          const updated = prev.map((i) =>
  ...
  const handleRenameConfirm = useCallback(async () => {
    ...
    // Ï¶âÏãú ?∏Ïßë Î™®Îìú ?¥Ï†ú (Ï§ëÎ≥µ ?∏Ï∂ú Î∞©Ï?)
