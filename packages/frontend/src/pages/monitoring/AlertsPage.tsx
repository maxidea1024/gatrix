...
const AlertsPage: React.FC = () => {
  ...
  const loadAlerts = useCallback(async () => {
    try {
      ...
      enqueueSnackbar(error.message || t('monitoring.alerts.loadFailed'), { variant: 'error' });
      setAlerts([]);
      setTotal(0);
    } finally {
      setLoading(false);
                    ...
                    {alerts.length === 0 ? (
                      ...
                      alerts.map((alert) => (
                        <TableRow key={alert.id} hover>
                          ...
                          <TableCell>{alert.startsAt ? new Date(alert.startsAt).toLocaleString() : '-'}</TableCell>
                          <TableCell>{alert.endsAt ? new Date(alert.endsAt).toLocaleString() : '-'}</TableCell>
                        </TableRow>
                      ))
                    )}
                  </TableBody>
