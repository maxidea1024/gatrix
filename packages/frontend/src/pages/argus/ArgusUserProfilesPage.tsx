...
const UserProfileDrawer: React.FC<UserProfileDrawerProps> = ({
  ...
  useEffect(() => {
    const timer = setTimeout(
      () => setPropertySearchDebounced(propertySearch),
      250
    );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getUserEvents(projectId, userId, { limit: 150 }).then((r) =>
        setEvents(r.data)
      ),
      getUserSessions(projectId, userId, { limit: SESSIONS_PAGE_SIZE }).then(
        (s) => {
          setSessions(s);
  ...
  const getEventIcon = (name: string) => {
    ...
    if (
      ...
      return (
        <LoginIcon sx={{ color: theme.palette.primary.main, fontSize: 18 }} />
      );
    }
  ...
  const drawerSubtitle = useMemo(() => {
    ...
    return t(
      'argus.userProfiles.drawerSubtitle',
        ...
        >
          ...
          <Box
            sx={{
              width: { xs: '100%', sm: 340 },
              borderRight: {
                sm: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`,
              },
              display: 'flex',
          ...
          >
              ...
              >
                {[
                  ...
                  >
                    ...
                    >
                      <Box sx={{ display: 'inline-flex', fontSize: 13 }}>
                        {card.icon}
                      </Box>
            ...
            <Box>
              ...
              >
                {t(
                  'argus.userProfiles.cohortsMembership',
                  'Cohorts Membership'
                )}
              </Typography>
              {userCohorts.length > 0 ? (
                ...
                <Typography variant="caption" color="text.secondary">
                  {t(
                    'argus.userProfiles.noCohortMembership',
                    'No cohort membership'
                  )}
                </Typography>
              )}
            </Box>
            ...
            >
              ...
              >
                ...
                >
                  {t(
                    'argus.userProfiles.userProperties',
              ...
              <TextField
                placeholder={t(
                  'argus.userProfiles.searchPropertiesPlaceholder',
                  'Search properties...'
                )}
                size="small"
              ...
              />
              <TableContainer
                sx={{ flex: 1, maxHeight: 350, overflowY: 'auto' }}
              >
                  ...
                  <TableBody>
                    {filteredProperties.map((p) => (
                      <TableRow key={p.key} hover>
                        ...
                        <TableCell
                          sx={{ p: 0.8, fontSize: 12, wordBreak: 'break-all' }}
                        >
                    ...
                    {filteredProperties.length === 0 && (
                      <TableRow>
                        <TableCell
                          colSpan={2}
                          align="center"
                          sx={{ py: 3, color: 'text.secondary', fontSize: 12 }}
                        >
                          {t(
                            'argus.userProfiles.noPropertiesFound',
                            'No properties found'
                          )}
                        </TableCell>
                      </TableRow>
          ...
          >
            ...
            >
              ...
              >
                {t(
                  'argus.userProfiles.engagementTrend',
                  'User Engagement Trend (Last 30 Days Event Frequency)'
                )}
              </Typography>
              <Box
                sx={{
              ...
              >
                {dailyActivity.map((d, idx) => (
                  <Tooltip
                    key={idx}
                    title={t(
                      'argus.userProfiles.engagementTrendTooltip',
                      '{{date}}: {{count}} events',
                      { date: d.date.toLocaleDateString(), count: d.count }
                    )}
                  >
                    <Box
                      sx={{
            ...
            <Box
              sx={{
                display: 'flex',
                alignItems: 'center',
                justifyComposite: 'space-between',
                px: 2,
                pt: 1,
                borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`,
              }}
            >
              ...
              >
                <Tab
                  label={`${t('argus.userProfiles.activityFeed', 'Activity Feed')} (${profile?.total_events ?? events.length})`}
                  icon={<EventIcon sx={{ fontSize: 14 }} />}
                  iconPosition="start"
                />
                <Tab
                  label={`${t('argus.userProfiles.sessionsLogs', 'Sessions Logs')} (${profile?.total_sessions ?? sessions.length})`}
                  icon={<SessionIcon sx={{ fontSize: 14 }} />}
                  iconPosition="start"
                />
                <Tab
                  label={`💰 ${t('argus.userProfiles.finance', 'Finance')}`}
                  icon={<PurchaseIcon sx={{ fontSize: 14 }} />}
                  iconPosition="start"
                />
              </Tabs>
              ...
              {rightTab === 0 && (
                <TextField
                  placeholder={t(
                    'argus.userProfiles.filterEventsPlaceholder',
                    'Filter events...'
                  )}
                  size="small"
                  value={eventSearch}
                ...
                (eventsLoading ? (
                  ...
                  >
                    <Typography variant="body2">
                      {t(
                        'argus.userProfiles.noEventsMatch',
                  ...
                  groupedEvents.map(([dateStr, items]) => (
                    <Box key={dateStr} sx={{ mb: 3.5 }}>
                      ...
                      {(expandedDates.has(dateStr)
                        ...
                        const isExpanded =
                          expandedEventId === `${evt.event_id}-${idx}`;
                        return (
                          <Accordion
                                ...
                                <Table size="small">
                                  <TableHead>
                                    <TableRow>
                                      ...
                                      >
                                        {t(
                                          'argus.userProfiles.attributeKey',
                                          'Attribute Key'
                                        )}
                                      </TableCell>
                                      <TableCell
                                      ...
                                      >
                                        {t(
                                          'argus.userProfiles.attributeValue',
                                          'Value'
                                        )}
                                      </TableCell>
                                    </TableRow>
                      ...
                      {!expandedDates.has(dateStr) && items.length > 5 && (
                        ...
                        >
                          {t(
                            'argus.userProfiles.showMoreEvents',
              ...
              {rightTab === 1 &&
                (sessions.length === 0 ? (
                  ...
                  >
                    <Typography variant="body2">
                      {t(
                        'argus.userProfiles.noSessionLogs',
                    ...
                    .map((s, i) => (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {s.duration_seconds === 0
                              ? t(
                                  'argus.userProfiles.sessionSingleEvent',
                                  'Single event'
                                )
                              : t(
                                  'argus.userProfiles.sessionDurationLabel',
                                  '{{duration}}',
                                  {
                        ...
                        >
                          <Typography variant="caption" color="text.secondary">
                            {t(
                              'argus.userProfiles.sessionEvents',
                              '{{total}} total ({{unique}} unique)',
                              { total: s.event_count, unique: s.unique_events }
                            )}
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                            ...
                            >
                              <Button
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    const more = await getUserSessions(
                                      projectId,
                                      userId,
                                      {
                                    ...
                                    setSessionsHasMore(
                                      more.length >= SESSIONS_PAGE_SIZE
                                    );
                                  } finally {
                              ...
                              >
                                {sessionsLoadingMore ? (
                                  ...
                                  t(
                                    'argus.userProfiles.loadMoreSessions',
                                    'Load more sessions'
                                  )
                                )}
                              </Button>
              ...
              {rightTab === 2 &&
                (() => {
                  ...
                  if (
                    ...
                    !finLoading &&
                    userId &&
                    finLoadedForUser !== userId
                  ) {
                    setFinLoading(true);
                    setFinLoadedForUser(userId);
                    getRevenueUserSummary(projectId, {
                      user_id: userId,
                      period: '90d',
                    })
                  ...
                  return finLoading ? (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {t(
                              'argus.userProfiles.totalPurchases',
                              'Purchases'
                            )}
                          </Typography>
                          <Typography
                          ...
                          <Typography
                            sx={{ fontSize: 10, color: 'text.secondary' }}
                          >
                            {finData.summary.purchase_count}{' '}
                            {t('argus.userProfiles.items', 'items')}
                          </Typography>
                        </Paper>
                        <Paper
                          variant="outlined"
                      ...
                      {(() => {
                        ...
                        return rows.length > 0 ? (
                          ...
                          >
                            {t(
                              'argus.userProfiles.noFinanceData',
                              'No records found'
                            )}
                          </Typography>
                        );
                    ...
                    >
                      {t(
                        'argus.userProfiles.financeLoadError',
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      if (result.data.length > 0) {
        getUserCohortMemberships(
          projectId,
          result.data.map((u: ArgusUserProfile) => u.user_id)
        )
      ...
      >
        ...
        >
          ...
          <Typography variant="body2" color="text.secondary">
            {t('argus.userProfiles.usersCount', '{{count}} users', {
              count: total,
            })}
          </Typography>
        ...
        >
          ...
          >
            <TableHead>
              <TableRow>
                <TableCell sx={{ fontWeight: 700, width: 280 }}>
                ...
                <TableCell sx={{ fontWeight: 700 }} align="right">
                  <TableSortLabel
                    ...
                    direction={
                      sortField === 'total_sessions' ? sortDir : 'desc'
                    }
