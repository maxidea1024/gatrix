...
import {
  Box, Typography, useTheme, alpha, Paper, LinearProgress, Tooltip,
} from '@mui/material';
import {
  FiberNew as NewIcon, Bolt as ActiveIcon, Replay as ReturningIcon,
  HotelClass as DormantIcon, PersonOff as ChurnedIcon, Autorenew as LifecycleIcon,
...
const ArgusLifecyclePage: React.FC = () => {
  ...
  const totalUsers = data ? data.stages.reduce((a, s) => a + s.user_count, 0) : 0;
    ...
    <Box>
      ...
      <PageContentLoader loading={loading}>
        {data ? (
          <>
            {/* Lifecycle Stages */}
            <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 3, mb: 3, bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fff' }}>
              <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 2 }}>{t('argus.lifecycle.lifecycleStages')}</Typography>
              {/* Stacked bar */}
              <Box sx={{ display: 'flex', height: 40, borderRadius: 2, overflow: 'hidden', mb: 2 }}>
                {data.stages.map((s) => {
                  const pct = totalUsers > 0 ? (s.user_count / totalUsers) * 100 : 0;
                  const cfg = STAGE_CONFIG[s.stage] || { color: '#999', icon: <></>, labelKey: s.stage };
                  return pct > 0 ? (
                    <Tooltip key={s.stage} title={`${t(cfg.labelKey)}: ${s.user_count.toLocaleString()} (${pct.toFixed(1)}%)`}>
                      <Box sx={{ width: `${pct}%`, bgcolor: cfg.color, transition: 'width 0.3s', minWidth: 4 }} />
                    </Tooltip>
                  ) : null;
                })}
              </Box>
              ...
              <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                {data.stages.map((s) => {
                  const cfg = STAGE_CONFIG[s.stage] || { color: '#999', icon: <></>, labelKey: s.stage };
                  const pct = totalUsers > 0 ? ((s.user_count / totalUsers) * 100).toFixed(1) : '0';
                  return (
                    <Box key={s.stage} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Box sx={{ width: 12, height: 12, borderRadius: '50%', bgcolor: cfg.color }} />
                      <Typography variant="body2" fontWeight={600} sx={{ color: cfg.color }}>{t(cfg.labelKey)}</Typography>
                      <Typography variant="body2" color="text.secondary">{s.user_count.toLocaleString()} ({pct}%)</Typography>
                    </Box>
                  );
                })}
              </Box>
            ...
            {/* Engagement Metrics */}
            <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 3, bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fff' }}>
              <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 2 }}>{t('argus.lifecycle.engagementMetrics')}</Typography>
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 2 }}>
                {[
                  ...
                  { label: t('argus.lifecycle.stickiness'), value: `${data.stickiness}%`, isPercent: true },
                ].map((m) => (
                  <Paper key={m.label} variant="outlined" sx={{ p: 2, borderRadius: 2, textAlign: 'center', bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fafafa' }}>
