import type { RevenueData, TopSpendersData, ProductRevenue } from '../../services/argus/argusAnalytics';
...
export interface InsightDrilldown {
  ...
  ledgerFilter?: {           // ledger: transaction filter
    type?: string;
    reason?: string;
  };
}
...
type TFunc = (key: string, defaultValue: string, opts?: Record<string, any>) => string;
...
export function generateInsights(
  ...
  if (data.revenue_by_country.length > 0) {
    ...
    const topPct = data.total_revenue > 0 ? (topCountry.revenue / data.total_revenue) * 100 : 0;
    if (topPct > 40) {
      insights.push({
        severity: 'warning',
        icon: '🌍',
  ...
  if (data.revenue_by_platform.length > 1 && data.prev_revenue_by_platform?.length > 0) {
    for (const plat of data.revenue_by_platform) {
      const prev = data.prev_revenue_by_platform.find(p => p.platform === plat.platform);
      if (prev) {
  ...
  if (data.prev_refund_count > 0 && data.refund_count > 0) {
    const prevRefundRate = data.prev_total_refunds > 0 ? (data.prev_refund_count / data.prev_total_transactions) * 100 : 0;
  ...
  if (data.total_granted > 0 && data.total_revenue > 0) {
    const grantRatio = (data.total_granted / (data.total_revenue + data.total_granted)) * 100;
    if (grantRatio > 30) {
      insights.push({
        severity: 'warning',
        icon: '🎁',
  ...
  if (data.revenue_over_time.length >= 14) {
    ...
    const firstAvg = firstHalf.reduce((s, d) => s + (d.arpdau || 0), 0) / firstHalf.length;
    const secondAvg = secondHalf.reduce((s, d) => s + (d.arpdau || 0), 0) / secondHalf.length;
  ...
  if (data.total_ad_revenue > 0 && data.prev_total_ad_revenue > 0 && data.total_revenue > 0) {
    const prevBlended = data.prev_total_revenue + data.prev_total_ad_revenue;
    const prevIapShare = prevBlended > 0 ? (data.prev_total_revenue / prevBlended) * 100 : 100;
