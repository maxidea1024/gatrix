...
export const DsnKeysSettings: React.FC<DsnKeysSettingsProps> = ({
  ...
  const [showActiveKeysOnly, setShowActiveKeysOnly] = useLocalStorage(
    'argus-dsn-active-only',
    false
  );
  ...
  const handleCreateKey = async () => {
    ...
    try {
      ...
      enqueueSnackbar(t('argus.settings.keyCreateFailed'), {
        variant: 'error',
      });
    }
  };
  ...
  const handleToggleActive = async (keyId: number, isActive: boolean) => {
    try {
      ...
      enqueueSnackbar(t('argus.settings.keyRevokeFailed'), {
        variant: 'error',
      });
    }
  };
  ...
  const handleHardDeleteKey = async (keyId: number) => {
    try {
      ...
      setProject((prev) =>
        prev
          ? { ...prev, dsn_keys: prev.dsn_keys?.filter((k) => k.id !== keyId) }
          : prev
      );
    } catch {
  ...
  const handleSaveKeyEdit = async () => {
    ...
    try {
      await argusService.updateDsnKey(projectId, id, {
        label: label.trim(),
        rate_limit_count,
        rate_limit_window,
      });
      ...
      enqueueSnackbar(
        t('common.saveFailed', { defaultValue: 'Failed to save' }),
        { variant: 'error' }
      );
    } finally {
    ...
    <PageContentLoader loading={!project}>
      ...
      >
        {!project?.dsn_keys?.length ? (
          ...
          (() => {
            const filteredKeys = showActiveKeysOnly
              ? project.dsn_keys.filter((k) => k.is_active)
              : project.dsn_keys;
            return (
                  ...
                  <FormControlLabel
                    ...
                    label={
                      ...
                      >
                        {showActiveKeysOnly
                          ? t(
                              'argus.settings.showingActiveOnly',
                              'Showing Active Only'
                            )
                          : t('argus.settings.activeOnly', 'Active Only')}
                      </Typography>
                    }
                  />
                </Box>
                    ...
                    >
                        ...
                        {key.is_active ? (
                          ...
                          >
                            <IconButton
                              size="small"
                              color="warning"
                              onClick={() => handleToggleActive(key.id, true)}
                            >
                          ...
                          <Tooltip title={t('common.delete', 'Delete')}>
                            <IconButton
                              size="small"
                              color="error"
                              onClick={() => handleHardDeleteKey(key.id)}
                            >
                      ...
                      <Box sx={{ display: 'flex', gap: 2, mt: 1, px: 0.5 }}>
                        <Typography
                          ...
                          sx={{ fontSize: '0.65rem' }}
                        >
                          {t('argus.settings.firstSeen', 'First Seen')}:{' '}
                          {key.first_seen
                            ? new Date(key.first_seen).toLocaleString()
                        ...
                        <Typography
                          ...
                          sx={{ fontSize: '0.65rem' }}
                        >
                          {t('argus.settings.lastSeen', 'Last Seen')}:{' '}
                          {key.last_seen
          ...
          <Box>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <TextField
                size="small"
                type="number"
                label={t('argus.settings.rateLimitCount', 'Count')}
                value={newKeyRateCount}
                onChange={(e) =>
                  setNewKeyRateCount(parseInt(e.target.value) || 0)
                }
                inputProps={{ min: 0 }}
                sx={{
                  flex: 1,
              ...
              <TextField
                size="small"
                type="number"
                label={t('argus.settings.rateLimitWindow', 'Window (s)')}
                value={newKeyRateWindow}
                onChange={(e) =>
                  setNewKeyRateWindow(parseInt(e.target.value) || 0)
                }
                inputProps={{ min: 0 }}
                sx={{
                  flex: 1,
      ...
      >
                ...
                onChange={(e) =>
                  setEditKeyDialog((prev) =>
                    prev
                      ? {
                          ...prev,
                          rate_limit_count: parseInt(e.target.value) || 0,
                        }
                      : prev
                  )
                }
                ...
                onChange={(e) =>
                  setEditKeyDialog((prev) =>
                    prev
                      ? {
                          ...prev,
                          rate_limit_window: parseInt(e.target.value) || 0,
                        }
                      : prev
                  )
                }
        ...
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
          {(() => {
            const originalKey = project?.dsn_keys?.find(
              (k) => k.id === editKeyDialog?.id
            );
            const hasChanges =
              !!originalKey &&
              !!editKeyDialog &&
              (originalKey.label !== editKeyDialog.label ||
                (originalKey.rate_limit_count ?? 0) !==
                  editKeyDialog.rate_limit_count ||
                (originalKey.rate_limit_window ?? 0) !==
                  editKeyDialog.rate_limit_window);
            return (
              <>
                <Button
                  onClick={() => setEditKeyDialog(null)}
                  sx={{
                    textTransform: 'none',
                    fontWeight: 600,
                    borderRadius: '8px',
                ...
                <Button
                  ...
                  disabled={editKeySaving || !hasChanges}
                  startIcon={
                    editKeySaving ? (
