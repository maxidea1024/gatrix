...
import { Box, Button, Paper, Typography, Tooltip, IconButton } from '@mui/material';
...
export const SourceMapsSettings: React.FC<SourceMapsSettingsProps> = ({
  ...
  useEffect(() => {
    argusService.listSourcemapReleases(projectId)
      .then(d => { setSourcemaps(d); setSmLoaded(true); })
            ...
            <Paper key={rel.id} elevation={0} sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 2, border: `1px solid ${bdr}`, borderRadius: '8px' }}>
              ...
              <Tooltip title={t('common.delete')}><IconButton size="small" color="error" onClick={async () => {
                try {
                  ...
                  enqueueSnackbar(t('argus.settings.smDeleteSuccess'), { variant: 'success' });
