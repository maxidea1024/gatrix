...
/** Field Block — Sentry style (Label+Desc above, Input below, consistent width) */
export const FieldBlock: React.FC<{
  ...
  >
    ...
    <Typography
      variant="body2"
      color="text.secondary"
      sx={{ fontSize: '0.78rem', mb: 1.5, lineHeight: 1.5 }}
    >
      {desc}
    </Typography>
    {children}
...
/** Provider Card (for marketplace grid) */
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        '&:hover': {
          borderColor: alpha(prov.color, 0.5),
          transform: 'translateY(-1px)',
          boxShadow: `0 4px 16px ${alpha(prov.color, 0.1)}`,
        },
      }}
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
        <Avatar
          sx={{
            width: 36,
            height: 36,
            backgroundColor: alpha(prov.color, isDark ? 0.2 : 0.08),
            color: prov.color,
        ...
        <Box>
          ...
          {count > 0 && (
            <Chip
              label={`${count} ${t('argus.settings.configured')}`}
              size="small"
              sx={{
                height: 18,
                fontSize: '0.58rem',
      ...
      <Typography
        variant="caption"
        color="text.secondary"
        sx={{ fontSize: '0.73rem', lineHeight: 1.5, flex: 1 }}
      >
        {t(prov.descKey)}
      </Typography>
      <Button
...
/** Connected Item (Common for Integration / Tracker) */
      ...
      >
        ...
        {onToggle ? (
          <Tooltip
            title={
              active
                ? t('argus.settings.deactivateKey', 'Deactivate')
                : t('common.active', 'Activate')
            }
          >
            <Chip
...
/** Config Dialog (Common for Integration + Tracker) */
...
/** Sampling Rate Progress Bar — drag with mouse to set */
export const RateBar: React.FC<{
  value: number;
...
export const StatusBadge: React.FC<{ active: boolean; t: any }> = ({
  active,
  t,
}) => (
  <Chip
...
export const EmptyState: React.FC<{
  ...
  <Box sx={{ py: 5, textAlign: 'center' }}>
    <Box
      sx={{
        mb: 1.5,
        '& .MuiSvgIcon-root': { fontSize: 44, color: 'text.disabled' },
      }}
    >
      {icon}
    </Box>
