...
import { Chat as ChatIcon, Webhook as WebhookIcon, Email as EmailIcon, Notifications as NotificationsIcon } from '@mui/icons-material';
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 2 }}>
          {NOTIFICATION_PROVIDERS.map(prov => (
            <ProviderCard key={prov.id} prov={prov} isDark={isDark} t={t}
              ...
              onAdd={() => { setAddNotifDialog(prov.id); }}
            />
          ))}
        </Box>
      </SettingsCard>
