...
import { GitHub as GitHubIcon, BugReport as BugIcon } from '@mui/icons-material';
import { useSnackbar } from 'notistack';
import { SettingsCard, ProviderCard, ConnectedItem, Spinner } from './components/SettingsShared';
            ...
            {trackers.map(trk => {
              ...
              return (
                <ConnectedItem key={trk.id} isDark={isDark} color={prov?.color || '#666'} icon={prov?.icon || <BugIcon sx={{ fontSize: 18 }} />}
                  ...
                  onToggle={async () => {
                    await argusService.updateIssueTracker(projectId, trk.id, { enabled: !trk.enabled });
                    await loadTrackers();
                  }}
                  onTest={async () => {
                    try {
                      const r = await argusService.testIssueTracker(projectId, trk.id);
                      enqueueSnackbar(r.ok ? r.message : `Failed: ${r.message}`, { variant: r.ok ? 'success' : 'error' });
        ...
        return (
          <ProviderWizardModal
            ...
            onSubmit={async (data) => {
              ...
              tp.configFields.forEach(f => { if (data[f.key]) config[f.key] = data[f.key]; });
