...
import {
  ...
  useTheme, alpha, CircularProgress, Popover,
...
import {
  ...
  Timeline as TraceIcon, ExpandMore as ExpandMoreIcon,
  FilterList as FilterIcon,
  ArrowDownward as SortDescIcon, ArrowUpward as SortAscIcon,
  ViewColumn as ViewIcon, Terminal as LogsIcon,
...
const ArgusTraceExplorerPage: React.FC = () => {
  ...
  const [tags, setTags] = useState<{ op: any[]; status: any[]; domain: any[] }>({ op: [], status: [], domain: [] });

  // Aggregates
  const [aggData, setAggData] = useState<{
    groupBy: string;
    topValues: { group_value: string; count: number; avg_duration?: number; p95_duration?: number }[];
  ...
  const fetchSpans = useCallback(async () => {
    ...
    try {
      ...
      setSpans(data);
    } catch (err) { console.error('Failed to search spans', err); }
  ...
  const fetchTraceSamples = useCallback(async () => {
    ...
    } catch (err) { console.error('Failed to get trace samples', err); }
    finally { setLoading(false); }
  ...
  const fetchVolume = useCallback(async () => {
    try {
      ...
      setVolume(data);
    } catch (err) { console.error('Failed to fetch span volume', err); }
  }, [projectId, filters, currentPeriod, search]);
  ...
  const fetchTags = useCallback(async () => {
    try {
      ...
      setTags(data);
    } catch (err) { console.error('Failed to fetch span tags', err); }
  ...
  const fetchAggregates = useCallback(async (groupByVal?: string) => {
    ...
    try {
      ...
      setAggData(data);
    } catch (err) { console.error('Failed to fetch span aggregates', err); }
  ...
  const handleSaveQuery = async () => {
    ...
    try {
      const res = await argusService.createSavedQuery(projectId, {
        name: saveName.trim(),
        query_config: { search, period: currentPeriod, tab: activeTab, groupBy: aggGroupBy },
        display_type: 'table',
        query_type: 'spans' as any,
      });
      const updated = await argusService.listSavedQueries(projectId, 'spans' as any);
      setSavedQueries(updated);
      setQueryName(saveName.trim());
      if (res.id) setCurrentQueryId(res.id);
      setSaveDialogOpen(false);
      setSaveName('');
    } catch (err) { console.error('Failed to save trace query:', err); }
  };
  ...
  const SPAN_COLUMNS = ['timestamp', 'op', 'description', 'duration', 'status', 'trace_id'];
    ...
    <PageContentLoader loading={loading} skeleton={<TableSkeleton />}>
      ...
      <Table size="small" sx={{ '& td, & th': { borderColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)' } }}>
        <TableHead>
          <TableRow>
            {SPAN_COLUMNS.map(col => (
              <TableCell
                key={col}
                onClick={() => ['duration', 'timestamp'].includes(col) ? handleColumnSort(col) : undefined}
                sx={{
                  fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase',
                  cursor: ['duration', 'timestamp'].includes(col) ? 'pointer' : 'default',
                  userSelect: 'none', py: 1,
                  color: orderCol === col ? theme.palette.primary.main : 'text.secondary',
                }}
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.3 }}>
                  {col === 'trace_id' ? 'TRACE' : col.toUpperCase()}
                  {orderCol === col && (orderDir === 'desc'
          ...
          {spans.map((span, idx) => (
            <TableRow key={idx} hover sx={{
              ...
              <TableCell sx={{ py: 0.8, maxWidth: 300 }}>
                <Typography sx={{ fontSize: '0.73rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {span.description || '—'}
                </Typography>
              </TableCell>
              <TableCell sx={{ py: 0.8 }}>
              ...
              <TableCell sx={{ py: 0.8 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  ...
                  >
                    {span.trace_id ? String(span.trace_id).slice(0, 12) + '…' : '—'}
                  </Typography>
                  {span.trace_id && (
                    <>
                      <Tooltip title={t('argus.traces.viewLogs', 'View Logs')}>
                        <IconButton size="small" onClick={(e) => { e.stopPropagation(); navigate(`/argus/explore/logs?q=trace_id:"${span.trace_id}"`); }} sx={{ p: 0.2 }}>
                          <LogsIcon sx={{ fontSize: 12, color: 'text.disabled' }} />
                        </IconButton>
                      </Tooltip>
                      <CopyButton text={span.trace_id} size={12} sx={{ p: 0.2 }} />
                    </>
                  )}
                </Box>
              </TableCell>
        ...
        <Box sx={{ py: 8, textAlign: 'center' }}>
          <TraceIcon sx={{ fontSize: 48, color: alpha(theme.palette.primary.main, 0.15), mb: 1 }} />
          ...
          <Typography color="text.disabled" sx={{ fontSize: '0.8rem' }}>
            {t('argus.traces.noSpansDesc', 'Try adjusting your filters or time range.')}
          </Typography>
        </Box>
      ) : null}
    </PageContentLoader>
  ), [loading, spans, orderCol, orderDir, theme, isDark, t, handleColumnSort, navigate]);
    ...
    <Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Box
              ...
              sx={{
                ...
                px: 1, py: 0.2, borderRadius: '6px', minWidth: 400, minHeight: 30,
            ...
            >
              ...
              <Box component="input" value={search}
                ...
                style={{
                  ...
                  color: 'inherit', fontFamily: 'inherit', fontSize: '0.85rem', fontWeight: 500, minWidth: 100, padding: '6px 8px'
                }}
              />
              {search && (
            ...
            >
              <Box sx={{ p: 1 }}>
                ...
                {tags.status.length > 0 && (
                  <>
                    <Typography variant="caption" sx={{ px: 1, color: 'text.disabled', fontWeight: 600, display: 'block', mt: 1, mb: 0.5 }}>
          ...
          <PageContentLoader loading={loading} skeleton={<TableSkeleton />}>
            {traceSamples.length > 0 ? (
              <Table size="small" sx={{ '& td, & th': { borderColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)' } }}>
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>TRACE ID</TableCell>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>{t('argus.traces.startTime', 'START TIME')}</TableCell>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>{t('argus.traces.spanCount', 'SPANS')}</TableCell>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>{t('argus.traces.totalDuration', 'TOTAL DURATION')}</TableCell>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>{t('argus.traces.operations', 'OPERATIONS')}</TableCell>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>{t('argus.traces.errors', 'ERRORS')}</TableCell>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1, width: 40 }} />
                  </TableRow>
                </TableHead>
                <TableBody>
                  {traceSamples.map((trace, idx) => {
                    const ops: string[] = Array.isArray(trace.operations) ? trace.operations : [];
                    return (
                      <TableRow key={idx} hover sx={{
                        cursor: 'pointer',
                        '&:hover': { backgroundColor: alpha(theme.palette.primary.main, 0.02) },
                      ...
                      >
                        ...
                        <TableCell sx={{ py: 0.8 }}>
                          <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                            ...
                            {ops.length > 4 && (
                              <Chip label={`+${ops.length - 4}`} size="small" sx={{
                                height: 18, fontSize: '0.65rem', borderRadius: '3px',
                              }} />
                            )}
                          </Box>
                        </TableCell>
                        ...
                        <TableCell sx={{ py: 0.8 }}>
                          <Tooltip title={t('argus.traces.viewLogs', 'View Logs')}>
                            <IconButton size="small" onClick={(e) => { e.stopPropagation(); navigate(`/argus/explore/logs?q=trace_id:"${trace.trace_id}"`); }} sx={{ p: 0.3 }}>
                              <LogsIcon sx={{ fontSize: 14, color: 'text.disabled' }} />
                            </IconButton>
                          </Tooltip>
                        </TableCell>
                      </TableRow>
              ...
              <Box sx={{ py: 8, textAlign: 'center' }}>
                ...
                <Typography color="text.disabled" sx={{ fontSize: '0.8rem' }}>
                  {t('argus.traces.noTracesDesc', 'Try adjusting your search or time range.')}
                </Typography>
              </Box>
            ) : null}
          </PageContentLoader>
      ...
      {activeTab === 2 && (
        <Paper elevation={0} sx={{
          ...
          <Box sx={{ px: 2, py: 1.5, display: 'flex', alignItems: 'center', gap: 1, borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}` }}>
            ...
            <FormControl size="small" sx={{ minWidth: 120 }}>
              ...
              >
                <MenuItem value="op" sx={{ fontSize: '0.75rem' }}>Operation (op)</MenuItem>
                <MenuItem value="status" sx={{ fontSize: '0.75rem' }}>Status</MenuItem>
                <MenuItem value="domain" sx={{ fontSize: '0.75rem' }}>Domain</MenuItem>
                <MenuItem value="action" sx={{ fontSize: '0.75rem' }}>Action</MenuItem>
              </Select>
            </FormControl>
            <Button size="small" variant="outlined" onClick={() => fetchAggregates()} disabled={aggLoading}
              sx={{ textTransform: 'none', fontSize: '0.72rem', ml: 'auto', borderRadius: '6px' }}>
              {aggLoading ? <CircularProgress size={14} /> : t('argus.traces.runAgg', 'Run')}
            </Button>
          </Box>
          ...
          <PageContentLoader loading={aggLoading} skeleton={<TableSkeleton />}>
            {aggData && aggData.topValues.length > 0 ? (
              <Table size="small" sx={{ '& td, & th': { borderColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)' } }}>
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>{aggGroupBy.toUpperCase()}</TableCell>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>{t('argus.traces.count', 'COUNT')}</TableCell>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>{t('argus.traces.avgDuration', 'AVG DURATION')}</TableCell>
                    <TableCell sx={{ fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase', py: 1 }}>P95</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
              ...
              <Box sx={{ py: 8, textAlign: 'center' }}>
                ...
                <Typography color="text.disabled" sx={{ fontSize: '0.8rem' }}>
                  {t('argus.traces.aggregatesDesc', 'Group spans by operation, status, or domain to find patterns.')}
                </Typography>
              </Box>
            ) : null}
          </PageContentLoader>
