...
import {
  ...
  useTheme, alpha, Tooltip, Divider, Collapse, InputAdornment, Checkbox, Autocomplete,
} from '@mui/material';
import {
  ...
  LocalOffer as TagIcon, Search as SearchIcon, ContentCopy as DuplicateIcon,
  PriorityHigh as PriorityIcon, FilterList as FilterIcon,
  DragIndicator as DragIcon, VolumeOff as MuteIcon, Schedule as ScheduleIcon,
  Chat as SlackIcon, ShowChart as MetricIcon,
  Assignment as JiraIcon, ViewKanban as LinearIcon, NotificationsActive as PagerDutyIcon,
...
const getConditionTypes = (t: (key: string) => string): { value: ArgusAlertCondition['type']; label: string; desc: string; icon: React.ReactElement; color: string }[] => [
  { value: 'new_issue', label: t('argus.alerts.condNewIssue'), desc: t('argus.alerts.condNewIssueDesc'), icon: <BugIcon sx={{ fontSize: 18 }} />, color: '#f44336' },
  { value: 'event_frequency', label: t('argus.alerts.condEventFreq'), desc: t('argus.alerts.condEventFreqDesc'), icon: <FrequencyIcon sx={{ fontSize: 18 }} />, color: '#ff9800' },
  { value: 'user_count', label: t('argus.alerts.condUserCount'), desc: t('argus.alerts.condUserCountDesc'), icon: <UsersIcon sx={{ fontSize: 18 }} />, color: '#2196f3' },
  { value: 'regression', label: t('argus.alerts.condRegression'), desc: t('argus.alerts.condRegressionDesc'), icon: <RegressionIcon sx={{ fontSize: 18 }} />, color: '#9c27b0' },
  { value: 'new_feedback', label: t('argus.alerts.condNewFeedback'), desc: t('argus.alerts.condNewFeedbackDesc'), icon: <FeedbackIcon sx={{ fontSize: 18 }} />, color: '#00bcd4' },
  { value: 'property_match', label: t('argus.alerts.condPropMatch') || 'Event Property Match', desc: t('argus.alerts.condPropMatchDesc') || 'Matches an event property (platform, url, etc.)', icon: <FilterIcon sx={{ fontSize: 18 }} />, color: '#3f51b5' },
  { value: 'high_priority_issue', label: t('argus.alerts.condHighPriority'), desc: t('argus.alerts.condHighPriorityDesc'), icon: <PriorityIcon sx={{ fontSize: 18 }} />, color: '#e91e63' },
  { value: 'project_error_rate', label: t('argus.alerts.condMetric') || 'Project Error Rate', desc: t('argus.alerts.condMetricDesc') || 'Global error rate exceeds threshold', icon: <MetricIcon sx={{ fontSize: 18 }} />, color: '#f44336' },
];
...
const getIntervals = (t: (key: string) => string) => [
  { value: 60, label: t('argus.alerts.1min') }, { value: 300, label: t('argus.alerts.5min') },
  { value: 900, label: t('argus.alerts.15min') }, { value: 1800, label: t('argus.alerts.30min') },
  { value: 3600, label: t('argus.alerts.1hour') }, { value: 86400, label: t('argus.alerts.1day') },
];

const getFrequencies = (t: (key: string) => string) => [
  { value: 60, label: t('argus.alerts.1min') }, { value: 300, label: t('argus.alerts.5min') },
  { value: 900, label: t('argus.alerts.15min') }, { value: 3600, label: t('argus.alerts.1hour') },
...
const ConditionCard: React.FC<{
  id: string; cond: ArgusAlertCondition; isDark: boolean;
  ...
  const needsThreshold = ['event_frequency', 'user_count', 'project_error_rate'].includes(cond.type);
  ...
  return (
    <Box ref={setNodeRef} style={style} sx={{
      ...
      <Box {...attributes} {...listeners} sx={{
        cursor: 'grab', mt: 1, color: 'text.disabled', '&:hover': { color: 'text.primary' }
      }}>
      ...
      <Box sx={{ flex: 1 }}>
        ...
        {needsThreshold && (
          <Box sx={{ display: 'flex', gap: 1 }}>
            <TextField size="small" type="number" label={t('argus.alerts.threshold')}
              value={cond.value || 10}
              onChange={(e) => onChange({ ...cond, value: Number(e.target.value) })}
              sx={{ width: 100, '& .MuiOutlinedInput-root': { fontSize: '0.82rem' } }}
              InputProps={{ startAdornment: <InputAdornment position="start" sx={{ '& .MuiTypography-root': { fontSize: '0.72rem' } }}>{isMetric ? '%' : '≥'}</InputAdornment> }}
            />
            <FormControl size="small" sx={{ minWidth: 120 }}>
              <InputLabel sx={{ fontSize: '0.75rem' }}>{t('argus.alerts.interval')}</InputLabel>
              <Select value={cond.interval || 3600} label={t('argus.alerts.interval')}
        ...
        {cond.type === 'property_match' && (
          <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
            <FormControl size="small" sx={{ minWidth: 110 }}>
              <Select value={cond.property || 'platform'} onChange={(e) => onChange({ ...cond, property: e.target.value })}
            ...
            <TextField size="small" sx={{ flex: 1, '& .MuiOutlinedInput-root': { fontSize: '0.82rem' } }}
...
const ActionCard: React.FC<{
  id: string; action: ArgusAlertAction; isDark: boolean;
  ...
  return (
    <Box ref={setNodeRef} style={style} sx={{
      ...
      <Box sx={{ flex: 1 }}>
        <FormControl size="small" fullWidth sx={{ mb: action.type !== 'slack' ? 1 : 0 }}>
        ...
        {(action.type === 'webhook' || action.type === 'email' || action.type === 'slack' || action.type === 'jira' || action.type === 'linear' || action.type === 'pagerduty') && (
          action.type === 'slack' ? (
            <TextField size="small" fullWidth placeholder="e.g. #general, #alerts" value={action.channel || ''}
            ...
            <TextField size="small" fullWidth placeholder="Project Key (e.g. ENG, GAT)" value={action.channel || ''}
              onChange={(e) => onChange({ ...action, channel: e.target.value })}
              sx={{ '& .MuiOutlinedInput-root': { fontSize: '0.82rem' } }} />
          ) : action.type === 'pagerduty' ? (
            <TextField size="small" fullWidth placeholder="Integration Key" value={action.target_url || ''}
              onChange={(e) => onChange({ ...action, target_url: e.target.value })}
              sx={{ '& .MuiOutlinedInput-root': { fontSize: '0.82rem' } }} />
          ) : (
...
const ArgusAlertRulesPage: React.FC<ArgusAlertRulesPageProps> = ({ projectId: propProjectId }) => {
  ...
  const [stats, setStats] = useState<{ rule_id: number; bucket: string; count: number }[]>([]);
  ...
  const [formConditionLogic, setFormConditionLogic] = useState<'any' | 'all'>('any');
  ...
  // Mute dialog
  const [muteDialogRule, setMuteDialogRule] = useState<ArgusAlertRule | null>(null);
  ...
  useEffect(() => {
    if (!projectId) return;
    argusService.getFilterOptions(projectId)
    ...
    try {
      await Promise.all(
        Array.from(selectedRules).map(id =>
          argusService.updateAlertRule(projectId, id, { enabled: enable } as any)
        )
      );
      fetchRules();
      setSelectedRules(new Set());
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        Array.from(selectedRules).map(id => argusService.deleteAlertRule(projectId, id))
      );
      fetchRules();
      setSelectedRules(new Set());
  ...
  const handleMute = async () => {
    ...
    try {
      const mutedUntil = new Date(Date.now() + muteDuration * 1000).toISOString();
      await argusService.updateAlertRule(projectId, muteDialogRule.id, { muted_until: mutedUntil } as any);
      fetchRules();
  ...
  const handleConditionDragEnd = (event: DragEndEvent) => {
    ...
    const oldIndex = formConditions.findIndex((_, i) => `cond-${i}` === active.id);
    const newIndex = formConditions.findIndex((_, i) => `cond-${i}` === over.id);
  ...
  const openCreate = () => {
    setEditingRule(null);
    setFormName(''); setFormDescription(''); setFormConditions([{ type: 'new_issue' }]);
    setFormActions([{ type: 'webhook', target_url: '' }]);
    setFormFrequency(300); setFormEnvironment(''); setFormLevel('');
    setFormTags({}); setFormConditionLogic('any'); setNewTagKey(''); setNewTagValue('');
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    // Validation
    if (!formName.trim()) {
      enqueueSnackbar(t('argus.alerts.validationNameRequired'), { variant: 'warning' });
      return;
    }
    if (formConditions.length === 0) {
      enqueueSnackbar(t('argus.alerts.validationConditionRequired'), { variant: 'warning' });
      return;
    }
    if (formActions.length === 0) {
      enqueueSnackbar(t('argus.alerts.validationActionRequired'), { variant: 'warning' });
      return;
    }
    // Validate actions
    const invalidWebhook = formActions.find(a => a.type === 'webhook' && (!a.target_url || !a.target_url.startsWith('http')));
    if (invalidWebhook) {
      enqueueSnackbar(t('argus.alerts.validationWebhookUrl') || 'Invalid Webhook URL', { variant: 'warning' });
      return;
    }
    const invalidSlack = formActions.find(a => (a.type === 'slack' || a.type === 'jira' || a.type === 'linear') && !a.channel?.trim());
    if (invalidSlack) {
      enqueueSnackbar('Channel or Project Key is required', { variant: 'warning' });
      return;
    }
    const invalidEmail = formActions.find(a => a.type === 'email' && (!a.target_url || !a.target_url.includes('@')));
    if (invalidEmail) {
      enqueueSnackbar('Valid email address is required', { variant: 'warning' });
      return;
    }
    const invalidPagerDuty = formActions.find(a => a.type === 'pagerduty' && !a.target_url?.trim());
    if (invalidPagerDuty) {
      enqueueSnackbar('Integration Key is required for PagerDuty', { variant: 'warning' });
      return;
    }
    try {
      if (editingRule) {
        await argusService.updateAlertRule(projectId, editingRule.id, {
          ...
          conditions: formConditions, actions: formActions, frequency: formFrequency,
  ...
  return (
    <Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            ...
            {rules.length > 3 && (
              <TextField
                ...
                sx={{ mb: 0.5, '& .MuiOutlinedInput-root': { fontSize: '0.82rem', borderRadius: 1.5 } }}
              />
            )}
            ...
            {selectedRules.size > 0 && (
              <Paper elevation={0} sx={{
                display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 0.8, borderRadius: 1.5,
                backgroundColor: alpha('#ff9800', 0.06), border: `1px solid ${alpha('#ff9800', 0.15)}`,
              }}>
                <Checkbox size="small" checked={selectedRules.size === filteredRules.length} indeterminate={selectedRules.size > 0 && selectedRules.size < filteredRules.length}
                  onChange={toggleSelectAll} color="warning" />
                <Typography sx={{ fontSize: '0.78rem', fontWeight: 600, flex: 1 }}>
                  {t('argus.alerts.selectedCount', { count: selectedRules.size })}
                </Typography>
                <Button size="small" onClick={() => handleBulkToggle(true)} sx={{ textTransform: 'none', fontSize: '0.72rem' }}>
                  {t('argus.alerts.enableAll')}
                </Button>
                <Button size="small" onClick={() => handleBulkToggle(false)} sx={{ textTransform: 'none', fontSize: '0.72rem' }}>
                  {t('argus.alerts.disableAll')}
                </Button>
                <Button size="small" color="error" onClick={handleBulkDelete} sx={{ textTransform: 'none', fontSize: '0.72rem' }}>
                  {t('common.delete')}
                </Button>
              </Paper>
            )}
            ...
            {filteredRules.map(rule => {
              ...
              const isMuted = !!(rule as any).muted_until && new Date((rule as any).muted_until) > new Date();
              ...
              return (
                <Paper key={rule.id} elevation={0} sx={{
                  ...
                  {/* Bulk Checkbox */}
                  <Checkbox size="small" checked={selectedRules.has(rule.id)}
                    onChange={() => toggleSelectRule(rule.id)} color="warning"
                    sx={{ p: 0.3 }} />

                  {/* Toggle */}
                  <Tooltip title={rule.enabled ? t('argus.alerts.disable') : t('argus.alerts.enable')}>
                    <Switch size="small" checked={!!rule.enabled} onChange={() => handleToggle(rule)} color="warning" />
                  </Tooltip>

                  {/* Muted badge */}
                  {isMuted && (
                    <Chip icon={<MuteIcon sx={{ fontSize: '12px !important' }} />}
                      label={t('argus.alerts.mutedLabel')} size="small"
                      sx={{ height: 18, fontSize: '0.58rem', backgroundColor: alpha('#9e9e9e', 0.1), color: '#9e9e9e', border: 'none' }} />
                  )}
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    {rule.description && (
                      <Typography sx={{ fontSize: '0.72rem', color: 'text.secondary', mt: 0.2, mb: 0.3 }}>
                        {rule.description}
                      </Typography>
                    )}
                    <Box sx={{ display: 'flex', gap: 0.5, mt: 0.5, flexWrap: 'wrap', alignItems: 'center' }}>
                      {/* IF conditions */}
                      <Chip label={t('argus.alerts.stepIf')} size="small" sx={{ height: 18, fontSize: '0.58rem', fontWeight: 800, backgroundColor: alpha('#f44336', 0.08), color: '#f44336', border: 'none' }} />
                      {conditions.map((c: ArgusAlertCondition, i: number) => {
                        const cfg = getConditionTypes(t).find(ct => ct.value === c.type);
                        return (
                      ...
                      {/* THEN actions */}
                      <Chip label={t('argus.alerts.stepThen')} size="small" sx={{ height: 18, fontSize: '0.58rem', fontWeight: 800, backgroundColor: alpha('#4caf50', 0.08), color: '#4caf50', border: 'none' }} />
                      {actions.map((a: ArgusAlertAction, i: number) => {
                        const cfg = getActionTypes(t).find(at => at.value === a.type);
                  ...
                  <Box sx={{ width: 100, height: 36, mx: 1 }}>
                    {(() => {
                      ...
                      const sortedStats = [...ruleStats].sort((a, b) => a.bucket.localeCompare(b.bucket));
                      const data = {
                        labels: sortedStats.map(s => {
                          const d = new Date(s.bucket + (s.bucket.length === 10 ? 'T00:00:00Z' : 'Z'));
                          return d.toLocaleString('en-US', { month: 'short', day: 'numeric', hour: s.bucket.length > 10 ? '2-digit' : undefined, hour12: false });
                        }),
                        datasets: [{
                          data: sortedStats.map(s => s.count),
                          borderColor: isDark ? alpha('#ff9800', 0.8) : alpha('#f57c00', 0.8),
                          backgroundColor: isDark ? alpha('#ff9800', 0.1) : alpha('#f57c00', 0.1),
                          borderWidth: 1.5,
                          pointRadius: 0,
                          fill: true,
                          tension: 0.4,
                  ...
                  <Box sx={{ display: 'flex', gap: 0.3 }}>
                    ...
                    <Tooltip title={t('argus.alerts.mute')}>
                      <IconButton size="small" sx={{ color: 'text.disabled' }} onClick={() => setMuteDialogRule(rule)}>
                        <MuteIcon sx={{ fontSize: 16 }} />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title={t('argus.alerts.duplicate')}>
                      <IconButton size="small" sx={{ color: 'text.disabled' }} onClick={() => openDuplicate(rule)}>
          ...
          <Paper elevation={0} sx={{
              ...
              {rules.length > 0 && (
                <FormControl size="small" sx={{ minWidth: 150 }}>
                  <Select
                    ...
                    onChange={(e) => setHistoryFilterRule(e.target.value as number | '')}
                    displayEmpty
                    sx={{ fontSize: '0.72rem', height: 28 }}
                  >
                    <MenuItem value="" sx={{ fontSize: '0.72rem' }}><em>{t('argus.alerts.allRules')}</em></MenuItem>
                    {rules.map(r => (
            ...
            {stats.length > 0 && (
              <Box sx={{ height: 120, mb: 3 }}>
                {(() => {
                  const filteredStats = historyFilterRule === '' ? stats : stats.filter(s => s.rule_id === historyFilterRule);
                  const dateMap = new Map<string, number>();
                  filteredStats.forEach(s => dateMap.set(s.bucket, (dateMap.get(s.bucket) || 0) + s.count));
                  const dates = Array.from(dateMap.keys()).sort();
                  const chartData = dates.map(d => {
                    const dt = new Date(d + (d.length === 10 ? 'T00:00:00Z' : 'Z'));
                    const label = dt.toLocaleString('en-US', { month: 'short', day: 'numeric', hour: d.length > 10 ? '2-digit' : undefined, hour12: false });
                    return { label, count: dateMap.get(d) || 0 };
                  });

                  return <InteractiveTimeSeriesChart data={chartData} type="bar" height={120} />;
                })()}
              </Box>
            )}
            ...
            {history.length === 0 ? (
              <Typography variant="body2" color="text.disabled" textAlign="center" py={3}>
                {t('argus.alerts.noHistory')}
              </Typography>
            ) : (
              <Box>
                {history
                  .filter(h => historyFilterRule === '' || h.rule_id === historyFilterRule)
                  .map(h => (
                  <Box key={h.id} sx={{
                    ...
                    <Box sx={{ flex: 1 }}>
                      <Typography sx={{ fontSize: '0.78rem', fontWeight: 500 }}>{h.rule_name || t('argus.alerts.ruleNumber', { id: h.rule_id })}</Typography>
                    ...
                    {h.status && (
                      <Tooltip title={h.response_body || 'No details'}>
                        <Chip size="small" label={h.status.toUpperCase()} 
                          sx={{ height: 18, fontSize: '0.58rem', fontWeight: 700, border: 'none',
                            backgroundColor: h.status === 'success' ? alpha('#4caf50', 0.1) : alpha('#f44336', 0.1),
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth
        ...
        <DialogTitle sx={{
          fontWeight: 700, fontSize: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start',
          pb: 1.5, borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}`,
        }}>
          <Box>
            {editingRule ? t('argus.alerts.editRule') : t('argus.alerts.createRule')}
            <Typography variant="body2" sx={{ fontSize: '0.75rem', color: 'text.secondary', fontWeight: 400, mt: 1 }}>
        ...
        <DialogContent sx={{ pt: '24px !important', overflow: 'auto' }}>
          {/* Rule Name */}
          <TextField fullWidth size="small" label={t('argus.alerts.ruleName')}
            value={formName} onChange={(e) => setFormName(e.target.value)}
            placeholder={t('argus.alerts.ruleNamePlaceholder')}
            sx={{ mb: 1.5, '& .MuiOutlinedInput-root': { fontSize: '0.88rem', fontWeight: 600 } }}
          />

          {/* Description (optional) */}
          <TextField fullWidth size="small" label={t('argus.alerts.ruleDescription')}
          ...
          {/* IF Step */}
          <StepCard step={t('argus.alerts.stepIf')} label={t('argus.alerts.conditionsDesc')} color="#f44336" isDark={isDark}>
            {/* ANY/ALL Toggle */}
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                ...
                <Box sx={{ display: 'flex', borderRadius: 1, overflow: 'hidden', border: `1px solid ${isDark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.12)'}` }}>
                  {(['any', 'all'] as const).map(logic => (
                    <Button key={logic} size="small"
                      variant={formConditionLogic === logic ? 'contained' : 'text'}
                      onClick={() => setFormConditionLogic(logic)}
                      sx={{
                        textTransform: 'none', fontSize: '0.68rem', fontWeight: 700, px: 1.5, py: 0.2,
                        minWidth: 0, borderRadius: 0,
                        ...(formConditionLogic === logic ? { backgroundColor: alpha('#f44336', 0.9), color: '#fff' } : {}),
                      }}
                    >
                      {t(`argus.alerts.conditionLogic${logic === 'any' ? 'Any' : 'All'}`)}
                    </Button>
                  ))}
                </Box>
              </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleConditionDragEnd}>
                <SortableContext items={formConditions.map((_, i) => `cond-${i}`)} strategy={verticalListSortingStrategy}>
                  {formConditions.map((cond, i) => (
                    <ConditionCard key={`cond-${i}`} id={`cond-${i}`} cond={cond} isDark={isDark}
          ...
          {/* THEN Step */}
          <StepCard step={t('argus.alerts.stepThen')} label={t('argus.alerts.actionsDesc')} color="#4caf50" isDark={isDark}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleActionDragEnd}>
                <SortableContext items={formActions.map((_, i) => `act-${i}`)} strategy={verticalListSortingStrategy}>
                  {formActions.map((action, i) => (
                    <ActionCard key={`act-${i}`} id={`act-${i}`} action={action} isDark={isDark}
          ...
          {/* FILTERS Step */}
          <StepCard step={t('argus.alerts.stepFilters')} label={t('argus.alerts.filtersDesc')} color="#7c4dff" isDark={isDark}>
            <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
              <FormControl size="small" sx={{ minWidth: 120 }}>
                <InputLabel sx={{ fontSize: '0.75rem' }}>{t('argus.alerts.alertFreq')}</InputLabel>
                <Select value={formFrequency} onChange={(e) => setFormFrequency(Number(e.target.value))} label={t('argus.alerts.alertFreq')}
                  sx={{ fontSize: '0.82rem' }}>
                  {getFrequencies(t).map(f => <MenuItem key={f.value} value={f.value} sx={{ fontSize: '0.82rem' }}>{f.label}</MenuItem>)}
                </Select>
              </FormControl>
              <Autocomplete size="small" freeSolo
                options={availableEnvironments}
                value={formEnvironment || null}
                onChange={(_, v) => setFormEnvironment(typeof v === 'string' ? v : v || '')}
                onInputChange={(_, v) => setFormEnvironment(v)}
                renderInput={(params) => (
                  <TextField {...params} label={t('argus.alerts.environment')}
              ...
              <FormControl size="small" sx={{ minWidth: 100 }}>
                <InputLabel sx={{ fontSize: '0.75rem' }}>{t('argus.alerts.level')}</InputLabel>
                <Select value={formLevel} onChange={(e) => setFormLevel(e.target.value)} label={t('argus.alerts.level')}
                  sx={{ fontSize: '0.82rem' }}>
                  <MenuItem value="" sx={{ fontSize: '0.82rem' }}><em>{t('argus.alerts.allLevels')}</em></MenuItem>
                  <MenuItem value="fatal" sx={{ fontSize: '0.82rem' }}>{t('argus.common.fatal')}</MenuItem>
                  <MenuItem value="error" sx={{ fontSize: '0.82rem' }}>{t('argus.common.error')}</MenuItem>
                  <MenuItem value="warning" sx={{ fontSize: '0.82rem' }}>{t('argus.common.warning')}</MenuItem>
                </Select>
              </FormControl>
            </Box>
            ...
            <Box sx={{ mt: 2 }}>
              ...
              <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                <TextField size="small" placeholder={t('argus.alerts.tagKey')} value={newTagKey}
                  onChange={(e) => setNewTagKey(e.target.value)}
                  sx={{ width: 100, '& .MuiOutlinedInput-root': { fontSize: '0.78rem' } }}
                />
                <Typography sx={{ color: 'text.disabled', fontSize: '0.78rem' }}>:</Typography>
                <TextField size="small" placeholder={t('argus.alerts.tagValue')} value={newTagValue}
      ...
      <Dialog open={deleteConfirm !== null} onClose={() => setDeleteConfirm(null)} maxWidth="xs"
        ...
        <DialogTitle sx={{ fontWeight: 700, fontSize: '0.95rem' }}>{t('argus.alerts.confirmDelete')}</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          <Button onClick={() => setDeleteConfirm(null)} sx={{ textTransform: 'none' }}>{t('common.cancel')}</Button>
      ...
      {/* Mute Dialog */}
      <Dialog open={muteDialogRule !== null} onClose={() => setMuteDialogRule(null)} maxWidth="xs"
        PaperProps={{ sx: { borderRadius: 2 } }}>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth size="small">
            <InputLabel sx={{ fontSize: '0.78rem' }}>{t('argus.alerts.muteDuration')}</InputLabel>
            <Select value={muteDuration} onChange={(e) => setMuteDuration(Number(e.target.value))}
              label={t('argus.alerts.muteDuration')} sx={{ fontSize: '0.82rem' }}>
              <MenuItem value={1800} sx={{ fontSize: '0.82rem' }}>{t('argus.alerts.30min')}</MenuItem>
              <MenuItem value={3600} sx={{ fontSize: '0.82rem' }}>{t('argus.alerts.1hour')}</MenuItem>
              <MenuItem value={14400} sx={{ fontSize: '0.82rem' }}>{t('argus.alerts.4hours')}</MenuItem>
              <MenuItem value={86400} sx={{ fontSize: '0.82rem' }}>{t('argus.alerts.1day')}</MenuItem>
              <MenuItem value={604800} sx={{ fontSize: '0.82rem' }}>{t('argus.alerts.1week')}</MenuItem>
            </Select>
          </FormControl>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setMuteDialogRule(null)} sx={{ textTransform: 'none' }}>{t('common.cancel')}</Button>
          <Button variant="contained" onClick={handleMute} sx={{ textTransform: 'none', fontWeight: 700 }}>
