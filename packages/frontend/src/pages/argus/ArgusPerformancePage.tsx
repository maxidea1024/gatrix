        ...
        <PageContentLoader loading={detailLoading} skeleton={<><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2, mb: 2 }}><Paper elevation={0} sx={{ p: 2, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 2 }}><ArgusChartSkeleton type="line" height={260} color="#ff9800" /></Paper><Paper elevation={0} sx={{ p: 2, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 2 }}><ArgusChartSkeleton type="bar" height={220} color={theme.palette.info.main} /></Paper></Box></>}>
          ...
          {detail?.summary && (
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 2, mb: 2 }}>
              {[
                { label: t('argus.performance.totalTransactions', 'Total'), value: formatCompactNumber(Number(detail.summary.count)), color: '#7c4dff', icon: <SpeedIcon /> },
                { label: t('argus.performance.avgP95', 'P95'), value: `${Number(detail.summary.p95).toFixed(0)}ms`, color: Number(detail.summary.p95) > 3000 ? '#f44336' : Number(detail.summary.p95) > 1000 ? '#ff9800' : '#4caf50', icon: <TimelineIcon /> },
                { label: t('argus.performance.avgDuration', 'Avg'), value: `${Number(detail.summary.avg_duration).toFixed(0)}ms`, color: '#2196f3', icon: <ScheduleIcon /> },
                { label: t('argus.performance.errorRate', 'Error Rate'), value: `${Number(detail.summary.error_rate).toFixed(2)}%`, color: Number(detail.summary.error_rate) > 5 ? '#f44336' : '#4caf50', icon: <BugReportIcon /> },
              ].map((card, idx) => (
                <Paper key={idx} elevation={0} sx={{
                  p: 1.5, background: isDark ? `linear-gradient(135deg, ${alpha(card.color, 0.12)}, ${alpha(card.color, 0.03)})` : `linear-gradient(135deg, ${alpha(card.color, 0.06)}, ${alpha(card.color, 0.01)})`,
                  border: `1px solid ${alpha(card.color, 0.2)}`, borderRadius: 2, display: 'flex', alignItems: 'center', gap: 1.5
                }}>
                  <Box sx={{ width: 32, height: 32, borderRadius: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: alpha(card.color, isDark ? 0.2 : 0.1), color: card.color }}>
                    {React.cloneElement(card.icon, { sx: { fontSize: 18 } })}
                  </Box>
                  <Box>
                    <Typography variant="h6" fontWeight={800} sx={{ lineHeight: 1.1, fontSize: '1.1rem', color: card.color }}>{card.value}</Typography>
                    <Typography variant="caption" sx={{ color: isDark ? '#888' : '#777', fontWeight: 500, fontSize: '0.65rem' }}>{card.label}</Typography>
                  </Box>
                </Paper>
              ))}
            </Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Paper elevation={0} sx={{ p: 2, border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}`, borderRadius: 2 }}>
              <Typography variant="subtitle2" fontWeight={600} sx={{ mb: 1.5, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <LightbulbIcon fontSize="small" sx={{ color: theme.palette.warning.main }} />
                {t('argus.performance.insights', 'Insights & Suspect Tags')}
              </Typography>
                
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                  {/* Variance Insight */}
                  {detail?.summary && detail.summary.p95 > detail.summary.p50 * 3 && detail.summary.p50 > 50 && (
                    <Box sx={{ p: 1.5, borderRadius: 1.5, backgroundColor: alpha('#ff9800', 0.1), border: `1px solid ${alpha('#ff9800', 0.2)}` }}>
                      <Typography variant="body2" fontWeight={700} sx={{ color: '#ff9800', display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
                        <WarningIcon sx={{ fontSize: 16 }} />
                        {t('argus.performance.highVariance', 'High Latency Variance Detected')}
                      </Typography>
                      <Typography variant="caption" sx={{ color: isDark ? '#ccc' : '#555', display: 'block', lineHeight: 1.4 }}>
                        {t('argus.performance.varianceDesc', 'P95 duration is significantly higher than the median (P50), indicating inconsistent performance.')}
                      </Typography>
                    </Box>
                  )}

                  {/* Suspect Tags */}
                  {detail?.suspect_tags && detail.suspect_tags.length > 0 && (() => {
                    const slowestTag = [...detail.suspect_tags].sort((a, b) => b.p95 - a.p95)[0];
                    if (detail.summary && slowestTag.p95 > detail.summary.p95 * 1.5) {
                      return (
                        <Box sx={{ p: 1.5, borderRadius: 1.5, backgroundColor: alpha('#2196f3', 0.1), border: `1px solid ${alpha('#2196f3', 0.2)}` }}>
                          <Typography variant="body2" fontWeight={700} sx={{ color: '#2196f3', display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
                            {slowestTag.tag_key === 'browser' ? <DevicesIcon sx={{ fontSize: 16 }} /> : <EnvIcon sx={{ fontSize: 16 }} />}
                            {t('argus.performance.slowestTag', 'Slowest Environment/Tag')}
                          </Typography>
                          <Typography variant="caption" sx={{ color: isDark ? '#ccc' : '#555', display: 'block', lineHeight: 1.4 }}>
                            {t('argus.performance.slowestTagDesc', `This transaction is unusually slow on {{tag_value}} ({{tag_key}}).`, { tag_value: slowestTag.tag_value, tag_key: slowestTag.tag_key })}
                            <strong style={{ marginLeft: 4 }}>P95: {Number(slowestTag.p95).toFixed(0)}ms</strong>
                          </Typography>
                        </Box>
                      );
                    }
                  ...
                  {(!detail?.summary || (!(detail.summary.p95 > detail.summary.p50 * 3 && detail.summary.p50 > 50) && !(detail?.suspect_tags?.some(tag => detail.summary && tag.p95 > detail.summary.p95 * 1.5)))) && (
                    <Typography variant="body2" color="text.secondary" sx={{ py: 1, textAlign: 'center' }}>
                      {t('argus.performance.noInsights', 'No significant anomalies detected.')}
                    </Typography>
                  )}
                </Box>
              </Paper>
            ...
            <Paper elevation={0} sx={{ p: 2, border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}`, borderRadius: 2, display: 'flex', flexDirection: 'column' }}>
                <Typography variant="subtitle2" fontWeight={600} sx={{ mb: 1.5, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                ...
                {!detail?.related_issues?.length ? (
                  <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', py: 3 }}>
                    <BugReportIcon sx={{ fontSize: 36, color: 'text.disabled', mb: 1, opacity: 0.3 }} />
                    <Typography variant="body2" color="text.secondary">
                      {t('argus.performance.noIssues', 'No related issues found.')}
                    </Typography>
                  </Box>
                ) : (
                    ...
                    {detail.related_issues.map((issue, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Box sx={{ px: 1.2, py: 0.3, borderRadius: 1, backgroundColor: alpha(levelColor, isDark ? 0.15 : 0.08), display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0 }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2 }}>
              {/* Slowest Spans */}
              <Paper elevation={0} sx={{ p: 2, border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}`, borderRadius: 2, flex: 1 }}>
                <Typography variant="subtitle2" fontWeight={600} sx={{ mb: 1.5 }}>
