...
import { Box, Typography, Avatar, TextField, alpha, Link, CircularProgress } from '@mui/material';
import {
...
const FeedbackActivityTimeline: React.FC<FeedbackActivityTimelineProps> = ({
  ...
  const fetchActivities = useCallback(async (silent = false, customLimit = 5) => {
    ...
    try {
      const data = await argusService.getFeedbackActivity(projectId, feedbackId, customLimit, 0);
      setActivities(data);
  ...
  const fetchMoreActivities = async () => {
    ...
    try {
      ...
      const data = await argusService.getFeedbackActivity(projectId, feedbackId, 5, offset);
      if (data.length > 0) {
