...
const WizardFormContainer: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => (
  <form
    autoComplete="off"
      ...
      case 1:
        return (
          <Box>
            ...
            <CopyableUrl
              label={t('argus.settings.githubWizard.homepageUrl')}
              value={baseUrl}
              isDark={isDark}
            />
            <CopyableUrl
              label={t('argus.settings.githubWizard.callbackUrl')}
              value={`${baseUrl}/api/argus/integrations/github/callback`}
              isDark={isDark}
            />
            <CopyableUrl
              label={t('argus.settings.githubWizard.webhookUrl')}
      ...
      case 3:
        return (
          <Box>
            ...
            >
              <WizardInput
                label={t('argus.settings.githubWizard.appId')}
                value={ghForm.appId}
                onChange={(v) => setGhForm((p) => ({ ...p, appId: v }))}
                isDark={isDark}
                required
              />
              <WizardInput
                label={t('argus.settings.githubWizard.clientId')}
                value={ghForm.clientId}
                onChange={(v) => setGhForm((p) => ({ ...p, clientId: v }))}
                isDark={isDark}
                required
              />
            </Box>
            ...
            >
              <WizardInput
                label={t('argus.settings.githubWizard.clientSecret')}
                value={ghForm.clientSecret}
                onChange={(v) => setGhForm((p) => ({ ...p, clientSecret: v }))}
                isDark={isDark}
                type="password"
                required
              />
              <WizardInput
                label={t('argus.settings.githubWizard.webhookSecret')}
                value={ghForm.webhookSecret}
                onChange={(v) => setGhForm((p) => ({ ...p, webhookSecret: v }))}
                isDark={isDark}
                type="password"
              />
            </Box>
            <WizardInput
              label={t('argus.settings.githubWizard.privateKey')}
              value={ghForm.privateKey}
              onChange={(v) => setGhForm((p) => ({ ...p, privateKey: v }))}
              isDark={isDark}
              multiline
              rows={5}
              required
            ...
            >
              ...
              >
                <Box>
                  <Typography sx={{ fontWeight: 600, fontSize: '0.88rem' }}>
                    {t(
                      'argus.settings.providerWizard.testTitle',
                      '연결 테스트'
                    )}
                  </Typography>
                  <Typography
              ...
              {ghTestResult && (
                ...
                >
                  {ghTestResult.ok
                    ? `연결 성공! (${ghTestResult.name})`
                    : `연결 실패: ${ghTestResult.error}`}
                </Alert>
              )}
            </Box>
          </Box>
      ...
      case 1:
        return (
          <Box>
            ...
            <CopyableUrl
              label={t(
                'argus.settings.gitlabWizard.redirectUri',
                'Redirect URI'
              )}
        ...
        return (
          <Box>
            ...
            >
              ...
              >
                💡 <strong>Confidential</strong> 옵션을 체크하고,{' '}
                <strong>Expire access tokens</strong>는 체크 해제하세요.
              </Typography>
            </Box>
          </Box>
        );
        ...
        return (
          <Box>
            ...
            <WizardInput
              ...
              value={glForm.applicationId}
              onChange={(v) => setGlForm((p) => ({ ...p, applicationId: v }))}
              isDark={isDark}
              required
            />
            <WizardInput
              ...
              value={glForm.applicationSecret}
              onChange={(v) =>
                setGlForm((p) => ({ ...p, applicationSecret: v }))
              }
              isDark={isDark}
              type="password"
              required
            ...
            <WizardInput
              ...
              value={glForm.webhookSecret}
              onChange={(v) => setGlForm((p) => ({ ...p, webhookSecret: v }))}
              isDark={isDark}
              type="password"
            ...
            >
              ...
              >
                ...
                <Button
                  onClick={async () => {
                    if (!glForm.applicationId || !glForm.applicationSecret)
                      return;
                    setGlTesting(true);
                    setGlTestResult(null);
                    try {
              ...
              {glTestResult && (
                ...
                >
                  {glTestResult.ok
                    ? `연결 성공! (${glTestResult.message})`
                    : `연결 실패: ${glTestResult.error}`}
                </Alert>
              )}
            </Box>
          </Box>
          ...
          <Box>
            ...
            >
              {t(
                ...
                'Bitbucket의 App Passwords 페이지에서 새 앱 비밀번호를 생성합니다.'
              )}
            </Typography>
            <Box
              sx={{
            ...
            >
              ...
              >
                {t(
                  ...
                  'Personal Settings > App passwords로 이동하여 새 비밀번호를 생성하세요.'
                )}
              </Typography>
              <Button
                variant="contained"
        ...
        return (
          <Box>
            ...
            <WizardInput
              ...
              onChange={(v) => setBbForm((p) => ({ ...p, appPassword: v }))}
              isDark={isDark}
              type="password"
              required
              hint={t(
                'argus.settings.bitbucketWizard.appPasswordHint',
                '방금 생성한 App Password를 붙여넣으세요'
              )}
            />
            ...
            >
              ...
              >
                ...
                <Button
                  ...
                  disabled={
                    bbTesting || !bbForm.username || !bbForm.appPassword
                  }
                  variant="outlined"
                  size="small"
                  startIcon={
              ...
              {bbTestResult && (
                ...
                >
                  {bbTestResult.ok
                    ? `연결 성공! (${bbTestResult.display_name})`
                    : `연결 실패: ${bbTestResult.error}`}
                </Alert>
              )}
            </Box>
          </Box>
          ...
          <Box>
            ...
            <Box
              sx={{
                ...
                p: 2,
                borderRadius: '8px',
                backgroundColor: alpha(cfg.accentColor, 0.06),
                border: `1px solid ${alpha(cfg.accentColor, 0.15)}`,
              }}
            >
              <Typography
                sx={{
        ...
        return (
          <Box>
            ...
            <WizardInput
              ...
              onChange={(v) =>
                setSlackForm((p) => ({ ...p, signingSecret: v }))
              }
              isDark={isDark}
              type="password"
              hint={t(
            ...
            >
              {t(
                'argus.settings.slackWizard.testConnection',
                'Test Connection'
              )}
            </Button>
            {slackTestResult && (
              ...
              >
                {slackTestResult.ok
                  ? t(
                      ...
                      'Connected to {{team}}'
                    ).replace('{{team}}', slackTestResult.team || 'Slack')
        ...
        <Box sx={{ flex: 1, px: 4, pb: 2, overflow: 'auto' }}>
          <Fade in key={activeStep} timeout={300}>
            <Box>
              ...
              {/* Step Body */}
              {provider === 'github'
                ? renderGithubStep(activeStep)
                : provider === 'gitlab'
                  ? renderGitlabStep(activeStep)
                  : provider === 'slack'
                    ? renderSlackStep(activeStep)
                    : renderBitbucketStep(activeStep)}
              {error && (
