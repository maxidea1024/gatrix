...
import {
  Box, Typography, alpha, Paper, Table, TableHead, TableRow, TableCell, TableBody,
  TextField, InputAdornment, Button, LinearProgress, Avatar, Skeleton, Tooltip, CircularProgress, useTheme, Chip
} from '@mui/material';
import {
...
export const ArgusMonetizationProducts: React.FC<ArgusMonetizationProductsProps> = ({
  products,
  productSearch,
  setProductSearch,
  productTrend,
  ...
  if (products.length === 0) return <EmptyPagePlaceholder icon={<ProductIcon sx={{ fontSize: 48 }} />} message={t('argus.monetization.noProducts', 'No product data')} subtitle={t('argus.monetization.noProductsDesc', 'No product_name property found in purchase events')} />;
  ...
  const trendRawPeriods = hasTrend ? productTrend[0].trend.map((d: any) => d.period) : [];
  ...
  return (
    <>
      ...
      {hasTrend && (
        <ArgusVolumeChart
          title={t('argus.monetization.productTrend', 'Top 5 Product Revenue Trend')}
      ...
      <Paper variant="outlined" sx={{ borderRadius: 2, overflow: 'hidden', bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fff' }}>
        <Box sx={{ p: 1.5, pl: 2, display: 'flex', gap: 1, alignItems: 'center' }}>
          <TextField
            size="small"
            placeholder={t('argus.monetization.searchProducts', 'Search products...')}
            value={productSearch}
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ fontWeight: 700 }}>{t('argus.monetization.productName', 'Product')}</TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">{t('argus.monetization.totalRevenue', 'Revenue')}</TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">{t('argus.monetization.share', 'Share')}</TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">{t('argus.monetization.transactions', 'Transactions')}</TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">{t('argus.monetization.buyers', 'Buyers')}</TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">{t('argus.monetization.refundRate', 'Refund Rate')}</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {filtered.map((p, i) => (
              <TableRow key={p.product_name} hover sx={{ cursor: 'pointer' }} onClick={() => openProductDetail(p)}>
                ...
                <TableCell align="right">
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 0.5 }}>
                    <Typography fontSize={12} color={p.refund_rate > 10 ? ARGUS_SEMANTIC.negative : p.refund_rate > 5 ? ARGUS_SEMANTIC.warning : 'text.secondary'}>
                      {p.refund_rate > 0 ? `${p.refund_rate.toFixed(1)}%` : '—'}
                    </Typography>
                    {p.refund_rate > 10 && (
                      <Chip size="small" label="!" sx={{ height: 18, fontSize: 10, fontWeight: 700, bgcolor: alpha(ARGUS_SEMANTIC.negative, isDark ? 0.15 : 0.08), color: ARGUS_SEMANTIC.negative, border: 'none', minWidth: 0, px: 0.5 }} />
                    )}
                  </Box>
                </TableCell>
              </TableRow>
      ...
      {categoryBreakdown.length > 1 && (
        <Paper variant="outlined" sx={{ borderRadius: 2, overflow: 'hidden', bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fff', mt: 3 }}>
          <Box sx={{ p: 2 }}>
            <Typography variant="subtitle1" fontWeight={700}>{t('argus.monetization.categoryBreakdown', 'Revenue by Category')}</Typography>
          </Box>
          <Table size="small">
            <TableHead>
              <TableRow>
                <TableCell sx={{ fontWeight: 700 }}>{t('argus.monetization.category', 'Category')}</TableCell>
                <TableCell sx={{ fontWeight: 700 }} align="right">{t('argus.monetization.totalRevenue', 'Revenue')}</TableCell>
                <TableCell sx={{ fontWeight: 700 }} align="right">{t('argus.monetization.transactions', 'Transactions')}</TableCell>
      ...
      {firstPurchaseProducts.length > 0 && (
        <Paper variant="outlined" sx={{ borderRadius: 2, overflow: 'hidden', bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fff', mt: 3 }}>
          <Box sx={{ p: 2 }}>
            <Typography variant="subtitle1" fontWeight={700}>{t('argus.monetization.firstPurchaseProducts', 'First Purchase Products')}</Typography>
            <Typography variant="caption" color="text.secondary">{t('argus.monetization.firstPurchaseDesc', 'Products that drive first-time conversions')}</Typography>
          </Box>
          <Table size="small">
            <TableHead>
              <TableRow>
                <TableCell sx={{ fontWeight: 700 }}>#</TableCell>
                <TableCell sx={{ fontWeight: 700 }}>{t('argus.monetization.productName', 'Product')}</TableCell>
                <TableCell sx={{ fontWeight: 700 }} align="right">{t('argus.monetization.firstPurchaseCount', 'First Purchases')}</TableCell>
                <TableCell sx={{ fontWeight: 700 }} align="right">%</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
                    ...
                    <TableCell align="right">
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 1 }}>
                        <LinearProgress variant="determinate" value={totalFirst > 0 ? Math.min((p.first_purchase_count / totalFirst) * 100, 100) : 0}
                          sx={{ flex: 1, maxWidth: 60, height: 6, borderRadius: 3, '& .MuiLinearProgress-bar': { bgcolor: ARGUS_SEMANTIC.positive } }} />
        ...
        {productDetailLoading ? (
          ...
          <Box sx={{ p: 3, display: 'flex', flexDirection: 'column', gap: 3, overflow: 'auto', flex: 1 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, flex: 1, textAlign: 'center', bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fff' }}>
                <Typography variant="caption" color="text.secondary">{t('argus.monetization.transactions', 'Transactions')}</Typography>
                <Typography variant="h6" fontWeight={700}>{productDetail.summary.total_transactions.toLocaleString()}</Typography>
              </Paper>
              <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, flex: 1, textAlign: 'center', bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fff' }}>
                <Typography variant="caption" color="text.secondary">{t('argus.monetization.buyers', 'Buyers')}</Typography>
            ...
            {productDetail.trend.length > 0 && (
              <ArgusVolumeChart
                ...
                labels={[]}
                datasets={[
                  { label: t('argus.monetization.totalRevenue', 'Revenue'), data: productDetail.trend.map((d: any) => d.revenue), color: ARGUS_SEMANTIC.positive },
                  { label: t('argus.monetization.transactions', 'Transactions'), data: productDetail.trend.map((d: any) => d.transactions), color: theme.palette.primary.main },
                ]}
                loading={false}
                storagePrefix="argus_product_detail_trend"
                showCompactToggle={false}
                mb={0}
                onZoom={handleChartZoom(productDetail.trend.map((d: any) => d.period))}
              />
            )}

            {/* Buyer list */}
            <Box>
              <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1 }}>{t('argus.monetization.buyerList', 'Buyers')}</Typography>
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell sx={{ fontWeight: 700 }} align="right">{t('argus.monetization.lastPurchase', 'Last Purchase')}</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {productDetail.buyers.map((b: any) => (
                    <TableRow key={b.user_id} hover>
                      <TableCell>
                        ...
                        >
                          <Avatar sx={{ width: 28, height: 28, fontSize: 12, bgcolor: stringToColor(b.user_id) }}>
              ...
              {productDetail.has_more && (
                <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2 }}>
                  ...
                  >
                    {loadingMoreBuyers ? t('common.loading', 'Loading...') : t('argus.monetization.loadMore', 'Load More')}
                  </Button>
                </Box>
              )}
            </Box>
            ...
            {selectedProduct && (() => {
              ...
              const maxRevenue = Math.max(...heatmapData.map((h: any) => h.revenue), 1);
              ...
              return (
                <Box>
                  ...
                  {heatmapLoading ? (
                    ...
                    <Box sx={{ overflowX: 'auto' }}>
                      ...
                      {dayLabels.map((day, di) => (
                        <Box key={di} sx={{ display: 'flex', gap: '2px', mb: '2px', alignItems: 'center' }}>
                          ...
                          {Array.from({ length: 24 }, (_, h) => {
                            const cell = heatmapData.find((c: any) => c.day_of_week === (di + 1) && c.hour === h);
                            ...
                            return (
                              <Tooltip key={h} title={cell ? `${fmt(cell.revenue)} (${cell.count} txn)` : '-'} arrow>
                                <Box sx={{
                                  ...
                                  bgcolor: intensity > 0 ? alpha(ARGUS_SEMANTIC.positive, Math.min(intensity * 0.85 + 0.15, 1)) : (isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)'),
                                  cursor: 'default',
                    ...
                    <Typography fontSize={12} color="text.secondary">{t('argus.monetization.noHeatmapData', 'No hourly data available')}</Typography>
                  )}
                </Box>
              );
            })()}
