...
export const ArgusMonetizationSpenders: React.FC<
  ArgusMonetizationSpendersProps
> = ({
  spenders,
  segmentComparison,
  loading,
  ...
  if (!spenders || spenders.total_spenders === 0) {
    return (
      <EmptyPagePlaceholder
        ...
        message={t('argus.monetization.noSpenders', 'No spender data')}
        subtitle={t(
          'argus.monetization.noSpendersDesc',
  ...
  return (
    <>
      ...
      >
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          {spenders.segments.map((seg: any) => {
            const segColor =
              seg.segment === 'top_1_pct'
                ? ARGUS_SEMANTIC.negative
                : seg.segment === 'top_10_pct'
              ...
              >
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    ...
                    <Box
                      sx={{ display: 'flex', alignItems: 'baseline', gap: 0.8 }}
                    >
                      <Typography
                        fontSize={13}
                        fontWeight={800}
                      ...
                      <Typography fontSize={11} color="text.secondary">
                        {fmt(seg.revenue)} · {seg.user_count.toLocaleString()}{' '}
                  ...
                  <LinearProgress
                    variant="determinate"
                    value={Math.min(seg.percentage, 100)}
                    sx={{
                      height: 8,
                      borderRadius: 4,
                      bgcolor: isDark
                ...
                {isRisky && (
                  <Chip
                    size="small"
                    label={t('argus.monetization.highRisk', 'High')}
                    sx={{
                      height: 20,
                      fontSize: 10,
                      fontWeight: 700,
        ...
        {/* Whale Insight */}
        {(() => {
          const top10 = spenders.segments.find(
            (s: any) => s.segment === 'top_10_pct'
          );
          const top1 = spenders.segments.find(
            (s: any) => s.segment === 'top_1_pct'
          );
            ...
            return (
              <Box
                sx={{
                  ...
                  borderRadius: 2,
                  bgcolor: isDark
                    ? alpha(ARGUS_SEMANTIC.negative, 0.06)
                    : alpha(ARGUS_SEMANTIC.negative, 0.03),
                  border: '1px solid',
                  borderColor: alpha(
                    ARGUS_SEMANTIC.negative,
                    isDark ? 0.12 : 0.06
              ...
              >
                ...
                >
                  {t(
                    'argus.monetization.whaleConcentrationWarning',
                    '{{pct}}% of revenue concentrated in Top 10%',
                    { pct: top10.percentage.toFixed(0) }
                  )}
                  {top1 ? ` (Top 1% → ${top1.percentage.toFixed(0)}%)` : ''}
                </Typography>
                <Typography
                ...
                >
                  →{' '}
                  {t(
                    'argus.monetization.whaleConcentrationDetail',
                    'Revenue crash risk if whale users churn. Consider mid-tier growth programs & VIP retention.'
                  )}
                </Typography>
              </Box>
            );
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              {spenders.top_users.slice(0, 5).map((u: any, i: number) => (
                <Chip
                  ...
                  avatar={
                    <Avatar
                      sx={{
                        ...
                        height: 20,
                        fontSize: 9,
                        bgcolor: stringToColor(u.user_id),
                      }}
                    >
                  ...
                  onClick={() =>
                    navigate(
                      `/argus/analytics/users/${encodeURIComponent(u.user_id)}`
                    )
                  }
                  sx={{
                    fontSize: 11,
                    ...
                    bgcolor:
                      i === 0
                        ? alpha(ARGUS_SEMANTIC.negative, isDark ? 0.12 : 0.06)
                        : 'transparent',
                    '&:hover': {
                      bgcolor: isDark
        ...
        {spenders.segments.map((seg: any) => (
          <Paper
            ...
            sx={{
              ...
              textAlign: 'center',
              bgcolor: isDark
                ? alpha(segmentColors[seg.segment] || '#999', 0.06)
                : alpha(segmentColors[seg.segment] || '#999', 0.03),
              border: '1px solid',
              borderColor: alpha(
                segmentColors[seg.segment] || '#999',
                isDark ? 0.15 : 0.1
              ),
            }}
          >
            ...
            <Typography
              variant="h4"
              fontWeight={800}
              sx={{ color: segmentColors[seg.segment] }}
            >
              {seg.percentage.toFixed(1)}%
            </Typography>
            <Typography variant="caption" color="text.secondary">
              {t('argus.monetization.ofRevenue', 'of revenue')}
            </Typography>
            <Box sx={{ mt: 1 }}>
              <Typography variant="body2" fontSize={13}>
                {seg.user_count.toLocaleString()}{' '}
                {t('argus.monetization.users', 'users')} · {fmt(seg.revenue)}
              </Typography>
            </Box>
          </Paper>
        ))}
      ...
      {spenders.distribution.length > 0 && (
        <ArgusVolumeChart
          title={t(
            'argus.monetization.spendDistribution',
            'Spending Distribution'
          )}
          labels={spenders.distribution.map(
            (d: any) => `$${fmtNum(d.range_start)}~$${fmtNum(d.range_end)}`
          )}
          datasets={[
            {
              label: t('argus.monetization.users', 'Users'),
              data: spenders.distribution.map((d: any) => d.user_count),
      ...
      {spenders.whale_trend.length > 0 && (
        <ArgusVolumeChart
          title={t(
            'argus.monetization.whaleTrend',
            'Top 10% Revenue Share Trend'
          )}
          rawPeriods={spenders.whale_trend.map((d: any) => d.period)}
          labels={[]}
          datasets={[
            {
              label: t('argus.monetization.top10PctShare', 'Top 10% Share'),
              data: spenders.whale_trend.map((d: any) => d.top10_pct_share),
              color: ARGUS_SEMANTIC.negative,
            },
          ]}
          loading={loading}
          ...
          onZoom={handleChartZoom(
            spenders.whale_trend.map((d: any) => d.period)
          )}
        />
      )}
      ...
      >
        ...
        >
          <Typography variant="subtitle1" fontWeight={700}>
            {t(
              'argus.monetization.topIndividualSpenders',
      ...
      {segmentComparison && segmentComparison.segments.length > 0 && (
        <Box sx={{ mt: 3 }}>
          <Typography variant="subtitle1" fontWeight={700} sx={{ mb: 2 }}>
            {t(
              'argus.monetization.segmentComparison',
              'Whale vs Normal Comparison'
            )}
          </Typography>
          <Box
            sx={{
              ...
              gridTemplateColumns: {
                xs: '1fr',
                md: `repeat(${segmentComparison.segments.length}, 1fr)`,
              },
              gap: 2,
            }}
          >
            {segmentComparison.segments.map((seg: any) => (
              <Paper
                ...
                sx={{
                  ...
                  borderColor:
                    seg.segment === 'whales'
                      ? alpha(ARGUS_SEMANTIC.negative, 0.4)
                      : alpha(ARGUS_SEMANTIC.positive, 0.4),
                }}
              >
                <Typography
                  variant="subtitle2"
                ...
                >
                  {seg.segment === 'whales'
                    ? `${t('argus.monetization.whales', 'Whales')} (Top 10%)`
                    : t('argus.monetization.normalUsers', 'Normal Users')}
                </Typography>
                <Box
                  sx={{
                    display: 'grid',
                ...
                >
                  ...
                  <Box>
                    ...
                    <Typography fontWeight={700}>
                      {fmt(seg.total_revenue)}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary">
                      {t('argus.monetization.avgSpend', 'Avg Spend')}
                    </Typography>
                    <Typography fontWeight={700}>
                      {fmt(seg.avg_spend)}
                    </Typography>
                  ...
                  <Box>
                    <Typography variant="caption" color="text.secondary">
                      {t('argus.monetization.avgPurchases', 'Avg Purchases')}
                    </Typography>
                    <Typography fontWeight={700}>
                      {seg.avg_purchases.toFixed(1)}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary">
                      {t('argus.monetization.aov', 'AOV')}
                    </Typography>
                    <Typography fontWeight={700}>
                      {fmt(seg.avg_order_value)}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary">
                      {t('argus.monetization.avgActiveDays', 'Avg Active Days')}
                    </Typography>
                    <Typography fontWeight={700}>
                      {seg.avg_active_days.toFixed(1)}
                    </Typography>
                  </Box>
                </Box>
                {seg.top_products.length > 0 && (
                  <>
                    <Typography
                      ...
                      fontWeight={600}
                    >
                      {t('argus.monetization.topProducts', 'Top Products')}
                    </Typography>
