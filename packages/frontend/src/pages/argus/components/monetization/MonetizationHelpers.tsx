...
import { ChangeIndicator } from '../argusSharedComponents';
...
export const NAV_ITEMS: NavItem[] = [
  {
    id: 'overview',
    labelKey: 'argus.monetization.sectionOverview',
    fallback: 'Overview',
    icon: <MoneyIcon sx={{ fontSize: 18 }} />,
  },
  {
    id: 'ledger',
    labelKey: 'argus.monetization.sectionLedger',
    fallback: 'Ledger',
    icon: <LedgerIcon sx={{ fontSize: 18 }} />,
  },
  {
    id: 'discover',
    labelKey: 'argus.monetization.sectionDiscover',
    fallback: 'Discover',
    icon: <DiscoverIcon sx={{ fontSize: 18 }} />,
  },
  {
    id: 'products',
    labelKey: 'argus.monetization.sectionProducts',
    fallback: 'Products',
    icon: <ProductIcon sx={{ fontSize: 18 }} />,
  },
  {
    id: 'economy',
    labelKey: 'argus.monetization.sectionEconomy',
    fallback: 'Economy',
    icon: <EconomyIcon sx={{ fontSize: 18 }} />,
  },
  {
    id: 'spenders',
    labelKey: 'argus.monetization.sectionSpenders',
    fallback: 'Top Spenders',
    icon: <DiamondIcon sx={{ fontSize: 18 }} />,
  },
  {
    id: 'ltv',
    labelKey: 'argus.monetization.sectionLtv',
    fallback: 'LTV',
    icon: <LtvIcon sx={{ fontSize: 18 }} />,
  },
  {
    id: 'acquisition',
    labelKey: 'argus.monetization.sectionAcquisition',
...
export const SHORT_PRESETS = new Set([
  '5min',
  '10min',
  '15min',
  '30min',
...
export function getPresetsForSection(
  sectionId: SectionId
): DateRangePresetOption[] | undefined {
...
export const BalanceGauge: React.FC<BalanceGaugeProps> = ({ ratio }) => {
  ...
  let desc = t(
    'argus.monetization.healthyDesc',
    'Currency faucet and sink are well balanced.'
  );
  ...
  if (ratio > 1.2) {
    ...
    desc = t(
      'argus.monetization.inflationRiskDesc',
      'Sources are significantly higher than sinks.'
    );
  } else if (ratio < 0.8) {
    statusText = t('argus.monetization.sinkDeficit', '⚠ Sink Deficit');
    color = ARGUS_SEMANTIC.warning;
    desc = t(
      'argus.monetization.sinkDeficitDesc',
      'Sinks exceed sources; currency is being drained.'
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ color: ratio >= 0.8 && ratio <= 1.2 ? color : undefined }}
        >
          1.0 ({t('argus.monetization.balanced', 'Balanced')})
        </span>
        <span>
          2.0+ ({t('argus.monetization.sourceHeavy', 'Source Heavy')})
        </span>
      </Box>
      <Typography
        variant="caption"
        color="text.secondary"
        display="block"
        sx={{ fontSize: 11, mt: 1, fontStyle: 'italic' }}
      >
        {desc}
      </Typography>
    </Paper>
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        {sub && (
          <Typography
            sx={{ fontSize: '0.6875rem', mt: 0.2, color: 'text.secondary' }}
          >
            {sub}
          </Typography>
        )}
  ...
  function SidebarItem({ item, active, isDark, onClick, t }) {
    ...
    return (
      ...
      >
        ...
        <Box
          sx={{ display: 'flex', opacity: active ? 1 : 0.6, color: 'inherit' }}
        >
          {item.icon}
        </Box>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography
            sx={{ fontSize: '0.85rem', fontWeight: active ? 600 : 400 }}
          >
            {t(item.labelKey)}
          </Typography>
        </Box>
