...
import {
  ...
  Button, Select, MenuItem, CircularProgress, alpha
} from '@mui/material';
import {
...
export const ArgusMonetizationDiscover: React.FC<ArgusMonetizationDiscoverProps> = ({
  discGroupBy,
  setDiscGroupBy,
  discQuery,
  setDiscQuery,
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <Paper elevation={0} sx={{
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                {t('argus.monetization.discoverDesc', 'Write AQL queries, slice and aggregate revenue dataset freely')}
              </Typography>
            </Box>
          </Box>
        ...
        {discError && (
          <Paper variant="outlined" sx={{ p: 2, mb: 3, borderColor: ARGUS_SEMANTIC.negative, bgcolor: alpha(ARGUS_SEMANTIC.negative, 0.04), borderRadius: 2 }}>
            <Typography color={ARGUS_SEMANTIC.negative} fontSize={12} fontWeight={600}>
        ...
        {discHasQueried && discVolume.length > 0 && (
          <Box sx={{ mb: 4 }}>
            <VolumeChart
              ...
              loading={discLoading}
              onZoom={(start: any, end: any) => setDateRange({ type: 'custom', start: new Date(start), end: new Date(end) })}
            />
          </Box>
        )}
