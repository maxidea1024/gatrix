...
export const ArgusMonetizationLtv: React.FC<ArgusMonetizationLtvProps> = ({
  ...
  if (!ltv || ltv.ltv_curve.length === 0) {
    return (
      <EmptyPagePlaceholder
        icon={<LtvIcon sx={{ fontSize: 48 }} />}
        message={t('argus.monetization.noLtv', 'No LTV data')}
        subtitle={t(
          'argus.monetization.noLtvDesc',
          'Need purchase events with user tracking'
        )}
      />
    );
  }
  ...
  const milestones = [0, 1, 7, 14, 30, 60, 90].filter((d) =>
    ltv.ltv_curve.some((c: any) => c.day === d)
  );
  const milestoneData = milestones
    .map((d) => ltv.ltv_curve.find((c: any) => c.day === d)!)
    .filter(Boolean);
  ...
  const paybackObj = ltv.ltv_curve.find(
    (c: any) => c.cumulative_revenue >= cacInput
  );
  const isPaybackReached = !!paybackObj;
  const paybackDayText = isPaybackReached
    ? `D${paybackObj.day}`
    : t('argus.monetization.notReached', 'Not Reached');
    ...
    <>
      ...
      >
        <Box>
          ...
          <Typography variant="caption" color="text.secondary">
            {t(
              'argus.monetization.ltvDesc',
              'Track Lifetime Value progression, Customer Acquisition Cost (CAC) payback, and multiplier growth.'
            )}
          </Typography>
        </Box>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <TextField
            ...
            onChange={(e) =>
              setCacInput(Math.max(0, Number(e.target.value) || 0))
            }
            InputProps={{
              startAdornment: (
      ...
      >
        ...
        >
          ...
          <Typography
            variant="h5"
            fontWeight={800}
            color={isPaybackReached ? ARGUS_SEMANTIC.positive : 'text.primary'}
            sx={{ my: 0.5 }}
          >
            {paybackDayText}
          </Typography>
        ...
        {milestoneData.map((m: any) => {
          const roasVal =
            cacInput > 0
              ? ((m.cumulative_revenue / cacInput) * 100).toFixed(1) + '%'
              : '-';
          const mult =
            d1LtvVal > 0
              ? (m.cumulative_revenue / d1LtvVal).toFixed(1) + 'x'
              : '1.0x';
          return (
            <Paper
              key={m.day}
      ...
      {ltv.pltv_predictions && ltv.pltv_predictions.length > 0 && (
        <Paper
          ...
          sx={{
            ...
            mb: 3,
            bgcolor: isDark
              ? alpha(ARGUS_SERIES[4], 0.04)
              : alpha(ARGUS_SERIES[4], 0.02),
            border: '1px dashed',
            borderColor: alpha(ARGUS_SERIES[4], isDark ? 0.2 : 0.15),
          }}
        >
          <Box
            sx={{
          ...
          >
            <Box>
              ...
              <Typography fontSize={11} color="text.secondary">
                {t(
                  'argus.monetization.pltvDesc',
                  'Log-curve fit: y = a·ln(x+1) + b'
                )}
              </Typography>
            </Box>
            <Chip
              ...
              sx={{
                ...
                fontWeight: 700,
                bgcolor: alpha(
                  ltv.pltv_confidence >= 0.9
                    ? ARGUS_SEMANTIC.positive
                    : ltv.pltv_confidence >= 0.7
                      ? ARGUS_SEMANTIC.warning
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            {ltv.pltv_predictions.map((pred: any) => {
              const roasVal =
                cacInput > 0
                  ? ((pred.predicted_ltv / cacInput) * 100).toFixed(1) + '%'
                  : '-';
              return (
                <Box
                  key={pred.day}
        ...
        return (
          <ArgusVolumeChart
            ...
            datasets={[
              {
                label: t(
                  'argus.monetization.cumulativeRevenue',
                  'Cumulative Revenue'
                ),
                data: actualCumulative as number[],
                color: ARGUS_SEMANTIC.positive,
              },
              {
                label: t('argus.monetization.dailyRevenue', 'Daily Revenue'),
                data: actualDaily as number[],
        ...
        <Box sx={{ mt: 3, mb: 3 }}>
          ...
          >
            <Typography variant="subtitle1" fontWeight={700}>
              {t('argus.monetization.cohortLtv', 'LTV by Cohort')}
            </Typography>
            <Select
              size="small"
              value={ltvCohortBy}
            ...
            >
              <MenuItem value="week">
                {t('argus.monetization.byWeek', 'By Install Week')}
              </MenuItem>
              <MenuItem value="platform">
                {t('argus.monetization.byPlatform', 'By Platform')}
              </MenuItem>
              <MenuItem value="country">
                {t('argus.monetization.byCountry', 'By Country')}
              </MenuItem>
            </Select>
          </Box>
          <ArgusVolumeChart
            ...
            labels={(() => {
              const maxLen = Math.max(
                ...cohortLtv.cohorts.map((c: any) => c.ltv_curve.length)
              );
              const ref = cohortLtv.cohorts.find(
                (c: any) => c.ltv_curve.length === maxLen
              );
      ...
      >
        <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
          <Typography variant="subtitle1" fontWeight={700}>
            {t(
              'argus.monetization.ltvBreakdownTable',
              'LTV Detailed Breakdown'
            )}
          </Typography>
        </Box>
        <Table size="small">
          <TableHead>
            <TableRow>
              <TableCell sx={{ fontWeight: 700 }}>
                {t('argus.monetization.cohortDay', 'Day')}
              </TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">
                {t('argus.monetization.cumulativeLtv', 'Cumulative LTV')}
              </TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">
                {t('argus.monetization.dailyRevenue', 'Daily Revenue')}
              </TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">
                {t('argus.monetization.multiplier', 'Multiplier')}
              </TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">
                {t('argus.monetization.roas', 'ROAS')}
              </TableCell>
              <TableCell sx={{ fontWeight: 700 }} align="right">
                {t('argus.monetization.users', 'Cohort Users')}
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {ltv.ltv_curve.map((row: any) => {
              const multVal =
                d1LtvVal > 0
                  ? (row.cumulative_revenue / d1LtvVal).toFixed(2) + 'x'
                  : '-';
              const roasPercent =
                cacInput > 0
                  ? ((row.cumulative_revenue / cacInput) * 100).toFixed(1) + '%'
                  : '-';
                ...
                <TableRow key={row.day} hover>
                  ...
                  <TableCell align="right">
                    <Typography fontWeight={600} fontSize={13}>
                      {fmt(row.cumulative_revenue)}
                    </Typography>
                  </TableCell>
                  <TableCell align="right">{fmt(row.daily_revenue)}</TableCell>
                  <TableCell align="right">
                    {row.day === 1 ? '1.00x (Ref)' : multVal}
                  </TableCell>
                  <TableCell align="right">
                    <Typography
                      fontWeight={600}
                      fontSize={13}
                      color={
                        isBep ? ARGUS_SEMANTIC.positive : ARGUS_SEMANTIC.warning
                      }
                    >
                      {roasPercent}
                    </Typography>
