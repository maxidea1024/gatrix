...
import {
  Box, Typography, ToggleButtonGroup, ToggleButton, Paper, Table, TableHead, TableRow, TableCell, TableBody, Chip, useTheme
} from '@mui/material';
import {
...
export const ArgusMonetizationEconomy: React.FC<ArgusMonetizationEconomyProps> = ({
  economy,
  economyCurrency,
  setEconomyCurrency,
  loading,
  ...
  if (!economy || (economy.by_currency.length === 0 && economy.flow_over_time.length === 0)) {
    return <EmptyPagePlaceholder icon={<EconomyIcon sx={{ fontSize: 48 }} />} message={t('argus.monetization.noEconomy', 'No economy data')} subtitle={t('argus.monetization.noEconomyDesc', 'No resource_source / resource_sink events found')} />;
  }

  const selectedCurrencyData = economy.by_currency.find((c: any) => c.currency_type === economyCurrency) || economy.by_currency[0];
  ...
  const filteredTopSinks = economy.top_sinks.filter((s: any) => s.currency_type === economyCurrency);
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '340px 1fr' }, gap: 3 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <KpiCard
            ...
            color={currentNetFlow >= 0 ? ARGUS_SEMANTIC.warning : ARGUS_SEMANTIC.positive}
            sub={currentNetFlow > 0 ? t('argus.monetization.inflationRisk', 'Inflation risk') : t('argus.monetization.healthy', 'Healthy')}
          />
          ...
          <Paper variant="outlined" sx={{ borderRadius: 3, overflow: 'hidden', bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fff' }}>
            <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
              <Typography variant="subtitle2" fontWeight={700}>{t('argus.monetization.byCurrency', 'Currency Summary')}</Typography>
            </Box>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 700, fontSize: 11 }}>{t('argus.monetization.currencyType', 'Currency')}</TableCell>
                  <TableCell sx={{ fontWeight: 700, fontSize: 11 }} align="right">{t('argus.monetization.netFlow', 'Net Flow')}</TableCell>
                  <TableCell sx={{ fontWeight: 700, fontSize: 11 }} align="right">{t('argus.monetization.sourceSinkRatio', 'Ratio')}</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="right" sx={{ py: 1.5 }}>
                        <Typography fontSize={12} fontWeight={600} color={c.net_flow >= 0 ? ARGUS_SEMANTIC.warning : ARGUS_SEMANTIC.positive}>
                          {c.net_flow >= 0 ? '+' : ''}{fmtNum(c.net_flow)}
                        </Typography>
                      </TableCell>
                      <TableCell align="right" sx={{ py: 1.5 }}>
                        <Typography fontSize={12} fontWeight={600} color={r > 1.2 ? ARGUS_SEMANTIC.negative : r < 0.8 ? ARGUS_SEMANTIC.warning : ARGUS_SEMANTIC.positive}>
                          {r.toFixed(2)}
                        </Typography>
                      </TableCell>
                    </TableRow>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: 0 }}>
          ...
          <ArgusVolumeChart
            ...
            labels={[]}
            datasets={[
              { label: t('argus.monetization.source', 'Source'), data: economy.flow_over_time.map((d: any) => d.source), color: ARGUS_SEMANTIC.positive },
              { label: t('argus.monetization.sink', 'Sink'), data: economy.flow_over_time.map((d: any) => d.sink), color: ARGUS_SEMANTIC.negative },
            ]}
            loading={loading}
            storagePrefix="argus_economy_flow"
            showCompactToggle={false}
            onZoom={handleChartZoom(economy.flow_over_time.map((d: any) => d.period))}
          />
          ...
          {economy.ratio_trend && economy.ratio_trend.length > 0 && (
            <ArgusVolumeChart
              ...
              labels={[]}
              datasets={[
                { label: t('argus.monetization.sourceSinkRatio', 'Ratio'), data: economy.ratio_trend.map((d: any) => d.ratio), color: ARGUS_SEMANTIC.warning },
              ]}
              loading={loading}
              storagePrefix="argus_economy_ratio"
              showCompactToggle={false}
              onZoom={handleChartZoom(economy.ratio_trend.map((d: any) => d.period))}
            />
          )}
            ...
            <Paper variant="outlined" sx={{ borderRadius: 3, overflow: 'hidden', bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fff' }}>
              <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
                <Typography variant="subtitle1" fontWeight={700}>
                  {t('argus.monetization.topSinks', 'Top Spending Items')} ({economyCurrency.toUpperCase()})
                </Typography>
              </Box>
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell sx={{ fontWeight: 700 }}>{t('argus.monetization.itemName', 'Item')}</TableCell>
