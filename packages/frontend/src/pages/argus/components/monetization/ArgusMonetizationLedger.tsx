...
export const ArgusMonetizationLedger: React.FC<
  ArgusMonetizationLedgerProps
> = ({
  txGroupBy,
  setTxGroupBy,
  txData,
  ...
  const isDataMatchingMode =
    txData && (isGrouped ? txData.mode === 'grouped' : txData.mode === 'flat');
  const hasData =
    isDataMatchingMode &&
    (isGrouped
      ? Array.isArray((txData as TransactionGroupedResponse).groups) &&
        (txData as TransactionGroupedResponse).groups.length > 0
      : Array.isArray((txData as TransactionResponse).transactions) &&
        (txData as TransactionResponse).transactions.length > 0);

  const eventTypeLabels: Record<string, { label: string; color: string }> = {
    purchase: {
      label: t('argus.monetization.txPurchase', 'Purchase'),
      color: ARGUS_SEMANTIC.positive,
    },
    refund: {
      label: t('argus.monetization.txRefund', 'Refund'),
      color: ARGUS_SEMANTIC.negative,
    },
    grant: {
      label: t('argus.monetization.txGrant', 'Free Grant'),
      color: ARGUS_SEMANTIC.warning,
    },
    ad_impression: {
      label: t('argus.monetization.txAd', 'Ad'),
  ...
  const presets = [
    ...
    },
    {
      ...
      action: () => {
        setTxQuery(
          'event_name:refund (reason:technical_issue OR reason:policy_violation)'
        );
        setTxGroupBy('none');
        setTxOffset(0);
      },
    ...
    >
      <Box sx={{ flexShrink: 0 }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box>
              ...
              >
                {t(
                  'argus.monetization.transactionLedger',
                  'Transaction Ledger'
                )}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                {t(
                  'argus.monetization.ledgerDesc',
                  'Audit and analyze individual transaction events'
                )}
              </Typography>
            </Box>
          </Box>
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  downloadCsv(
                    rowsToExport,
                    txCsvColumns,
                    `ledger_${txGroupBy}_${txQuery ? 'query' : 'all'}_${new Date().toISOString().slice(0, 10)}`
                  );
                } catch {
                  const rowsToExport = isGrouped
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3 }}>
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <QueryAQLEditor
              ...
              placeholder={t(
                'argus.monetization.txSearchPlaceholder',
          ...
          <FilterChipSelect
            ...
            options={[
              {
                value: 'none',
                label: t('argus.monetization.groupNone', 'None'),
              },
              {
                value: 'product',
                label: t('argus.monetization.groupProduct', 'Product'),
              },
              {
                value: 'user',
                label: t('argus.monetization.groupUser', 'User'),
              },
              { value: 'day', label: t('argus.monetization.groupDay', 'Day') },
              {
                value: 'hour',
                label: t('argus.monetization.groupHour', 'Hour'),
              },
              {
                value: 'reason',
                label: t('argus.monetization.groupReason', 'Reason'),
              },
            ]}
            ...
            onSelect={(v) => {
              setTxGroupBy(v as LedgerGroupBy);
              setTxOffset(0);
            }}
          />
        </Box>
        ...
        >
          ...
          <Typography variant="body2" color="text.secondary" fontSize={12}>
            {t('argus.monetization.purchaseShort', 'Purchase')}:{' '}
            {isDataMatchingMode && txData
              ? `${fmt(txData.summary.purchase_total)} (${txData.summary.purchase_count.toLocaleString()})`
              : '—'}
          </Typography>
          <Typography variant="body2" color="text.secondary" fontSize={12}>
            {t('argus.monetization.refundShort', 'Refund')}:{' '}
            {isDataMatchingMode && txData && txData.summary.refund_count > 0
              ? `-${fmt(txData.summary.refund_total)} (${txData.summary.refund_count.toLocaleString()})`
              : '—'}
          </Typography>
          <Typography variant="body2" color="text.secondary" fontSize={12}>
            {t('argus.monetization.grantShort', 'Free Grant')}:{' '}
            {isDataMatchingMode && txData && txData.summary.grant_count > 0
              ? `${fmt(txData.summary.grant_total)} (${txData.summary.grant_count.toLocaleString()})`
              : '—'}
          </Typography>
          <Typography variant="body2" color="text.secondary" fontSize={12}>
            {t('argus.monetization.adShort', 'Ad')}:{' '}
            {isDataMatchingMode && txData && txData.summary.ad_count > 0
              ? `${fmt(txData.summary.ad_total)} (${txData.summary.ad_count.toLocaleString()})`
              : '—'}
          </Typography>
        </Box>
        ...
        {!data ? (
          ...
          (() => {
            ...
            const refundMap = hasAqlFilter
              ? new Map()
              : new Map(
                  data.refunds_over_time.map((r: any) => [
                    r.period,
                    r.refund_count,
                  ])
                );
            const grantMap = hasAqlFilter
              ? new Map()
              : new Map(
                  (data.grants_over_time || []).map((g: any) => [
                    g.period,
                    g.grant_count,
                  ])
                );
            const adMap = hasAqlFilter
              ? new Map()
              : new Map(
                  data.ad_revenue_over_time.map((a: any) => [
                    a.period,
                    a.impressions,
                  ])
                );
            return (
              <ArgusVolumeChart
                ...
                datasets={[
                  {
                    label: t('argus.monetization.purchaseShort', 'Purchase'),
                    data: rot.map((d: any) => d.transactions),
                    color: ARGUS_SEMANTIC.positive,
                  },
                  {
                    label: t('argus.monetization.refundShort', 'Refund'),
                    data: periods.map((p) => refundMap.get(p) || 0),
                    color: ARGUS_SEMANTIC.negative,
                  },
                  {
                    label: t('argus.monetization.grantShort', 'Free Grant'),
                    data: periods.map((p) => grantMap.get(p) || 0),
                    color: ARGUS_SEMANTIC.warning,
                  },
                  {
                    label: t('argus.monetization.adShort', 'Ad'),
                    data: periods.map((p) => adMap.get(p) || 0),
                    color: ARGUS_SEMANTIC.info,
                  },
                ]}
                loading={false}
          ...
          {!isGrouped ? (
              ...
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 700, width: 120 }}>
                    {t('argus.monetization.txType', 'Type')}
                  </TableCell>
                  ...
                  <TableCell sx={{ fontWeight: 700 }}>
                    {t('argus.monetization.txUser', 'User')}
                  </TableCell>
                  <TableCell sx={{ fontWeight: 700 }}>
                    {t('argus.monetization.txProduct', 'Product')}
                  </TableCell>
                  ...
                  <TableCell sx={{ fontWeight: 700 }}>
                    {t('argus.monetization.txReason', 'Details')}
                  </TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {(txData as TransactionResponse).transactions.map((tx, idx) => {
                  const meta = eventTypeLabels[tx.event_type] || {
                    icon: '❓',
                    label: tx.event_type,
                    color: '#999',
                  };
                  return (
                    <TableRow key={`${tx.event_id}-${idx}`} hover>
                      ...
                      <TableCell>
                        {tx.user_id ? (
                          <Typography
                            ...
                            onClick={() =>
                              navigate(
                                `/argus/analytics/users/${encodeURIComponent(tx.user_id)}`
                              )
                            }
                            sx={{
                              fontSize: 12,
                      ...
                      <TableCell sx={{ fontSize: 12, fontWeight: 600 }}>
                        {tx.product_name ||
                          (tx.ad_type ? tx.ad_type.replace(/_/g, ' ') : '—')}
                      </TableCell>
                      <TableCell
                        align="right"
                        sx={{
            ...
            >
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 700 }}>
                    {t('argus.monetization.groupKey', 'Group Key')}
                  </TableCell>
                  <TableCell align="right" sx={{ fontWeight: 700 }}>
                    {t('argus.monetization.groupCount', 'Count')}
                  </TableCell>
                  <TableCell align="right" sx={{ fontWeight: 700 }}>
                    {t('argus.monetization.groupTotal', 'Total Amount')}
                  </TableCell>
                  <TableCell align="right" sx={{ fontWeight: 700 }}>
                    {t('argus.monetization.groupAvg', 'Average Amount')}
                  </TableCell>
                  <TableCell align="right" sx={{ fontWeight: 700 }}>
                    {t('argus.monetization.groupUniqueUsers', 'Unique Users')}
                  </TableCell>
                  <TableCell sx={{ fontWeight: 700 }}>
                    {t('argus.monetization.groupRange', 'Activity Range')}
                  </TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {(txData as TransactionGroupedResponse).groups.map(
                  (group, idx) => (
                    <TableRow key={idx} hover>
                      <TableCell sx={{ fontSize: 12, fontWeight: 700 }}>
                        {txGroupBy === 'user' ? (
                          <Typography
                            ...
                            onClick={() =>
                              navigate(
                                `/argus/analytics/users/${encodeURIComponent(group.group_key)}`
                              )
                            }
                            sx={{
                              fontSize: 12,
                      ...
                      <TableCell
                        ...
                        sx={{ fontSize: 12, fontWeight: 600 }}
                      >
                        {fmt(group.avg_amount)}
                      </TableCell>
                      <TableCell
                        align="right"
                        sx={{ fontSize: 12, fontWeight: 600 }}
                      >
                        {group.unique_users.toLocaleString()}
                      </TableCell>
                      <TableCell
                        sx={{
                          fontSize: 11,
                      ...
                      >
                        {group.first_at
                          ? new Date(group.first_at).toLocaleDateString()
                          : ''}
                        {group.last_at && group.last_at !== group.first_at
                          ? ` ~ ${new Date(group.last_at).toLocaleDateString()}`
                          : ''}
                      </TableCell>
                    </TableRow>
                  )
      ...
      <Box sx={{ mt: 2, flexShrink: 0 }}>
        {isDataMatchingMode &&
          txData &&
          (isGrouped
            ? (txData as TransactionGroupedResponse).total_groups || 0
            : (txData as TransactionResponse).total_count || 0) > 0 && (
            <SimplePagination
              count={
                isGrouped
                  ? (txData as TransactionGroupedResponse).total_groups || 0
                  : (txData as TransactionResponse).total_count || 0
              }
