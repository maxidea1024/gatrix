            ...
            {heatmapGrid && (
              ...
              >
                ...
                <Box sx={{ display: 'flex', gap: 0.5 }}>
                  {/* Day labels */}
                  <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', gap: 0, width: 28, flexShrink: 0 }}>
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    {/* Hour labels */}
                    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(24, 1fr)', gap: '2px', mb: '2px' }}>
                      {HOURS.map((h) => (
                        <Box key={h} sx={{ textAlign: 'center', height: 14 }}>
                          <Typography sx={{ fontSize: '0.55rem', color: 'text.secondary', lineHeight: '14px' }}>
                            {h % 3 === 0 ? h : ''}
                          </Typography>
                        </Box>
                      ))}
                    ...
                    {heatmapGrid.grid.map((row, rowIdx) => (
                      <Box
                        key={rowIdx}
                        sx={{ display: 'grid', gridTemplateColumns: 'repeat(24, 1fr)', gap: '2px', mb: '2px' }}
                      >
                          ...
                          return (
                            <Tooltip
                              ...
                              title={
                                <Box sx={{ textAlign: 'center' }}>
                                  <Typography sx={{ fontSize: '0.75rem', fontWeight: 600 }}>
                                    {t(DOW_KEYS[rowIdx])} {String(colIdx).padStart(2, '0')}:00
                                  </Typography>
                                  <Typography sx={{ fontSize: '0.85rem', fontWeight: 700 }}>
                                    {formatCompactNumber(count)} {t('argus.analytics.events', 'events')}
                                  </Typography>
                                </Box>
                              }
                              arrow
                              ...
                              slotProps={{
                                tooltip: {
                                  sx: {
                                    bgcolor: isDark ? 'rgba(30,30,40,0.95)' : 'rgba(255,255,255,0.95)',
                                ...
                                arrow: {
                                  sx: {
                                    color: isDark ? 'rgba(30,30,40,0.95)' : 'rgba(255,255,255,0.95)',
                                  },
                                },
                              }}
                            >
            ...
            <Grid container spacing={1.5}>
              {FEATURES.map((f) => (
                <Grid size={{ xs: 6 }} key={f.key}>
                  ...
                  >
                    <FeatureIconBox color={f.color} sx={{ width: 36, height: 36, borderRadius: '10px' }}>
                      ...
                      const pctLabel =
                        pctValue === 0
                          ...
                          : pctValue < 1
                            ? '<1%'
                            : `${Math.round(pctValue)}%`;
                      const barColor = ev.icon_color || DONUT_COLORS[i % DONUT_COLORS.length];
                      return (
                        <Box
                          key={ev.name}
                          sx={{
