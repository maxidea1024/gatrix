  ...
  return (
    ...
    >
      ...
      <Typography
        variant="h4"
        fontWeight={800}
        sx={{ color: color || 'text.primary', my: 0.5 }}
      >
        {value}
      </Typography>
      {sub && (
        ...
        >
          ...
          <FormControl size="small" sx={{ minWidth: 200 }}>
            ...
            <Select
              ...
              onChange={(e) => setCauseEvent(e.target.value)}
            >
              {eventNames.map((en) => (
          ...
          <Typography variant="body2" fontWeight={600}>
            {t('argus.impact.thenDid')}
          </Typography>
          <FormControl size="small" sx={{ minWidth: 200 }}>
            <InputLabel>{t('argus.impact.effectEvent')}</InputLabel>
            <Select
              value={effectEvent}
              label={t('argus.impact.effectEvent')}
              onChange={(e) => setEffectEvent(e.target.value)}
            >
              {eventNames.map((en) => (
                <MenuItem key={en.name} value={en.name}>
                  {en.display_name || en.name}
          ...
          <TextField
            ...
            value={windowDays}
            onChange={(e) =>
              setWindowDays(Math.max(1, parseInt(e.target.value) || 1))
            }
            sx={{ width: 70 }}
          ...
          <Button
            variant="contained"
            onClick={runAnalysis}
            disabled={!causeEvent || !effectEvent || loading}
            sx={{ ml: 'auto' }}
          >
      ...
      <PageContentLoader loading={loading}>
        {result ? (
          <>
            ...
            >
              <MetricCard
                label={t('argus.impact.causeUsers')}
                value={result.cause_users.toLocaleString()}
                sub={`"${result.cause_event}"`}
                color={theme.palette.primary.main}
              />
              <MetricCard
                label={t('argus.impact.converted')}
                value={result.converted_users.toLocaleString()}
                sub={`${result.conversion_rate}% ${t('argus.impact.conversion')}`}
                color={theme.palette.secondary.main}
              />
              <MetricCard
                label={t('argus.impact.baselineRate')}
                value={`${result.baseline_rate}%`}
                sub={`${result.baseline_converted} / ${result.baseline_users}`}
              />
              <MetricCard
                label={t('argus.impact.lift')}
                value={`${result.lift > 0 ? '+' : ''}${result.lift}%`}
                sub={
                  result.lift > 0
                    ? t('argus.impact.positiveImpact')
                    : result.lift < 0
                      ? t('argus.impact.negativeImpact')
                      : t('argus.impact.noImpact')
                }
                color={liftColor}
              />
            </Box>
            ...
            >
              <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 2 }}>
                {t('argus.impact.conversionComparison')}
              </Typography>
              <Box sx={{ mb: 2 }}>
                <Box
                  sx={{
                ...
                <LinearProgress
                  variant="determinate"
                  value={Math.min(result.conversion_rate, 100)}
                  sx={{
                    height: 24,
                    borderRadius: 2,
                    bgcolor: isDark
              ...
              <Box>
                ...
                <LinearProgress
                  variant="determinate"
                  value={Math.min(result.baseline_rate, 100)}
                  color="inherit"
                  sx={{
                    height: 24,
                    borderRadius: 2,
