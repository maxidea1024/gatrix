        ...
        <Box sx={{ display: 'flex', gap: 4, flex: 1, mb: -2 }}>
          ...
          <Box sx={{ flex: 1, minWidth: 0, pb: 6 }}>
            ...
            {currentSection === 'ownership' && (
              <SettingsCard title={t('argus.settings.ownership')} desc={t('argus.settings.ownershipDesc')} isDark={isDark}>
                <Paper elevation={0} sx={{ p: 2.5, mb: 2.5, borderRadius: '10px', border: `1px dashed ${bdr}`, backgroundColor: bgSubtle }}>
                  <Typography sx={{ fontWeight: 700, fontSize: '0.62rem', textTransform: 'uppercase', color: 'text.disabled', letterSpacing: '0.1em', mb: 1.5 }}>
                    {t('argus.settings.addRule')}
                  </Typography>
                  <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                    <TextField size="small" label={t('argus.settings.ruleName')} placeholder="Frontend Team"
                      value={newRule.name} onChange={e => setNewRule({ ...newRule, name: e.target.value })} sx={{ width: 160, ...inpSx }} />
                    <FormControl size="small" sx={{ minWidth: 100 }}>
                      <InputLabel>{t('argus.settings.matchType')}</InputLabel>
                      <Select value={newRule.type} onChange={e => setNewRule({ ...newRule, type: e.target.value })} label={t('argus.settings.matchType')}>
                        <MenuItem value="path">Path</MenuItem><MenuItem value="module">Module</MenuItem>
                        <MenuItem value="tag">Tag</MenuItem><MenuItem value="url">URL</MenuItem>
                      </Select>
                    </FormControl>
                    <TextField size="small" label={t('argus.settings.pattern')} placeholder="src/components/**"
                      value={newRule.pattern} onChange={e => setNewRule({ ...newRule, pattern: e.target.value })}
                      sx={{ flex: 1, minWidth: 160, ...inpSx, '& .MuiOutlinedInput-root': { fontFamily: 'monospace' } }} />
                    <TextField size="small" label={t('argus.settings.owners')} placeholder="alice, bob"
                      value={newRule.owners} onChange={e => setNewRule({ ...newRule, owners: e.target.value })} sx={{ width: 160, ...inpSx }} />
                    <Button variant="contained" size="small" startIcon={<AddIcon />}
                      ...
                      sx={{ textTransform: 'none', fontWeight: 600, borderRadius: '8px' }}>{t('common.add')}</Button>
                  </Box>
                </Paper>

                {!ruleLoaded ? <Spinner /> : rules.length === 0 ? (
                  <EmptyState icon={<SecurityIcon />} text={t('argus.settings.noRules')} hint={t('argus.settings.noRulesHint')} />
                ) : (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {rules.map(rule => {
                        ...
                        <Paper key={rule.id} elevation={0} sx={{
                          ...
                          <Box sx={{ flex: 1, minWidth: 0 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.3 }}>
                              <Typography sx={{ fontWeight: 600, fontSize: '0.85rem' }}>{rule.name}</Typography>
                              <Chip label={rule.match_type} size="small" sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700 }} />
                              {rule.auto_assign && <Chip label="auto-assign" size="small" sx={{ height: 18, fontSize: '0.6rem', backgroundColor: alpha('#7c4dff', 0.1), color: '#7c4dff', border: 'none' }} />}
                            </Box>
                            <Typography variant="caption" sx={{ fontFamily: 'monospace', color: isDark ? '#aaa' : '#666' }}>{rule.match_pattern}</Typography>
                            <Box sx={{ display: 'flex', gap: 0.5, mt: 0.5 }}>
                              {(owners as string[]).map((o: string, i: number) => <Chip key={i} label={o} size="small" sx={{ height: 20, fontSize: '0.65rem', fontWeight: 600 }} />)}
                            </Box>
                          </Box>
                          <IconButton size="small" color="error" onClick={async () => {
                            await argusService.deleteOwnershipRule(projectId, rule.id); setRules(p => p.filter(r => r.id !== rule.id));
                            enqueueSnackbar(t('common.deleted'), { variant: 'success' });
