...
const ArgusRetentionPage: React.FC = () => {
  ...
  // ── Condition handlers ──
  const handleAddCondition = useCallback((target: 'cohort' | 'return') => {
    const event = target === 'cohort' ? cohortEvent : returnEvent;
    const setter = target === 'cohort' ? setCohortEvent : setReturnEvent;
    setter({
      ...event,
      conditions: [...(event.conditions || []), { property: '', operator: 'is', value: '' }],
    });
  }, [cohortEvent, returnEvent, setCohortEvent, setReturnEvent]);

  const handleConditionChange = useCallback(
    (target: 'cohort' | 'return', condIndex: number, field: keyof EventCondition, value: string) => {
      const event = target === 'cohort' ? cohortEvent : returnEvent;
      const setter = target === 'cohort' ? setCohortEvent : setReturnEvent;
      setter({
        ...event,
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      const data = await argusService.getAnalyticsRetention(projectId, {
        ...
        criteria,
        measurement,
        measurement_property: ['property_sum', 'property_avg'].includes(measurement)
          ? measurementProperty
          : undefined,
  ...
  }, [cohortEvent, returnEvent, dateRange, projectId, retentionType, criteria, measurement, measurementProperty, breakdownProperty, minFrequency]);
  ...
  const curveData = useMemo(() => {
    ...
    return Array.from({ length: maxPeriods }, (_, i) => ({
      ...
      average: counts[i] > 0 ? Math.round((avgRetention[i] / counts[i]) * 10) / 10 : 0,
      ...Object.fromEntries(
        cohorts.map((c, cIdx) => [
    ...
    <>
      {event.conditions?.map((cond, cIdx) => (
        <Box key={cIdx} sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 0.5, pl: 0.5 }}>
          <Typography variant="caption" color="text.secondary">{t('argus.analytics.where', 'where')}</Typography>
          <PropertyPicker
            projectId={projectId}
            eventName={event.name}
            value={cond.property}
            onChange={(val) => handleConditionChange(target, cIdx, 'property', val)}
            emptyLabel={t('argus.analytics.property', 'Property')} highlightEmpty
          />
          <InlineSelect
            value={cond.operator}
            onChange={(val) => handleConditionChange(target, cIdx, 'operator', val)}
            options={OPERATORS}
          />
          {!['set', 'not_set'].includes(cond.operator) && (
            <input
              value={cond.value}
              onChange={(e) => handleConditionChange(target, cIdx, 'value', e.target.value)}
              ...
              style={{
                ...
                color: 'inherit', outline: 'none', width: 80,
                fontSize: '0.8rem', fontFamily: 'inherit',
              }}
            />
          )}
          <IconButton size="small" onClick={() => handleRemoveCondition(target, cIdx)}
            sx={{ p: 0.25, ml: 0.5, opacity: 0.6, '&:hover': { opacity: 1 } }}>
            <CloseIcon sx={{ fontSize: 14 }} />
          </IconButton>
        </Box>
      ))}
      <Button size="small" onClick={() => handleAddCondition(target)}
        sx={{ alignSelf: 'flex-start', textTransform: 'none', opacity: 0.7, pl: 0.5, py: 0, minWidth: 0, fontSize: '0.75rem' }}>
    ...
    <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <Box sx={{ mt: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
          ...
          <EventBlock indexLabel="A" color="#6366f1">
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
              <Typography variant="caption" color="text.secondary" sx={{ minWidth: 48 }}>{t('argus.analytics.first', 'First')}</Typography>
              <InlineSelect
                value={cohortEvent.name}
                onChange={(val) => setCohortEvent({ ...cohortEvent, name: val })}
                options={eventOptions}
                emptyLabel={t('argus.analytics.selectEvent', 'Select Event')} highlightEmpty
              />
            </Box>
          ...
          <EventBlock indexLabel="B" color="#10b981">
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
              <Typography variant="caption" color="text.secondary" sx={{ minWidth: 48 }}>{t('argus.analytics.return', 'Return')}</Typography>
              <InlineSelect
                value={returnEvent.name}
                onChange={(val) => setReturnEvent({ ...returnEvent, name: val })}
                options={eventOptions}
                emptyLabel={t('argus.analytics.selectEvent', 'Select Event')} highlightEmpty
              />
            </Box>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexWrap: 'nowrap' }}>
          <Typography variant="caption" color="text.secondary" sx={{ minWidth: 50, flexShrink: 0, fontSize: '0.7rem' }}>{t('argus.analytics.retentionType', 'Period')}</Typography>
          <InlineSelect
            value={retentionType}
        ...
        {/* Criteria */}
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexWrap: 'nowrap' }}>
          <Typography variant="caption" color="text.secondary" sx={{ minWidth: 50, flexShrink: 0, fontSize: '0.7rem' }}>{t('argus.analytics.criteria', 'Criteria')}</Typography>
          <InlineSelect
            value={criteria}
            onChange={(val) => setCriteria(val as any)}
            options={[
              { value: 'on', label: t('argus.analytics.criteriaOn', 'On (N-Day)') },
              { value: 'on_or_after', label: t('argus.analytics.criteriaOnOrAfter', 'On or After') },
            ]}
          />
        </Box>
        ...
        {/* Measurement */}
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexWrap: 'nowrap' }}>
          <Typography variant="caption" color="text.secondary" sx={{ minWidth: 50, flexShrink: 0, fontSize: '0.7rem' }}>{t('argus.analytics.measure', 'Measure')}</Typography>
          <InlineSelect
            value={measurement}
            onChange={(val) => setMeasurement(val as any)}
            options={[
              { value: 'retention_rate', label: t('argus.analytics.retentionRate', 'Retention Rate') },
              { value: 'unique_users', label: t('argus.analytics.uniqueUsers', 'Unique Users') },
              { value: 'property_sum', label: t('argus.analytics.propertySum', 'Property Sum') },
              { value: 'property_avg', label: t('argus.analytics.propertyAvg', 'Property Avg') },
            ]}
          />
        </Box>
        ...
        {['property_sum', 'property_avg'].includes(measurement) && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Typography variant="caption" color="text.secondary" sx={{ minWidth: 70 }}>{t('argus.analytics.property', 'Property')}</Typography>
            <PropertyPicker
              projectId={projectId}
              eventName={returnEvent.name}
              value={measurementProperty}
              onChange={setMeasurementProperty}
              emptyLabel={t('argus.analytics.selectProperty', 'Select Property')} highlightEmpty
            />
          </Box>
        )}
        ...
        {/* Frequency */}
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          <Typography variant="caption" color="text.secondary" sx={{ minWidth: 50, flexShrink: 0, fontSize: '0.7rem' }}>{t('argus.analytics.atLeast', 'At least')}</Typography>
          <input
            type="number"
            value={minFrequency}
            ...
            style={{
              ...
              borderRadius: 4, color: 'inherit', outline: 'none',
              width: 50, fontSize: '0.8rem', fontFamily: 'inherit',
              padding: '2px 6px', textAlign: 'center',
            }}
          />
          <Typography variant="caption" color="text.secondary">{t('argus.analytics.times', 'times')}</Typography>
        </Box>
      </Box>
      ...
      <Box sx={{ mt: 'auto', pt: 2 }}>
        <Button fullWidth variant="contained" size="small" startIcon={<RunIcon />}
          onClick={handleRun}
  ...
  const toolbar = (
    <>
      ...
      <CsvExportButton data={csvData} filename="retention" disabled={csvData.length === 0} />
    </>
  );
        ...
        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
          <LineChart data={curveData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="average" stroke="#6366f1" strokeWidth={3}
              dot={{ r: 3 }} name={t('argus.analytics.average', 'Average')} />
            {/* Individual cohorts (thin, transparent) */}
            {cohortKeys.slice(0, 10).map((key, idx) => (
              <Line key={key} type="monotone" dataKey={key}
                stroke={SERIES_COLORS[idx % SERIES_COLORS.length]}
                strokeWidth={1} strokeOpacity={0.4} dot={false} />
            ))}
          </LineChart>
        </ResponsiveContainer>
      </Box>
        ...
        <Box sx={{ textAlign: 'center' }}>
          ...
          <Typography variant="subtitle1" color="text.secondary" sx={{ mt: 1 }}>
            {t('argus.analytics.avgRetentionLabel', 'Average {{period}} Retention', {
              period: retentionType === 'day' ? t('argus.analytics.day1', 'Day 1') : retentionType === 'week' ? t('argus.analytics.week1', 'Week 1') : t('argus.analytics.month1', 'Month 1'),
            })}
          </Typography>
        </Box>
      </Box>
  ...
  const renderHeatmapTable = () => {
    ...
    return (
      <Box sx={{ overflowX: 'auto', mt: 2, borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}` }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
          <thead>
            <tr>
              <th style={{ textAlign: 'left', padding: '12px 16px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, color: theme.palette.text.secondary, fontWeight: 600, position: 'sticky', left: 0, background: theme.palette.background.paper, zIndex: 2 }}>
                {t('argus.analytics.cohortDate', 'Cohort Date')}
              </th>
              <th style={{ textAlign: 'center', padding: '12px 16px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, color: theme.palette.text.secondary, fontWeight: 600 }}>
                {t('argus.analytics.cohortSize', 'Size')}
              </th>
              {cohorts[0]?.retention?.map((_: any, i: number) => (
                <th key={i} style={{ padding: '8px 8px', textAlign: 'center', borderBottom: `2px solid ${isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, color: theme.palette.text.secondary, fontWeight: 600, minWidth: 42 }}>
                  {retentionType === 'day' ? `D${i}` : retentionType === 'week' ? `W${i}` : `M${i}`}
                </th>
              ))}
            </tr>
          </thead>
          ...
          >
            {!hasQueried ? (
              <EmptyPagePlaceholder
                message={t('argus.analytics.emptyRetention', 'Define cohort events and click Run to see retention.')}
                minHeight={300}
              />
            ) : cohorts.length === 0 ? (
              <EmptyPagePlaceholder
                message={t('argus.analytics.noData', 'No retention data for the selected period.')}
                minHeight={300}
              />
            ) : (
