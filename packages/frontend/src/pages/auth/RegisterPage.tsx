...
const RegisterPage: React.FC = () => {
  ...
  const {
    ...
    resolver,
    mode: 'onChange', // ?§ÏãúÍ∞?Í≤ÄÏ¶ùÏùÑ ?ÑÌï¥ onChangeÎ°?Î≥ÄÍ≤?    defaultValues: {
      name: '',
      email: '',
      password: '',
      confirmPassword: '',
  ...
  const validateInvitation = async (token: string) => {
    ...
    try {
      ...
      if (result.valid && result.invitation) {
        ...
        // Ï¥àÎ?Î∞õÏ? Í≤ΩÏö∞ ?¥Î©î???ÑÎìúÎ•?ÎØ∏Î¶¨ Ï±ÑÏ? (?àÎäî Í≤ΩÏö∞)
        if (result.invitation.email) {
          setValue('email', result.invitation.email);
        }
      } else {
        // Ï¥àÎ? ÎßÅÌÅ¨Í∞Ä ?†Ìö®?òÏ? ?äÏ? Í≤ΩÏö∞ ?ÑÏö© ?òÏù¥ÏßÄÎ°?Î¶¨Îã§?¥Î†â??        navigate('/invalid-invite', { replace: true });
      }
    } catch (error: any) {
      console.error('Failed to validate invitation:', error);
      // 404??Í∏∞Ì? ?êÎü¨??Í≤ΩÏö∞???†Ìö®?òÏ? ?äÏ? Ï¥àÎ?Î°?Ï≤òÎ¶¨
  ...
  const onSubmit = async (data: RegisterData & { confirmPassword: string }) => {
    ...
    try {
      ...
      if (inviteToken && invitation) {
        // Ï¥àÎ?Î•??µÌïú Í∞Ä??        await invitationService.acceptInvitation(inviteToken, {
          username: data.name,
      ...
      // ?±Í≥µ ??ÏµúÏÜå 2Ï¥??ÄÍ∏?      const elapsed = Date.now() - startTime;
      const remainingTime = Math.max(0, 2000 - elapsed);
      if (remainingTime > 0) {
        await new Promise(resolve => setTimeout(resolve, remainingTime));
      }

      // ?±Í≥µ ?úÏóêÎß??êÎü¨ Î©îÏãúÏßÄ ÏßÄ?∞Í∏∞
      setRegisterError(null);
      setRegisteredEmail(data.email); // Save registered email
      setRegisterSuccess(true);
    } catch (err: any) {
      // ?êÎü¨ ?úÏóê??ÏµúÏÜå 2Ï¥??ÄÍ∏?      const elapsed = Date.now() - startTime;
      const remainingTime = Math.max(0, 2000 - elapsed);
      if (remainingTime > 0) {
        await new Promise(resolve => setTimeout(resolve, remainingTime));
      }
