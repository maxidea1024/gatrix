...
// ?¨Ïö©??ÏπúÌôî?ÅÏù∏ ?§Î•ò Î©îÏãúÏßÄ ?®Ïàò
...
const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // OAuth Î°úÎî© ?ÅÌÉú Ï¥àÍ∏∞??    setOauthLoading(null);
  ...
  const {
    ...
    resolver,
    mode: 'onChange', // ?§ÏãúÍ∞?validation ?úÏÑ±??    defaultValues: {
      email: '',
      password: '',
      rememberMe: false,
    },
  ...
  useEffect(() => {
    ...
    if (registeredEmail) {
      ...
      // Í∏∞Ïñµ???¥Î©î?ºÍ≥º ?§Ï†ï??Î™®Îëê ?àÏùÑ ?åÎßå ?ºÏóê ?§Ï†ï
      if (rememberedEmail && isRememberMeEnabled) {
        ...
        // Í∏∞Ïñµ???§Ï†ï???ÜÍ±∞??Î∂àÏùºÏπòÌïòÎ©?Ï¥àÍ∏∞??        reset({
          email: '',
          password: '',
          rememberMe: false,
        });
  ...
  const onSubmit = async (data: LoginCredentials & { rememberMe: boolean }) => {
    ...
    try {
      ...
      // ÏµúÏÜå 2Ï¥??ÄÍ∏?      const startTime = Date.now();
      ...
      // ÏµúÏÜå 2Ï¥àÍ? ÏßÄ?òÏ? ?äÏïò?§Î©¥ Ï∂îÍ? ?ÄÍ∏?      const elapsed = Date.now() - startTime;
      if (elapsed < 2000) {
        await Promise.all([
          loginPromise,
      ...
      if (err.message === 'Network Error' || err.code === 'ERR_NETWORK' || !err.status) {
        const networkError = t('auth.errors.networkError') || '?úÎ≤Ñ???∞Í≤∞?????ÜÏäµ?àÎã§. ?§Ìä∏?åÌÅ¨ ?∞Í≤∞???ïÏù∏?òÍ±∞???†Ïãú ???§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.';
  ...
  const handleOAuthLogin = async (provider: string, authUrl: string) => {
    ...
    // ÏµúÏÜå 2Ï¥??ÄÍ∏?    await new Promise(resolve => setTimeout(resolve, 2000));
    ...
    // ?òÏù¥ÏßÄ ?¥Îèô ?ÑÏóê ?Ä?ÑÏïÑ???ïÎ≥¥Î•?sessionStorage???Ä??    sessionStorage.setItem('oauthTimeout', timeout.toString());
  ...
  // ?¥Î©î??Í∏∞Ïñµ?òÍ∏∞ Ï≤¥ÌÅ¨Î∞ïÏä§ Î≥ÄÍ≤??∏Îì§??  const handleRememberMeChange = (checked: boolean, onChange: (value: boolean) => void) => {
    if (checked) {
      // Ï≤¥ÌÅ¨?òÎ†§Í≥?????Î≥¥Ïïà Í≤ΩÍ≥† ?úÏãú
      setPendingRememberMe(true);
      setShowRememberMeWarning(true);
    } else {
      // Ï≤¥ÌÅ¨ ?¥Ï†ú???åÎäî Ï¶âÏãú ?ÅÏö©?òÍ≥† ?Ä?•Îêú ?¥Î©î?ºÎèÑ ??†ú
      onChange(false);
      AuthService.clearRememberedCredentials();
    }
  };

  // Î≥¥Ïïà Í≤ΩÍ≥† ?ïÏù∏ ??  const handleRememberMeConfirm = () => {
    setValue('rememberMe', true);
    setShowRememberMeWarning(false);
    setPendingRememberMe(false);
  };

  // Î≥¥Ïïà Í≤ΩÍ≥† Ï∑®ÏÜå ??  const handleRememberMeCancel = () => {
    setShowRememberMeWarning(false);
    setPendingRememberMe(false);
  };
