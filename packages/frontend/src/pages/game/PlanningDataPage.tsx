...
const PlanningDataPage: React.FC = () => {
    ...
    if (
      ...
      const loadItems = async () => {
        try {
          ...
          setRewardTypeItems((prev) => ({
            ...prev,
            [currentRewardType.value]: items,
          }));
        } catch (error: any) {
          // Extract user-friendly error message
          let errorMessage = t("planningData.errors.loadItemsFailed");
  ...
  const filteredHotTimeBuffItems = useMemo(() => {
    ...
    debouncedHotTimeBuffSearchTerm,
    hotTimeBuffSortBy,
    hotTimeBuffSortOrder,
  ]);
                                  ...
                                  (item: HotTimeBuffItem) => {
                                    ...
                                    const formatHour = (utcHour: number) => {
                                      ...
                                      const localTime =
                                        ...
                                        "00:00:00";
                                      const localHourMin = localTime.substring(
                                        0,
                                        5,
                                      ); // HH:mm
                                            ...
                                            item.worldBuffId.length > 0
                                              ? item.worldBuffId.map(
                                                  (buffId, index) => {
                                                    ...
                                                    const enName =
                                                      item.worldBuffNamesEn?.[
                                                        index
                                                      ];
                                                    const cnName =
                                                      item.worldBuffNamesCn?.[
                                                        index
                                                      ];
                                      ...
                                      .map((item: any) => {
                                        ...
                                        const calculatePeriod = (
                                          ...
                                          if (!startDate || !endDate)
                                            return "-";
                                          try {
                                            const startISO = startDate.includes(
                                              "T",
                                            )
                                              ? startDate
                                              : new Date(
