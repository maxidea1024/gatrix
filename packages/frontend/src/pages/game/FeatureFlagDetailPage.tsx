...
const FeatureFlagDetailPage: React.FC = () => {
  ...
  const loadEnvironments = useCallback(async () => {
    try {
      ...
      if (
        selectedEnvironment &&
        envs.some((e) => e.environment === selectedEnvironment)
      ) {
  ...
  const distributeWeights = (variants: Variant[]): Variant[] => {
    ...
    const totalLockedWeight = lockedVariants.reduce(
                                                      ...
                                                      <Box sx={{ mb: 1.5 }}>
                                                        ...
                                                        >
                                                          ...
                                                          >
                                                            ...
                                                            >
                                                              {t(
                                                                "featureFlags.ofYourBaseMatching",
                                                              )}
                                                            </Typography>
                                                          </Box>
                                                        </Paper>
                    ...
                    {flag?.variantType === "none" ? (
                      ...
                      <Stack spacing={2}>
                        ...
                        {editingStrategy.variants.map((variant, index) => {
                          ...
                          const hasJsonError =
                            strategyJsonErrors[index] !== undefined &&
                            strategyJsonErrors[index] !== null;
                          const colors = [
                            "#7C4DFF",
                            "#448AFF",
                            "#00BFA5",
                          ...
                          const variantCount = editingStrategy.variants?.length || 0;
                          const hasAnyLockedVariant = editingStrategy.variants?.some(v => v.weightLock);
                          const isCurrentLocked = variant.weightLock;
                          // Show weight controls only when more than 1 variant
                          const showWeightControls = variantCount > 1;
                          // Show fixed checkbox only if no other variant is locked, or this one is locked
                          const showFixedCheckbox = showWeightControls && (!hasAnyLockedVariant || isCurrentLocked);
                          return (
                            <Paper
                              key={index}
                              variant="outlined"
                            ...
                            >
                              ...
                              {/* Weight Controls - only show when more than 1 variant */}
                              {showWeightControls && (
                                    ...
                                    <FormControlLabel
                                      control={
                                        <Checkbox
                                          ...
                                          onChange={(e) =>
                                            toggleWeightLock(index, e.target.checked)
                                          }
                                        />
                                      }
                                      label={
                                  ...
                                  <TextField
                                    ...
                                    onChange={(e) => {
                                      if (variant.weightLock) {
                                        ...
                                        updateVariantWeight(index, newWeight, true);
                                      }
                                    }}
                                    InputProps={{
                                      endAdornment: "%",
                              ...
                              <Box>
                                ...
                                >
                                  ...
                                  <Tooltip title={t("featureFlags.payloadHelp")}>
                                    <HelpOutlineIcon fontSize="small" color="action" />
                                  </Tooltip>
                                </Typography>
                        ...
                        {(() => {
                          const total =
                            editingStrategy.variants?.reduce(
                              (sum, v) => sum + (v.weight || 0),
                              0,
                            ) || 0;
                          if (total !== 100) {
