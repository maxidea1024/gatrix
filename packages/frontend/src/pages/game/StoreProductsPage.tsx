...
const StoreProductsPage: React.FC<{ embedded?: boolean }> = ({ embedded }) => {
  ...
  // Stats state
  const [productStats, setProductStats] = useState<StoreProductStats>({
    total: 0,
    active: 0,
    inactive: 0,
    overridden: 0,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await storeProductService.deleteStoreProduct(
        projectApiPath,
        deletingProduct.id
      );
  ...
  const handleBulkDeleteConfirm = async () => {
    ...
    try {
      await storeProductService.deleteStoreProducts(
        projectApiPath,
        selectedIds
      );
          ...
          >
            ...
            >
              ...
              <Button
                variant="text"
                size="small"
                startIcon={<BatchProcessIcon sx={{ fontSize: 16 }} />}
