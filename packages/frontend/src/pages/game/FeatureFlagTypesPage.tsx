...
const getTypeIcon = (iconName: string | null) => {
    switch (iconName) {
        case 'RocketLaunch': return <ReleaseIcon sx={{ color: 'primary.main' }} />;
        case 'Science': return <ExperimentIcon sx={{ color: 'secondary.main' }} />;
        case 'Build': return <OperationalIcon sx={{ color: 'warning.main' }} />;
        case 'PowerSettingsNew': return <KillSwitchIcon sx={{ color: 'error.main' }} />;
        case 'VpnKey': return <PermissionIcon sx={{ color: 'info.main' }} />;
...
const FeatureFlagTypesPage: React.FC = () => {
    ...
    const loadTypes = useCallback(async () => {
        ...
        try {
            ...
            enqueueSnackbar(parseApiErrorMessage(error, t('common.loadFailed')), { variant: 'error' });
        } finally {
            setLoading(false);
        }
    }, [enqueueSnackbar, t]);
                                ...
                                control={
                                    <Checkbox
                                        ...
                                        onChange={(e) => setEditingType(prev =>
                                            prev ? { ...prev, lifetimeDays: e.target.checked ? null : 40 } : prev
                                        )}
                                    />
                                }
