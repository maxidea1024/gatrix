    ...
    <Box sx={{ p: 3 }}>
                ...
                <TableBody>
                  {paginatedTables.map((table) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Chip label={`v${table.version}`} size="small" variant="outlined" sx={{ fontFamily: 'monospace' }} />
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell align="center">
                        {table.runtime ? (
                          table.runtime.synced ? (
                            <Chip label="동기화됨" size="small" color="default" sx={{ fontSize: '0.68rem', height: 22, color: 'text.secondary' }} />
                          ) : (
                            <Tooltip title={`서버: v${table.runtime.loadedVersion} / DB: v${table.version}`}>
                              <Button
                                size="small"
                                variant="contained"
                                color="error"
      ...
      >
        <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, p: 2, gap: 1.5 }}>
              {editorCurrentTable && (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
                  <Chip label={`v${editorCurrentTable.version}`} size="small" variant="outlined" sx={{ fontFamily: 'monospace' }} />
                  <Typography variant="caption" sx={{ fontFamily: 'monospace', color: 'text.disabled' }}>
                    {editorCurrentTable.contentHash?.slice(0, 12)}
                  </Typography>
                  {editorData !== editorOriginalData ? (
                    <Chip label="변경됨" size="small" color="warning" sx={{ fontSize: '0.65rem', height: 18 }} />
                  ) : (
                    <Chip label="동일" size="small" color="default" sx={{ fontSize: '0.65rem', height: 18 }} />
                  )}
                  <Box sx={{ ml: 'auto' }} />
              ...
              {editorLoading ? (
                ...
                <Box sx={{ flex: 1, minHeight: 0 }}>
                  <JsonEditor
                    ...
                    helperText={!editorJsonValid ? 'JSON 문법 오류가 있습니다' : editorData !== editorOriginalData ? '변경사항이 있습니다' : '현재 버전의 데이터입니다'}
                  />
                </Box>
              )}
              <TextField
          ...
          {/* ── Tab 1: History ── */}
          {detailTab === 1 && (
            <Box sx={{ p: 2, flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: viewingVersion != null ? 'hidden' : 'auto' }}>
              {historyLoading ? (
                <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4 }}><CircularProgress /></Box>
              ) : historyData?.history?.length ? (
                <Stack spacing={1.5} sx={{
                  ...(viewingVersion != null ? { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' } : { overflowY: 'auto' }),
                }}>
                  {historyData.history.map((entry) => (
                    <React.Fragment key={entry.version}>
                      <Paper variant="outlined" sx={{
                        borderRadius: 2, overflow: 'hidden', flexShrink: 0,
                        borderColor: entry.isActive ? 'success.main' : viewingVersion === entry.version ? 'primary.main' : 'divider',
                        borderWidth: entry.isActive ? 1.5 : 1,
                        transition: 'border-color 0.2s, box-shadow 0.2s',
                        '&:hover': { borderColor: entry.isActive ? 'success.main' : 'primary.light' },
                      }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 0.75, bgcolor: (theme) => theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.015)', borderBottom: '1px solid', borderColor: 'divider' }}>
                          <Chip label={`v${entry.version}`} size="small" color={entry.isActive ? 'success' : 'default'} sx={{ fontWeight: 700, fontFamily: 'monospace', fontSize: '0.75rem', height: 22 }} />
                          {entry.isActive && <Chip label="적용 중" size="small" color="success" sx={{ fontSize: '0.65rem', height: 20 }} />}
                          <Box sx={{ flex: 1 }} />
                          <Stack direction="row" spacing={0.25}>
                            <Tooltip title={t('cms.history.viewData')}>
                              <IconButton size="small" color={viewingVersion === entry.version ? 'primary' : 'default'} onClick={() => viewVersionData(entry.version)}>
                                <VisibilityIcon sx={{ fontSize: 18 }} />
                              </IconButton>
                            </Tooltip>
                            {!entry.isActive && (
                              <Tooltip title={`v${entry.version} 롤백`}>
                                <IconButton size="small" color="warning" onClick={() => confirmRollback(detailTableName, entry.version)}>
                        ...
                        <Box sx={{ px: 2, py: 1.25 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 0.75 }}>
                            ...
                            <Tooltip title={new Date(entry.uploadedAt).toLocaleString('ko-KR')}>
                              <Typography variant="caption" sx={{ color: 'text.secondary', cursor: 'help', fontWeight: 500 }}>{formatRelativeTime(entry.uploadedAt)}</Typography>
                            </Tooltip>
                            <Typography variant="caption" sx={{ color: 'text.disabled' }}>·</Typography>
                            <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 500 }}>{entry.uploadedBy || '—'}</Typography>
                          </Box>
                          {entry.comment ? (
                            <Typography variant="body2" sx={{ fontSize: '0.8rem', color: 'text.primary', lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                              {entry.comment}
                            </Typography>
                          ) : (
                            <Typography variant="body2" sx={{ fontSize: '0.78rem', color: 'text.disabled', fontStyle: 'italic' }}>{t('cms.history.noComment')}</Typography>
                          )}
                          <Typography variant="caption" sx={{ fontFamily: 'monospace', color: 'text.disabled', fontSize: '0.68rem', mt: 0.5, display: 'block' }}>{entry.contentHash?.slice(0, 16)}</Typography>
                        </Box>
                      </Paper>
          ...
          {detailTab === 2 && (
            <Box sx={{ p: 2, flex: 1, minHeight: 0, overflowY: 'auto' }}>
              {propagationLoading ? (
                ...
                <EmptyPlaceholder message="전파 내역이 없습니다" minHeight={120} />
              ) : (() => {
                          ...
                          <Box sx={{ px: 2, py: 1, display: 'flex', alignItems: 'center', gap: 1, bgcolor: (theme) => theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.02)' : 'grey.50', borderBottom: '1px solid', borderColor: 'divider' }}>
                            ...
                            {events[0].tableName && (
                              <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', alignItems: 'center' }}>
                                {events[0].tableName.split(',').map(t => t.trim()).filter(Boolean).map((t, idx, arr) => {
                                  if (idx > 2) {
                                    if (idx === 3) {
                                      return (
