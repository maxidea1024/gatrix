...
export function useIssueActions({
  ...
  const assign = useCallback(async (assignee: string) => {
    if (!projectId || !issueId || !issue) return;
    updateIssueOptimistic((prev) => ({ ...prev, assigned_to: assignee || null }));
    try {
  ...
  const subscribe = useCallback(async (value: boolean) => {
  ...
  const bookmark = useCallback(async (value: boolean) => {
    if (!projectId || !issueId) return;
    try {
