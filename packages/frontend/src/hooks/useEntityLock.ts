...
export function useEntityLock({
    ...
    useEffect(() => {
        const handleLockReleased = async (event: CustomEvent) => {
            const { table: releasedTable, entityId: releasedEntityId, environment: releasedEnv } = event.detail;
            ...
            if (
                ...
                try {
                    const result = await entityLockService.acquireLock(table, entityId, environmentRef.current);
                    if (result.success) {
                        setHasLock(true);
                        setLockedBy(null);
                        lockAcquiredRef.current = true;
                        enqueueSnackbar(t('entityLock.lockAcquired'), { variant: 'success' });
                    }
                } catch (error) {
    ...
    useEffect(() => {
        const handleLockTakenOver = (event: CustomEvent) => {
            const { table: takenTable, entityId: takenEntityId, environment: takenEnv, newOwner } = event.detail;
            ...
            if (
                ...
                // Show notification
                enqueueSnackbar(
                    t('entityLock.lockTakenBy', { userName: newOwner.userName, userEmail: newOwner.userEmail }),
                    { variant: 'warning' }
                );
        ...
        const handleLockAcquisition = async () => {
            ...
            if (lockAcquiredRef.current && softLockEnabled && environment) {
                ...
                heartbeatRef.current = setInterval(async () => {
                    ...
                    const success = await entityLockService.extendLock(table, entityId, environmentRef.current);
                    if (!success) {
