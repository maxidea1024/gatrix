  ...
  return (
    <>
      ...
      >
        ...
        {clipboardUrls.length > 0 && (
          <>
            ...
            <MenuItem
              onClick={() => {
                onAddFromClipboardBefore(clipboardUrls);
                handleCloseContextMenu();
              }}
            >
              <ListItemIcon>
            ...
            <MenuItem
              onClick={() => {
                onAddFromClipboardAfter(clipboardUrls);
                handleCloseContextMenu();
              }}
            >
              <ListItemIcon>
      ...
      >
        ...
        <DialogActions>
          <Button
            onClick={() => setEmptyFrameCountDialogOpen(null)}
            variant="contained"
          >
            {t('common.cancel')}
          </Button>
...
const SequenceEditor: React.FC<SequenceEditorProps> = ({
  ...
  const handleDeleteFrame = (frameIndex: number) => {
    onUpdate({
      ...sequence,
      frames: sequence.frames.filter((_, i) => i !== frameIndex),
    });
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      onUpdate({
        ...sequence,
        frames: arrayMove(sequence.frames, oldIndex, newIndex),
      });
    }
  };
                  ...
                  >
                    {sequence.frames.map((frame, frameIndex) => (
                      <FrameEditor
                        ...
                        onAddMultipleEmptyAfter={(count) =>
                          handleAddMultipleEmptyAfter(frameIndex, count)
                        }
                        onAddFromClipboardBefore={(urls) =>
                          handleAddFromClipboardBefore(frameIndex, urls)
                        }
                ...
                >
                  ...
                  >
                    {sequence.frames.map((frame, frameIndex) => {
                      ...
                      const startTime = sequence.frames
                        .slice(0, frameIndex)
