...
const DateRangePicker: React.FC<DateRangePickerProps> = ({
  ...
  onChange,
  preset = 'last7d',
  availablePresets = ['today', 'yesterday', 'last7d', 'last30d', 'last3m', 'last6m', 'last12m', 'ytd', 'custom'],
  size = 'small',
  showTime = false,
}) => {
  ...
  const CustomDay = (props: PickersDayProps<Dayjs>) => {
    ...
    const isStart = dayStart.year() === fromStart.year() &&
    ...
    const isInRange = toStart && tempFrom
  ...
  return (
    <Box>
      ...
      >
        {availablePresets.filter(p => p !== 'custom').map((presetValue) => (
          <Button
            key={presetValue}
