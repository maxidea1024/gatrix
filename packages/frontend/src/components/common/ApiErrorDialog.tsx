...
const ApiErrorDialog: React.FC<ApiErrorDialogProps> = ({
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, color: type === 'LOCKED' ? 'error.main' : type === 'DUPLICATE' ? 'error.main' : 'warning.main' }}>
                {type === 'LOCKED' ? <LockIcon /> : <WarningIcon />}
                {type === 'LOCKED' ? t('errors.RESOURCE_LOCKED_TITLE', 'Resource Locked') :
                    type === 'DUPLICATE' ? t('errors.DUPLICATE_ENTRY_TITLE', 'Duplicate Entry') :
            ...
            <DialogContent>
                <Box sx={{ mt: 1 }}>
                    {type === 'LOCKED' && (
                        <Box>
                            <Typography variant="body1" gutterBottom>
                                {t('errors.RESOURCE_LOCKED', { changeRequestTitle: lockedInfo?.changeRequestTitle || t('common.unknown') })}
                            </Typography>
                        ...
                        <Box>
                            ...
                            <Stack spacing={2}>
                                {conflictData?.currentLive && (
                                    <Box>
                                        <Typography variant="subtitle2" sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
                                            <StorageIcon fontSize="small" color="primary" /> {t('changeRequest.conflictDialog.liveData', 'Current Live Data')}
                                        </Typography>
                                        <Paper variant="outlined" sx={{ p: 1.5, bgcolor: 'action.hover', maxHeight: 200, overflow: 'auto' }}>
                                ...
                                {conflictData?.originalDraftBase && (
                                    <Box>
                                        <Typography variant="subtitle2" sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
                                            <HistoryIcon fontSize="small" color="info" /> {t('changeRequest.conflictDialog.originalData', 'Data When Request Created')}
                                        </Typography>
                                        <Paper variant="outlined" sx={{ p: 1.5, bgcolor: 'action.hover', maxHeight: 200, overflow: 'auto' }}>
                                            <Typography component="pre" variant="caption" sx={{ fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
                    ...
                    {type === 'DUPLICATE' && (
                        <Box>
                            <Typography variant="body1" gutterBottom sx={{ mb: 2 }}>
                                {t('changeRequest.duplicateDialog.description', 'A record with the same unique identifier already exists. This usually happens when the data you are trying to merge has already been added to the live server.')}
                            </Typography>
                            <Paper variant="outlined" sx={{ p: 2, bgcolor: (theme) => alpha(theme.palette.error.main, 0.05), borderColor: 'error.main' }}>
                            ...
                            <Typography variant="body2" sx={{ mt: 2, color: 'text.secondary' }}>
                                {t('changeRequest.duplicateDialog.guide', 'Please check the existing data on the live server and delete or modify this change request.')}
                            </Typography>
                        </Box>
                    )}
