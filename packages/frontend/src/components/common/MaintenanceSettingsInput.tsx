...
const MaintenanceSettingsInput: React.FC<MaintenanceSettingsInputProps> = ({
  ...
  const statusTooltip = useMemo(() => {
    ...
    parts.push(
      `${t("maintenance.tooltipStartTime")}: ${startDate ? formatDateTimeDetailed(startDate) : t("maintenance.immediateStart")}`,
    );
    // End time: show "manual stop" if not set
    parts.push(
      `${t("maintenance.tooltipEndTime")}: ${endDate ? formatDateTimeDetailed(endDate) : t("maintenance.manualStop")}`,
    );
    ...
    if (maintenanceMessage) {
      parts.push(
        `${t("maintenance.tooltipMessage")}: ${maintenanceMessage.length > 50 ? maintenanceMessage.substring(0, 50) + "..." : maintenanceMessage}`,
      );
    }
    return parts.join("\n");
  }, [
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e: SelectChangeEvent<number | "">) =>
                  onSelectedTemplateIdChange(e.target.value as number | "")
                }
