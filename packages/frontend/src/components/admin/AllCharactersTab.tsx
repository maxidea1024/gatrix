...
const COL_STORAGE_KEY = 'allCharacters.columns';
...
export default function AllCharactersTab({
  projectApiPath,
  worlds,
}: AllCharactersTabProps) {
  ...
  const MULTI_SEARCH_SIZE_KEY = 'allCharacters_multiSearchSize';
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const result = await playerConnectionService.getAllCharacters(
        projectApiPath,
        {
  ...
  const handleExport = useCallback(
    async (format: 'csv' | 'xlsx') => {
      ...
      try {
        ...
        // Build export columns (English headers for data export)
        ...
        // Transform data for export (format booleans and dates)
        ...
        exportToFile(exportData, exportColumns, 'all-characters', format);
