...
import changeRequestService, { ChangeRequest, ActionGroup, ChangeItem } from '@/services/changeRequestService';
...
const SubmitPreviewDrawer: React.FC<SubmitPreviewDrawerProps> = ({
    ...
    const getOpTypeIcon = (opType: string) => {
        switch (opType) {
            case 'CREATE': return <AddIcon fontSize="small" sx={{ color: 'success.main' }} />;
            case 'DELETE': return <DeleteIcon fontSize="small" sx={{ color: 'error.main' }} />;
            default: return <EditIcon fontSize="small" sx={{ color: 'warning.main' }} />;
        }
    };
        ...
        >
            <Box sx={{ p: 3, display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                <Box sx={{ flex: 1, overflow: 'auto', mb: 3 }}>
                    ...
                    {changeRequest.changeItems?.filter(item => !item.actionGroupId).map((item) => (
                ...
                {selectedCount < totalCount && (
                    <Alert severity="warning" sx={{ mb: 2 }}>
                        {t('changeRequest.submitExcludeWarning', { count: totalCount - selectedCount })}
                    </Alert>
                )}
