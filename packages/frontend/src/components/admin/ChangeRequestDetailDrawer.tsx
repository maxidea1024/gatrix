...
const STATUS_CONFIG: Record<ChangeRequestStatus, { color: 'default' | 'primary' | 'secondary' | 'error' | 'info' | 'success' | 'warning'; labelKey: string; bgColor: string }> = {
    ...
    conflict: { color: 'warning', labelKey: 'changeRequest.status.conflict', bgColor: '#d29922' },
};
...
const ChangeRequestDetailDrawer: React.FC<ChangeRequestDetailDrawerProps> = ({
    ...
    const allChanges = useMemo(() => {
        ...
        return cr.changeItems.map((item: any) => {
            ...
            const operation = item.opType === 'CREATE' ? 'create' :
                item.opType === 'DELETE' ? 'delete' : 'update';
                        ...
                        <Box sx={{ flex: 1, overflow: 'auto', p: 3 }}>
                            ...
                            {activeTab === 0 && (
                                <Box>
                                                    ...
                                                    <>
                                                        ...
                                                        <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                                                            <Button
                                                                variant="outlined"
                                                                color="error"
                                                                startIcon={actionLoading ? <CircularProgress size={16} color="inherit" /> : <CloseIcon />}
                                        ...
                                        <Paper sx={{ p: 2, bgcolor: (theme) => alpha(theme.palette.error.main, 0.1), border: 1, borderColor: 'error.main' }}>
                                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                ...
                                                <Box sx={{ display: 'flex', gap: 1 }}>
                                                    <Button variant="outlined" color="warning" startIcon={actionLoading ? <CircularProgress size={16} color="inherit" /> : <UndoIcon />} onClick={handleReopen} disabled={actionLoading}>
                                                        {t('changeRequest.actions.reopen')}
                                                    </Button>
                                                    <Button variant="outlined" color="error" startIcon={actionLoading ? <CircularProgress size={16} color="inherit" /> : <DeleteIcon />} onClick={handleDelete} disabled={actionLoading}>
                                                        {t('common.delete')}
                                                    </Button>
                                                </Box>
                                            </Box>
                                    ...
                                    {cr.status === 'conflict' && (cr.requesterId === user?.id || user?.role === 'admin' || user?.role === 0) && (
                                    ...
                                    {cr.status === 'approved' && (
                                        <Paper sx={{ p: 2, bgcolor: (theme) => alpha(theme.palette.success.main, 0.1), border: 1, borderColor: 'success.main' }}>
                                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                ...
                                                <Button variant="contained" color="success" startIcon={actionLoading ? <CircularProgress size={16} color="inherit" /> : <MergeIcon />} onClick={handleExecute} disabled={actionLoading}>
                                    ...
                                    {cr.status === 'applied' && (
                                        <Paper sx={{ p: 2, bgcolor: (theme) => alpha(theme.palette.info.main, 0.1), border: 1, borderColor: 'info.main' }}>
                                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                <Box>
                                        ...
                                        <Paper variant="outlined" sx={{ p: 2 }}>
                                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                ...
                                                <Box sx={{ display: 'flex', gap: 1 }}>
                                                    ...
                                                    <Button variant="contained" startIcon={<SendIcon />} onClick={() => setSubmitPreviewOpen(true)} disabled={actionLoading}>
                                                                        ...
                                                                        <Collapse in={isItemExpanded} timeout="auto">
                                                                            <Box sx={{ px: 2, py: 1 }}>
                                                                                ...
                                                                                {item.operation === 'delete' && (
                                                                                    <Typography variant="body2" color="error" sx={{ textAlign: 'center', py: 2, fontStyle: 'italic' }}>
                                                                                        {t('changeRequest.opDelete')}
                                                                                    </Typography>
                                                                                )}

                                                                                {/* CREATE: Table format (Field | Value) */}
                                                                                {item.operation === 'create' && item.changes.length > 0 && (
                                                                                    <Table size="small" sx={{ '& td, & th': { py: 0.75, px: 1.5, border: 'none' } }}>
                                                                                        <TableBody>
                                                                                            {item.changes.filter(c => !['updatedBy', 'createdBy', 'updatedAt', 'createdAt', 'id', 'version', 'environment'].includes(c.field)).map((change, i) => (
                                                                                                <TableRow key={i} sx={{ '&:nth-of-type(odd)': { bgcolor: 'action.hover' } }}>
                                                                                                    <TableCell sx={{ fontWeight: 600, color: 'text.secondary', width: '35%', fontSize: '0.875rem' }}>
                                                                                                        {formatFieldName(item.table, change.field)}
                                                                                                    </TableCell>
                                                                                                    <TableCell sx={{ color: 'success.main', fontWeight: 500, fontSize: '0.875rem' }}>
                                                                                ...
                                                                                {item.operation === 'update' && item.changes.filter(c => !['updatedBy', 'createdBy', 'updatedAt', 'createdAt'].includes(c.field)).map((change, i) => (
                                                                                    <Box
                                                                                        key={i}
                                                                                        sx={{
                                                                                            display: 'flex',
                                                                                ...
                                                                                {item.operation === 'update' && item.changes.filter(c => !['updatedBy', 'createdBy', 'updatedAt', 'createdAt'].includes(c.field)).length === 0 && (
