...
const SafeguardPanel: React.FC<SafeguardPanelProps> = ({ flowId, milestoneId, canManage }) => {
    ...
    const getTimeRangeLabel = (range: string) => {
        switch (range) {
            case 'hour': return t('releaseFlow.safeguard.timeRangeHour');
            case 'day': return t('releaseFlow.safeguard.timeRangeDay');
            case 'week': return t('releaseFlow.safeguard.timeRangeWeek');
            case 'month': return t('releaseFlow.safeguard.timeRangeMonth');
        ...
        <Box>
            ...
            {safeguards.length > 0 && (
                <Table size="small">
                    ...
                    <TableBody>
                        {safeguards.map((sg) => (
                            <TableRow key={sg.id} sx={sg.isTriggered ? { bgcolor: 'error.main', '& td': { color: 'error.contrastText' } } : undefined}>
                                <TableCell>
                                    ...
                                    <TableCell align="right">
                                        <Stack direction="row" spacing={0.5} justifyContent="flex-end">
                                            ...
                                            <Tooltip title={t('releaseFlow.safeguard.deleteSafeguard')}>
                                                <IconButton size="small" color="error" onClick={() => handleDeleteClick(sg)}>
                        ...
                        <FormControl size="small" sx={{ minWidth: 150 }}>
                            ...
                            >
                                {TIME_RANGE_OPTIONS.map((opt) => (
                                    <MenuItem key={opt} value={opt}>{getTimeRangeLabel(opt)}</MenuItem>
                                ))}
                            </Select>
                        </FormControl>
                    </Stack>
