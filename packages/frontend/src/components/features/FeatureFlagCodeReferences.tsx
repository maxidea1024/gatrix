...
const FeatureFlagCodeReferences: React.FC<FeatureFlagCodeReferencesProps> = ({
  flagName,
  onLoad,
}) => {
    ...
    const fetchReferences = async () => {
      ...
      try {
        const response = await api.get(
          `${projectApiPath}/features/${flagName}/code-references`
        );
        if (response.success) {
        ...
        {scanInfo && (
          ...
          >
            <Typography variant="caption" color="text.secondary">
              ...
              {formatRelativeTime(scanInfo.scanTime)}
            </Typography>
            {scanInfo.commitHash && (
        ...
        <TableContainer component={Paper} variant="outlined">
          <Table size="small">
            <TableHead sx={{ bgcolor: 'action.hover' }}>
              <TableRow>
                <TableCell width="40%">
                  {t('featureFlags.codeReferences.file')}
                </TableCell>
                <TableCell width="10%">
                  {t('featureFlags.codeReferences.line')}
                </TableCell>
                <TableCell width="20%">
                  {t('featureFlags.codeReferences.function')}
                </TableCell>
                <TableCell width="10%">
                  {t('featureFlags.codeReferences.language')}
                </TableCell>
                <TableCell width="10%">
                  {t('featureFlags.codeReferences.confidence')}
                </TableCell>
                ...
                <React.Fragment key={ref.id}>
                  <TableRow hover>
                    ...
                    <TableCell>
                      <Tooltip
                        title={t(
                          'featureFlags.codeReferences.confidenceLevel',
                          {
                    ...
                    <TableCell align="right">
                      {ref.codeUrl && (
                        <IconButton
                          ...
                          rel="noopener noreferrer"
                          title={t(
                            'featureFlags.codeReferences.viewInRepository'
                          )}
                        >
                    ...
                    <TableRow>
                      ...
                      >
                        <Box sx={{ px: 2, py: 1 }}>
                          ...
                          <SyntaxHighlighter
                            language={
                              ref.language?.toLowerCase() || 'typescript'
                            }
                            style={syntaxStyle}
