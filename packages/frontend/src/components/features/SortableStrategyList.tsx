...
const SortableStrategyItem: React.FC<SortableStrategyItemProps> = ({
    ...
    const getOperatorDisplay = (operator: string): string => {
        const opMap: Record<string, string> = {
            'str_eq': '=', 'str_neq': '≠', 'str_contains': '∋',
            'str_starts_with': '⊆', 'str_ends_with': '⊇',
            'str_in': '∈', 'str_not_in': '∉',
            'num_eq': '=', 'num_gt': '>', 'num_gte': '≥',
            'num_lt': '<', 'num_lte': '≤',
            'bool_is': '=',
            'date_gt': '>', 'date_gte': '≥', 'date_lt': '<', 'date_lte': '≤',
            'semver_eq': '=', 'semver_gt': '>', 'semver_gte': '≥',
            'semver_lt': '<', 'semver_lte': '≤',
        };
    ...
    const getValueDisplay = (c: Constraint): string => {
        if (c.values && c.values.length > 0) {
            return c.values.length <= 3 ? c.values.join(', ') : `${c.values.slice(0, 3).join(', ')}... (+${c.values.length - 3})`;
        }
        return c.value || '';
    };
                    ...
                    <Stack spacing={2}>
                        ...
                        <FormControl fullWidth size="small">
                            ...
                            >
                                ...
                                <MenuItem value="flexibleRollout">{t('featureFlags.strategyTypes.flexibleRollout')}</MenuItem>
                                <MenuItem value="userWithId">{t('featureFlags.strategyTypes.userWithId')}</MenuItem>
                                <MenuItem value="gradualRolloutUserId">{t('featureFlags.strategyTypes.gradualRolloutUserId')}</MenuItem>
                                <MenuItem value="remoteAddress">{t('featureFlags.strategyTypes.remoteAddress')}</MenuItem>
                                <MenuItem value="applicationHostname">{t('featureFlags.strategyTypes.applicationHostname')}</MenuItem>
                            </Select>
                        </FormControl>
                        ...
                        {strategy.name === 'flexibleRollout' && (
                            <Box sx={{ px: 1 }}>
                                ...
                                <Slider
                                    ...
                                    onChange={(_, value) => updateStrategy({
                                ...
                                <FormControl fullWidth size="small" sx={{ mt: 2 }}>
                                    ...
                                    <Select
                                        ...
                                        disabled={!canManage}
                                    >
                                        <MenuItem value="default">{t('featureFlags.stickinessOptions.default')}</MenuItem>
                                        <MenuItem value="userId">{t('featureFlags.stickinessOptions.userId')}</MenuItem>
                                        <MenuItem value="sessionId">{t('featureFlags.stickinessOptions.sessionId')}</MenuItem>
                                    ...
                                    return (
                                        <Box key={segmentName}>
                                            ...
                                            {isExpanded && segInfo.constraints?.length > 0 && (
                                                <Paper elevation={2} sx={{ mt: 1, p: 1.5, backgroundColor: 'background.default' }}>
                                                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                                        {segInfo.constraints.map((c: Constraint, cIdx: number) => (
                                                            <React.Fragment key={cIdx}>
                                                                {cIdx > 0 && (
                                                                    <Chip label="AND" size="small" sx={{ alignSelf: 'center', height: 18, fontSize: '0.65rem' }} />
                                                                )}
                                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                                                                    ...
                                                                    <Chip
                                                                        label={c.inverted ? `NOT ${getOperatorDisplay(c.operator)}` : getOperatorDisplay(c.operator)}
                                                                        size="small"
                                                                        variant="outlined"
