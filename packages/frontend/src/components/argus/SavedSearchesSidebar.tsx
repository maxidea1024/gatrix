...
import {
  Box, Typography, IconButton, Tooltip, Divider, Dialog,
...
const DEFAULT_SEARCHES: SavedSearch[] = [
  { id: 'default_unresolved', name: '', query: 'is:unresolved', sort: 'last_seen', isDefault: true, isPinned: false },
  { id: 'default_assigned_me', name: '', query: 'assigned:me is:unresolved', sort: 'last_seen', isDefault: true, isPinned: false },
  { id: 'default_bookmarked', name: '', query: 'is:unresolved bookmarks:me', sort: 'last_seen', isDefault: true, isPinned: false },
  { id: 'default_new_today', name: '', query: 'is:unresolved firstSeen:-24h', sort: 'first_seen', isDefault: true, isPinned: false },
  { id: 'default_errors', name: '', query: 'is:unresolved level:error OR level:fatal', sort: 'event_count', isDefault: true, isPinned: false },
];
...
const SavedSearchesSidebar: React.FC<SavedSearchesSidebarProps> = ({
  ...
  const [customSearches, setCustomSearches] = useState<SavedSearch[]>(loadSavedSearches);
  const [editDialog, setEditDialog] = useState<{ open: boolean; search: SavedSearch | null; isNew: boolean }>({
    open: false, search: null, isNew: false,
  });
  ...
  const allSearches = [
    ...DEFAULT_SEARCHES.map(s => ({ ...s, name: defaultSearchNames[s.id] || s.id })),
    ...customSearches,
  ];
  ...
  const handleSave = () => {
    ...
    if (editDialog.isNew) {
      ...
      setCustomSearches(prev =>
        prev.map(s => s.id === editDialog.search!.id
  ...
  return (
    ...
    >
      ...
      <Dialog
        ...
        onClose={() => setEditDialog({ open: false, search: null, isNew: false })}
        maxWidth="xs"
        fullWidth
        PaperProps={{ sx: { borderRadius: '12px' } }}
      >
        <DialogTitle sx={{ fontSize: '0.95rem', fontWeight: 700 }}>
          {editDialog.isNew ? t('argus.savedSearches.create') : t('argus.savedSearches.edit')}
        </DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button
            size="small"
            onClick={() => setEditDialog({ open: false, search: null, isNew: false })}
