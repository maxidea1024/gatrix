...
const SearchAutocompletePopover = forwardRef<
  ...
  (
    ...
    const getEditingContext = useCallback((q: string): EditingContext => {
      ...
      if (colonMatch) {
        ...
        if (field === 'has')
          return { state: 'has_field', partial: val.toLowerCase() };
        return {
          state: 'typing_value',
          field,
          partial: val.replace(/^["']/, '').toLowerCase(),
        };
      }
    ...
    const editingContext = useMemo(
      () => getEditingContext(query),
      [query, getEditingContext]
    );
    ...
    const items = useMemo((): AutocompleteItem[] => {
      ...
      switch (ctx.state) {
        ...
        case 'typing_field': {
          ...
          const isExactOp =
            p.toUpperCase() === 'AND' || p.toUpperCase() === 'OR';
          ...
          // Filtered fields
          fields
            .filter((f) => f.toLowerCase().includes(p))
            .forEach((f) => {
              result.push({
                key: `field-${f}`,
                type: 'field',
        ...
        case 'has_field': {
          const matched = ctx.partial
            ? fields.filter((f) => f.toLowerCase().includes(ctx.partial))
            : fields;
          matched.forEach((f) =>
            result.push({
              key: `has-${f}`,
              type: 'has-field' as const,
              label: f,
        ...
        if (elKey?.startsWith('syntax-')) {
          ...
          if (inSyntax && syntaxBuffer.length > 0) {
            finalElements.push(
              <Box
                key="syntax-wrap"
                sx={{ display: 'flex', gap: 1, px: 1, mb: 1 }}
              >
                {syntaxBuffer}
              </Box>
            );
