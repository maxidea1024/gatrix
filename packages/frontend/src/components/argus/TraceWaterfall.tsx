...
export const getOpColor = (op: string) =>
  OP_COLORS[op] || OP_COLORS[op.split('.')[0]] || '#9e9e9e';
...
const TraceWaterfall: React.FC<{
  trace: ArgusTraceDetail;
  isDark: boolean;
}> = ({ trace, isDark }) => {
  ...
  const { flatList, getDescendants, toggleCollapse, childrenMap } =
    useMemo(() => {
  ...
  const activeSearchSpanId =
    searchMatches[searchMatchIdx]?.span.span_id || null;
      ...
      if (e.key === 'ArrowDown' || e.key === 'j') {
        ...
        setFocusedSpanIdx((prev) => {
          ...
          const el = document.getElementById(
            `span-row-${flatList[next]?.span.span_id}`
          );
  ...
  return (
    <>
      <Box ref={waterfallContainerRef} tabIndex={0} sx={{ outline: 'none' }}>
        ...
        {root && (
          ...
          >
            ...
            >
              ...
              >
                {t('argus.performance.spanCount', '{{count}} spans', {
                  count: spans.length,
                })}{' '}
                · {root.environment} · {root.release}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Chip
                label={`${Number(root.duration).toLocaleString()}ms`}
                size="small"
                sx={{
                  fontWeight: 700,
                  fontSize: '0.78rem',
              ...
              <Chip
                ...
                sx={{
                  ...
                  backgroundColor: alpha(
                    root.transaction_status === 'ok' ? '#4caf50' : '#f44336',
                    0.12
                  ),
                  color:
                    root.transaction_status === 'ok' ? '#4caf50' : '#f44336',
              ...
              {root.http_status_code > 0 && (
                <Chip
                  label={`HTTP ${root.http_status_code}`}
                  size="small"
                  variant="outlined"
                  sx={{
                    fontSize: '0.72rem',
              ...
              {spans.length === 0 && (
                <Box sx={{ py: 4, textAlign: 'center' }}>
                  <Typography variant="body2" color="text.secondary">
                    {t(
                      'argus.performance.noTraceSpans',
        ...
        >
          ↑↓ {t('argus.performance.navigate', 'navigate')} · Enter{' '}
          {t('argus.performance.select', 'select')} · Esc{' '}
          {t('argus.performance.close', 'close')} · /{' '}
          {t('argus.performance.search', 'search')} ·{' '}
          {t('argus.performance.dblClickZoom', 'double-click to zoom')}
        </Typography>
      </Box>
...
const WaterfallRow: React.FC<{
  ...
  if (isSelected)
    rowBg = alpha(theme.palette.primary.main, isDark ? 0.08 : 0.04);
  else if (isActiveMatch)
    rowBg = alpha(theme.palette.warning.main, isDark ? 0.15 : 0.1);
  else if (isSearchMatch)
    rowBg = alpha(theme.palette.warning.main, isDark ? 0.06 : 0.04);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isRoot &&
          (() => {
            ...
            return (
              ...
              >
                {treeConnectors.map((show, i) =>
                  show ? (
                    <line
                      key={i}
                      x1={BASE + i * SLOT + SLOT / 2}
                      y1={0}
                      x2={BASE + i * SLOT + SLOT / 2}
                      y2={32}
                      stroke={lineColor}
                      strokeWidth={1}
                    />
        ...
        >
          <Chip
            ...
            sx={{
              ...
              fontSize: '0.6rem',
              fontWeight: 600,
              backgroundColor: alpha(opColor, isDark ? 0.15 : 0.08),
              color: opColor,
              border: 'none',
              borderRadius: 0.8,
              maxWidth: '100%',
        ...
        {isErr && (
          <ErrorIcon
            sx={{ fontSize: 14, color: '#f44336', ml: 0.3, flexShrink: 0 }}
          />
        )}
        {/* Label */}
        <Typography
