...
function generateAnalysis(
    ...
    for (const line of lines.slice(0, 5)) {
      ...
      if (match) {
        codePointers.push({
          file: match[2],
          line: match[3],
          hint: `Check ${match[1]} for error handling`,
        });
      }
    }
  }
  ...
  if (type.includes('TypeError') || type.includes('ReferenceError')) {
    ...
    rootCause =
      'Network request failure — the server endpoint is either unreachable, returning errors, or the request is timing out.';
    summary =
      'API call failure detected. This may indicate backend instability, DNS issues, or CORS misconfiguration.';
    ...
    rootCause =
      'Malformed data received from an external source — likely invalid JSON response from an API or corrupted user input.';
    summary =
      'Data parsing failure. The input data structure does not match expected format.';
    ...
    rootCause =
      'Memory exhaustion or infinite recursion detected. This typically happens with unbounded data growth or recursive calls without base case.';
    summary =
      'Resource exhaustion detected. The application is running out of memory or hitting stack limits.';
    suggestions.push(
      'Review recursive functions for proper termination conditions'
    );
          ...
          {analysis && severityCfg && (
            <Chip
              ...
              sx={{
                ...
                fontSize: '0.7rem',
                fontWeight: 700,
                backgroundColor: alpha(severityCfg.color, 0.15),
