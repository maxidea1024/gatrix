...
const QueryBuilderPanel: React.FC<QueryBuilderPanelProps> = ({
  ...
  const sortedFields = useMemo(() => {
    ...
    return [
      ...keys.filter((f) => (facets[f]?.length ?? 0) > 0),
      ...keys.filter((f) => (facets[f]?.length ?? 0) === 0),
    ];
  ...
  const dslPreview = useMemo(() => {
    const fmtDsl = (c: Condition, depth: number): string => {
      ...
      if (depth > 0) {
        const inner = parts
          .map((p, i) => (i === 0 ? `${pad}${p}` : `${indent}${conn}${p}`))
          .join('\n');
        const body = `(\n${indent}${inner}\n${indent})`;
        return c.negated ? `NOT ${body}` : body;
      }
      const joined = parts
        .map((p, i) => (i === 0 ? `${pad}${p}` : `${conn}${p}`))
        .join('\n');
  ...
  const sqlPreview = useMemo(() => {
    const fmtSql = (c: Condition, depth: number): string => {
      ...
      const joined = parts
        .map((p, i) => (i === 0 ? `${base}${pad}${p}` : `${base}${conn}${p}`))
        .join('\n');
      return c.negated ? `NOT ${joined}` : joined;
    };
        ...
        >
          ...
          >
            ...
            <MenuItem value="has" sx={{ fontSize: '0.7rem', fontWeight: 600 }}>
              <FilterListIcon
                sx={{ fontSize: 12, mr: 0.5, color: 'success.main' }}
              />
              has
            </MenuItem>
            <MenuItem value="!has" sx={{ fontSize: '0.7rem', fontWeight: 600 }}>
              <SearchOffIcon
                sx={{ fontSize: 12, mr: 0.5, color: 'error.main' }}
              />
              !has
            </MenuItem>
            <Divider sx={{ my: 0.3 }} />
            ...
            {['has', '!has'].map((f) => (
              ...
              >
                <ListItemIcon sx={{ minWidth: 24 }}>
                  <HasIcon sx={{ fontSize: 14, color: 'warning.main' }} />
                </ListItemIcon>
                {f}
              </MenuItem>
            ))}
            ...
            {sortedFields.map((f) => {
              ...
              const icon =
                fc?.type === 'datetime' ? (
                  ...
                  <NumIcon sx={{ fontSize: 14 }} />
                ) : fc?.type === 'boolean' ? (
                  <BoolIcon sx={{ fontSize: 14 }} />
                ) : (
              ...
              return (
                ...
                >
                  <ListItemIcon sx={{ minWidth: 24, color: 'text.disabled' }}>
                    {icon}
                  </ListItemIcon>
                  {f}
                  {facets[f]?.length > 0 && (
