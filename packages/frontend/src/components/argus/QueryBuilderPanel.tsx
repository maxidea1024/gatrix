...
const CATEGORY_BADGES: Record<string, { label: string; color: string; bg: string; bgLight: string }> = {
  ...
  custom:   { label: 'ATR', color: '#90a4ae', bg: 'rgba(144,164,174,0.12)', bgLight: 'rgba(96,125,139,0.10)' },
  has:      { label: 'HAS', color: '#4db6ac', bg: 'rgba(77,182,172,0.12)',  bgLight: 'rgba(0,121,107,0.10)' },
};
...
function CategoryBadge({ category, isDark }: { category: string; isDark: boolean }) {
  ...
  return (
    <Box component="span" sx={{
      ...
      color: badge.color, backgroundColor: isDark ? badge.bg : badge.bgLight,
      borderRadius: '3px', px: '3px', py: '1px', lineHeight: 1.3,
      letterSpacing: '0.02em', minWidth: 22, textAlign: 'center', whiteSpace: 'nowrap',
    }}>
      {badge.label}
    </Box>
  );
...
const QueryBuilderPanel: React.FC<QueryBuilderPanelProps> = ({
  ...
  const [previewCollapsed, setPreviewCollapsed] = useLocalStorage<boolean>('qb-preview-collapsed', false);
  ...
  const sortedFields = useMemo(() => {
    ...
    const dynamicKeys = Object.keys(facets).filter(k => !staticKeys.includes(k));
  ...
  const handleDragEnter = useCallback(() => { dragEnterCount.current++; }, []);
  ...
  const renderFilter = (filter: FilterCondition, groupId: string, index: number, depth: number) => {
    ...
    const showDropLine = drag.dropTargetId === groupId && drag.dropIndex === index;
    const lineColor = depthColor(depth);
    const fieldConfig = filter.field ? getFieldByKey(filter.field, config) : undefined;
        ...
        >
          ...
          <Select size="small" displayEmpty value={filter.field || ''}
            onChange={(e) => {
              ...
              update(filter.id, { field: newField, value: '', values: undefined });
          ...
          >
            <MenuItem value="" disabled sx={{ display: 'none' }}>field…</MenuItem>
            ...
            <MenuItem value="has" sx={{ fontSize: '0.7rem', fontWeight: 600, gap: 0.5 }}>
              <CategoryBadge category="has" isDark={isDark} />has
            </MenuItem>
            <MenuItem value="!has" sx={{ fontSize: '0.7rem', fontWeight: 600, gap: 0.5 }}>
              <CategoryBadge category="has" isDark={isDark} />not has
            </MenuItem>
            <Divider sx={{ my: 0.3 }} />
            ...
            {sortedFields.map((f) => {
              ...
              return (
                <MenuItem key={f} value={f} sx={{ fontSize: '0.7rem', gap: 0.5 }}>
                  <CategoryBadge category={fc?.category || 'custom'} isDark={isDark} />
                  {f}
                  {facets[f]?.length > 0 && (
                    <Typography component="span" sx={{ ml: 'auto', pl: 1, fontSize: '0.55rem', color: 'text.disabled' }}>
                      ({facets[f].length})
                    </Typography>
                  )}
                </MenuItem>
          ...
          {!isHas ? (
            fieldType === 'datetime' ? (
              <DatetimeValuePopover
                ...
                value={filter.value}
                isDark={isDark}
                onSelect={(val) => update(filter.id, { value: val, values: [val] })}
              />
            ) : fieldType === 'boolean' ? (
              <Select size="small" displayEmpty value={filter.value || ''}
                onChange={(e) => update(filter.id, { value: e.target.value, values: e.target.value ? [e.target.value] : [] })}
                sx={{ flex: 1, minWidth: 80, height: 26, fontSize: '0.7rem', '& .MuiSelect-select': { py: 0.3 } }}
              >
                <MenuItem value="" disabled sx={{ display: 'none' }}>value…</MenuItem>
                <MenuItem value="true" sx={{ fontSize: '0.7rem' }}>true</MenuItem>
                <MenuItem value="false" sx={{ fontSize: '0.7rem' }}>false</MenuItem>
              </Select>
            ) : fieldType === 'number' ? (
          ...
          >
            ...
            {['has', '!has'].map((f) => (
              <MenuItem key={f} sx={{ fontSize: '0.7rem', gap: 0.5 }} onClick={() => {
                ...
                <ListItemIcon sx={{ minWidth: 24 }}><CategoryBadge category="has" isDark={isDark} /></ListItemIcon>
                {f === '!has' ? 'not has' : f}
              </MenuItem>
            ))}
            <Divider sx={{ my: 0.3 }} />
            ...
            {sortedFields.map((f) => {
              ...
              return (
                <MenuItem key={f} sx={{ fontSize: '0.7rem', gap: 0.5 }} onClick={() => {
                  ...
                  <ListItemIcon sx={{ minWidth: 24 }}><CategoryBadge category={fc?.category || 'custom'} isDark={isDark} /></ListItemIcon>
                  {f}
                  {facets[f]?.length > 0 && (
        ...
        <ChildrenContainer isRoot={isRoot}>
          {group.children.length === 0 ? (
            <EmptyGroupHint
              ...
              sx={{
                border: drag.dropTargetId === group.id && drag.dropIndex === 0
                  ? `1px dashed ${primary}` : undefined,
                backgroundColor: drag.dropTargetId === group.id && drag.dropIndex === 0
                  ? alpha(primary, 0.06) : undefined,
              }}
            >
              <AddIcon sx={{ fontSize: 14, color: 'text.disabled', opacity: 0.4 }} />
            ...
            const showDropLine = drag.dropTargetId === group.id && drag.dropIndex === idx;
            return (
              <Box key={child.id} sx={{
                position: 'relative',
                mt: isFirst ? 0 : '4px',
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth={isFullscreen ? false : 'md'} fullWidth={!isFullscreen} fullScreen={isFullscreen}
      ...
      <DialogBody>
        ...
        <BottomSection sx={{ ...(previewCollapsed ? { flexShrink: 0 } : { height: previewHeight, flexShrink: 0 }) }}>

          {/* Preview header — click to toggle */}
          <PreviewHeader sx={{ cursor: 'pointer' }} onClick={() => setPreviewCollapsed((v) => !v)}>
...
// ═════════════════════════════════════════════════════════════════════════════
// DatetimeValuePopover — Compact button + Popover wrapping DatetimeValueEditor
// ═════════════════════════════════════════════════════════════════════════════

function DatetimeValuePopover({
  operator,
  value,
  isDark,
  onSelect,
  ...
  const isRelative = value?.startsWith('now');
  ...
  return (
    <>
      <Box
        ...
        sx={{
          flex: 1, minWidth: 100, height: 26,
          display: 'flex', alignItems: 'center',
          px: 1, borderRadius: '4px', cursor: 'pointer',
          fontSize: '0.7rem',
          border: `1px solid ${isDark ? 'rgba(255,255,255,0.23)' : 'rgba(0,0,0,0.23)'}`,
          color: value ? (isRelative ? '#7c8aff' : 'text.primary') : 'text.disabled',
          fontFamily: isRelative ? '"JetBrains Mono", monospace' : 'inherit',
          '&:hover': {
      ...
      <Popover
        ...
        slotProps={{ paper: { sx: { mt: 0.5, borderRadius: '8px' } } }}
      >
        <DatetimeValueEditor
          operator={operator}
          currentValue={value}
