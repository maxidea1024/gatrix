...
const EventDistributionChart: React.FC<EventDistributionChartProps> = ({
  projectId,
  issueId,
  isDark,
}) => {
  ...
  const [statsData, setStatsData] = useState<
    { timestamp: string; event_count: number; user_count: number }[]
  >([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setBrowserTag(
        tags.find(
          (tg: ArgusIssueTagGroup) =>
            tg.key === 'browser_name' || tg.key === 'browser'
        ) || null
      );
    } catch (error) {
  ...
  const browserData = useMemo(() => {
    ...
    const totalCount = browserTag.topValues.reduce(
    ...
    const top2 = browserTag.topValues.slice(0, 2).map((tv, idx) => {
      ...
      return {
        name: tv.value || 'Unknown',
        pct,
        color: BROWSER_COLORS[idx] || BROWSER_COLORS[0],
      };
    });
    ...
    if (browserTag.topValues.length > 2) {
      ...
      const restCount = restItems.reduce(
          ...
          {['24h', '7d', '14d', '30d'].map((p) => (
            <Box
              ...
              sx={{
                ...
                backgroundColor:
                  period === p
                    ? alpha(isDark ? '#bb86fc' : '#6200ea', 0.1)
                    : 'transparent',
                borderRadius: 0.5,
                cursor: 'pointer',
                '&:hover': {
                  backgroundColor: alpha(isDark ? '#bb86fc' : '#6200ea', 0.06),
                },
              }}
            >
              {p}
            ...
            >
              <Box
                ...
                sx={{
                  ...
                  '&:hover .bar-bg': {
                    backgroundColor: alpha(isDark ? '#bb86fc' : '#6200ea', 0.2),
                  },
                }}
              >
                <Typography
                ...
                >
                  +{browserData.rest.count}{' '}
                  {t('argus.issues.eventDistributionMore', 'more')}
                </Typography>
                <Typography
                  sx={{
                    fontSize: '0.65rem',
