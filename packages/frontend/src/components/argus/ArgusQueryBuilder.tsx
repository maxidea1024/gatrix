...
interface TagRule extends BaseRule {
  type: 'tag';
  field: string;
  op: string;
  value: string;
...
interface HasRule extends BaseRule {
  type: 'has';
  field: string;
}
...
function parseQueryToRules(
  q: string,
  defaultField: string
): { rules: Rule[]; isComplex: boolean } {
  if (!q.trim()) {
    return {
      rules: [
        {
  ...
  validRules.forEach((rule, idx) => {
    ...
    switch (rule.type) {
      ...
      case 'text': {
        ...
        lines.push(
          `${prefix}message ${op} ${escapeSqlString('%' + rule.value + '%')}`
        );
        break;
      }
    }
...
const HighlightedQuery: React.FC<{ sql: string; isDark: boolean }> = ({
  sql,
  isDark,
}) => {
  ...
  const renderLine = (line: string, i: number) => {
    if (line.startsWith('--')) {
      return (
        <div
          key={i}
          style={{ color: isDark ? '#607d8b' : '#9e9e9e', fontStyle: 'italic' }}
        >
          {line}
        </div>
      );
    ...
    const parts = line
      .split(
        /('(?:[^']|'')*')|(\bWHERE\b|\bAND\b|\bOR\b|\bIS NOT NULL\b|\bILIKE\b|\bNOT ILIKE\b)/g
      )
      .filter(Boolean);
      ...
      <div key={i}>
        {parts.map((p, j) => {
          if (p.startsWith("'") && p.endsWith("'")) {
            return (
              <span key={j} style={{ color: isDark ? '#a5d6a7' : '#2e7d32' }}>
                {p}
              </span>
            );
          }
          ...
          return (
            <span key={j} style={{ color: isDark ? '#e0e0e0' : '#424242' }}>
              {p}
            </span>
          );
        })}
...
const ArgusQueryBuilder: React.FC<ArgusQueryBuilderProps> = ({
  fields,
  query,
  facets = {},
  activeFilters,
  ...
  const updateRule = (id: string, patch: Partial<Rule>) => {
    setRules((prev) =>
      prev.map((r) => (r.id === id ? ({ ...r, ...patch } as Rule) : r))
    );
  };
  ...
  const addRule = (type: RuleType) => {
    ...
    switch (type) {
      ...
      case 'has':
        setRules((prev) => [
          ...prev,
          { ...base, type: 'has', field: fields[0] || '' },
        ]);
        break;
      case 'text':
        setRules((prev) => [
          ...prev,
          { ...base, type: 'text', contains: true, value: '' },
        ]);
        break;
    }
  };
    ...
    >
      <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
        ...
        {isComplex && (
          ...
          >
            <WarningIcon
              sx={{ fontSize: 18, color: theme.palette.warning.main, mt: 0.2 }}
            />
            <Typography
              sx={{ fontSize: '0.75rem', color: theme.palette.warning.main }}
            >
              {t(
                'argus.builder.complexWarning',
                'Complex query detected (Parentheses). The visual builder cannot parse it. Using the builder will overwrite your query.'
              )}
            </Typography>
          </Box>
        )}
                ...
                {rule.type === 'tag' && (
                  <>
                    ...
                    >
                      ...
                      {!fields.includes(rule.field) && rule.field && (
                        <MenuItem
                          value={rule.field}
                          sx={{ fontSize: '0.75rem' }}
                        >
                    ...
                    <Select
                      ...
                      value={rule.op}
                      onChange={(e) =>
                        updateRule(rule.id, { op: e.target.value })
                      }
                    ...
                    <Autocomplete
                      freeSolo
                      size="small"
                      options={(facets[rule.field] || []).map((v) =>
                        String(v.value ?? '')
                      )}
                      value={rule.value || ''}
                      ...
                      renderOption={(props, option) => {
                        const fv = (facets[rule.field] || []).find(
                          (v) => String(v.value ?? '') === option
                        );
                        return (
                          <Box
                            component="li"
                      ...
                      slotProps={{
                        paper: { sx: { fontSize: '0.75rem', maxHeight: 200 } },
                      }}
                      sx={{ flex: 1 }}
                    />
                  </>
                ...
                {rule.type === 'text' && (
                  <>
                    <Select
                      ...
                      onChange={(e) =>
                        updateRule(rule.id, {
                          contains: e.target.value === 'contains',
                        })
                      }
                      sx={{ width: 150, ...selectSx }}
                    >
                      <MenuItem value="contains" sx={{ fontSize: '0.75rem' }}>
                        {t('argus.builder.contains', 'Contains')}
                      </MenuItem>
                      <MenuItem
                        value="not_contains"
                        sx={{ fontSize: '0.75rem' }}
                      >
                        {t('argus.builder.notContains', 'Not contains')}
                      </MenuItem>
                    </Select>
                    <TextField
                      size="small"
        ...
        >
          ...
          <HighlightedQuery
            sql={generateSqlPreview(rules, activeFilters)}
            isDark={isDark}
          />
        </Box>
