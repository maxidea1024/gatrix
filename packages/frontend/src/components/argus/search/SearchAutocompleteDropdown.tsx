...
type DropdownRow =
  ...
  | { kind: 'item'; id: string; primary: string; secondary: string; type: 'suggestion' | 'field' | 'operator' | 'value' | 'keyword'; meta?: Record<string, any> };
...
const CATEGORY_ORDER: (FieldCategory | 'logic')[] = ['logic', 'log', 'resource', 'trace', 'event', 'user', 'custom'];

// ─── Component ───────────────────────────────────────────────────────────

export const SearchAutocompleteDropdown = forwardRef<SearchAutocompleteDropdownHandle, Props>(
  ({ composingText, fields, facets, open, anchorEl, isDark, onSelect, onSelectField, onSelectOperator, hasExplicitFields, editingPart, hasTokens }, ref) => {
    ...
    useEffect(() => { setHighlightIdx(-1); }, [composingText]);
    ...
    const fieldItems = useMemo((): FieldItem[] => {
      ...
      for (const f of fields) {
        ...
        items.push({ key: f, description: t(def.description), category: def.category });
      }
      ...
      if (!hasExplicitFields) {
        for (const [key, def] of Object.entries(FIELD_DEFINITIONS)) {
          ...
          seen.add(key);
          items.push({ key, description: t(def.description), category: def.category });
        }
      }
      return items;
    ...
    const { rows, selectableIndices } = useMemo(() => {
      ...
      const trimmed = (step === 'field')
        ? rawText
        : (rawText.includes(':') ? rawText.slice(rawText.indexOf(':') + 1).replace(/^["']/, '') : rawText);
      ...
      if (step === 'field') {
        ...
        if (trimmed && !trimmed.includes(':')) {
          allRows.push({
            kind: 'item', id: 'msg-contains',
            primary: `message contains "${trimmed}"`,
            secondary: t('argus.search.messageContains', 'Search text contained in messages'),
            type: 'suggestion',
          });
        ...
        const filtered = trimmed
          ? fieldItems.filter((f) => f.key.toLowerCase().includes(trimmed) || f.description.toLowerCase().includes(trimmed))
          : fieldItems;
        ...
        for (const cat of CATEGORY_ORDER) {
          ...
          allRows.push({ kind: 'header', label: t(CATEGORY_LABELS[cat as FieldCategory]), category: cat as FieldCategory });
          for (const f of items) {
            allRows.push({
              kind: 'item', id: `field-${f.key}`,
              primary: f.key, secondary: f.description,
              type: 'field', meta: { category: f.category },
            });
            selectable.push(allRows.length - 1);
          }
        }
        ...
        for (const op of operators) {
          allRows.push({
            kind: 'item', id: `op-${op.value}`,
            primary: t(op.label), secondary: op.shortLabel,
            type: 'operator', meta: { value: op.value },
          });
          selectable.push(allRows.length - 1);
        }
      } else if (step === 'value') {
        ...
        if (fieldDef.type === 'date') {
          for (const p of RELATIVE_TIME_PRESETS) {
            allRows.push({ kind: 'item', id: `time-${p.value}`, primary: t(p.label), secondary: p.value, type: 'value' });
            selectable.push(allRows.length - 1);
          }
        } else if (fieldDef.type === 'boolean') {
          for (const v of ['true', 'false']) {
            allRows.push({ kind: 'item', id: `val-${v}`, primary: v, secondary: '', type: 'value' });
            selectable.push(allRows.length - 1);
          }
        } else {
          const vals = facets[selectedField] || [];
          const total = vals.reduce((s, v) => s + v.count, 0);
          const filtered = trimmed ? vals.filter((v) => v.value.toLowerCase().includes(trimmed)) : vals;
          for (const v of filtered.slice(0, 20)) {
            allRows.push({
              kind: 'item', id: `val-${v.value}`, primary: v.value,
              secondary: total > 0 ? `${((v.count / total) * 100).toFixed(0)}%` : '',
              type: 'value', meta: { count: v.count, total },
            });
            selectable.push(allRows.length - 1);
          }
          // If composingText has content, offer to use it directly
          if (trimmed && !filtered.some((v) => v.value.toLowerCase() === trimmed)) {
            allRows.push({
              kind: 'item', id: 'val-custom',
    ...
    }, [step, composingText, selectedField, selectedOperator, fieldItems, facets, t]);
    ...
    useEffect(() => {
      if (highlightIdx >= 0 && listRef.current && selectableIndices.length > 0) {
    ...
    // ─── Select ───
    const handleSelectItem = useCallback((selectIdx: number) => {
      ...
      const resetState = () => { setStep('field'); setSelectedField(''); setSelectedOperator(''); setHighlightIdx(-1); };
      ...
      if (step === 'field') {
        ...
        if (row.type === 'suggestion') {
          ...
          onSelect({ field: 'message', operator: op, value: composingText.trim() });
          resetState();
          return;
        }
      ...
      if (step === 'value') {
        const value = getFieldDef(selectedField).type === 'date' && row.secondary ? row.secondary : row.primary;
        onSelect({ field: selectedField, operator: selectedOperator, value });
        resetState();
      }
    }, [step, rows, selectableIndices, selectedField, selectedOperator, composingText, onSelect, onSelectField, onSelectOperator, editingPart]);
    ...
    const handleKeyDown = useCallback((e: React.KeyboardEvent): boolean => {
      ...
      if (e.key === 'ArrowUp') {
        e.preventDefault();
        setHighlightIdx((i) => i <= 0 ? selectableIndices.length - 1 : i - 1);
        return true;
      }
      ...
      if (e.key === 'Escape') {
        if (step === 'value') { setStep('operator'); setHighlightIdx(-1); return true; }
        if (step === 'operator') { setStep('field'); setSelectedField(''); setHighlightIdx(-1); return true; }
        return true;
      }
      return false;
    ...
    useImperativeHandle(ref, () => ({
      handleKeyDown,
      reset: () => { setStep('field'); setSelectedField(''); setSelectedOperator(''); setHighlightIdx(-1); },
      setEditState: (s, f, o) => { setStep(s); if (f) setSelectedField(f); if (o) setSelectedOperator(o); setHighlightIdx(-1); },
    }));
    ...
    const breadcrumb = step !== 'field' ? (
      <Box sx={{
        ...
        <Box
          onClick={() => {
            if (step === 'value') { setStep('operator'); setHighlightIdx(-1); }
            else { setStep('field'); setSelectedField(''); setHighlightIdx(-1); }
          }}
          sx={{ cursor: 'pointer', color: 'text.disabled', fontSize: '0.72rem', '&:hover': { color: 'text.primary' }, mr: 0.25 }}
        >
          ←
        </Box>
        <Box sx={{
          fontFamily: '"JetBrains Mono", "Fira Code", monospace', fontWeight: 600, fontSize: '0.75rem',
          color: CATEGORY_COLORS[getFieldDef(selectedField).category] || theme.palette.primary.main,
        }}>
          {selectedField}
        </Box>
        {step === 'value' && (
          <Box sx={{ color: 'text.disabled', fontSize: '0.7rem', fontFamily: 'monospace' }}>
            {getOperatorsForField(selectedField).find((o) => o.value === selectedOperator)?.shortLabel || selectedOperator}
          </Box>
        )}
        <Box sx={{ ml: 'auto', color: 'text.disabled', fontSize: '0.62rem' }}>
          {step === 'operator' ? t('argus.search.selectOperator', 'Select operator') : t('argus.search.enterValue', 'Enter value')}
        </Box>
      </Box>
    ) : null;
      ...
      >
        ...
        <Box ref={listRef} sx={{ py: 0.25 }}>
          {rows.map((row, rowIdx) => {
            if (row.kind === 'header') {
              return (
                <Box key={`hdr-${row.category}`} sx={{
                  px: 1.5, py: 0.35, mt: rowIdx > 0 ? 0.4 : 0,
                  ...
                  <Box sx={{
                    ...
                    backgroundColor: CATEGORY_COLORS[row.category], opacity: 0.5,
                  }} />
                  {row.label}
                </Box>
              );
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.6, minWidth: 0, flex: 1 }}>
                  {row.type === 'field' && row.meta?.category && (
                    <Box sx={{
                      ...
                      backgroundColor: CATEGORY_COLORS[row.meta.category as FieldCategory] || '#888',
                    }} />
                  )}
                  {row.type === 'keyword' && (
                    <Box sx={{
                      ...
                      px: 0.4, borderRadius: '2px', flexShrink: 0,
                    }}>
                      KW
                    </Box>
                  )}
                  <Typography component="span" sx={{
                    fontFamily: (row.type === 'field' || row.type === 'keyword')
                      ? '"JetBrains Mono", "Fira Code", monospace' : 'inherit',
                    fontWeight: row.type === 'suggestion' ? 500 : row.type === 'field' ? 600 : 400,
                    fontSize: '0.76rem',
                    whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                    color: row.type === 'suggestion' ? theme.palette.primary.main : 'text.primary',
                  }}>
                    {row.primary}
                  </Typography>
                  {row.secondary && row.type !== 'value' && row.type !== 'operator' && (
                    <Typography component="span" sx={{
                ...
                {row.type === 'operator' && row.secondary && (
                  <Box sx={{
                    ...
                    px: 0.6, py: 0.1, borderRadius: '3px', ml: 1,
                  }}>
                    {row.secondary}
                  </Box>
                )}
                ...
                {row.type === 'value' && row.meta?.total > 0 && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0, ml: 1 }}>
                    ...
                    <Typography sx={{ fontSize: '0.58rem', color: 'text.disabled', minWidth: 24, textAlign: 'right' }}>
                      {row.secondary}
                    </Typography>
                  </Box>
                )}
        ...
        <Box sx={{
          display: 'flex', gap: 1.5, px: 1.5, py: 0.4,
          ...
          {[
            ...
            { key: 'Esc', label: step !== 'field' ? t('argus.search.hint.back', 'back') : t('argus.search.hint.close', 'close') },
          ].map((h) => (
            <Box key={h.key} sx={{ display: 'flex', alignItems: 'center', gap: 0.35 }}>
              <Box sx={{
                fontSize: '0.58rem', fontFamily: 'monospace', fontWeight: 600,
                backgroundColor: isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.05)',
                color: 'text.disabled', px: 0.4, py: 0.1, borderRadius: '3px', lineHeight: 1.3,
              }}>
                {h.key}
              </Box>
              <Typography sx={{ fontSize: '0.58rem', color: 'text.disabled' }}>{h.label}</Typography>
            </Box>
          ))}
        </Box>
      </Box>
