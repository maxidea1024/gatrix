...
function calculateImpact(
  ...
  const rawScore =
    ((eventScore * 3 + userScore * 4 + durationScore * 1 + severityWeight * 2) /
      15) *
    100;
  ...
  const estimatedLoss =
    userCount * avgRevenuePerUser * Math.max(durationHours / 24, 0.1);
  ...
  const recentHours = lastSeen
    ? (Date.now() - new Date(lastSeen).getTime()) / 3600000
    : 999;
  const trend: ImpactMetrics['trend'] =
    recentHours < 1 ? 'increasing' : recentHours < 6 ? 'stable' : 'decreasing';
...
const getTrendConfig = (
  ...
  increasing: {
    color: '#f44336',
    label: `↗ ${t('argus.impact.trendIncreasing', 'Increasing')}`,
  },
  stable: {
    color: '#ff9800',
    label: `→ ${t('argus.impact.trendStable', 'Stable')}`,
  },
  decreasing: {
    color: '#4caf50',
    label: `↘ ${t('argus.impact.trendDecreasing', 'Decreasing')}`,
  },
});
...
const MetricCard: React.FC<{
  ...
  >
    ...
    <Typography
      sx={{ fontSize: '1.1rem', fontWeight: 800, color, lineHeight: 1.2 }}
    >
      {value}
    </Typography>
    {subtitle && (
    ...
    <Box sx={{ mb: 2 }}>
      ...
      >
        ...
        <Chip
          ...
          sx={{
            ...
            fontSize: '0.6rem',
            fontWeight: 600,
            backgroundColor: alpha(trendConfig.color, 0.08),
      ...
      <Box sx={{ mb: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
          ...
          <Typography
            sx={{ fontSize: '0.85rem', fontWeight: 800, color: riskColor }}
          >
      ...
      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
        <MetricCard
          ...
          label={t('argus.impact.users', 'Users')}
        ...
        <MetricCard
          isDark={isDark}
          icon={<RevenueIcon sx={{ fontSize: 14 }} />}
          label={t('argus.impact.estLoss', 'Est. Loss')}
        ...
        <MetricCard
          ...
          label={t('argus.impact.errorBudget', 'Error Budget')}
          value={`${metrics.errorBudgetUsed}%`}
          color={metrics.errorBudgetUsed > 80 ? '#f44336' : '#ff9800'}
          subtitle={t('argus.impact.ofSla', 'of 99.9% SLA')}
        />
        <MetricCard
          isDark={isDark}
          icon={<LatencyIcon sx={{ fontSize: 14 }} />}
          label={t('argus.impact.mttr', 'Duration')}
          value={metrics.mttr}
          color="#7c4dff"
