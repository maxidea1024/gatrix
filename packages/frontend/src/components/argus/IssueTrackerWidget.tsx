...
const LinearIcon = (props: any) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
    <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM13.06 16.06L11.84 17.28C11.55 17.57 11.07 17.57 10.78 17.28L6.72 13.22C6.43 12.93 6.43 12.45 6.72 12.16L12.72 6.16C13.01 5.87 13.49 5.87 13.78 6.16L17.28 9.66C17.57 9.95 17.57 10.43 17.28 10.72L13.06 14.94V16.06Z" fill="currentColor"/>
  </svg>
);
...
function getTrackerIcon(provider: string, props: any = { fontSize: 'small' }) {
  switch (provider) {
    ...
    case 'jira': return <JiraIcon width={18} height={18} {...props} />;
    case 'linear': return <LinearIcon width={18} height={18} {...props} />;
    default: return <BugReportIcon {...props} />;
  }
}
...
const IssueTrackerWidget: React.FC<IssueTrackerWidgetProps> = ({
  ...
  const handleCreateExternalIssue = async (tracker: ArgusIssueTracker) => {
    ...
    try {
      ...
      const res = await argusService.createExternalIssue(projectId, tracker.id, payload);
      ...
      if (res.url && res.key) {
        ...
        // Save to backend
        await argusService.updateIssueExternalLink(projectId, issueId, res.url, res.key);
        enqueueSnackbar(t('argus.issues.trackerIssueCreated', { key: res.key }), { variant: 'success' });
      ...
      console.error(error);
      enqueueSnackbar(error.response?.data?.error || t('argus.issues.trackerCreateFailed', 'Failed to create issue'), { variant: 'error' });
    } finally {
      setCreatingId(null);
    }
  };
  ...
  const handleUnlink = async () => {
    ...
    try {
      ...
      await argusService.updateIssueExternalLink(projectId, issueId, null, null);
      enqueueSnackbar(t('argus.issues.trackerUnlinked', 'Issue unlinked successfully'), { variant: 'info' });
  ...
  return (
    <Box>
      ...
      {hasLinkedIssue && (
        <Box sx={{
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Link
              ...
              sx={{
                ...
                fontWeight: 600, fontSize: '0.85rem', color: theme.palette.primary.main,
                textDecoration: 'none',
          ...
          <Tooltip title={t('argus.issues.unlink', 'Unlink Issue')}>
            <IconButton size="small" onClick={handleUnlink} disabled={unlinking} sx={{ color: 'text.secondary' }}>
              {unlinking ? <CircularProgress size={16} color="inherit" /> : <LinkOffIcon sx={{ fontSize: 16 }} />}
            </IconButton>
          </Tooltip>
        </Box>
      )}
        ...
        <Box>
          {loadingTrackers ? (
            ...
            <Button
              ...
              startIcon={<AddIcon />}
              onClick={handleClick}
              sx={{ justifyContent: 'flex-start', textTransform: 'none', fontWeight: 600, color: 'text.secondary', borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }}
            >
            ...
            <Typography variant="body2" sx={{ color: 'text.disabled', fontStyle: 'italic', fontSize: '0.8rem' }}>
              {t('argus.issues.noTrackersConfigured', 'No issue trackers configured for this project.')}
            </Typography>
          )}
        </Box>
      )}
      ...
      <Menu
        ...
        onClose={handleClose}
        slotProps={{ paper: { sx: { borderRadius: 2, minWidth: 220, boxShadow: '0 4px 20px rgba(0,0,0,0.12)' } } }}
      >
        {activeTrackers.map(tracker => (
          <MenuItem key={tracker.id} onClick={() => handleCreateExternalIssue(tracker)} disabled={creatingId !== null}>
            <ListItemIcon>
              {creatingId === tracker.id ? <CircularProgress size={16} /> : getTrackerIcon(tracker.provider)}
            </ListItemIcon>
            <ListItemText
