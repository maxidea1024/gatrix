...
const IssueLogsSection: React.FC<IssueLogsSectionProps> = ({
  projectId,
  issueId,
  isDark,
}) => {
  ...
  const { logs, logsHasMore, logsLoading, loadMoreLogs, isFetchingMore } =
    useLogsData(projectId, issueId, showLogs);

  if (!showLogs) {
    return (
      <Box sx={{ mt: 2, mb: 2 }}>
        <EmptyPlaceholder message="" minHeight={120}>
          <Button
            variant="outlined"
            startIcon={<LogIcon />}
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && logGotoTime) {
                        ...
                        if (parts.length >= 2) {
                          ...
                          const targetMin =
                            targetH * 3600 + targetM * 60 + targetS;
                          // Find closest log
                          let closestIdx = 0;
                          ...
                          if (container) {
                            const rows =
                              container.querySelectorAll('[data-log-row]');
                            if (rows[closestIdx]) {
                        ...
                        <React.Fragment key={logKey}>
                          ...
                          {isExpanded && (
                            ...
                            >
                              ...
                              >
                                {allFields
                                  .filter((f) => f.key !== 'message' && f.value)
                                  .map((field) => (
                    ...
                    {filteredLogs.length === 0 && logSearch && (
                      <Box sx={{ p: 3, textAlign: 'center' }}>
                        <Typography variant="body2" color="text.disabled">
                          {t('argus.logs.noMatchingLogs', { query: logSearch })}
                        </Typography>
                      </Box>
                    )}
                  </Box>
