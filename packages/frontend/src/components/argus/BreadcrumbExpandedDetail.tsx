...
function isSqlMessage(msg?: string): boolean {
  if (!msg) return false;
  return /^(SELECT|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|BEGIN|COMMIT|ROLLBACK|TRUNCATE)\b/i.test(
    msg.trim()
  );
}
...
const DataNode: React.FC<DataNodeProps> = ({
  ...
  if (value === null || value === undefined) {
    return (
      <Box sx={{ display: 'flex', gap: 0.5, py: 0.15 }}>
        <Typography
          component="span"
          sx={{ fontSize: '0.72rem', color: colors.key, fontWeight: 600 }}
        >
          {keyName}:
        </Typography>
        <Typography
          component="span"
          sx={{ fontSize: '0.72rem', color: colors.null, fontStyle: 'italic' }}
        >
  ...
  return (
    <Box sx={{ display: 'flex', gap: 0.5, py: 0.15 }}>
      ...
      <Typography
        component="span"
        sx={{ fontSize: '0.72rem', color, wordBreak: 'break-all' }}
      >
        {displayValue}
      </Typography>
    </Box>
...
const SqlHighlighted: React.FC<{ sql: string; isDark: boolean }> = ({
  sql,
  isDark,
}) => {
  const html = useMemo(() => {
...
const HttpDetailPanel: React.FC<{ crumb: Breadcrumb; isDark: boolean }> = ({
  crumb,
  isDark,
}) => {
...
const NavigationDetailPanel: React.FC<{
  crumb: Breadcrumb;
  isDark: boolean;
}> = ({ crumb, isDark }) => {
  const data = crumb.data || {};
