...
import { LEVEL_CONFIG, PRIORITY_CONFIG, stringToColor, getInitials, formatRelative } from '@/utils/argusHelpers';
...
export interface IssueListItemProps {
  ...
  onAssigneeClick?: (e: React.MouseEvent<HTMLElement>, issue: ArgusIssue) => void;
...
const IssueListItem: React.FC<IssueListItemProps> = ({
  ...
  if (compact) {
    return (
      ...
      >
        <Box sx={{ width: 3, height: 32, borderRadius: 1, backgroundColor: lc.color, flexShrink: 0 }} />
        <Box sx={{ flex: 1, overflow: 'hidden', minWidth: 0 }}>
          <Typography variant="body2" fontWeight={600} noWrap sx={{ lineHeight: 1.3 }}>
            {highlight ? <HighlightText text={issue.title} highlight={highlight} /> : issue.title}
          </Typography>
        ...
        <Box sx={{
          ...
          <Typography variant="caption" fontWeight={700} sx={{ color: lc.color, fontSize: '0.72rem' }}>
  ...
  return (
    <Box
      ...
      sx={{
        ...
        cursor: onClick ? 'pointer' : 'default',
        transition: 'background 0.15s',
        borderBottom: showDivider ? `1px solid ${isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)'}` : 'none',
        '&:hover': onClick ? {
      ...
      <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', px: 1.5, py: 1.5, gap: 2, minWidth: 0 }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.2 }}>
            ...
            >
              {highlight
                ? <HighlightText text={issue.title} highlight={highlight} isDark={isDark} />
                : issue.title}
            </Typography>
            {issue.external_url && (
            ...
            <Box sx={{ minWidth: 28, display: 'flex', justifyContent: 'center' }}>
              {issue.assigned_to ? (
                <Tooltip title={`${t('argus.issues.assignedTo')}: ${issue.assigned_to}`}>
                  <Avatar
                    onClick={(e) => { e.stopPropagation(); onAssigneeClick?.(e, issue); }}
                    sx={{
