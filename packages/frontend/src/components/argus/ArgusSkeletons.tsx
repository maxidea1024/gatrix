...
/* ─── Chart Skeleton ─── */
export const ChartSkeleton: React.FC<{ height?: number }> = ({ height = 180 }) => {
  ...
  return (
    <Paper elevation={0} sx={{
      ...
      <Skeleton variant="text" width={120} height={18} sx={{ mb: 1, borderRadius: 0.5 }} />
...
export const StatsRowSkeleton: React.FC<{ count?: number }> = ({ count = 4 }) => {
  ...
  const isDark = theme.palette.mode === 'dark';
  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: `repeat(${count}, 1fr)`, gap: 2, mb: 2 }}>
      {Array.from({ length: count }).map((_, i) => (
        <Paper key={i} elevation={0} sx={{
          p: 2, borderRadius: 2,
          border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}`,
        }}>
          <Skeleton variant="text" width={80} height={14} sx={{ mb: 0.5, borderRadius: 0.5 }} />
          <Skeleton variant="text" width={60} height={28} sx={{ borderRadius: 0.5 }} />
        </Paper>
      ))}
    </Box>
  );
...
/* ─── Table Skeleton ─── */
export const TableSkeleton: React.FC<{ rows?: number; cols?: number }> = ({ rows = 8, cols = 4 }) => {
  const theme = useTheme();
  const isDark = theme.palette.mode === 'dark';
  return (
    <Paper elevation={0} sx={{
      ...
      {/* Header */}
      <Box sx={{
        display: 'flex', gap: 2, px: 2, py: 1.2,
        backgroundColor: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.01)',
        borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}`,
      }}>
        {Array.from({ length: cols }).map((_, i) => (
          <Skeleton key={i} variant="text" width={`${100 / cols}%`} height={16} sx={{ borderRadius: 0.5 }} />
        ))}
      </Box>
      {/* Rows */}
      {Array.from({ length: rows }).map((_, row) => (
        <Box key={row} sx={{
          ...
          {Array.from({ length: cols }).map((_, col) => (
            <Skeleton key={col} variant="text" animation="wave"
              width={`${60 + Math.random() * 30}%`} height={16}
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      {Array.from({ length: rows }).map((_, i) => (
        <Paper key={i} elevation={0} sx={{
          ...
          <Box sx={{ flex: 1 }}>
            <Skeleton variant="text" width={`${40 + Math.random() * 40}%`} height={18} sx={{ borderRadius: 0.5 }} />
            <Skeleton variant="text" width={`${20 + Math.random() * 30}%`} height={14} sx={{ borderRadius: 0.5 }} />
          </Box>
          <Skeleton variant="text" width={60} height={14} sx={{ borderRadius: 0.5 }} />
        </Paper>
      ))}
    </Box>
  );
...
export const FilterBarSkeleton: React.FC = () => (
  <Box sx={{ display: 'flex', gap: 1, mb: 2, alignItems: 'center' }}>
    <Skeleton variant="rounded" width={70} height={30} sx={{ borderRadius: 1 }} />
