...
// Service status type
export type ServiceStatus = 'initializing' | 'ready' | 'shutting_down' | 'terminated' | 'error' | 'no_response' | 'noResponse' | 'shuttingDown';

// View mode type
export type ViewMode = 'list' | 'grid' | 'card' | 'cluster' | 'checkerboard';

// Grouping field type for multi-level grouping
export type GroupingField = 'service' | 'group' | 'environment' | 'cloudProvider' | 'cloudRegion' | 'cloudZone';
    ...
    return Array.from(groupMap.entries())
        ...
        .map(([name, instances]) => {
            const group: ServerGroup = {
                ...
                children: hasMoreLevels ? buildServerGroups(instances, levels, getLabel, nextLevel) : undefined,
            };
            return group;
        });
};
