    ...
    return (
        <Box sx={{ display: 'flex', gap: 0.5 }}>
            {statusCounts.ready > 0 && (
                <Chip label={`${t('serverList.stats.ready')}: ${statusCounts.ready}`} size="small" color="success" variant="outlined" sx={{ height: 20, fontSize: '0.7rem' }} />
            )}
            {statusCounts.initializing > 0 && (
                <Chip label={`${t('serverList.stats.initializing')}: ${statusCounts.initializing}`} size="small" color="info" variant="outlined" sx={{ height: 20, fontSize: '0.7rem' }} />
            )}
            {statusCounts.shuttingDown > 0 && (
                <Chip label={`${t('serverList.stats.shuttingDown')}: ${statusCounts.shuttingDown}`} size="small" color="warning" variant="outlined" sx={{ height: 20, fontSize: '0.7rem' }} />
            )}
            {statusCounts.error > 0 && (
                <Chip label={`${t('serverList.stats.error')}: ${statusCounts.error}`} size="small" color="error" variant="outlined" sx={{ height: 20, fontSize: '0.7rem' }} />
            )}
        </Box>
    );
};
...
export const ServerGroupHeader: React.FC<ServerGroupHeaderProps> = ({ group, getGroupingLabel, t }) => {
