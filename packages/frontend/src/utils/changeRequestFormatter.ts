...
export const formatChangeRequestTitle = (title: string, t: TFunction): string => {
    ...
    if (agMatch) {
        ...
        const actionLocalized = t(actionMap[action] || `common.${action.toLowerCase()}`, { defaultValue: action });
 ...
 */
export const formatChangeItemTitle = (table: string, targetId: string, afterData: any, t: TFunction): string => {
    ...
    if (afterData && typeof afterData === 'object') {
        ...
        if (friendlyName === targetId || friendlyName.startsWith('NEW_')) {
            ...
            for (const field of nameFields) {
                if (afterData[field] && (typeof afterData[field] === 'string' || typeof afterData[field] === 'number')) {
