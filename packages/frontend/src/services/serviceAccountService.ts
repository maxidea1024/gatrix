...
class ServiceAccountService {
    ...
    async create(data: { name: string; permissions?: string[] }): Promise<ServiceAccount> {
        const response = await api.post('/admin/service-accounts', data);
        return response.data;
    }

    async update(id: number, data: { name?: string; permissions?: string[] }): Promise<ServiceAccount> {
