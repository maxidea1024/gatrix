...
async function main() {
    ...
    try {
        ...
        const token = await knex('g_api_access_tokens').where('tokenName', 'ts-only-server-token').first();
        if (!token) {
        ...
        console.log('  allowAllEnvironments:', token.allowAllEnvironments, '(type:', typeof token.allowAllEnvironments, ')');
        ...
        console.log('  Environment mappings:', envMappings.map(e => e.environmentId));
