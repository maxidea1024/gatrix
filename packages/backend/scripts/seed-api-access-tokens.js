...
async function seedApiAccessTokens() {
  ...
  try {
    ...
    for (let i = 0; i < 1; i++) {
      ...
      console.log(
        `Debug - Token ${i + 1}: permissions array = ${JSON.stringify(permissionsArray)}, JSON string = ${permissions}`
      );
    ...
    for (let i = 0; i < tokens.length; i += batchSize) {
      ...
      console.log(
        `Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(tokens.length / batchSize)}`
      );
    }
    ...
    const [totalCount] = await connection.execute(
      'SELECT COUNT(*) as count FROM g_api_access_tokens'
    );
