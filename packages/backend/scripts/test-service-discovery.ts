 ...
 */
function generateServerData(status: typeof STATUSES[number] = 'initializing', internalAddress?: string) {
 ...
 */
async function registerServer(permanent: boolean = false, internalAddress?: string, forceLifespan?: number) {
  try {
    ...
    const lifespan = isPermanent ? null : (forceLifespan || (30000 + Math.floor(Math.random() * 300000))); // null = permanent, or 30s-5min
    ...
    if (isPermanent) {
      console.log(`✅ Server registered: ${server.type}:${server.instanceId} (init delay: ${server.initDelay}ms, lifespan: PERMANENT)`);
    } else {
      console.log(`✅ Server registered: ${server.type}:${server.instanceId} (init delay: ${server.initDelay}ms, lifespan: ${server.lifespan}ms)`);
    }
    ...
    server.stateUpdateInterval = setInterval(async () => {
      if (server.status === 'ready') {
        await updateStatusInStorage(server);
      }
    }, 5000 + Math.floor(Math.random() * 5000)); // 5-10 seconds
