...
async function addMigrationRecord() {
  ...
  try {
    ...
    const [existing] = await connection.execute(
      'SELECT * FROM g_migrations WHERE name = ?',
    ...
    if (existing.length > 0) {
      ...
      const hasAutoIncrement = columns.some(col => col.Field === 'id' && col.Extra.includes('auto_increment'));

      if (hasAutoIncrement) {
        await connection.execute(
          'INSERT INTO g_migrations (name, executedAt) VALUES (?, NOW())',
          ['009_create_api_access_tokens']
        );
      } else {
        // Get next ID
        const [maxId] = await connection.execute('SELECT COALESCE(MAX(id), 0) + 1 as nextId FROM g_migrations');
        await connection.execute(
