...
exports.up = async function (connection) {
    ...
    if (duplicates.length > 0) {
        console.log(`Found ${duplicates.length} duplicate segment names across environments. Deduplicating...`);
        ...
        for (const dup of duplicates) {
            ...
            console.log(`  Deduplicated segment "${dup.segmentName}": kept ${keepId}, removed ${deleteIds.length} duplicates`);
        }
    }
    ...
    try {
        await connection.execute('ALTER TABLE g_feature_segments ADD UNIQUE INDEX idx_segment_name_unique (segmentName)');
...
exports.down = async function (connection) {
    ...
    // Add index
    await connection.execute('ALTER TABLE g_feature_segments ADD INDEX idx_environment (environment)');
