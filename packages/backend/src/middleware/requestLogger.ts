...
export const requestLogger = (
  ...
  if (isDevelopment) {
    ...
    // Request body ì¶”ê? (Content-Type??application/json??ê²½ìš°ë§?
  ...
  res.end = function (chunk?: any, encoding?: any): Response {
    ...
    // ê°œë°œ ?˜ê²½?ì„œë§?response body ì¶”ê? (Content-Type??application/json??ê²½ìš°ë§?
    if (isDevelopment && chunk && res.get('Content-Type')?.includes('application/json')) {
      try {
        let responseText: string | undefined;

        if (Buffer.isBuffer(chunk)) {
          // Bufferë¥?ë¬¸ì?´ë¡œ ë³€??          responseText = chunk.toString('utf8');
