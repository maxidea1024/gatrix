...
export class Project extends Model implements ProjectData {
   ...
   * Create new project
   */
  static async createProject(data: Omit<ProjectData, 'id' | 'createdAt' | 'updatedAt'>): Promise<Project> {
    if (!this.isValidProjectName(data.projectName)) {
      throw new Error('Invalid project name. Use only lowercase letters, numbers, underscore, and hyphen.');
    }
