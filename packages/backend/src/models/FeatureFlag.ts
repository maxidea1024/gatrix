...
export class FeatureFlagModel {
  ...
  static async update(
    ...
    try {
      ...
      if (data.staleAfterDays !== undefined)
        updateData.staleAfterDays = data.staleAfterDays;
...
export class FeatureContextFieldModel {
  static async findAll(
    ...
    try {
      let query = db("g_feature_context_fields")
        ...
        .leftJoin(
          "g_users",
          "g_feature_context_fields.createdBy",
          "g_users.id",
        );
      ...
      if (search) {
        query = query.where((qb: any) => {
          qb.where(
            "g_feature_context_fields.fieldName",
            "like",
            `%${search}%`,
          ).orWhere(
      ...
      const fields = await query.orderBy(
        "g_feature_context_fields.sortOrder",
        "asc",
      );
  ...
  static async findByFieldName(
    fieldName: string,
  ): Promise<FeatureContextFieldAttributes | null> {
    try {
      const field = await db("g_feature_context_fields")
  ...
  static async create(
    ...
    try {
      ...
      return this.findByFieldName(
        data.fieldName,
      ) as Promise<FeatureContextFieldAttributes>;
    } catch (error) {
  ...
  static async update(
    ...
    try {
      ...
      if (data.description !== undefined)
        updateData.description = data.description;
      if (data.legalValues !== undefined)
        updateData.legalValues = JSON.stringify(data.legalValues);
      if (data.tags !== undefined) updateData.tags = JSON.stringify(data.tags);
      if (data.stickiness !== undefined)
        updateData.stickiness = data.stickiness;
      if (data.sortOrder !== undefined) updateData.sortOrder = data.sortOrder;
      if (data.isEnabled !== undefined) updateData.isEnabled = data.isEnabled;
      if (data.updatedBy !== undefined) updateData.updatedBy = data.updatedBy;

      await db("g_feature_context_fields")
        .where("fieldName", fieldName)
        .update(updateData);
