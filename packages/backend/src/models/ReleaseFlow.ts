...
export class ReleaseFlowModel {
  static async listTemplates(search?: string): Promise<ReleaseFlowAttributes[]> {
    try {
      let query = db('g_release_flows')
        ...
        .select(
          db.raw(
            '(SELECT COUNT(*) FROM g_release_flow_milestones WHERE flowId = g_release_flows.id) as milestoneCount'
          )
        )
  ...
  static async update(
    id: string,
    data: Partial<ReleaseFlowAttributes>
  ): Promise<ReleaseFlowAttributes> {
    try {
...
export class ReleaseFlowMilestoneModel {
  ...
  static async create(
    ...
    try {
      ...
      await db('g_release_flow_milestones').insert({
        ...
        transitionCondition: data.transitionCondition
  ...
  static async update(
    ...
    try {
      ...
      if (data.progressionExecutedAt !== undefined)
        updateData.progressionExecutedAt = data.progressionExecutedAt;
