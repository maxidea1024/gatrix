...
export class UserModel {
      ...
      const applyFilters = (query: any) => {
        ...
        if (filters.role) {
          if (Array.isArray(filters.role)) {
            logger.info(
              `[UserModel] Applying role filter (array): ${filters.role.join(", ")}`,
            );
            query.whereIn("g_users.role", filters.role);
          } else {
            logger.info(
              `[UserModel] Applying role filter (single): ${filters.role}`,
            );
        ...
        if (filters.status) {
          if (Array.isArray(filters.status)) {
            logger.info(
              `[UserModel] Applying status filter (array): ${filters.status.join(", ")}`,
            );
            query.whereIn("g_users.status", filters.status);
          } else {
            logger.info(
              `[UserModel] Applying status filter (single): ${filters.status}`,
            );
   ...
   */
  static async getAccessibleEnvironments(
    userId: number,
  ): Promise<string[] | "all"> {
    try {
      const user = await db("g_users")
