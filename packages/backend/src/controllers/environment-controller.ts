...
import {
  ...
  CopyOptions,
} from '../services/environment-copy-service';
...
export class EnvironmentController {
   ...
   */
  static getEnvironments = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      if (userId) {
        ...
        try {
          const accessibleEnvIds =
            await permissionService.getAccessibleEnvironmentIds(
              userId,
              orgId,
              projectId
            );
          ...
          if (accessibleEnvIds.length > 0) {
            environments = environments.filter((env) =>
              accessibleEnvIds.includes(env.id)
            );
          }
        } catch (err) {
          logger.warn('Failed to check user environment access', {
            userId,
            err,
          });
        }
   ...
   */
  static createEnvironment = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (!baseEnvironment) {
          ...
          logger.info(
            `System KV items initialized for new environmentId: ${newEnv.id}`
          );
        }
   ...
   * Update environment
   */
  static updateEnvironment = asyncHandler(
    async (req: Request, res: Response) => {
      const { environmentId } = req.params;
      const {
        displayName,
   ...
   */
  static getEnvironmentRelatedData = asyncHandler(
    async (req: Request, res: Response) => {
   ...
   * Delete environment
   */
  static deleteEnvironment = asyncHandler(
    async (req: Request, res: Response) => {
      const { environmentId: envParam } = req.params;
      ...
      try {
        ...
        logger.info(
          `Environment deleted: ${environmentName} by user ${userId}`,
          {
            force,
            environmentId: envParam,
          }
        ...
        if (error.message === 'ENVIRONMENT_HAS_RELATED_DATA') {
          return res.status(400).json({
            ...
            message:
              'Environment has related data. Use force=true to delete all data.',
   ...
   */
  static getEnvironmentSegments = asyncHandler(
    async (req: Request, res: Response) => {
      const { environmentId } = req.params;
   ...
   */
  static createPredefinedSegments = asyncHandler(
    async (req: Request, res: Response) => {
      const { environmentId } = req.params;
      const userId = (req.user as any)?.userId;
      ...
      res.json({
        ...
        data: [],
        message:
          'Segments feature is being rebuilt - no predefined segments created',
      });
    }
  );
   ...
   */
  static getEnvironmentStats = asyncHandler(
    async (req: Request, res: Response) => {
      const { environmentId } = req.params;
   ...
   */
  static validateEnvironmentName = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      const existing = await Environment.query()
   ...
   */
  static copyEnvironmentData = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        res.status(500).json({
          ...
          message:
            error instanceof Error
              ? error.message
              : 'Failed to copy environment data',
        });
      }
    }
  );
  ...
  static getCopyPreview = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          error instanceof Error ? error.message : 'Failed to get copy preview',
      });
    }
  });
}
