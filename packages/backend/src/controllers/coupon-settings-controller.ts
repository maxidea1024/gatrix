...
const createSchema = Joi.object({
  ...
  tags: Joi.alternatives(Joi.object(), Joi.array(), Joi.string())
    .optional()
    .allow(null),
  maxTotalUses: Joi.number().integer().min(1).allow(null),
  perUserLimit: Joi.number().integer().min(1).default(1),
  usageLimitType: Joi.string()
    .valid('USER', 'CHARACTER')
    .optional()
    .default('USER'),
  ...
  rewardData: Joi.alternatives(Joi.object(), Joi.array(), Joi.string()).allow(
    null
  ),
...
const updateSchema = Joi.object({
  ...
  rewardData: Joi.alternatives(Joi.object(), Joi.array(), Joi.string())
    .allow(null)
    .optional(),
...
export class CouponSettingsController {
  ...
  static getById = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      const setting = await CouponSettingsService.getSettingById(
        id,
        environmentId
      );
  ...
  static getIssuedCodesStats = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const { id } = req.params;
      if (!id) throw new GatrixError('id is required', 400);
  ...
  static getIssuedCodes = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const { id } = req.params;
      if (!id) throw new GatrixError('id is required', 400);
  ...
  static getGenerationStatus = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const { id } = req.params;
      if (!id) throw new GatrixError('id is required', 400);
  ...
  static recalculateCacheAll = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
  ...
  static exportUsage = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const {
        settingId,
        couponCode,
      ...
      const { convertMySQLDateTimeToTimezone } = require('../utils/date-utils');
      ...
      const transformedRecords = records.map((r: any) => ({
        ...
        'Coupon Start Date':
          convertMySQLDateTimeToTimezone(r.couponStartsAt, tz) || '-',
        'Coupon Expiry Date':
          convertMySQLDateTimeToTimezone(r.couponExpiresAt, tz) || '-',
