...
function buildPreviewFromHtml(html: string, url: string): LinkPreview {
  ...
  const ogDesc = extractMeta(html, 'og:description') || extractMeta(html, 'twitter:description', 'name');
...
export class LinkPreviewController {
  static async getPreview(req: express.Request, res: express.Response) {
    try {
      ...
      if (typeof response.data !== 'string') {
        return res.json({ success: true, data: { url, siteName: getHostname(url) } as LinkPreview });
      }
      ...
      return res.status(200).json({ success: true, data: { url: req.body?.url || req.query?.url, siteName: getHostname(req.body?.url || req.query?.url) } });
    }
  }
  ...
  static async getBatch(req: express.Request, res: express.Response) {
    try {
      ...
      const results = await Promise.all(limited.map(async (u) => {
        try {
          ...
          const data = html ? buildPreviewFromHtml(html, u) : ({ url: u, siteName: getHostname(u) } as LinkPreview);
