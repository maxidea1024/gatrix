...
export class IpWhitelistController {
  static getIpWhitelists = asyncHandler(
  ...
  static getIpWhitelistById = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      const ipWhitelist = await IpWhitelistService.getIpWhitelistById(
        id,
        environment,
      );
  ...
  static createIpWhitelist = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const environment = req.environment;
      if (!environment) {
        throw new GatrixError("Environment not specified", 400);
      ...
      const gatewayResult = await UnifiedChangeGateway.requestCreation(
        ...
        async () => {
          return await IpWhitelistService.createIpWhitelist(
            environment,
            createData,
          );
        },
      ...
      if (gatewayResult.mode === "DIRECT") {
        ...
        res.status(202).json({
          ...
          message:
            "Change request created. The IP whitelist entry will be created after approval.",
        });
      }
    },
  );

  static updateIpWhitelist = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const id = parseInt(req.params.id);
      const environment = req.environment;
  ...
  static deleteIpWhitelist = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const id = parseInt(req.params.id);
      const environment = req.environment;
    ...
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      if (gatewayResult.mode === "DIRECT") {
        ...
        res.status(202).json({
          ...
          message:
            "Change request created. Toggle will be applied after approval.",
        });
      }
    },
  );

  static bulkCreateIpWhitelists = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const environment = req.environment;
      if (!environment) {
        throw new GatrixError("Environment not specified", 400);
      ...
      if (requiresApproval) {
        ...
        res.status(202).json({
          ...
          message:
            "Change request created. IP whitelists will be created after approval.",
        });
      } else {
  ...
  static checkIpWhitelist = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
