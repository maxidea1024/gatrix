    ...
    static async getFeatureFlags(req: Request, res: Response): Promise<void> {
        try {
            ...
            const flags: EvaluationFlag[] = await Promise.all(
                rawFlags.map(async (flag: FeatureFlagAttributes & { isEnabled: boolean }) => {
                    ...
                    const evaluationStrategies: EvaluationStrategy[] = strategies
                        .sort((a: FeatureStrategyAttributes, b: FeatureStrategyAttributes) => a.sortOrder - b.sortOrder)
                    ...
                    const evaluationVariants: EvaluationVariant[] = variants.map((v: FeatureVariantAttributes) => ({
                        name: v.variantName,
                        weight: v.weight,
                        payload: v.payload,
            ...
            if (referencedSegmentNames.size > 0) {
                const rawSegments = await FeatureSegmentModel.findByNames(Array.from(referencedSegmentNames));
    ...
    static async getFeatureFlag(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!environment || !flagName) {
                res.status(400).json({ success: false, error: 'Environment and flag name are required' });
                return;
            }
