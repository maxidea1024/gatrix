...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      return res.json({
        success,
        message: success ? "Lock acquired" : "Failed to acquire lock",
      });
    } catch (error) {
      next(error);
    }
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      return res.json({
        success,
        message: success ? "Lock released" : "Failed to release lock",
      });
    } catch (error) {
      next(error);
    }
    ...
    try {
      ...
      if (!table || !entityId || !environment) {
        return res
          ...
          .json({
            error:
              "Missing required query params: table, entityId, environment",
          });
      }

      const [lockInfo, pendingCR] = await Promise.all([
        entityLockService.checkLock(
          table as string,
          entityId as string,
