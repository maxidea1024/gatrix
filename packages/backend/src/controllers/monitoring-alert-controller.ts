...
import { MonitoringAlert } from '../models/monitoring-alert';
...
export class MonitoringAlertController {
   ...
   */
  static receiveAlert = asyncHandler(
   ...
   */
  static getAlertById = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const { id } = req.params;
   ...
   */
  static updateAlertStatus = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const { id } = req.params;
