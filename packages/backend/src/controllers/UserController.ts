...
export class UserController {
  ...
  static promoteToAdmin = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const userId = parseInt(req.params.id);
  ...
  static demoteFromAdmin = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const userId = parseInt(req.params.id);
   ...
   */
  static getEnvironmentAccess = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const userId = parseInt(req.params.id);
      if (isNaN(userId)) {
        throw new GatrixError("Invalid user ID", 400);
   ...
   */
  static setEnvironmentAccess = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const userId = parseInt(req.params.id);
      if (isNaN(userId)) {
        throw new GatrixError("Invalid user ID", 400);
   ...
   */
  static getMyEnvironmentAccess = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const userId = req.user!.userId;
