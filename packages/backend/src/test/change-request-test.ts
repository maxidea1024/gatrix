...
async function runTest() {
    ...
    try {
        ...
        if (!existingEnv) {
            ...
            await Environment.query().patchAndFetchById(TEST_ENV, { requiresApproval: true, requiredApprovers: 1 });
        }
        ...
        try {
            await UnifiedChangeGateway.requestModification(1, TEST_ENV, TEST_TABLE, 'mock-id-not-used-by-real-vars-yet', {});
        ...
        const res2 = await UnifiedChangeGateway.requestModification(1, TEST_ENV, TEST_TABLE, 'rej-item', { val: 1 });
        await ChangeRequestService.updateChangeRequestMetadata(res2.changeRequestId!, { title: 'To Reject', reason: 'Test' });
        await ChangeRequestService.submitChangeRequest(res2.changeRequestId!);

        const rejected = await ChangeRequestService.rejectChangeRequest(res2.changeRequestId!, 1, 'Bad idea');
