...
async function testAuditLog() {
  try {
    ...
    if (!testUser) {
      ...
      // Create a test user
      const createResponse = await axios.post(`${API_URL}/admin/users`, {
      ...
      await axios.put(`${API_URL}/admin/users/${newUser.id}`, {
      ...
      // Test 1: Update user
      console.log('\n🔄 Test 1: Updating user...');
      await axios.put(`${API_URL}/admin/users/${testUser.id}`, {
    ...
    // Get game worlds
    const worldsResponse = await axios.get(`${API_URL}/admin/game-worlds`, { headers });
    ...
    if (worlds.length > 0) {
      ...
      await axios.patch(`${API_URL}/admin/game-worlds/${testWorld.id}/toggle-visibility`, {}, { headers });
      ...
      await axios.patch(`${API_URL}/admin/game-worlds/${testWorld.id}/toggle-maintenance`, {}, { headers });
      ...
      await axios.put(`${API_URL}/admin/game-worlds/${testWorld.id}`, {
    ...
    const inviteResponse = await axios.post(`${API_URL}/admin/invitations`, {
    ...
    await axios.delete(`${API_URL}/admin/invitations/${invitation.id}`, { headers });
