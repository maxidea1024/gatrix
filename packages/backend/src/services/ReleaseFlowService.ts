...
export class ReleaseFlowService {
    ...
    async applyTemplateToFlag(
        ...
        if (existingPlan) {
            throw new GatrixError('A release flow is already active for this flag and environment', 400, true, ErrorCodes.BAD_REQUEST);
        }
     ...
     */
    async startMilestone(planId: string, milestoneId: string, userId: number): Promise<ReleaseFlowAttributes> {
        ...
        try {
            ...
            await ReleaseFlowModel.update(plan.id, { activeMilestoneId: milestone.id, status: 'active', updatedBy: userId });
            await ReleaseFlowMilestoneModel.update(milestone.id, { startedAt: new Date(), pausedAt: null, progressionExecutedAt: null });
            ...
            await db('g_feature_strategies').where('flagId', plan.flagId).where('environment', plan.environment).del();
    ...
    async resumePlan(planId: string, userId: number): Promise<ReleaseFlowAttributes> {
        ...
        if (plan.activeMilestoneId) {
            ...
            if (activeMilestone && activeMilestone.pausedAt && activeMilestone.startedAt) {
                ...
                const newStartedAt = new Date(new Date(activeMilestone.startedAt).getTime() + pauseDuration);
                await ReleaseFlowMilestoneModel.update(plan.activeMilestoneId, {
                    pausedAt: null,
     ...
     */
    async progressToNextMilestone(planId: string, userId?: number): Promise<ReleaseFlowAttributes | null> {
        const plan = await ReleaseFlowModel.findById(planId);
    ...
    async setTransitionCondition(
        ...
        if (transitionCondition.intervalMinutes < 1) {
            throw new GatrixError('Interval must be at least 1 minute', 400, true, ErrorCodes.BAD_REQUEST);
        }
     ...
     */
    async removeTransitionCondition(milestoneId: string, userId: number): Promise<ReleaseFlowMilestoneAttributes> {
        const milestone = await ReleaseFlowMilestoneModel.findById(milestoneId);
        if (!milestone) {
            throw new GatrixError('Milestone not found', 404, true, ErrorCodes.NOT_FOUND);
        }

        const updated = await ReleaseFlowMilestoneModel.update(milestoneId, { transitionCondition: null });
    ...
    async updateTemplate(id: string, input: Partial<CreateTemplateInput>, userId: number): Promise<ReleaseFlowAttributes> {
        const trx = await db.transaction();
        try {
    ...
    async deleteTemplate(id: string, userId: number): Promise<void> {
        ...
        await ReleaseFlowModel.update(id, { isArchived: true, updatedBy: userId, archivedAt: new Date() });
