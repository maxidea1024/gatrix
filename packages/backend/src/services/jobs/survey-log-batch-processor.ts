...
export class SurveyLogBatchProcessor {
  ...
  private readonly BATCH_SIZE = parseInt(
    process.env.SURVEY_LOG_BATCH_SIZE || '1000',
    10
  );
  ...
  public async start(intervalMs: number = 2000) {
    ...
    logger.info(
      `Started SurveyLogBatchProcessor (consumer: ${this.CONSUMER_NAME}, batchSize: ${this.BATCH_SIZE})`
    );
  }
  ...
  private async processMessages(messages: any[]): Promise<void> {
    ...
    try {
      ...
      await trx('g_survey_logs').insert(insertData).onConflict('id').ignore();
      ...
      logger.info(
        `Successfully committed survey log batch of ${payloads.length} items`
      );
    } catch (dbError) {
      await trx.rollback();
      logger.error(
        'Failed to commit survey log batch. Messages will remain pending.',
        { error: dbError }
      );
    }
  }
