...
export class CouponGenerationJob {
  ...
  static async process(job: Job<any>): Promise<void> {
    ...
    try {
      ...
      logger.info('Job data received', { jobId, jobData: JSON.stringify(job.data) });
      ...
      if (!settingId || !quantity) {
        logger.error('Invalid job data', { jobId, jobData: job.data, settingId, quantity });
        throw new Error('Invalid job data: settingId and quantity are required');
      }

      logger.info('Starting coupon generation job', { jobId, settingId, quantity });
      ...
      for (let i = 0; i < quantity; i += this.BATCH_SIZE) {
        ...
        for (let j = 0; j < batchSize; j++) {
          ...
          if (!found) {
            logger.warn('Failed to generate unique code after 10 attempts', { jobId, settingId, codeIndex: i + j });
            continue;
          }
        ...
        if (batchCodes.length > 0) {
          ...
          if (totalGenerated - lastProgressUpdate >= this.PROGRESS_UPDATE_INTERVAL) {
            ...
            logger.info('Coupon generation progress', { jobId, settingId, progress, generated: totalGenerated, total: quantity });
      ...
      logger.info('Coupon generation job completed successfully', { jobId, settingId, totalGenerated });
    } catch (error) {
      ...
      try {
        ...
        logger.error('Failed to update coupon status to FAILED', { jobId, settingId, error: updateError });
      }
