...
} from '../models/ip-whitelist';
import { GatrixError } from '../middleware/error-handler';
import { normalizeIPOrCIDR, isValidIPOrCIDR } from '../utils/ip-validation';
...
export class IpWhitelistService {
  ...
  static async getAllIpWhitelists(
    ...
    filters: Omit<IpWhitelistFilters, 'environmentId'> = {},
  ...
  static async createIpWhitelist(
    environmentId: string,
    data: Omit<CreateIpWhitelistData, 'environmentId'>
  ): Promise<IpWhitelist> {
    try {
      ...
      // Publish ip_whitelist.updated event for SDK real-time updates
      try {
        ...
        logger.warn('Failed to publish ip_whitelist.updated event:', eventError);
  ...
  static async deleteIpWhitelist(
    ...
    try {
      ...
      await IpWhitelistModel.delete(id, environmentId);
  ...
  static async bulkCreateIpWhitelists(
    environmentId: string,
    entries: Omit<CreateIpWhitelistData, 'createdBy' | 'environmentId'>[],
    createdBy: string
  ): Promise<number> {
    try {
      ...
      for (const entry of entries) {
        try {
          await this.createIpWhitelist(environmentId, {
            ...entry,
            createdBy,
          });
          createdCount++;
