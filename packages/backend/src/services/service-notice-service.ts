...
import { convertDateFieldsFromMySQL, convertToMySQLDateTime } from '../utils/date-utils';
  ...
  async getServiceNotices(
    ...
    const notices = rows.map((row) => {
      const notice = {
        ...
        subchannels:
          typeof row.subchannels === 'string' ? JSON.parse(row.subchannels) : row.subchannels,
