...
export class SurveyService {
  ...
  static async getSurveys(params: {
    ...
    if (params.search) {
      query = query.where(function () {
        this.where('surveyTitle', 'like', `%${params.search}%`).orWhere(
          'platformSurveyId',
          'like',
          `%${params.search}%`
        );
    ...
    const surveys = rows.map((row) => ({
      ...row,
      triggerConditions:
        typeof row.triggerConditions === 'string'
  ...
  static async createSurvey(input: CreateSurveyInput): Promise<Survey> {
    ...
    await db('g_surveys').insert({
      ...
      participationRewards: input.participationRewards
        ? JSON.stringify(input.participationRewards)
        : null,
  ...
  static async updateSurvey(
    ...
    if (input.participationRewards !== undefined)
      updateData.participationRewards = input.participationRewards
        ? JSON.stringify(input.participationRewards)
        : null;
    ...
    if (input.targetPlatforms !== undefined)
      updateData.targetPlatforms = input.targetPlatforms
        ? JSON.stringify(input.targetPlatforms)
        : null;
    if (input.targetPlatformsInverted !== undefined)
      updateData.targetPlatformsInverted = input.targetPlatformsInverted;
    if ((input as any).targetChannels !== undefined)
      updateData.targetChannels = (input as any).targetChannels
        ? JSON.stringify((input as any).targetChannels)
        : null;
    if ((input as any).targetChannelsInverted !== undefined)
      updateData.targetChannelsInverted = (input as any).targetChannelsInverted;
    if ((input as any).targetSubchannels !== undefined)
      updateData.targetSubchannels = (input as any).targetSubchannels
        ? JSON.stringify((input as any).targetSubchannels)
        : null;
  ...
  static async updateSurveyConfig(
    ...
    if (input.joinedSecretKey !== undefined) {
      updates.push({
        key: 'survey.joinedSecretKey',
        value: input.joinedSecretKey,
      });
    }
