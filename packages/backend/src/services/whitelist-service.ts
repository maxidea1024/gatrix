...
export class WhitelistService {
  ...
  static async testWhitelist(
    ...
    try {
      ...
      // Check IP whitelist
      if (ipAddress) {
        const { IpWhitelistModel } = await import('../models/ip-whitelist');
        const ipWhitelists = await IpWhitelistModel.findAll(1, 1000, {
          environmentId,
          isEnabled: true,
        });
        const now = new Date();
        const { ipMatchesCIDR } = await import('../utils/ip-validation');
        ...
        for (const ipWhitelist of ipWhitelists.ipWhitelists) {
          ...
          if (
            ipWhitelist.ipAddress === ipAddress ||
            ipMatchesCIDR(ipAddress, ipWhitelist.ipAddress)
          ) {
            matchedRules.push({
              type: 'ip',
