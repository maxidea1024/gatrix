...
class CommandBuilder {
  ...
  constructor(
    name: string,
    private onBuild: (def: BuiltCommandDef) => void
  ) {
    this.def = {
      name,
      action: async () => ({ output: '' }),
  ...
  audit(enabled: boolean = true) {
    this.def.audit = enabled;
    return this;
  }
  action(fn: BuiltCommandDef['action']) {
    this.def.action = fn;
    return this;
  }
  register() {
    this.onBuild(this.def);
    return this;
  }
}
...
class ConsoleService {
  ...
  constructor() {
    ...
    this.command('gen-login-password')
      .description(
        'Generate login-friendly password (starts with a letter; includes digits and symbols)'
      )
  ...
  private parseOptions(argv: string[]): {
    args: string[];
    opts: Record<string, any>;
  } {
    const args: string[] = [];
  ...
  public async execute(
    ...
    if (built) {
      try {
        ...
        return {
          output: `\x1b[31mError:\x1b[0m ${e?.message || 'Unknown error'}`,
        };
      }
    }
  ...
  private timeCommand = async (args: string[] = []): Promise<ConsoleExecutionResult> => {
    ...
    return {
      output: `${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`,
    };
  };
  ...
  private base64Command = async (
    ...
    if (hasEncode && !hasDecode) {
      if (!text) {
        return {
          output: '\u001b[31mError:\u001b[0m Please provide text to encode',
        };
      }
    ...
    if (hasDecode && !hasEncode) {
      ...
      try {
        return { output: Buffer.from(text, 'base64').toString('utf-8') };
      } catch {
        return { output: '\u001b[31mInvalid base64\u001b[0m' };
      }
    }
  ...
  private ulidCommand = async (args: string[] = []): Promise<ConsoleExecutionResult> => {
    ...
    if (isNaN(count) || count < 1 || count > 100) {
      return {
        output: '\u001b[31mError:\u001b[0m Count must be between 1 and 100',
      };
    }
  ...
  private jwtSecretCommand = async (
    args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
  ...
  private hashCommand = async (
    ...
    if (!text) {
      return {
        output: '\u001b[31mError:\u001b[0m Please provide text to hash',
      };
    }
    ...
    if (isNaN(rounds) || rounds < 4 || rounds > 20) {
      return {
        output: '\u001b[31mError:\u001b[0m Rounds must be between 4 and 20',
      };
    }
    ...
    try {
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Hash failed'}`,
      };
    }
  };

  // Command: encrypt - Encrypt text using specified algorithm
  private encryptCommand = async (
    args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    ...
    if (!text) {
      return {
        output: '\u001b[31mError:\u001b[0m Please provide text to encrypt',
      };
    }
    ...
    if (!supportedAlgos.includes(algo)) {
      return {
        output: `\u001b[31mError:\u001b[0m Unsupported algorithm. Use: ${supportedAlgos.join(', ')}`,
      };
    }
    ...
    try {
      ...
      if (opts?.key) {
        ...
        if (keyHex.length !== expectedLength) {
          return {
            output: `\u001b[31mError:\u001b[0m Key must be ${expectedLength} hex characters (${keySize} bytes)`,
          };
        }
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Encryption failed'}`,
      };
    }
  };

  // Command: decrypt - Decrypt text using specified algorithm
  private decryptCommand = async (
    args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    ...
    if (!encryptedData) {
      return {
        output:
          '\u001b[31mError:\u001b[0m Please provide encrypted data in format: encrypted:iv[:authTag]',
      };
    }
    ...
    try {
      ...
      if (requiresAuthTag && parts.length !== 3) {
        return {
          output:
            '\u001b[31mError:\u001b[0m Invalid format for AEAD cipher. Use: encrypted:iv:authTag',
        };
      }
      if (!requiresAuthTag && parts.length !== 2) {
        return {
          output: '\u001b[31mError:\u001b[0m Invalid format. Use: encrypted:iv',
        };
      }
      ...
      const lines = ['\u001b[32mDecrypted:\u001b[0m', '', decrypted];
      return { output: lines.join('\n') };
    } catch (error: any) {
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Decryption failed'}`,
      };
    }
  };

  // Command: api-token - Generate API access token
  private apiKeyCommand = async (
    args: string[],
    ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    ...
    if (expiresInDays !== null && (isNaN(expiresInDays) || expiresInDays < 1)) {
      return {
        output: '\u001b[31mError:\u001b[0m --expires must be a positive number',
      };
    }
    ...
    try {
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Failed to create API token'}`,
      };
    }
  };

  // Command: api-token-delete - Delete API access token
  private apiTokenDeleteCommand = async (
    args: string[],
    ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    ...
    if (!tokenId && !tokenName) {
      return {
        output: '\u001b[31mError:\u001b[0m Either --id or --name is required',
      };
    }
    ...
    try {
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Failed to delete API token'}`,
      };
    }
  };

  // Command: random - Generate random string
  private randomCommand = async (
    args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
  ...
  private dbStatsCommand = async (
    ...
    try {
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Failed to get database stats'}`,
      };
    }
  };

  // Command: cache-clear - Clear Redis cache
  private cacheClearCommand = async (
    args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    try {
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Failed to clear cache'}`,
      };
    }
  };

  // Command: cache-stats - Show cache statistics
  private cacheStatsCommand = async (
    _args: string[],
    _ctx?: ConsoleContext,
    _opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    try {
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Failed to get cache stats'}`,
      };
    }
  };

  // Command: api-token-list - List API tokens
  private tokenListCommand = async (
    args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    try {
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Failed to list tokens'}`,
      };
    }
  };

  // Command: user-info - Get user information
  private userInfoCommand = async (
    args: string[],
    _ctx?: ConsoleContext,
    _opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    ...
    if (!identifier) {
      return {
        output: '\u001b[31mError:\u001b[0m Please provide user ID or email',
      };
    }
    ...
    try {
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Failed to get user info'}`,
      };
    }
  };

  // Command: timestamp - Timestamp utilities
  private timestampCommand = async (
    args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    ...
    try {
      ...
      return {
        output: `\u001b[31mError:\u001b[0m ${error?.message || 'Invalid timestamp'}`,
      };
    }
  };

  // Command: md5 - Generate MD5 hash
  private md5Command = async (
    args: string[],
    _ctx?: ConsoleContext,
    _opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
  ...
  private genLoginIdCommand = async (
    _args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    ...
    if (isNaN(length) || length < 6 || length > 32) {
      return {
        output: '\u001b[31mError:\u001b[0m Length must be between 6 and 32',
      };
    }
    if (isNaN(count) || count < 1 || count > 100) {
      return {
  ...
  private genLoginPasswordCommand = async (
    _args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    ...
    if (isNaN(length) || length < 8 || length > 64) {
      return {
        output: '\u001b[31mError:\u001b[0m Length must be between 8 and 64',
      };
    }
    if (isNaN(count) || count < 1 || count > 100) {
      return {
  ...
  private unixtimestampCommand = async (
    _args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
  ...
  private chatTokenCommand = async (
    args: string[],
    _ctx?: ConsoleContext,
    opts?: Record<string, any>
  ): Promise<ConsoleExecutionResult> => {
    try {
      const { ChatServerService } = await import('./chat-server-service');
      ...
      return {
        output: `\x1b[31mError generating chat token:\x1b[0m ${error?.message || 'Unknown error'}`,
      };
    }
  };
}
