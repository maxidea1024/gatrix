...
const featureFlagDraftHandler = {
  /**
   * Create a snapshot from the current published state (all environments)
   * targetId = flagId (not flagName)
   * environmentId is IGNORED — we always capture ALL environments
   */
  async createSnapshot(
    ...
    // Get all environments for this flag
    const envSettings =
      await FeatureFlagEnvironmentModel.findByFlagId(targetId);
  ...
  /**
   * Apply draft data to real tables for ALL environments
   * draftData structure: { [environmentId]: { strategies, variants, values } }
   */
  async publish(
    targetId: string,
...
export function registerFeatureFlagDraftHandler(): void {
  registerDraftHandler('feature_flag', featureFlagDraftHandler);
}
