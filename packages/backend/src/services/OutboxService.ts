...
export class OutboxService {
    ...
    static async recordEvent(
        ...
        logger.debug(`[OutboxService] Event recorded: ${data.entityType}:${data.entityId} (${data.eventType})`);
        return event;
    }
    ...
    static async recordCREvents(
        ...
        for (const [, entity] of entityMap) {
            ...
            if (entity.wasCreated && entity.wasDeleted) {
                logger.debug(`[OutboxService] Pruned event: ${entity.entityType}:${entity.entityId} (created then deleted)`);
                continue;
            }

            // Skip if no actual change (before === after)
            if (!entity.wasCreated && !entity.wasDeleted) {
                const hasChange = JSON.stringify(entity.originalBefore) !== JSON.stringify(entity.finalAfter);
                if (!hasChange) {
                    logger.debug(`[OutboxService] Pruned event: ${entity.entityType}:${entity.entityId} (no net change)`);
                    continue;
                }
            }
        ...
        logger.info(`[OutboxService] Recorded ${events.length} events for CR ${changeRequestId} (pruned ${changes.length - events.length})`);
        return events;
    }
        ...
        for (const event of events) {
            try {
                ...
                if (newRetryCount >= maxRetries) {
                    ...
                    logger.error(`[OutboxService] Event ${event.id} failed after ${maxRetries} retries:`, error);
                } else {
                    await event.$query().patch({
                    ...
                    logger.warn(`[OutboxService] Event ${event.id} failed, will retry (${newRetryCount}/${maxRetries})`);
                }
            }
        }
