...
export interface EvaluationReason {
    kind: 'enabled' | 'disabled' | 'notFound' | 'constraintFailed' | 'rolloutFailed' | 'fallback' | 'archived';
    message?: string;
    failedConstraint?: {
        contextName: string;
        operator: string;
...
export class FeatureEvaluator {
    ...
    private evaluateStrategy(
        ...
        return {
            enabled: true,
            reason: { kind: 'enabled', message: `Strategy ${strategy.strategyName} matched` }
        };
    }
    ...
    private evaluateOperator(
        ...
        switch (operator) {
            ...
            // Boolean operators
            case 'bool_is':
                return ctx === val || (val === 'true' && !!contextValue) || (val === 'false' && !contextValue);
