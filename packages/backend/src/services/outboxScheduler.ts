...
export async function initializeOutboxProcessorJob(): Promise<void> {
    ...
    try {
        ...
        if (!processorExists) {
            await queueService.addJob('scheduler', 'outbox:process', { batchSize }, {
        ...
        if (!cleanupExists) {
            await queueService.addJob('scheduler', 'outbox:cleanup', { retentionDays: cleanupRetentionDays }, {
        ...
        if (!lockCleanupExists) {
            await queueService.addJob('scheduler', 'lock:cleanup', {}, {
