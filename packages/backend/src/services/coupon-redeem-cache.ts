...
const K = {
  ...
  targetSet: (env: string, id: string, type: string) =>
    `coupon:t:${env}:${id}:${type}`,
};
...
export async function resolveSettingCached(
  ...
  if (cachedSettingId) {
    ...
    const settingData = await redis.get(
      K.setting(environmentId, cachedSettingId)
    );
    if (settingData) {
      return {
  ...
  const coupon = await db('g_coupons')
    .where('code', code)
  ...
  if (coupon) {
    ...
    if (cachedSetting) {
      return {
        setting: JSON.parse(cachedSetting),
        couponId: coupon.id,
        isSpecialCoupon: false,
      };
    }
    ...
    const setting = await db('g_coupon_settings')
      ...
      .first();
    if (!setting)
      return { setting: null, couponId: null, isSpecialCoupon: false };
  ...
  pipe.set(
    K.setting(environmentId, specialSetting.id),
    JSON.stringify(specialSetting),
    'EX',
    CACHE_TTL
  );
  await pipe.exec();
...
export async function validateTargetingCached(
  ...
  if (!(await redis.exists(loadedKey))) {
    ...
    if (worlds.length > 0) {
      pipe.sadd(prefix('worlds'), ...worlds.map((w: any) => w.gameWorldId));
      pipe.expire(prefix('worlds'), CACHE_TTL);
    }
    if (platforms.length > 0) {
      pipe.sadd(prefix('platforms'), ...platforms.map((p: any) => p.platform));
      pipe.expire(prefix('platforms'), CACHE_TTL);
    }
    if (channels.length > 0) {
      pipe.sadd(prefix('channels'), ...channels.map((c: any) => c.channel));
      pipe.expire(prefix('channels'), CACHE_TTL);
    }
    if (subchannels.length > 0) {
      pipe.sadd(
        prefix('subchannels'),
        ...subchannels.map((s: any) => s.subchannel)
      );
      pipe.expire(prefix('subchannels'), CACHE_TTL);
    }
    if (users.length > 0) {
      pipe.sadd(prefix('users'), ...users.map((u: any) => u.userId));
      pipe.expire(prefix('users'), CACHE_TTL);
    }
  ...
  if (hasSubchannels && request.subChannel) {
    const match = await redis.sismember(
      prefix('subchannels'),
      request.subChannel
    );
 ...
 */
export async function getRewardCached(
  environmentId: string,
  setting: any
): Promise<any[]> {
  const redis = redisClient.getClient();
  ...
  if (setting.rewardTemplateId) {
    ...
    if (tpl) {
      const items =
        typeof tpl.rewardItems === 'string'
          ? JSON.parse(tpl.rewardItems)
          : tpl.rewardItems;
