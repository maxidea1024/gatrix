...
async function cleanupInvitations() {
  try {
    ...
    pendingInvitations.forEach((inv) => {
      console.log(
        `  ID: ${inv.id}, Channel: ${inv.channelId}, Inviter: ${inv.inviterId}, Invitee: ${inv.inviteeId}, Created: ${inv.createdAt}`
      );
    });
    ...
    if (duplicates.length > 0) {
      ...
      duplicates.forEach((dup) => {
        console.log(
          `  Channel ${dup.channelId}, Invitee ${dup.inviteeId}: ${dup.count} invitations`
        );
      });
      ...
      for (const dup of duplicates) {
        ...
        for (const inv of toCancel) {
          await knex('chat_channel_invitations').where('id', inv.id).update({
            status: 'cancelled',
            updatedAt: new Date(),
          });
