...
export class RedisManager {
  ...
  public async getUserSocketInfo(userId: number): Promise<{ socketId?: string; serverId?: string } | null> {
  ...
  public async getRecentMessages(channelId: number, limit = 50): Promise<any[]> {
    const messages = await this.redisClient!.lrange(`channel:${channelId}:recent_messages`, 0, limit - 1);
