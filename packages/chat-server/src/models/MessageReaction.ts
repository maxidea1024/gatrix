...
export class MessageReactionModel {
   ...
   */
  static async toggleReaction(messageId: number, userId: number, emoji: string): Promise<{ action: 'added' | 'removed', reaction?: MessageReaction }> {
    try {
      const db = databaseManager.getKnex();
   ...
   */
  static async getReactionSummary(messageId: number, currentUserId?: number): Promise<ReactionSummary[]> {
    try {
      const db = databaseManager.getKnex();
