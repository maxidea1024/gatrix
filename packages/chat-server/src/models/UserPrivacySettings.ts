...
export class UserPrivacySettingsModel {
  ...
  // 기본 설정값
  private static getDefaultSettings(): Omit<UserPrivacySettingsType, 'userId' | 'createdAt' | 'updatedAt'> {
    return {
  ...
  // 프라이버시 설정 업데이트
  static async update(userId: number, data: UpdatePrivacySettingsData): Promise<UserPrivacySettingsType> {
    try {
  ...
  static async canInviteUser(
    ...
    try {
      ...
      // 2. 정책 확인
      const policy = inviteType === 'direct' 
