...
export default async function cronsRoutes(app: FastifyInstance) {
  ...
  }>('/:projectId/crons', async (request, reply) => {
    try {
      const { projectId } = request.params;
      const {
        name,
      ...
      try {
        ...
        nextCheckinAt = getNextSchedule(
          new Date(),
          scheduleConfig,
          timezone !== 'UTC' ? timezone : undefined
        );
      } catch (schedErr) {
        logger.warn('Invalid schedule expression', {
          schedule_value,
          error: schedErr,
        });
        return reply
          .code(400)
          .send({ success: false, error: 'Invalid schedule expression' });
      }
      ...
      if (error.code === 'ER_DUP_ENTRY') {
        return reply
          .code(409)
          .send({ success: false, error: 'Monitor slug already exists' });
      }
  ...
  }>('/:projectId/crons/:monitorId', async (request, reply) => {
    try {
  ...
  }>('/:projectId/crons/:slug/checkin', async (request, reply) => {
    try {
      const { projectId, slug } = request.params;
      const { status, check_in_id, duration, environment, trace_id } =
        request.body;
      ...
      if (check_in_id && (status === 'ok' || status === 'error')) {
        ...
        if (existing.length > 0) {
          ...
          const computedDuration =
            duration ??
            Date.now() - new Date(inProgressCheckin.date_in_progress).getTime();
          await db.raw(
      ...
      const checkinId =
        check_in_id || require('crypto').randomBytes(16).toString('hex');
      const now = new Date();
      ...
      if (status === 'in_progress') {
        ...
        timeoutAt = new Date(
          now.getTime() + (monitor.max_runtime || 30) * 60_000
        );
      }
  ...
  }>('/:projectId/crons/:monitorId/checkins', async (request, reply) => {
    try {
      const { projectId, monitorId } = request.params;
  ...
  }>('/:projectId/crons/:monitorId/test-checkin', async (request, reply) => {
    try {
      const { projectId, monitorId } = request.params;
