...
export default async function integrationsRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { projectId } = request.params as { projectId: string };
      const { provider, repo_url, default_branch, access_token } =
        request.body as {
          provider: string;
          repo_url: string;
  ...
  app.patch(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { projectId, integrationId } = request.params as {
        projectId: string;
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { projectId } = request.params as { projectId: string };
      const { release, limit = '20' } = request.query as {
        release?: string;
        limit?: string;
      };
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        logger.error('Failed to ingest commits', {
          projectId,
          error: error instanceof Error ? error.message : String(error),
        });
      ...
      try {
        ...
        const commits = await db('g_argus_commits')
          ...
          .andWhere(
            'timestamp',
            '>=',
            db.raw('DATE_SUB(UTC_TIMESTAMP(), INTERVAL 7 DAY)')
          )
          .orderBy('timestamp', 'desc')
          .limit(20);
        ...
        logger.error('Failed to get suspect commits', {
          projectId,
          issueId,
          error: error instanceof Error ? error.message : String(error),
        });
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        logger.error('Failed to list ownership rules', {
          projectId,
          error: error instanceof Error ? error.message : String(error),
        });
        return reply
          .code(500)
          .send({ error: 'Failed to list ownership rules' });
      }
    }
  );
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { projectId } = request.params as { projectId: string };
      const { name, match_type, match_pattern, owners, priority, auto_assign } =
        request.body as {
          name: string;
          match_type: string;
      ...
      try {
        ...
        logger.error('Failed to create ownership rule', {
          projectId,
          error: error instanceof Error ? error.message : String(error),
        });
        return reply
          .code(500)
          .send({ error: 'Failed to create ownership rule' });
      }
    }
  );
  ...
  app.patch(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        if (body.owners !== undefined)
          ownUpdateObj.owners = JSON.stringify(body.owners);
        await db('g_argus_ownership_rules')
          .where({ id: ruleId, project_id: projectId })
          .update(ownUpdateObj);
        return reply.send({ success: true });
      } catch (error) {
        logger.error('Failed to update ownership rule', {
          projectId,
          ruleId,
          error: error instanceof Error ? error.message : String(error),
        });
        return reply
          .code(500)
          .send({ error: 'Failed to update ownership rule' });
      }
    }
  );
  ...
  app.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        logger.error('Failed to delete ownership rule', {
          error: error instanceof Error ? error.message : String(error),
        });
        return reply
          .code(500)
          .send({ error: 'Failed to delete ownership rule' });
      }
    }
  );
}
