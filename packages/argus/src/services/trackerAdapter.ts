...
async function clickupCreateIssue(config: TrackerConfig, issue: IssuePayload): Promise<ExternalIssueResult> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`ClickUp API error: ${response.status} ${err.substring(0, 200)}`);
  }
  const data = await response.json() as { id: string; url: string };
  return { url: data.url, key: `#${data.id}` };
}

async function clickupTestConnection(config: TrackerConfig): Promise<ConnectionTestResult> {
  try {
...
async function asanaCreateIssue(config: TrackerConfig, issue: IssuePayload): Promise<ExternalIssueResult> {
  ...
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`Asana API error: ${response.status} ${err.substring(0, 200)}`);
  }
  const data = await response.json() as { data: { gid: string; permalink_url: string } };
  return { url: data.data.permalink_url, key: data.data.gid };
}

async function asanaTestConnection(config: TrackerConfig): Promise<ConnectionTestResult> {
  try {
    ...
    if (response.ok) {
      ...
      return { ok: true, message: `Connected as ${data.data.name} (${data.data.email})` };
    }
    return { ok: false, message: `HTTP ${response.status}` };
  } catch (error) {
    return { ok: false, message: (error as Error).message };
...
async function notionCreateIssue(config: TrackerConfig, issue: IssuePayload): Promise<ExternalIssueResult> {
  ...
  const body = {
    ...
    children: issue.description ? [{
      ...
      paragraph: { rich_text: [{ type: 'text' as const, text: { content: issue.description } }] },
    }] : [],
  };
  ...
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`Notion API error: ${response.status} ${err.substring(0, 200)}`);
  }
  const data = await response.json() as { id: string; url: string };
  return { url: data.url, key: data.id.replace(/-/g, '').substring(0, 8) };
}

async function notionTestConnection(config: TrackerConfig): Promise<ConnectionTestResult> {
  try {
    ...
    if (response.ok) {
      ...
      return { ok: true, message: `Connected as ${data.name || data.type || 'bot'}` };
    }
    return { ok: false, message: `HTTP ${response.status}` };
  } catch (error) {
    return { ok: false, message: (error as Error).message };
...
async function shortcutCreateIssue(config: TrackerConfig, issue: IssuePayload): Promise<ExternalIssueResult> {
  ...
  if (issue.labels?.length) body.labels = issue.labels.map(l => ({ name: l }));

  const response = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Shortcut-Token': config.apiToken },
    body: JSON.stringify(body),
  });
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`Shortcut API error: ${response.status} ${err.substring(0, 200)}`);
  }
  const data = await response.json() as { id: number; app_url: string };
  return { url: data.app_url, key: `sc-${data.id}` };
}

async function shortcutTestConnection(config: TrackerConfig): Promise<ConnectionTestResult> {
  try {
...
async function azureDevOpsCreateIssue(config: TrackerConfig, issue: IssuePayload): Promise<ExternalIssueResult> {
  const organization = config.config.organization;
  const project = config.config.project;
  if (!organization || !project) throw new Error('Azure DevOps organization and project are required');
  ...
  const body = [
    ...
    { op: 'add', path: '/fields/System.Description', value: issue.description || 'Created from Argus' },
  ];
  ...
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`Azure DevOps API error: ${response.status} ${err.substring(0, 200)}`);
  }
  const data = await response.json() as { id: number; _links: { html: { href: string } } };
  return { url: data._links.html.href, key: `#${data.id}` };
}

async function azureDevOpsTestConnection(config: TrackerConfig): Promise<ConnectionTestResult> {
  try {
    const organization = config.config.organization;
    if (!organization) return { ok: false, message: 'organization is not configured' };
    const apiBase = config.apiUrl?.replace(/\/$/, '') || 'https://dev.azure.com';
    const authHeader = `Basic ${Buffer.from(`:${config.apiToken}`).toString('base64')}`;
    const response = await fetch(`${apiBase}/${organization}/_apis/projects?api-version=7.0`, {
      headers: { 'Authorization': authHeader },
    });
    if (response.ok) {
...
async function redmineCreateIssue(config: TrackerConfig, issue: IssuePayload): Promise<ExternalIssueResult> {
  const projectId = config.config.project_id;
  ...
  const body = {
    issue: {
      ...
      subject: issue.title,
      description: issue.description || 'Created from Argus',
      tracker_id: config.config.tracker_id ? Number(config.config.tracker_id) : 1, // 1 = Bug
    },
  };

  const response = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-Redmine-API-Key': config.apiToken },
    body: JSON.stringify(body),
  });
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`Redmine API error: ${response.status} ${err.substring(0, 200)}`);
  }
  const data = await response.json() as { issue: { id: number } };
  return { url: `${apiBase}/issues/${data.issue.id}`, key: `#${data.issue.id}` };
}

async function redmineTestConnection(config: TrackerConfig): Promise<ConnectionTestResult> {
  try {
    const apiBase = config.apiUrl.replace(/\/$/, '');
    ...
    if (response.ok) {
      const data = await response.json() as { user: { login: string; firstname: string } };
      return { ok: true, message: `Connected as ${data.user.firstname} (${data.user.login})` };
    }
    return { ok: false, message: `HTTP ${response.status}` };
  } catch (error) {
    return { ok: false, message: (error as Error).message };
...
async function youtrackCreateIssue(config: TrackerConfig, issue: IssuePayload): Promise<ExternalIssueResult> {
  const projectShortName = config.config.project_id;
  if (!projectShortName) throw new Error('YouTrack project_id (short name) is not configured');
  ...
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`YouTrack API error: ${response.status} ${err.substring(0, 200)}`);
  }
...
async function youtrackTestConnection(config: TrackerConfig): Promise<ConnectionTestResult> {
  try {
    const apiBase = config.apiUrl.replace(/\/$/, '');
...
async function trelloCreateIssue(config: TrackerConfig, issue: IssuePayload): Promise<ExternalIssueResult> {
  const listId = config.config.list_id;
  ...
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`Trello API error: ${response.status} ${err.substring(0, 200)}`);
  }
  const data = await response.json() as { id: string; shortUrl: string; idShort: number };
  return { url: data.shortUrl, key: `#${data.idShort}` };
}

async function trelloTestConnection(config: TrackerConfig): Promise<ConnectionTestResult> {
  try {
    const apiKey = config.config.api_key || '';
    const response = await fetch(`https://api.trello.com/1/members/me?key=${apiKey}&token=${config.apiToken}`);
    if (response.ok) {
      const data = await response.json() as { username: string; fullName: string };
      return { ok: true, message: `Connected as ${data.fullName} (@${data.username})` };
    }
    return { ok: false, message: `HTTP ${response.status}` };
  } catch (error) {
    return { ok: false, message: (error as Error).message };
...
const adapters: Record<string, {
  ...
  clickup: { createIssue: clickupCreateIssue, testConnection: clickupTestConnection },
  asana: { createIssue: asanaCreateIssue, testConnection: asanaTestConnection },
  notion: { createIssue: notionCreateIssue, testConnection: notionTestConnection },
  shortcut: { createIssue: shortcutCreateIssue, testConnection: shortcutTestConnection },
  azure_devops: { createIssue: azureDevOpsCreateIssue, testConnection: azureDevOpsTestConnection },
  redmine: { createIssue: redmineCreateIssue, testConnection: redmineTestConnection },
  youtrack: { createIssue: youtrackCreateIssue, testConnection: youtrackTestConnection },
  trello: { createIssue: trelloCreateIssue, testConnection: trelloTestConnection },
};
