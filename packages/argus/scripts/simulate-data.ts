...
function generateContextualLogs(
  scenario: ErrorScenario,
...
async function main() {
  ...
  for (const table of chTables) {
    try {
      await ch.exec({
        query: `TRUNCATE TABLE IF EXISTS ${CH_CONFIG.database}.${table}`,
      });
  ...
  for (let i = 0; i < TOTAL_ERROR_EVENTS; i++) {
    ...
    const HTTP_ENDPOINTS: {
      ...
      {
        method: 'DELETE',
        url: '/api/session/expire',
        statuses: [204, 204, 404],
        durMin: 8,
        durMax: 30,
        sizeMin: 0,
        sizeMax: 64,
    ...
    function generateHttpBreadcrumb(ts: string): {
      type: string;
      category: string;
      message: string;
      level: string;
    ...
    const EXTRA_BREADCRUMB_POOL: {
    ...
    }[] = [
      {
        ...
        category: 'ws',
        messages: [
          'WS message: player_position_update',
          'WS message: chat_broadcast',
          'WS message: inventory_sync',
          'WS keepalive ping sent',
          'WS keepalive pong received',
          'WS message: guild_notification',
          'WS message: trade_request',
          'WS reconnect attempt #1',
          'WS message: weather_change',
          'WS message: server_announcement',
        ],
        levels: ['info', 'info', 'debug'],
      },
      {
        type: 'default',
        category: 'console',
        messages: [
          '[Debug] Frame time: 16.4ms',
          '[Debug] Draw calls: 1247',
          '[Debug] Memory: 412MB / 2048MB',
          '[Info] Asset bundle loaded: world_assets_03',
          '[Info] Texture streaming: 89% complete',
          '[Warn] GC pause: 12ms',
          '[Debug] Physics step: 8.2ms',
          '[Info] Audio: 24 active sources',
          '[Debug] Network RTT: 45ms',
        ],
        levels: ['debug', 'info', 'info', 'warning'],
      },
      {
        type: 'default',
        category: 'db',
        messages: [
          ...
          'UPDATE inventory SET quantity = ? WHERE slot_id = ?',
          'INSERT INTO chat_log (sender, message, channel) VALUES (?, ?, ?)',
          'SELECT COUNT(*) FROM guild_members WHERE guild_id = ?',
          'BEGIN TRANSACTION',
          'COMMIT',
          'SELECT * FROM market_listings WHERE expires_at > NOW()',
        ],
        levels: ['info', 'debug'],
      },
      {
        ...
        messages: [
          ...
          'SET player:position:12345',
          'PUBLISH chat:channel:global',
          'EXPIRE session:user:12345 3600',
          'HGETALL guild:info:567',
          'ZADD leaderboard:pvp 1500 user:12345',
        ],
        levels: ['info', 'debug'],
      },
      {
        ...
        messages: [
          ...
          'Session validated',
          'Permission check: inventory.write → allowed',
          'Rate limit check: 42/1000 requests',
          'JWT claims verified',
      ...
      },
      {
        ...
        category: 'network',
        messages: [
          'Connection quality: GOOD (45ms RTT)',
          'Packet loss rate: 0.1%',
          'Bandwidth usage: 12.4 KB/s',
          'DNS resolution: game.server.io → 10.0.1.5',
          'TCP keepalive sent',
      ...
      },
      {
        ...
        category: 'anticheat',
        messages: [
          'Position validation: OK',
          'Speed check: within threshold',
          'Input frequency check: normal',
          'Memory scan: clean',
      ...
      },
      {
        ...
        category: 'matchmaking',
        messages: [
          'Queue joined: PvP Ranked',
          'Match found: 5v5 Arena',
          'ELO rating check: 1450',
          'Team balance score: 0.92',
          'Map selected: Coral Harbor',
        ],
        levels: ['info'],
      },
      {
        ...
        category: 'inventory',
        messages: [
          'Item equipped: Steel Cutlass +2',
          'Item moved: slot 3 → slot 7',
          'Stack merged: Healing Potion x5',
          'Item durability check: 78%',
      ...
      },
      {
        ...
        category: 'payment',
        messages: [
          'Premium currency balance checked: 1250',
          'Shop catalog loaded: 47 items',
          'Purchase validation started',
          'Receipt verified with store API',
        ],
        levels: ['info', 'warning'],
      },
      {
        ...
        messages: [
          ...
          'Whisper received from player',
          'Chat filter: message approved',
          'Emote triggered: /wave',
          'Chat history loaded: 50 messages',
        ],
        levels: ['info', 'debug'],
      },
    ];
    ...
    for (let bi = 0; bi < extraCount; bi++) {
      const ts = new Date(
        timestamp.getTime() - (totalCount - bi) * randomInt(500, 8000)
      ).toISOString();
      ...
      const pool =
        EXTRA_BREADCRUMB_POOL[randomInt(0, EXTRA_BREADCRUMB_POOL.length - 1)];
    ...
    scenario.breadcrumbTemplates.forEach((tmpl, idx) => {
      breadcrumbs.push({
        ...tmpl,
        timestamp: new Date(
          timestamp.getTime() - (templateCount - idx) * randomInt(1000, 5000)
        ).toISOString(),
      });
    });
    ...
    allEvents.push({
      ...
      received_at: new Date(
        timestamp.getTime() + randomInt(50, 500)
      ).toISOString(),
      ...
      stacktrace_frames: JSON.stringify(
        scenario.frames.map((f) => {
          ...
          if (isCpp) {
            ...
            contextLine = f.in_app
              ? `    check(bIsReady); // [Error: ${scenario.type}]`
            ...
            contextLine = f.in_app
              ? `    assert(ready, "${scenario.type}") -- [Error occurred here]`
              : `    -- Library code`;
            postContext = [`    return ready`, `end`];
          } else {
            // JS / Node
            preContext = [
            ...
            contextLine = f.in_app
              ? `    throw new Error("${scenario.type}"); // [Error occurred here]`
              : `    // System execution`;
            postContext = [`    return isReady;`, `}`];
          }
      ...
      device_family:
        os.name === 'iOS'
          ...
          : os.name === 'Android'
            ...
            : 'Desktop',
      runtime_name:
        scenario.runtime === 'nodejs'
          ? 'node'
          : scenario.runtime === 'lua'
            ? 'lua'
            : 'ue4',
      runtime_version:
        scenario.runtime === 'nodejs'
      ...
      http_method:
        scenario.transaction.startsWith('GET') ||
        scenario.transaction.startsWith('WS')
          ? 'GET'
          : 'POST',
  ...
  console.log(
    `   ✓ ${allEvents.length.toLocaleString()} events generated (${issueMap.size} unique issues)`
  );
  ...
  for (const [hash, tracker] of issueMap) {
    ...
    const [result] = await pool.query(
      ...
      [
        ...
        randomPick(s.releases),
        s.releases[s.releases.length - 1],
        s.level === 'fatal'
          ? 'critical'
          : s.level === 'error'
            ? randomPick(['high', 'high', 'medium'])
            : 'medium',
      ]
    );
    tracker.id = (result as any).insertId;
    console.log(
      `   ✓ [${s.runtime}/${s.level}] ${s.title.substring(0, 55)}... (${tracker.count.toLocaleString()} events)`
    );
  }
  ...
  for (let i = 0; i < allEvents.length; i += CHUNK_SIZE) {
    ...
    await ch.insert({
      table: `${CH_CONFIG.database}.errors`,
      values: chunk,
      format: 'JSONEachRow',
    });
    if ((i + CHUNK_SIZE) % 50000 < CHUNK_SIZE) {
      console.log(
        `   ... ${Math.min(i + CHUNK_SIZE, allEvents.length).toLocaleString()} / ${allEvents.length.toLocaleString()}`
      );
    }
  }
  ...
  for (let i = 0; i < TOTAL_TRANSACTIONS; i++) {
    ...
    for (let s = 0; s < spanTemplates.length; s++) {
      ...
      const spanDur = randomInt(
        ...
        Math.max(
          st.durMin,
          Math.min(st.durMax, Math.floor(duration / spanTemplates.length))
        )
      );
      spanBatch.push({
        span_id: uuid().substring(0, 16),
        ...
        status:
          isError && s === spanTemplates.length - 1 ? 'internal_error' : 'ok',
        action: st.op.split('.')[1] || '',
        domain: st.op.startsWith('db')
          ? 'mysql'
          : st.op.startsWith('cache')
            ? 'redis'
            : st.op.startsWith('http')
              ? 'internal-api'
              : '',
        data: {},
        tags: {
          'server.region': randomPick([
            'ap-northeast-2',
            'eu-west-1',
            'us-east-1',
          ]),
    ...
    txnBatch.push({
      ...
      transaction_status: isError
        ? randomPick(['internal_error', 'deadline_exceeded'])
        : 'ok',
      http_method: tmpl.name.startsWith('WS') ? 'WS' : tmpl.name.split(' ')[0],
      http_status_code: isError
        ? randomPick([500, 502, 503, 504])
        : randomPick([200, 200, 200, 201]),
      platform: 'node',
      environment: env,
      release,
      user_id: user.id,
    ...
    if (txnBatch.length >= CHUNK_SIZE) {
      await ch.insert({
        table: `${CH_CONFIG.database}.transactions`,
        values: txnBatch,
        format: 'JSONEachRow',
      });
      ...
      if (spanBatch.length > 0) {
        try {
          await ch.insert({
            table: `${CH_CONFIG.database}.spans`,
            values: spanBatch,
            format: 'JSONEachRow',
          });
        } catch {
  ...
  for (let i = 0; i < TOTAL_SESSIONS; i++) {
    ...
    const duration =
      status === 'crashed' || status === 'abnormal'
        ? randomInt(1000, 60000)
        : randomInt(60000, 7200000);
  ...
  try {
    for (let i = 0; i < feedbackBatch.length; i += CHUNK_SIZE) {
      await ch.insert({
        ...
        values: feedbackBatch.slice(i, i + CHUNK_SIZE),
        format: 'JSONEachRow',
      });
    }
    console.log(
  ...
  for (let i = 0; i < TOTAL_METRICS; i++) {
    ...
    let value_counter = 0,
      ...
      value_set: string[] = [];
    if (tmpl.type === 'counter')
      value_counter = randomFloat(tmpl.valMin, tmpl.valMax);
    else if (tmpl.type === 'gauge')
      value_gauge = randomFloat(tmpl.valMin, tmpl.valMax);
    else if (tmpl.type === 'distribution') {
      const len = randomInt(1, 10);
      for (let j = 0; j < len; j++)
        value_distribution.push(randomFloat(tmpl.valMin, tmpl.valMax));
    }
    ...
    if (metricsBatch.length >= CHUNK_SIZE) {
      try {
        await ch.insert({
          table: `${CH_CONFIG.database}.metrics`,
          values: metricsBatch,
          format: 'JSONEachRow',
        });
      } catch (e: any) {
        console.log(
          `   ⚠ Metrics insert failed: ${e.message?.substring(0, 80)}`
        );
      }
  ...
  if (dsnKeyTimestamps.size > 0) {
    ...
    for (const [keyId, ts] of dsnKeyTimestamps) {
      ...
      try {
        ...
        console.log(
          `   ✓ DSN key ${keyId}: first=${formatDate(ts.min)}, last=${formatDate(ts.max)}`
        );
      } catch (e) {
  ...
  for (const [, tracker] of sorted.slice(0, 10)) {
    const s = tracker.scenario;
    console.log(
      `     [${s.runtime.padEnd(6)}] ${tracker.count.toLocaleString().padStart(8)} events | ${s.title.substring(0, 50)}`
    );
  }
