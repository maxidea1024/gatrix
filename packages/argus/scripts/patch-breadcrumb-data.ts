...
function parseHttpMessage(
  msg: string
): { method: string; url: string; status: number; duration: number } | null {
  // "GET /api/character/info → 200 (45ms)" or "POST /api/inventory/move → 200 (120ms)"
  const m = msg.match(
    /^(GET|POST|PUT|DELETE|PATCH)\s+(\S+)\s*→\s*(\d+)\s*\((\d+)ms\)/
  );
  if (m) {
    return {
      method: m[1],
...
async function main() {
  ...
  for (const row of rows) {
    ...
    for (const bc of breadcrumbs) {
      ...
      if (
        ...
        if (parsed) {
          const ep =
            HTTP_ENDPOINTS.find((e) => bc.message?.includes(e.url)) ||
            HTTP_ENDPOINTS[0];
          bc.data = {
      ...
      if (
        (bc.category === 'navigation' || bc.type === 'navigation') &&
        !bc.data
      ) {
        const parsed = parseNavigationMessage(bc.message || '');
        if (parsed) {
          const nav = NAV_PAGES.find((n) => n.to === parsed.to) || {
            from: '/game/world',
            to: parsed.to,
          };
  ...
  for (const upd of updates) {
    ...
    try {
      ...
      console.error(
        `   ✗ Failed event_id=${upd.event_id}: ${e.message?.substring(0, 100)}`
      );
    }
  }
