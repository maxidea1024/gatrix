# AQL (Argus Query Language) — Engine Specification

> **Status**: Draft v1.2  
> **Last Updated**: 2025-06-11  
> **Author**: Gatrix Frontend Team  
> **Scope**: AQL Engine for Argus (Logs, Issues, Discover, Performance, Feedback, Sessions, Traces)
...
Argus Query Language (AQL) Engine은 Sentry / Datadog / Kibana 스타일의 **Context-Aware Query Editor**를 구현하는 프로젝트이다.

단순 검색 입력창이 아니라, AQL 문법을 이해하고 현재 입력 상태·커서 위치·문법 상태에 따라 적절한 추천을 제공하는 **스마트 쿼리 엔진**이다. 도메인별로 사용 가능한 필드와 집계 함수를 동적으로 제어하며, 사용자를 올바른 문법으로 유도하는 선제적 자동완성을 제공한다.
...
5. **AQL 로직과 UI는 완전히 분리** — UI는 렌더링만 담당
6. **문법 오류 시 쿼리 실행 차단** — 오류가 있으면 Enter를 눌러도 검색 실행하지 않음
7. **자동완성으로 오류 예방** — 자동완성이 사용자를 올바른 문법으로 유도하여 오류 발생을 최소화
8. **도메인 인식 엔진** — 각 도메인(Logs, Issues, Performance 등)이 자신의 필드·집계 함수를 독립적으로 정의
...
AQL의 기본 문법은 `field:value`이다. Colon(`:`)은 필드와 값을 구분하는 구분자이다.
...
Primary       = LPAREN Expression RPAREN | AggregateFilter | Filter | FreeText

Filter        = FIELD COLON ValueExpr
FreeText      = QuotedString | UnquotedString    — 필드 없이 단독 입력 → FreeTextExpression AST 노드

# ── Aggregate Filter (Section 21 참조) ──
# Parser가 FIELD + LPAREN 패턴을 감지하면 DomainConfig.aggregates를 참조하여 판별
# Lexer는 변경 없음 — FIELD 토큰 그대로 사용
AggregateFilter = AggFunc COLON CompareOp? Value
AggFunc         = FIELD LPAREN AggArgList? RPAREN
AggArgList      = Identifier ( COMMA Identifier )*
 ...
 * 
 * 이유: Parser는 AQL 문법을 구조화만 할 뿐, 의미 해석(어떤 필드에 매핑할지)은
...
type Expression =
  ...
  | GroupExpression
  | PartialExpression
  | AggregateFilterExpression;   // Section 21

/**
 * 집계 함수 필터 (Section 21).
 * count():>100, avg(duration):>=500, log_rate(error):>0.5 등.
 * Lexer는 변경 없음 — Parser가 FIELD+LPAREN 패턴을 DomainConfig.aggregates에서 확인하여 생성.
 */
interface AggregateFilterExpression {
  type: 'AggregateFilter';
  /** 함수 이름: count, avg, p95, log_rate, etc. */
  func: string;
  /** 함수 인자: ['duration'], ['error'], [] */
  args: string[];
  /** 비교 연산자: '>', '>=', '<', '<=', '=', '!=' */
  operator: string;
  /** 비교 값 (숫자) */
...
const ALL_QUERY_FIELDS: QueryField[] = [
  // ── Log fields ──
  { key: 'level',        label: 'aql.field.level',        type: 'string',   category: 'log',      operators: ['=', '!='], searchable: true, description: 'aql.field.level.desc', staticValues: ['debug', 'info', 'warning', 'error', 'fatal'] },
  { key: 'message',      label: 'aql.field.message',      type: 'string',   category: 'log',      operators: ['=', '!=', 'contains', 'startsWith', 'endsWith'], searchable: true, description: 'aql.field.message.desc' },
  { key: 'body',         label: 'aql.field.body',         type: 'string',   category: 'log',      operators: ['=', '!=', 'contains', 'startsWith', 'endsWith'], searchable: true, description: 'aql.field.body.desc' },
  { key: 'logger_name',  label: 'aql.field.loggerName',   type: 'string',   category: 'log',      operators: ['=', '!='], searchable: true, description: 'aql.field.loggerName.desc' },
  { key: 'timestamp',    label: 'aql.field.timestamp',    type: 'datetime', category: 'log',      operators: ['=', '!=', 'before', 'after'], searchable: false, description: 'aql.field.timestamp.desc' },

  // ── Resource fields ──
  { key: 'service',      label: 'aql.field.service',      type: 'string',   category: 'resource', operators: ['=', '!='], searchable: true, description: 'aql.field.service.desc' },
  { key: 'environment',  label: 'aql.field.environment',  type: 'string',   category: 'resource', operators: ['=', '!='], searchable: true, description: 'aql.field.environment.desc' },
  { key: 'release',      label: 'aql.field.release',      type: 'string',   category: 'resource', operators: ['=', '!='], searchable: true, description: 'aql.field.release.desc' },

  // ── Trace fields ──
  { key: 'trace_id',     label: 'aql.field.traceId',      type: 'string',   category: 'trace',    operators: ['=', '!='], searchable: true, description: 'aql.field.traceId.desc' },
  { key: 'span_id',      label: 'aql.field.spanId',       type: 'string',   category: 'trace',    operators: ['=', '!='], searchable: true, description: 'aql.field.spanId.desc' },
  { key: 'log_id',       label: 'aql.field.logId',        type: 'string',   category: 'trace',    operators: ['=', '!='], searchable: true, description: 'aql.field.logId.desc' },
  { key: 'issue_id',     label: 'aql.field.issueId',      type: 'number',   category: 'trace',    operators: ['=', '!=', '>', '>=', '<', '<='], searchable: true, description: 'aql.field.issueId.desc' },

  // ── Event/Issue fields ──
  { key: 'type',         label: 'aql.field.type',         type: 'string',   category: 'event',    operators: ['=', '!='], searchable: true, description: 'aql.field.type.desc' },
  { key: 'value',        label: 'aql.field.value',        type: 'string',   category: 'event',    operators: ['=', '!=', 'contains', 'startsWith', 'endsWith'], searchable: true, description: 'aql.field.value.desc' },
  { key: 'handled',      label: 'aql.field.handled',      type: 'boolean',  category: 'event',    operators: ['=', '!='], searchable: false, description: 'aql.field.handled.desc' },
  { key: 'platform',     label: 'aql.field.platform',     type: 'string',   category: 'event',    operators: ['=', '!='], searchable: true, description: 'aql.field.platform.desc' },
  { key: 'status',       label: 'aql.field.status',       type: 'string',   category: 'event',    operators: ['=', '!='], searchable: true, description: 'aql.field.status.desc' },
  { key: 'assigned',     label: 'aql.field.assigned',     type: 'string',   category: 'event',    operators: ['=', '!='], searchable: true, description: 'aql.field.assigned.desc', staticValues: ['me', 'none', 'my_teams'] },

  // ── User fields ──
  { key: 'browser_name', label: 'aql.field.browserName',  type: 'string',   category: 'user',     operators: ['=', '!='], searchable: true, description: 'aql.field.browserName.desc' },
  { key: 'os_name',      label: 'aql.field.osName',       type: 'string',   category: 'user',     operators: ['=', '!='], searchable: true, description: 'aql.field.osName.desc' },
  { key: 'device',       label: 'aql.field.device',       type: 'string',   category: 'user',     operators: ['=', '!='], searchable: true, description: 'aql.field.device.desc' },

  // ── Performance fields ──
  { key: 'transaction',  label: 'aql.field.transaction',  type: 'string',   category: 'event',    operators: ['=', '!=', 'contains'], searchable: true, description: 'aql.field.transaction.desc' },
  { key: 'duration',     label: 'aql.field.duration',     type: 'number',   category: 'event',    operators: ['=', '!=', '>', '>=', '<', '<='], searchable: false, description: 'aql.field.duration.desc' },

  // ── Feedback fields ──
  { key: 'contact_email', label: 'aql.field.contactEmail', type: 'string',  category: 'user',     operators: ['=', '!=', 'contains'], searchable: true, description: 'aql.field.contactEmail.desc' },
  { key: 'feedback',     label: 'aql.field.feedback',     type: 'string',   category: 'event',    operators: ['=', '!=', 'contains'], searchable: true, description: 'aql.field.feedback.desc' },
];
```

### 5.4 DomainConfig (도메인별 설정)

각 페이지(로그, 이슈, 퍼포먼스, 디스커버 등)는 **서로 다른 필드 목록**을 사용한다. AQL 엔진은 동일하되, 자동완성에서 노출하는 필드가 달라진다.
...
interface DomainConfig {
  ...
  /** 이 도메인에서 사용 가능한 집계 함수 목록 (Section 21) */
  aggregates?: AggregateFunctionDef[];
}

/**
 * 집계 함수 정의 (Section 21).
 * 도메인별로 사용 가능한 함수만 등록한다.
 */
interface AggregateFunctionDef {
  ...
  description: string;
  /** 인자 정의 */
  args: { name: string; type: 'field' | 'number' | 'duration'; required: boolean }[];
  /** 반환값 타입 (연산자 유효성 검증에 사용) */
...
각 페이지에서 config를 정의하여 `QueryAQLEditor`에 전달한다.
...
```typescript
// 로그 페이지
<QueryAQLEditor config={LOGS_CONFIG} onSearch={handleSearch} />

// 이슈 페이지
<QueryAQLEditor config={ISSUES_CONFIG} onSearch={handleSearch} />

// 디스커버 (모든 필드)
<QueryAQLEditor config={DISCOVER_CONFIG} onSearch={handleSearch} />

// 피드백 (freeText → feedback 필드)
<QueryAQLEditor config={FEEDBACK_CONFIG} onSearch={handleSearch} />
```
...
#### DomainConfig가 영향을 미치지 않는 것 (= AQL 엔진 자체는 불변)
...
| 설정 | 기본값 | 변경 방법 |
|------|--------|-----------|
| `maxSuggestions` | `20` | `<QueryAQLEditor maxSuggestions={30} />` |
...
|---|---|---|---|
| `DANGLING_OPERATOR` | `AQL.error.danglingOperator` | `"{{op}}" 뒤에 검색 조건이 필요합니다` | `Expected expression after "{{op}}"` |
| `UNEXPECTED_TOKEN` | `AQL.error.unexpectedToken` | `예상하지 못한 "{{token}}"이(가) 있습니다` | `Unexpected "{{token}}"` |
| `INCOMPLETE_FILTER` | `AQL.error.incompleteFilter` | `"{{field}}" 필드에 값이 필요합니다` | `Field "{{field}}" requires a value` |
| `UNCLOSED_PAREN` | `AQL.error.unclosedParen` | `닫는 괄호 ")"가 필요합니다` | `Missing closing parenthesis ")"` |
| `UNCLOSED_QUOTE` | `AQL.error.unclosedQuote` | `닫는 따옴표가 필요합니다` | `Missing closing quote` |
| `INCOMPLETE_FUNCTION` | `AQL.error.incompleteFunction` | `{{op}}() 함수의 인자가 필요합니다` | `{{op}}() requires an argument` |
| `UNKNOWN_FIELD` | `AQL.error.unknownField` | `"{{field}}"은(는) 알 수 없는 필드입니다` | `Unknown field "{{field}}"` |
| `INVALID_OPERATOR` | `AQL.error.invalidOperator` | `"{{field}}" 필드에 "{{op}}" 연산자를 사용할 수 없습니다` | `Operator "{{op}}" is not valid for field "{{field}}"` |
| `INVALID_VALUE_TYPE` | `AQL.error.invalidValueType` | `"{{field}}" 필드에 {{expected}} 타입의 값이 필요합니다` | `Field "{{field}}" expects a {{expected}} value` |
...
|---|---|---|---|
| `DANGLING_OPERATOR` | `AQL.hint.danglingOperator` | `예: {{example}}` | `e.g. {{example}}` |
| `UNEXPECTED_TOKEN` | `AQL.hint.unexpectedToken` | `이 위치에서는 {{expected}}이(가) 와야 합니다` | `Expected {{expected}} at this position` |
| `INCOMPLETE_FILTER` | `AQL.hint.incompleteFilter` | `예: {{field}}:{{exampleValue}}` | `e.g. {{field}}:{{exampleValue}}` |
| `UNCLOSED_PAREN` | `AQL.hint.unclosedParen` | `표현식 끝에 ")"를 추가하세요` | `Add ")" at the end of the expression` |
| `INCOMPLETE_FUNCTION` | `AQL.hint.incompleteFunction` | `예: {{field}}:{{op}}("값")` | `e.g. {{field}}:{{op}}("value")` |
| `UNKNOWN_FIELD` | `AQL.hint.unknownField` | `사용 가능한 필드: {{availableFields}}` | `Available fields: {{availableFields}}` |
| `INVALID_OPERATOR` | `AQL.hint.invalidOperator` | `"{{field}}"에 사용 가능한 연산자: {{validOps}}` | `Valid operators for "{{field}}": {{validOps}}` |
| `INVALID_VALUE_TYPE` | `AQL.hint.invalidValueType` | `예: {{field}}:{{exampleValue}}` | `e.g. {{field}}:{{exampleValue}}` |
...
### 13.2 Frontend AQL → Backend 변환

프론트엔드의 새 AQL 문법을 백엔드가 이해하는 형식으로 **serialize** 해야 한다.

| Frontend AQL | Backend Format |
|---|---|
...
- `localStorage` 기반 (`argus_recent_AQL_searches`)
- 최대 10개
- 검색 실행(Enter) 시 자동 저장
...
```
src/components/argus/query-aql/
...
├── serializer.ts                   # Frontend AQL → Backend format converter
├── QueryAQLEditor.tsx              # Main React component (contentEditable)
...
describe('Validator', () => {
  ...
  describe('Error i18n keys', () => {
    it('모든 에러에 messageKey 포함', () => {
      // error.messageKey === 'aql.error.invalidOperator' 등
    });
    it('모든 에러에 hintKey 포함', () => {
      // error.hintKey === 'aql.hint.invalidOperator' 등
    });
    it('params에 보간 변수 포함', () => {
...
```typescript
describe('Serializer (Frontend AQL → Backend)', () => {
  it('단순 필터', () => {
...
```bash
npx vitest run src/components/argus/query-aql/__tests__/
```
...
집계 함수 지원은 **Section 21**에서 상세히 정의한다.

---

## 21. Aggregate Functions

> **Status**: Spec Complete (v1.2)  
> 집계 함수를 AQL 엔진에서 1등 시민(first-class citizen)으로 지원한다.

### 21.1 설계 원칙

1. **예약어 Zero 원칙** — Lexer에 어떤 토큰도 추가하지 않는다. `count`, `avg` 등은 `FIELD` 토큰 그대로 사용.
2. **도메인이 함수를 소유** — 전역 레지스트리의 부분집합이 아니라, 각 도메인이 자신의 고유 함수를 직접 정의한다.
3. **Parser 기반 판별** — `FIELD + LPAREN` 패턴을 감지하면 `DomainConfig.aggregates`를 참조하여 집계 함수 여부를 판별한다.
4. **칩 시각 구분** — 집계 필터 칩은 `Σ` 아이콘 + 시안 색상으로 일반 필터와 구분한다.
5. **백엔드 전달** — 구조화된 JSON (`aggregateFilters: [...]`) 형태로 전달한다.
...
# 예시
count():>100                    — 이벤트 수 100건 초과
avg(duration):>=500             — 평균 응답시간 500ms 이상
p95(transaction.duration):<2000  — 95th percentile 2000ms 미만
count_unique(user):>20          — 고유 사용자 20명 초과
failure_rate():>0.05            — 실패율 5% 초과
log_rate(error):>0.5            — 에러 로그 비율 50% 초과 (Logs 전용)
growth_rate(count, 24h):>50     — 24시간 대비 증가율 50% 초과
```

### 21.3 토큰 플로우

집계 함수는 Lexer에서 별도 토큰을 발행하지 않는다. 기존 토큰 조합으로 표현된다.

```
Input: count():>100

Lexer 출력:
  FIELD("count")  LPAREN  RPAREN  COLON  GT  NUMBER(100)

Parser 판별:
  1. FIELD("count") 다음에 LPAREN이 온다
  2. DomainConfig.aggregates에서 "count" 탐색
  3. 있으면 → AggregateFilterExpression으로 파싱
  4. 없으면 → 기존 로직 (일반 필드 or free text)
```

```
Input: count:100   (일반 필드 필터)

Lexer 출력:
  FIELD("count")  COLON  NUMBER(100)

Parser 판별:
  1. FIELD("count") 다음에 COLON이 온다 (LPAREN이 아님)
  2. 기존 필터 로직 → FilterExpression { field: "count", op: "=", value: 100 }
```

**핵심**: `(` 유무로 집계 함수와 일반 필드를 구분한다. 예약어가 아니므로 충돌 없음.

### 21.4 도메인별 집계 함수 레지스트리

각 도메인은 자기에게 의미 있는 함수만 등록한다. 도메인 전용 함수(🆕)는 해당 도메인에서만 존재한다.

| 집계 함수 | 인자 | 반환 타입 | Logs | Issues | Perf | Traces | Feedback | Discover |
|---|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| `count()` | — | number | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| `count_unique(field)` | field | number | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| `avg(field)` | field | number | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `sum(field)` | field | number | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `min(field)` | field | number | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `max(field)` | field | number | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `p50(field)` | field | duration | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `p75(field)` | field | duration | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `p90(field)` | field | duration | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `p95(field)` | field | duration | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `p99(field)` | field | duration | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `failure_rate()` | — | percentage | ❌ | ❌ | ✅ | ✅ | ❌ | ✅ |
| `eps()` | — | number | ✅ | ✅ | ✅ | ✅ | ❌ | ✅ |
| `epm()` | — | number | ✅ | ✅ | ✅ | ✅ | ❌ | ✅ |
| 🆕 `error_rate()` | — | percentage | ✅ | ✅ | ✅ | ✅ | ❌ | ✅ |
| 🆕 `growth_rate(metric, period)` | field, duration? | percentage | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 🆕 `log_rate(level)` | field | percentage | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ |
...
### 21.5 DomainConfig 설정

```typescript
import { AGG_COUNT, AGG_COUNT_UNIQUE, AGG_EPS, AGG_EPM,
         AGG_LOG_RATE, AGG_PATTERN_COUNT, AGG_ERROR_RATE, AGG_GROWTH_RATE,
         AGG_AVG, AGG_SUM, AGG_MIN, AGG_MAX,
         AGG_P50, AGG_P75, AGG_P90, AGG_P95, AGG_P99,
         AGG_FAILURE_RATE,
         AGG_REGRESSION_SCORE, AGG_AFFECTED_USERS,
         AGG_SENTIMENT_SCORE, AGG_RESPONSE_RATE } from './aggregates';

const LOGS_CONFIG: DomainConfig = {
  name: 'logs',
  fields: pickFields([...]),
  aggregates: [
    AGG_COUNT, AGG_COUNT_UNIQUE, AGG_EPS, AGG_EPM,
    AGG_ERROR_RATE, AGG_GROWTH_RATE,
    AGG_LOG_RATE,          // 🆕 Logs 전용
    AGG_PATTERN_COUNT,     // 🆕 Logs 전용
  ],
};
...
const ISSUES_CONFIG: DomainConfig = {
  ...
  aggregates: [
    AGG_COUNT, AGG_COUNT_UNIQUE, AGG_EPS, AGG_EPM,
    AGG_ERROR_RATE, AGG_GROWTH_RATE,
    AGG_REGRESSION_SCORE,  // 🆕 Issues 전용
    AGG_AFFECTED_USERS,    // 🆕 Issues 전용
  ],
};
...
const PERFORMANCE_CONFIG: DomainConfig = {
  ...
  fields: pickFields([...]),
  aggregates: [
    AGG_COUNT, AGG_COUNT_UNIQUE, AGG_AVG, AGG_SUM, AGG_MIN, AGG_MAX,
    AGG_P50, AGG_P75, AGG_P90, AGG_P95, AGG_P99,
    AGG_FAILURE_RATE, AGG_EPS, AGG_EPM,
    AGG_ERROR_RATE, AGG_GROWTH_RATE,
  ],
};
...
const FEEDBACK_CONFIG: DomainConfig = {
  ...
  fields: pickFields([...]),
  aggregates: [
    AGG_COUNT, AGG_COUNT_UNIQUE, AGG_GROWTH_RATE,
    AGG_SENTIMENT_SCORE,   // 🆕 Feedback 전용
    AGG_RESPONSE_RATE,     // 🆕 Feedback 전용
  ],
};
...
const DISCOVER_CONFIG: DomainConfig = {
  ...
  aggregates: ALL_AGGREGATE_FUNCTIONS,  // 전체 (파워유저용)
};
```

### 21.6 DomainConfig의 영향 범위 (집계 관련)

| 영향 받는 기능 | 동작 |
|---|---|
| **자동완성 함수 추천** | `config.aggregates`에 포함된 함수만 추천 |
| **Parser** | `config.aggregates`에 있는 이름만 AggregateFilter로 파싱 |
| **Validator** | 인자 타입/개수 검증, 비교 연산자 유효성 검증 |
| **칩 렌더링** | `chipKind: 'aggregate'` → Σ 아이콘 + 시안 색상 |
| **QueryBuilderPanel** | 필드 선택 드롭다운에 AGGREGATES 그룹 추가 |
| **Y축/집계 드롭다운** | 하드코딩 대신 `config.aggregates`에서 동적 생성 |

| 영향 없는 기능 | 이유 |
|---|---|
| **Lexer** | 토큰화는 집계 함수와 무관 (FIELD 토큰 그대로) |
| **Serializer** | AggregateFilter 노드 직렬화는 함수 목록과 무관 |
| **Formatter** | 포매팅은 AST 노드 타입만 참조 |

### 21.7 백엔드 API 전달 형식

집계 필터는 기존 `conditions`와 분리하여 **구조화된 JSON**으로 전달한다.

```typescript
// Serializer가 AST를 분리하여 생성
interface SerializedQuery {
  /** 일반 필터 조건 (기존) */
  conditions: string;                    // "level:error environment:production"
  /** 집계 필터 조건 (신규) */
...
```typescript
// AQL: level:error count():>100 avg(duration):>=500

argusService.discoverQuery(projectApiPath, {
  conditions: 'level:error',
  aggregateFilters: [
    { func: 'count', args: [], operator: '>', value: 100 },
    { func: 'avg', args: ['duration'], operator: '>=', value: 500 },
  ],
});
```
...
`EXPECT_FIELD` 상태에서 집계 함수를 추천한다. **도메인의 `config.aggregates`에 등록된 함수만 추천**.

| 상태 | 입력 | 추천 (Logs 도메인) |
|---|---|---|
| `EXPECT_FIELD` | `\|` | 필드 목록 + `count()`, `count_unique(field)`, `eps()`, ... |
| `EXPECT_FIELD` | `cou\|` | `count()`, `count_unique(field)` |
| `EXPECT_FIELD` | `p\|` | `pattern_count()` (Logs에 p 시작 집계만 있음) |
| `EXPECT_FIELD` | `log\|` | `log_rate(level)` (Logs 전용) |

같은 입력이라도 도메인에 따라 다른 추천:

| 입력 `p\|` | Logs | Performance |
|---|---|---|
| 추천 | `pattern_count()` | `p50(field)`, `p75(field)`, `p90(field)`, `p95(field)`, `p99(field)` |

#### 21.8.2 함수 인자 추천

`count_unique(|)` 상태에서 **해당 도메인의 필드 목록**을 추천한다.

| 입력 | 추천 |
|---|---|
| `count_unique(\|)` | `user`, `service`, `environment`, ... (도메인 필드) |
| `avg(\|)` | `duration` (number 타입 필드만) |
| `p95(\|)` | `duration`, `transaction.duration` (number 타입 필드만) |

#### 21.8.3 비교 연산자 추천

`count():` 상태에서 비교 연산자를 추천한다.

| 입력 | 추천 |
|---|---|
| `count():\|` | `>`, `>=`, `<`, `<=`, `=`, `!=` |

#### 21.8.4 CursorContext 확장

| Input (`\|` = cursor) | type | field | prefix |
|---|---|---|---|
| `cou\|` | FIELD | — | `cou` (집계 함수도 매칭) |
| `count(\|` | AGG_ARG | `count` | `` |
| `count_unique(us\|` | AGG_ARG | `count_unique` | `us` |
| `count():\|` | AGG_OPERATOR | `count()` | `` |
| `count():>\|` | AGG_VALUE | `count()` | `` |
| `count():>10\|` | AGG_VALUE | `count()` | `10` |
...
interface FilterChip {
  id: string;
  type?: ChipType;                  // 'filter' | 'logical' | 'paren'
  chipKind?: 'filter' | 'aggregate'; // ← NEW
  // ... 기존 필드 유지
}
```
...
일반 필터:
┌─────────────────────────┐
│ level   is    error  ×  │   ← 보라색 필드, 회색 연산자, 주황색 값
└─────────────────────────┘

집계 필터:
┌──────────────────────────────┐
│ Σ count()   >    100     ×   │   ← Σ 아이콘 + 시안 배경 + 시안 필드색
└──────────────────────────────┘
...
| 요소 | 일반 필터 | 집계 필터 |
|---|---|---|
| **아이콘** | 없음 | `Σ` (Functions 아이콘) |
| **필드 색상 (dark)** | `#c4b5fd` (보라) | `#67e8f9` (시안) |
| **필드 색상 (light)** | `#7c3aed` (보라) | `#0891b2` (시안) |
| **칩 배경 (dark)** | `rgba(255,255,255,0.04)` | `rgba(103,232,249,0.06)` |
| **칩 보더 (dark)** | `rgba(255,255,255,0.08)` | `rgba(103,232,249,0.15)` |

### 21.10 QueryBuilderPanel 통합

QueryBuilderPanel의 필드 선택 드롭다운에 **AGGREGATES** 그룹을 추가한다.
...
- `config.aggregates`에 등록된 함수만 표시
- 집계 함수 선택 시 연산자는 비교 연산자(`>`, `>=`, `<`, `<=`, `=`, `!=`)만 표시
- 값 입력은 숫자만 허용
...
| 검증 | 규칙 | 에러 예시 |
|---|---|---|
| 함수 존재 | `config.aggregates`에 등록 필수 | `xyz():>100` → "알 수 없는 집계 함수" |
| 인자 개수 | `def.args.filter(a => a.required).length` 이상 | `count_unique():>10` → "field 인자 필요" |
| 인자 타입 | `type: 'field'` → 도메인 필드, `type: 'number'` → 숫자 | `avg("hello"):>100` → "숫자 필드 필요" |
| 비교 연산자 | `>`, `>=`, `<`, `<=`, `=`, `!=`만 허용 | `count():contains(100)` → "비교 연산자 필요" |
| 비교 값 | 숫자만 허용 | `count():>"abc"` → "숫자 값 필요" |

### 21.12 도메인 전용 함수 상세

| 함수 | 도메인 | 설명 | 인자 |
|---|---|---|---|
| `log_rate(level)` | Logs | 전체 로그 중 특정 레벨의 비율 | `level`: 로그 레벨 (error, warning 등) |
| `pattern_count()` | Logs | 유니크 로그 패턴(fingerprint) 수 | — |
| `regression_score()` | Issues | 이슈 재발 위험도 점수 (0~1) | — |
| `affected_users()` | Issues | 이슈에 영향받은 고유 사용자 수 | — |
| `sentiment_score()` | Feedback | 피드백 감성 분석 점수 (0~1) | — |
| `response_rate()` | Feedback | 피드백 응답률 (0~1) | — |
| `error_rate()` | 공용 | 전체 이벤트 중 에러 이벤트 비율 | — |
| `growth_rate(metric, period)` | 공용 | 이전 기간 대비 증가율 (%) | `metric`: 측정 대상, `period`: 비교 기간 (예: 24h, 7d) |

### 21.13 Serializer 규칙

AggregateFilterExpression은 다음과 같이 직렬화한다:

| AST | 직렬화 결과 |
|---|---|
| `{ func: "count", args: [], op: ">", value: 100 }` | `count():>100` |
| `{ func: "avg", args: ["duration"], op: ">=", value: 500 }` | `avg(duration):>=500` |
| `{ func: "count_unique", args: ["user"], op: ">", value: 20 }` | `count_unique(user):>20` |
| `{ func: "growth_rate", args: ["count", "24h"], op: ">", value: 50 }` | `growth_rate(count, 24h):>50` |
| `{ func: "count", args: [], op: "=", value: 100 }` | `count():100` (= 연산자 생략) |

### 21.14 파일 구조

```
query-aql/
  aggregates.ts         — [NEW] 집계 함수 마스터 레지스트리 + 도메인별 프리셋
  types.ts              — [MODIFY] AggregateFilterExpression, AggregateFunctionDef, DomainConfig.aggregates 추가
  parser.ts             — [MODIFY] parsePrimary()에 FIELD+LPAREN 분기, parseAggregateFilter() 추가
  serializer.ts         — [MODIFY] AggregateFilter 케이스 추가
  validator.ts          — [MODIFY] 집계 함수 인자/연산자 검증
  fields.ts             — [MODIFY] 각 DomainConfig에 aggregates 프리셋 할당
  useFilterChips.ts     — [MODIFY] chipKind: 'aggregate' + queryToChips/chipsToQuery 확장
  FilterTokenGroup.tsx  — [MODIFY] Σ 아이콘 + 시안 색상 + 집계 칩 스타일
  suggestion-engine.ts  — [MODIFY] config.aggregates 기반 함수 추천
  cursor-context.ts     — [MODIFY] AGG_ARG, AGG_OPERATOR, AGG_VALUE 상태 추가
  smart-suggestions.ts  — [NEW] 쿼리 맥락 기반 집계 함수 선제 제안
  query-templates.ts    — [NEW] 도메인별 추천 쿼리 템플릿
  query-narrator.ts     — [NEW] AQL → 자연어 해석
```
