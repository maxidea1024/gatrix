...
describe('Dataset Schemas', () => {
  describe('errors dataset', () => {
    ...
    it('should have all expected columns', () => {
      const expectedColumns = [
        ...
        'platform', 'level', 'logger', 'type', 'value', 'mechanism',
        'fingerprint', 'primary_hash', 'exception', 'stacktrace_frames',
        'breadcrumbs', 'user_id', 'user_email', 'user_ip', 'user_name',
        'environment', 'release', 'dist', 'server_name', 'transaction',
        'os_name', 'os_version', 'browser_name', 'browser_version',
        'device_name', 'device_family', 'runtime_name', 'runtime_version',
        'sdk_name', 'sdk_version', 'geo_country', 'geo_city', 'geo_region',
        'http_method', 'http_url', 'http_referer', 'tags', 'extra',
        'contexts', 'is_handled', 'is_symbolicated',
      ];
    ...
    it('should have materialized view config', () => {
      ...
      expect(ds.materializedViews[0].table).toBe('argus.error_frequency_hourly');
    });
  });
  ...
  describe('transactions dataset', () => {
    ...
    it('should have MV config', () => {
      expect(ds.materializedViews).toHaveLength(1);
      expect(ds.materializedViews[0].table).toBe('argus.transaction_metrics_hourly');
    });
  });
