 ...
 * 
 * Usage:
 *   npx ts-node archived/refactor-environment.ts [--dry-run]
 */
...
function renameIdentifiers(project: Project): { filesChanged: number; identifiersRenamed: number } {
    let filesChanged = 0;
        ...
        sourceFile.forEachDescendant((node) => {
            if (Node.isIdentifier(node)) {
                ...
                if (newName) {
                    ...
                    // Skip property access on string/object literals that map to DB columns
                    // We only want to rename variable declarations, parameters, etc.

                    // Check if this identifier is:
                    // 1. A variable declaration
                    // 2. A parameter
                    // 3. A property declaration in an interface/type
                    // 4. A property access (obj.environmentId)
                    // 5. A shorthand property (destructuring)
                    ...
                    const isPropertyAccessExpression = Node.isPropertyAccessExpression(parent) &&
                    ...
                    // For property assignments like { environmentId: value }, we need to check
                    // if this is the property name or the value
                    const isPropertyAssignment = Node.isPropertyAssignment(parent);
                    const isPropertyName = isPropertyAssignment &&
                        parent.getNameNode() === node;

                    if (isVariableDeclaration || isParameter || isPropertySignature ||
                        isPropertyDeclaration || isPropertyAccessExpression || isBindingElement ||
                        isShorthandPropertyAssignment || isPropertyName) {

                        console.log(`  ${filePath}: ${identifierName} → ${newName}`);

                        if (!DRY_RUN) {
                            // Use rename to update all references
                            try {
                                node.rename(newName);
                                identifiersRenamed++;
                                fileModified = true;
