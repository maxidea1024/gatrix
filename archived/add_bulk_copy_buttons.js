...
// <TableCell>\n<Typography ...>\n{item.service}\n</Typography>\n</TableCell>
const serviceRegex =
  /<TableCell>\s*<Typography variant="body2" fontWeight="medium">\s*{item\.service}\s*<\/Typography>\s*<\/TableCell>/;

// 2. Group
const groupRegex =
  /<TableCell>\s*<Typography variant="body2" color="text\.secondary">\s*{item\.group \|\| '-'}\s*<\/Typography>\s*<\/TableCell>/;

// 3. Env
const envRegex =
  /<TableCell>\s*<Typography variant="body2" color="text\.secondary">\s*{item\.env \|\| '-'}\s*<\/Typography>\s*<\/TableCell>/;

// 4. Hostname: uses D2Coding font style
const hostnameRegex =
  /<TableCell>\s*<Typography variant="body2" sx={{ fontFamily: 'D2Coding, monospace', fontSize: '0.75rem' }}>\s*{item\.hostname \|\| '-'}\s*<\/Typography>\s*<\/TableCell>/;

// 5. InternalIp
const internalIpRegex =
  /<TableCell>\s*<Typography variant="body2" sx={{ fontFamily: 'D2Coding, monospace', fontSize: '0.75rem' }}>\s*{item\.internalIp \|\| '-'}\s*<\/Typography>\s*<\/TableCell>/;

// 6. Port
const portRegex =
  /<TableCell>\s*<Typography variant="body2" sx={{ fontFamily: 'D2Coding, monospace', fontSize: '0.75rem' }}>\s*{item\.healthPort \|\| '-'}\s*<\/Typography>\s*<\/TableCell>/;
...
if (internalIpRegex.test(content)) {
  content = content.replace(internalIpRegex, (match) =>
    createReplacement(match, 'item.internalIp')
  );
  modified = true;
} else console.log('InternalIP regex failed');

if (portRegex.test(content)) {
  content = content.replace(portRegex, (match) =>
    createReplacement(match, 'String(item.healthPort)')
  );
  modified = true;
