...
const keys = {
  ko: {
    'featureFlags.weightDistribution':
      '가중치 분배: 고정 {fixed}%, 자동 분배 {remaining}% ({autoCount}개)',
  },
  en: {
    'featureFlags.weightDistribution':
      'Weight Distribution: Fixed {fixed}%, Auto {remaining}% ({autoCount} variants)',
  },
  zh: {
    'featureFlags.weightDistribution':
      '权重分配：固定 {fixed}%，自动 {remaining}%（{autoCount}个）',
  },
};
