...
const koToEn = {
  ...
  '이미 등록된 클라이언트 버전입니다: {{duplicates}}. 같은 환경에서 동일한 플랫폼과 버전 조합은 중복될 수 없습니다.':
    'Client version already exists: {{duplicates}}. The same platform and version combination cannot be duplicated in the same environment.',
  태그: 'Tags',
  '슈퍼 관리자': 'Super Admin',
  '슈퍼 관리자는 모든 권한을 가지며, 새로운 권한이 추가되어도 자동으로 권한이 부여됩니다.':
    'Super admins have all permissions and automatically receive any new permissions added.',
};

// Translation mappings (Korean -> Chinese)
const koToZh = {
  '이미 등록된 클라이언트 버전입니다: {{duplicates}}. 같은 환경에서 동일한 플랫폼과 버전 조합은 중복될 수 없습니다.':
    '客户端版本已存在: {{duplicates}}。同一环境中相同平台和版本组合不能重复。',
  태그: '标签',
  '슈퍼 관리자': '超级管理员',
  '슈퍼 관리자는 모든 권한을 가지며, 새로운 권한이 추가되어도 자동으로 권한이 부여됩니다.':
    '超级管理员拥有所有权限，新增权限时自动获得授权。',
};
