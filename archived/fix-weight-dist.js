...
const files = [
  {
    file: 'ko.ini',
    value: '가중치 분배: 고정 {{fixed}}%, 자동 분배 {{remaining}}% ({{autoCount}}개)',
  },
  {
    file: 'en.ini',
    value: 'Weight Distribution: Fixed {{fixed}}%, Auto {{remaining}}% ({{autoCount}} variants)',
  },
...
for (const { file, value } of files) {
  ...
  const filteredLines = lines.filter(
    (line) => !line.startsWith('featureFlags.weightDistribution=')
  );
