  ...
  "memberships": [
    {
      ...
      "roleBindings": [
        ...
        { "roleId": "ROLE_ID", "scopeType": "project", "scopeId": "PROJ_ID_1" }
      ]
    }
  ]
}
...
> [!NOTE]
> 멤버십(조직/프로젝트)은 기본 역할로 가입. 실제 권한은 `roleBindings`만으로 결정.
...
for each membership in config.memberships:
  1. g_organisation_members INSERT (기본 역할, 중복 skip)
  2. for each projectId in membership.projectIds:
     → g_project_members INSERT (기본 역할, 중복 skip)
