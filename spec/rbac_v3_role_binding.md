...
## 18. 사용자 온보딩: 자동 가입 및 역할 바인딩

### 18.1 개요

사용자를 초대하거나 직접 추가할 때, 지정된 **조직**, **프로젝트**에 자동 가입시키고 **역할 바인딩**까지 한 번에 설정하는 기능.

```
기존: 사용자 생성 → (수동) 조직 가입 → (수동) 프로젝트 가입 → (수동) 역할 바인딩
변경: 사용자 생성 → autoJoinConfig에 의해 자동으로 3가지 한 번에 처리
```
...
| 시나리오 | 흐름 |
|----------|------|
| **초대 생성** | 관리자가 초대 생성 시 `autoJoinConfig` 설정 → DB 저장 |
| **초대 수락** | 사용자 계정 생성 → `autoJoinConfig` 읽어 자동 가입 |
| **직접 추가** | 관리자가 사용자 생성 시 `autoJoinConfig` 포함 → 즉시 자동 가입 |

### 18.3 autoJoinConfig 구조

`g_invitations` 테이블에 `autoJoinConfig JSON NULL` 컬럼 추가:
  ...
  "memberships": [
    {
      ...
      "orgRole": "user",
      "projects": [
        { "projectId": "PROJ_ID", "projectRole": "member" }
      ],
      "roleBindings": [
        { "roleId": "ROLE_ID", "scopeType": "org", "scopeId": "ORG_ID" },
        { "roleId": "ROLE_ID", "scopeType": "project", "scopeId": "PROJ_ID" }
      ]
    }
  ]
}
...
```
applyAutoJoinConfig(userId, config, assignedBy)

for each membership in config.memberships:
  1. g_organisation_members INSERT (중복 skip)
     → orgId, userId, orgRole, invitedBy=assignedBy
  2. for each project in membership.projects:
     → g_project_members INSERT (중복 skip)
     → projectId, userId, projectRole, invitedBy=assignedBy
  3. for each binding in membership.roleBindings:
     → RoleBindingModel.create() (중복 skip)
     → userId, roleId, scopeType, scopeId, assignedBy
  4. permissionService.invalidateUserCache(userId)
```
...
| 항목 | 규칙 |
|------|------|
| **초대자 범위** | 초대자가 속한 조직/프로젝트만 선택 가능 |
| **복수 선택** | 복수 조직, 복수 프로젝트 동시 선택 가능 |
| **중복 방지** | 이미 멤버인 경우 skip, 이미 바인딩 있는 경우 skip |
| **권한 캐시** | 자동 가입 완료 후 반드시 `invalidateUserCache` 호출 |

### 18.6 사용자 목록 조직 스코프 필터링

사용자 관리 목록에서 **현재 조직에 속한 사용자만 표시**:
- `*` 권한(System Admin): 전체 보기 가능
- 일반 admin: 자신의 조직 멤버만 표시
- `UserModel.findAll`의 기존 `orgId` 필터 활용

### 18.7 UI

#### 초대/사용자 추가 시 (관리자 화면)

트리형 체크박스 UI로 조직/프로젝트/역할 선택:
...
- 현재 로그인한 관리자의 `getMyAccess()` 기반으로 선택 가능 범위 제한
- 역할 목록은 `rbacService.getRoles()`로 로드

#### 초대 수락 시 (피초대자 화면)

초대 수락(회원가입) 페이지에서 가입될 조직/프로젝트 안내:
...
- `validateInvitation` API 응답에 `autoJoinConfig`의 조직/프로젝트명 포함
- 읽기 전용 표시 (피초대자는 변경 불가)
