#!/usr/bin/env pwsh
#
# Gatrix Secret Key Generator (same as Swarm version + AWS Secrets Manager support)
#
# Usage:
...
function Show-Help {
    ...
    Write-Host "  -c, --count <n>           Number of keys (default: 1)"
    Write-Host "  --env                     Generate all .env security keys"
...
function Generate-Key {
    ...
    switch ($KeyEncoding) {
        ...
        "hex" { return ($bytes | ForEach-Object { $_.ToString("x2") }) -join '' }
        "alphanumeric" {
            $chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
            return ($bytes | ForEach-Object { $chars[$_ % $chars.Length] }) -join ''
        }
        default { Write-Host "Unknown encoding: $KeyEncoding" -ForegroundColor Red; exit 1 }
    }
}
...
if ($GenEnv) {
    ...
    Write-Host ""
    Write-Host "After setting these in .env, run ./setup-secrets.ps1 to store them in AWS Secrets Manager." -ForegroundColor Green
} else {
    for ($j = 1; $j -le $Count; $j++) {
        $key = Generate-Key -KeyLength $Length -KeyEncoding $Encoding
        if ($Count -gt 1) { Write-Host "[$j] $key" } else { Write-Host $key }
    }
}
