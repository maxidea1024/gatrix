...
show_health() {
    ...
    for service in $services; do
        ...
        if [ "$running" -eq "$replicas" ] 2>/dev/null; then
            echo -e "  ${GREEN}OK${NC} $service ($running/$replicas)"
        elif [ "$running" -gt 0 ] 2>/dev/null; then
            echo -e "  ${YELLOW}WARN${NC} $service ($running/$replicas)"
        else
            echo -e "  ${RED}DOWN${NC} $service ($running/$replicas)"
        fi
    done
}
...
main() {
    ...
    if [ -n "$SHOW_LOGS" ]; then show_logs "$SHOW_LOGS"
    elif [ "$SHOW_SERVICES" = true ]; then show_services
    elif [ "$SHOW_TASKS" = true ]; then show_tasks
    elif [ "$SHOW_HEALTH" = true ]; then show_health
    else show_all; fi
}
