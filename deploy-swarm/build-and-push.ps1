#!/usr/bin/env pwsh
#
# Gatrix Build and Push Script (Swarm Edition)
#
# Usage:
...
# Show help function
function Show-Help {
    Write-Host "Gatrix Build and Push Script (Swarm Edition)"
    Write-Host ""
...
# Define available services (Swarm edition: no chat-server, no event-lens)
$allServices = @{
...
# Root directory of the project (parent of 'deploy-swarm')
$rootDir = Resolve-Path "$PSScriptRoot\.."
$loginScript = Join-Path $PSScriptRoot "login-registry.ps1"

Write-Host "Gatrix Build & Push (Swarm Edition)" -ForegroundColor Cyan
