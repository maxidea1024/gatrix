...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Presets:"
            echo "  minimal   - backend:1  frontend:1  edge:1  (testing)"
            echo "  standard  - backend:2  frontend:1  edge:2  (production)"
...
apply_preset() {
    ...
    case $preset in
        ...
        high)
            ...
            scale_service "edge" 8
            ;;
        *) log_error "Unknown preset: $preset"; exit 1 ;;
    esac
}
...
main() {
    ...
    if [ -n "$PRESET" ]; then apply_preset "$PRESET"
    elif [ -n "$SERVICE_NAME" ] && [ -n "$REPLICAS" ]; then scale_service "$SERVICE_NAME" "$REPLICAS"
    else log_error "Please specify --service and --replicas, --preset, or --status"; exit 1; fi
