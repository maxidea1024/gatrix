...
function Show-HelpText {
    ...
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  ./status.ps1                # Show all status"
    Write-Host "  ./status.ps1 -s             # Services only"
    Write-Host "  ./status.ps1 -l backend     # Backend logs"
    exit 0
}
...
while ($i -lt $args.Count) {
    switch ($args[$i]) {
        ...
        { $_ -eq "-h" -or $_ -eq "--help" } { Show-HelpText }
...
function Show-HealthStatus {
    ...
    foreach ($service in $services) {
        ...
        if ($runningCount -eq $replicas -and $replicas -gt 0) {
            Write-Host "  OK $service ($runningCount/$replicas)" -ForegroundColor Green
        }
        elseif ($runningCount -gt 0) {
            Write-Host "  WARN $service ($runningCount/$replicas)" -ForegroundColor Yellow
        }
        else {
            Write-Host "  DOWN $service ($runningCount/$replicas)" -ForegroundColor Red
        }
    }
}
